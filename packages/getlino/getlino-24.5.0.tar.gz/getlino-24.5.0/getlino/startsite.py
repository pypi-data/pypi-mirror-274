...
@click.option(
    '--db-password',
    help=
    "Password for database user. Leave empty to generate a secure password.")
@click.pass_context
def startsite(
    ctx,
    appname,
    ...
    context.update({
        ...
        ' '.join(pip_packages),
        "db_name":
        os.environ.get('GITLAB_CI_DB_NAME', None) or prjname,
        "python_path":
        sites_base,
        "usergroup":
        usergroup,
        "provider":
        "lino" if not ini_file or
        (ini_file and not use_django_settings) else "django"
    })
    ...
    if ifroot():
        ...
        i.run_in_env(envdir, "python manage.py buildcache")
