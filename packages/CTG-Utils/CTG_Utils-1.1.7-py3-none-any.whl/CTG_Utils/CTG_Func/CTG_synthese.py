...
def synthese(year,ctg_path):
    ...
    path_dir_list = [ctg_path / Path(str(year)) / Path('SORTIES DU SAMEDI') / Path('EXCEL'),
                     ctg_path / Path(str(year)) / Path('SORTIES DU DIMANCHE') / Path('EXCEL'),
                     ctg_path / Path(str(year)) / Path('SORTIES DU JEUDI') / Path('EXCEL'),
                     ctg_path / Path(str(year)) / Path('SORTIES HIVER') / Path('EXCEL'),
                     ctg_path / Path(str(year)) / Path('SORTIES DU LUNDI')/ Path('EXCEL'),
                     ctg_path / Path(str(year)) / Path('SEJOUR') / Path('EXCEL')]
    ...
    file = ctg_path / Path(str(year)) / Path('STATISTIQUES') / Path('synthese.xlsx')
    df_total.to_excel(file)

def plot_pie_synthese(year,ctg_path):
    ...
    file_in = ctg_path / Path(str(year)) / Path('STATISTIQUES') / Path('synthese.xlsx')
    ...
    plt.title(str(year), pad=50, fontsize=20)
...
def synthese_adherent(year,ctg_path):
    ...
    file_out = ctg_path / Path(str(year)) / Path('STATISTIQUES') / Path('synthese_adherent.xlsx')
    dg.to_excel(file_out)
    
def synthese_randonnee(year,ctg_path):
    ...
    file_in = Path(ctg_path) / Path(str(year)) / Path('STATISTIQUES') / Path('synthese.xlsx')
    df_total = pd.read_excel(file_in)
    df_total = df_total.dropna(subset=['Nom']) 
    dg = df_total.query('Type=="RANDONNEE"').groupby('sejour').agg('count')['N° Licencié']
    ...
    _ = plt.title(f'# randos : {len(dg)} , # participants : {sum(dg)}')
    plt.tight_layout()
    plt.show()
    
    fig_file = 'SYNTHESE_RANDONNEES.png'
    plt.savefig(ctg_path / Path(str(year)) / Path('STATISTIQUES') / Path(fig_file),bbox_inches='tight')
    
def nbr_sejours_adherent(year, ctg_path):
    ...
    plt.title(str(year),pad=20, fontsize=20)
...
def read_memory_sorties():
    ...
    path_config_file = Path(__file__).parent.parent / Path('CTG_Func') / Path('CTG_RefFiles') / Path('memory_sorties.yml')
...
def evolution_sorties(type,ctg_path):
    ...
    from CTG_Utils.CTG_GUI.GUI_Globals import ACTIVITE_LIST 
    
    def add_memory(stat_dic,years):
        memory = read_memory_sorties()
    ...
    def fill_stat_year(year):
    
        # Internal library imports
        from CTG_Utils.CTG_Func.CTG_utility import get_sejour_info
