...
def record_advancement_line(target, line):
    ...
    if "Requirements" not in target:
        target["Requirements"] = {}
    target["Requirements"]["Date"] = find_min_date(
        line
...
def record_whole_award(scout, award_type, line):
    ...
    ic(line["Advancement"], award_type, badge)

    if f"{award_type}s" not in scout:
        scout[f"{award_type}s"] = {}  # "Adventures", "Merit Badges", "Ranks", etc.

    scout[f"{award_type}s"][badge] = {}  # "Ranks"."Eagle"."Date"
    record_advancement_line(scout[f"{award_type}s"][badge], line)
    if (
