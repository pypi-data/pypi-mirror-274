...
def record_whole_award(scout, award_type, line):
    ...
    if (
        ...
        and REQUIREMENTS.get(rank)  # and it is listed in the requirements list
        and REQUIREMENTS[rank].get("rank_order")
        > scout["Data"].get(
            "_rank_order", -1
        )  # and the rank_order is bigger than the ones the scout has gotten before
    ):
        print(
            "Higher rank being assigned",
            scout["Data"].get("_rank_order"),
            file=sys.stderr,
        )
        ic(rank)
        ...
        print(
            "resulting rank: ", rank, REQUIREMENTS[rank]["rank_order"], file=sys.stderr
        )
