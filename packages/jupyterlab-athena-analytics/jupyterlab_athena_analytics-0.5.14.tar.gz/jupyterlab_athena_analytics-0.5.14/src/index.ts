...
//   onDragStart: () =>
//     console.log('I am called whenever any external enters this window'),
// });
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (app: JupyterFrontEnd, tracker: INotebookTracker, docManager: IDocumentManager) => {
    ...
    // Initialize the monitor for external drag events
    const cleanup = monitorForExternal({
      onDragStart: () => {
        console.log('External drag event started');
      },
    });

    // Clean up the monitor when the window is unloaded
