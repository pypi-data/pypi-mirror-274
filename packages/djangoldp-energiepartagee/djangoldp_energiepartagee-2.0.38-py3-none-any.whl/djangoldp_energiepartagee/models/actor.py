...
class Actor(Model):
    ...
    def technology_used(self):
        technology = ""
        for citizen_project in self.founded_projects.all():
            if citizen_project.visible:
                if citizen_project.technology_used == "multi":
                    return "multi"
                technology = (
                    "multi"
                    if (
                        technology != "" and technology != citizen_project.technology_used
                    )
                    else citizen_project.technology_used
                )
        if not technology:
            technology = "unknown"
        return technology
    ...
    def energy_type(self):
        ...
        for citizen_project in self.founded_projects.all():
            if citizen_project.visible:
                if citizen_project.energy_type == "multi":
                    return "multi"
                energy = (
                    "multi"
                    if (energy != "" and energy != citizen_project.energy_type)
                    else citizen_project.energy_type
                )
        if not energy:
            energy = "unknown"
        return energy
    ...
    def progress_status(self):
        ...
        for citizen_project in self.founded_projects.all():
            if citizen_project.visible:
                status = status.union(citizen_project.progress_status)
        return status
