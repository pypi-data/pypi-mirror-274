...
class apiPermissionCheck:
    ...
    def check(self)->bool:
        ...
        for menu in self.currentUserMenus:
            if self._check(menu):return True
        return False
    ...
    def _check(self,menu:Dict):
        ...
        if method not in methods and "ALL" not in methods:return False 
        pathArr=path.split("/") 
        if "**" in url: 
            url=url[0:url.index("**")] 
            urlArr=url.split("/") 
            #log.warn(pathArr,urlArr)
            if len(pathArr)>=len(urlArr)-1 and  pathArr[0:len(urlArr)-1]==urlArr[0: len(urlArr)-1]:return True 
        if "*" in url:
            url=url[0:url.index("*")]
            urlArr=url.split("/") 
            log.warn(pathArr,urlArr)
