...
class KerberosCredential:
    def __init__(self):
        ...
        self.luid: Optional[int] = None
        self.tickets: List[KerberosTicket] = []
        self.pin: Optional[str] = None
        self.pin_raw: Optional[bytes] = None
        self.cardinfo = None
...
class KerberosDec(PackageDecryptor):
    ...
    def handle_ticket(self, kerberos_ticket):
        try:
            kt = KerberosTicket.parse(kerberos_ticket, self.reader, self.decryptor_template.sysinfo,
    ...
    def process_session(self, kerberos_logon_session):
        ...
        if kerberos_logon_session.SmartcardInfos.value != 0:
            csp_info = kerberos_logon_session.SmartcardInfos.read(self.reader,
        ...
        if kerberos_logon_session.pKeyList.value != 0:
            key_list = kerberos_logon_session.pKeyList.read(self.reader,
        ...
        if self.with_tickets is True:
            if kerberos_logon_session.Tickets_1.Flink.value != 0 and \
                ...
                self.walk_list(kerberos_logon_session.Tickets_1.Flink, self.handle_ticket,
            ...
            if kerberos_logon_session.Tickets_2.Flink.value != 0 and \
                ...
                self.walk_list(kerberos_logon_session.Tickets_2.Flink, self.handle_ticket,
                               override_ptr=self.decryptor_template.kerberos_ticket_struct)
            ...
            if kerberos_logon_session.Tickets_3.Flink.value != 0 and \
                ...
                self.walk_list(kerberos_logon_session.Tickets_3.Flink, self.handle_ticket,
                               override_ptr=self.decryptor_template.kerberos_ticket_struct)
            self.current_ticket_type = None
