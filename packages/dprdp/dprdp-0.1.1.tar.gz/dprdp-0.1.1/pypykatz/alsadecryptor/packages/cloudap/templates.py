...
from pypykatz.alsadecryptor.win_datatypes import ULONG, LUID, KIWI_GENERIC_PRIMARY_CREDENTIAL, POINTER, DWORD, PVOID, \
    PSID, GUID, DWORD64
...
class KIWI_CLOUDAP_CACHE_LIST_ENTRY:
    ...
    async def load(reader):
        ...
        res.unkLogin0 = await PVOID.load(reader)
        res.unkLogin1 = await PVOID.load(reader)
        res.toname = await reader.read(130)
        await reader.align()
        ...
        await reader.align()
        res.PRT = await PVOID.load(reader)
        return res
