...
class KerberosCredential:
    ...
    def to_dict(self):
        t = {'credtype': self.credtype, 'username': self.username, 'password': self.password,
             'password_raw': self.password_raw, 'domainname': self.domainname, 'luid': self.luid, 'pin': self.pin,
             'pin_raw': self.pin_raw, 'cardinfo': self.cardinfo, 'tickets': []}
...
class KerberosDec(PackageDecryptor):
    ...
    async def handle_ticket(self, kerberos_ticket):
        try:
            ...
            kt = await KerberosTicket.aparse(kerberos_ticket, self.reader, self.decryptor_template.sysinfo,
                                             self.current_ticket_type)
    ...
    async def process_session(self, kerberos_logon_session):
        ...
        if kerberos_logon_session.SmartcardInfos.value != 0:
            csp_info = await kerberos_logon_session.SmartcardInfos.read(self.reader,
                                                                        override_finaltype=self.decryptor_template.csp_info_struct)
        ...
        if kerberos_logon_session.pKeyList.value != 0:
            key_list = await kerberos_logon_session.pKeyList.read(self.reader,
                                                                  override_finaltype=self.decryptor_template.keys_list_struct)
        ...
        if kerberos_logon_session.Tickets_1.Flink.value != 0 and \
            ...
            await self.walk_list(kerberos_logon_session.Tickets_1.Flink, self.handle_ticket,
                                 override_ptr=self.decryptor_template.kerberos_ticket_struct)
        ...
        if kerberos_logon_session.Tickets_2.Flink.value != 0 and \
            ...
            await self.walk_list(kerberos_logon_session.Tickets_2.Flink, self.handle_ticket,
                                 override_ptr=self.decryptor_template.kerberos_ticket_struct)
        ...
        if kerberos_logon_session.Tickets_3.Flink.value != 0 and \
            ...
            await self.walk_list(kerberos_logon_session.Tickets_3.Flink, self.handle_ticket,
                                 override_ptr=self.decryptor_template.kerberos_ticket_struct)
        self.current_ticket_type = None
