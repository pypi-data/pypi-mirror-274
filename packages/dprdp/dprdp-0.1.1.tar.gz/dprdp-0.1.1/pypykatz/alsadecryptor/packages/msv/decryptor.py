...
import json
import base64
from pypykatz.commons.common import WindowsBuild, WindowsMinBuild, KatzSystemArchitecture, AGenericReader, \
    UniversalEncoder, hexdump
...
class MsvCredential:
    ...
    def to_dict(self):
        t = {'username': self.username, 'domainname': self.domainname, 'NThash': self.NThash, 'LMHash': self.LMHash,
             'SHAHash': self.SHAHash, 'DPAPI': self.DPAPI}
...
class CredmanCredential:
    ...
    def to_dict(self):
        t = {'credtype': self.credtype, 'username': self.username, 'domainname': self.domainname,
             'password': self.password, 'password_raw': self.password_raw, 'luid': self.luid}
        return t
...
class LogonSession:
    grep_header = ['packagename', 'domain', 'user', 'NT', 'LM', 'SHA1', 'masterkey', 'sha1_masterkey', 'key_guid',
                   'plaintext']
    ...
    def to_dict(self):
        t = {'authentication_id': self.authentication_id, 'session_id': self.session_id, 'username': self.username,
             'domainname': self.domainname, 'logon_server': self.logon_server, 'logon_time': self.logon_time,
             'sid': self.sid, 'luid': self.luid, 'msv_creds': [], 'wdigest_creds': [], 'ssp_creds': [],
             'livessp_creds': [], 'dpapi_creds': [], 'kerberos_creds': [], 'credman_creds': [], 'tspkg_creds': [],
             'cloudap_creds': []}
    ...
    def to_row(self):
        for cred in self.msv_creds:
            t = cred.to_dict()
            yield [self.luid, 'msv', self.session_id, self.sid, 'msv', '', self.domainname, self.username, 'NT',
                   t['NThash'].hex() if t['NThash'] else '']
            yield [self.luid, 'msv', self.session_id, self.sid, 'msv', '', self.domainname, self.username, 'LM',
                   t['LMHash'].hex() if t['LMHash'] else '']
            yield [self.luid, 'msv', self.session_id, self.sid, 'msv', '', self.domainname, self.username, 'sha1',
                   t['SHAHash'].hex() if t['SHAHash'] else '']
        for cred in self.wdigest_creds:
            t = cred.to_dict()
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'plaintext', t['password']]
        for cred in self.ssp_creds:
            t = cred.to_dict()
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'plaintext', t['password']]
        ...
        for cred in self.dpapi_creds:
            t = cred.to_dict()
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'masterkey', t['masterkey']]
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'sha1', t['sha1_masterkey']]
        for cred in self.kerberos_creds:
            t = cred.to_dict()
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'plaintext', t['password']]
        ...
        for cred in self.cloudap_creds:
            t = cred.to_dict()
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'masterkey', str(cred.get_masterkey_hex())]
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'sha1', str(t['dpapi_key_sha1'])]
            yield [self.luid, t['credtype'], self.session_id, self.sid, t['credtype'], '', self.domainname,
                   self.username, 'PRT', str(t['PRT'])]
    ...
    def to_grep_rows(self):
        ...
        for package in [self.wdigest_creds, self.ssp_creds, self.livessp_creds, self.credman_creds, self.tspkg_creds]:
            for cred in package:
                ...
                if t['password'] is not None:
                    yield [str(t['credtype']), str(t['domainname']), str(t['username']), '', '', '', '', '', '',
                           str(t['password'])]
        ...
        for cred in self.dpapi_creds:
            t = cred.to_dict()
            yield [str(t['credtype']), '', '', '', '', '', str(t['masterkey']), str(t['sha1_masterkey']),
                   str(t['key_guid']), '']

        for cred in self.cloudap_creds:
            t = cred.to_dict()
            yield [str(t['credtype']), '', '', '', '', '', str(cred.get_masterkey_hex()), str(t['dpapi_key_sha1']),
                   str(t['key_guid']), str(t['PRT'])]


class MsvDec(PackageDecryptor):
    ...
    async def find_first_entry(self):
        ...
        if self.sysinfo.architecture == KatzSystemArchitecture.X64:
            if self.sysinfo.buildnumber >= WindowsBuild.WIN_8.value or (
                    WindowsMinBuild.WIN_8.value <= self.sysinfo.buildnumber < WindowsMinBuild.WIN_BLUE.value and self.sysinfo.msv_dll_timestamp > 0x60000000):
    ...
    async def parse_credman_credentials(self, logon_session):
        ...
        credman_set_list_entry = await logon_session.CredentialManager.read(self.reader,
                                                                            override_finaltype=KIWI_CREDMAN_SET_LIST_ENTRY)
        await self.log_ptr(credman_set_list_entry.list1.value, 'KIWI_CREDMAN_LIST_STARTER')
        list_starter = await credman_set_list_entry.list1.read(self.reader,
                                                               override_finaltype=KIWI_CREDMAN_LIST_STARTER)
        if list_starter.start.value != list_starter.start.location:
            await self.walk_list(list_starter.start, self.add_credman_credential,
                                 override_ptr=self.credman_decryptor_template.list_entry)
    ...
    async def add_credman_credential(self, credman_credential_entry):
        ...
        if credman_credential_entry.cbEncPassword and credman_credential_entry.cbEncPassword != 0:
            enc_data = await credman_credential_entry.encPassword.read_raw(self.reader,
                                                                           credman_credential_entry.cbEncPassword)
