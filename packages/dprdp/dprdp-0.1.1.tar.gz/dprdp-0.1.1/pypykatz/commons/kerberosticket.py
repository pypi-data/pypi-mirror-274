...
class KerberosTicket:
    ...
    def to_dict(self):
        ...
        t = {'type': self.type, 'ServiceName': self.ServiceName, 'DomainName': self.DomainName,
             'ETargetName': self.ETargetName, 'TargetDomainName': self.TargetDomainName,
             'EClientName': self.EClientName, 'AltTargetDomainName': self.AltTargetDomainName,
             'StartTime': self.StartTime, 'EndTime': self.EndTime, 'RenewUntil': self.RenewUntil,
             'KeyType': self.KeyType, 'Key': self.Key}

        return t

    def to_asn1(self):
        krbcredinfo = {'key': EncryptionKey({'keytype': self.KeyType, 'keyvalue': self.Key}),
                       'prealm': self.AltTargetDomainName,
                       'pname': PrincipalName({'name-type': self.EClientName_type, 'name-string': self.EClientName}),
                       'flags': core.IntegerBitString(self.TicketFlags).cast(TicketFlags), 'starttime': self.StartTime,
                       'endtime': self.EndTime, 'renew-till': self.RenewUntil, 'srealm': self.DomainName,
                       'sname': PrincipalName({'name-type': self.ServiceName_type, 'name-string': self.ServiceName})}

        enc_krbcred = {'ticket-info': [KrbCredInfo(krbcredinfo)]}

        ticket = {'tkt-vno': krb5_pvno, 'realm': self.DomainName,
                  'sname': PrincipalName({'name-type': NAME_TYPE.SRV_INST.value, 'name-string': self.ServiceName}),
                  'enc-part': EncryptedData(
                      {'etype': self.TicketEncType, 'kvno': self.TicketKvno, 'cipher': self.Ticket})}

        krbcred = {'pvno': krb5_pvno, 'msg-type': MESSAGE_TYPE.KRB_CRED.value, 'tickets': [Ticket(ticket)],
                   'enc-part': EncryptedData(
                       {'etype': EncryptionType.NULL.value, 'cipher': EncKrbCredPart(enc_krbcred).dump()})}
    ...
    def generate_filename(self):
        t = '%s' % ('_'.join([self.type.name, self.DomainName, '_'.join(self.EClientName), '_'.join(self.ServiceName),
                              hashlib.sha1(self.Ticket).hexdigest()[:8]]))
