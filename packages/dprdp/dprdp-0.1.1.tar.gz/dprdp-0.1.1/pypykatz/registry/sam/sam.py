...
class SAM:
    ...
    @staticmethod
    def rid_to_key(rid):
        key = int(rid, 16).to_bytes(4, 'little')
    ...
    def decrypt_hash(self, rid, hashobj, constant):
        ...
        if isinstance(hashobj, SAM_HASH):
            rc4key = hashlib.md5(
                self.hashed_bootkey[:0x10] + int(rid, 16).to_bytes(4, 'little') + constant).digest()
            ...
            n = 16
            for block in [hashobj.data[i:i + n] for i in range(0, len(hashobj.data), n)]:
    ...
    def get_HBoot_key(self):
        ...
        if isinstance(domain_properties.key_0, SAM_KEY_DATA):
            ...
            n = 16
            for block in [domain_properties.key_0.data[i:i + n] for i in
                          range(0, len(domain_properties.key_0.data), n)]:
    ...
    def get_machine_sid(self):
        ...
        try:
            ...
            p1 = int.from_bytes(uac_data[:4], 'little')
            p2 = int.from_bytes(uac_data[4:8], 'little')
            p3 = int.from_bytes(uac_data[8:12], 'little')
    ...
    def to_dict(self):
        t = {'HBoot_key': self.hashed_bootkey, 'local_users': []}
