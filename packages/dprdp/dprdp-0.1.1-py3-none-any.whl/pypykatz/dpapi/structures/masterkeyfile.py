    ...
    def decrypt(self, key):
        ...
        while len(temp_key_blob) < keylen:
            ...
            for _ in range(self.iteration_count - 1):
                ...
                derived = (int.from_bytes(derived, sys.byteorder) ^ int.from_bytes(actual, sys.byteorder)).to_bytes(
                    len(actual), sys.byteorder)
        ...
        cipher = ALGORITHMS_DATA[self.crypto_algorithm][1](crypt_key, mode=ALGORITHMS_DATA[self.crypto_algorithm][2],
                                                           IV=iv)
