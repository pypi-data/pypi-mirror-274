...
class DpapiCredential:
    ...
    def to_dict(self):
        t = {'credtype': self.credtype, 'key_guid': self.key_guid, 'masterkey': self.masterkey,
             'sha1_masterkey': self.sha1_masterkey, 'luid': self.luid}
        return t
...
class DpapiDec(PackageDecryptor):
    ...
    async def add_entry(self, dpapi_entry):
        ...
        if dpapi_entry and dpapi_entry.keySize > 0:
