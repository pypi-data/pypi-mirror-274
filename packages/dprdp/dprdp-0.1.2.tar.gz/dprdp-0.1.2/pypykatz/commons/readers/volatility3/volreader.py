...
class Vol3Reader:
    ...
    def get_buildnumber(self):
        ...
        if self.framework_version == 1:
            ...
            vol_object = self.vol_obj.context.modules[
                             self.vol_obj.config['kernel']].symbol_table_name + constants.BANG + "_PEB"
        else:
            raise Exception('Unsupported Volatility Framework Version')
    ...
    def get_arch(self):
        if self.framework_version == 1:
            symbol_table_is_64bit = symbols.symbol_table_is_64bit(self.vol_obj.context,
                                                                  self.vol_obj.config["nt_symbols"])
        elif self.framework_version == 2:
            symbol_table_is_64bit = symbols.symbol_table_is_64bit(self.vol_obj.context, self.vol_obj.context.modules[
                self.vol_obj.config['kernel']].symbol_table_name)
        else:
            raise Exception('Unsupported Volatility Framework Version')
