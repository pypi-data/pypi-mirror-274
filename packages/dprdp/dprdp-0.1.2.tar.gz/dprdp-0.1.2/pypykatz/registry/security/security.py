...
class SECURITY:
    ...
    def decrypt_lsa_key(self, data):
        ...
        if self.lsa_secret_key_vista_type is True:
            ...
            cipher = AES(key)
            n = 16
            for block in [record.data[32:][i:i + n] for i in range(0, len(record.data[32:]), n)]:
                if len(block) < n:
    ...
    def decrypt_secret(key, value):
        dec_blob = b''
        enc_size = int.from_bytes(value[:4], 'little')
            ...
            if record.IV != b'\x00' * 16:
                if record.Flags & 1 == 1:
                    ...
                    if self.lsa_secret_key_vista_type is True:
                        ...
                        n = 16
                        for block in [record.EncryptedData[i:i + n] for i in range(0, len(record.EncryptedData), n)]:
                            if len(block) < 16:
                                block += b'\x00' * (16 - len(block))
                ...
                secret = LSADCCSecret(version, domain, username, dcc_hash, iteration=self.dcc_iteration_count,
                                      last_write_ts=record.LastWrite)
    ...
    def to_dict(self):
        t = {
            'dcc_iteration_count': self.dcc_iteration_count,
            'secrets_format': 'VISTA' if self.lsa_secret_key_vista_type else 'OLD', 'lsa_key': self.lsa_key,
            'NK$LM': None
        }
