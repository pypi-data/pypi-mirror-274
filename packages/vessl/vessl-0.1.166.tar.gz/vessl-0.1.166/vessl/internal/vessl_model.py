...
from vessl.cli._util import print_info
from vessl.service import RunnerBase, _load_model_v2, _ModelServer, _run_model_server
...
class VesslModel:
    ...
    def _validate_from_requirements(self):
        try:
            with open(self.requirements_path, "r") as f:
                requirements_st = f.read()
                r = parse_requirements(requirements_st)
                reqs = {}
                for req in r:
                ...
                if "tensorflow" in reqs:
                    _validate_version("tensorflow", "requirements", self.tensorflow_version, reqs["tensorflow"])
                if "python" in reqs:
            ...
            with open(self.pyproject_path, "r") as f:
                ...
                if deps is not None:
                    ...
                    if "tensorflow" in deps:
                        _validate_version("tensorlow","pyproject", self.tensorflow_version, deps["tensorflow"])
                    if "python" in deps:
                        _validate_version("python", "pyproject", self.python_version, deps["python"])
                ...
                if deps is not None:
                    ...
                    if "tensorflow" in deps:
                        _validate_version("tensorlow","poetry", self.tensorflow_version, deps["tensorflow"])
                    if "python" in deps:
    ...
    def save_lockfile(self):
        with open(".vessl.model.lock", "w") as f:
            f.write(self.build_lockfile())

    @staticmethod
    def create_ignore_file():
        if not os.path.exists(VESSL_IGNORE_FILE_PATH):
            ...
            with open(VESSL_IGNORE_FILE_PATH, "w") as f:
    ...
    def install_deps(self):
        if self.requirements_path is not None:
            ...
            if API_HOST == "https://api.dev.vssl.ai":
                command.extend(["--index-url", "https://test.pypi.org/simple", "--extra-index-url", "https://pypi.org/simple", "--pre"])
