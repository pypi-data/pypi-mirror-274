...
from metrics import MESSAGE_METRICS, TOKEN_METRICS
...
class Context:
    ...
    def __init__(
        ...
        self.prompt_parts = self._load_yaml(rendered_template)
    ...
    def _load_yaml(self, rendered_template: str) -> list[PromptPart]:
        ...
        try:
            ...
            self._logger.error(
                f"### Hermes: Error loading yaml from rendered template: {rendered_template=}; {ex=}",
                exc_info=True,
            )
            raise ex
    ...
    def _observe_context_metrics(self):
        ...
        # TODO: use constants.
        TOKEN_METRICS.labels(metric_type="total_tokens").observe(self.num_total_tokens)
        TOKEN_METRICS.labels(metric_type="truncated_tokens").observe(
            self.num_tokens_truncated
        )
        TOKEN_METRICS.labels(metric_type="active_tokens").observe(
            self.total_active_tokens
        )
        MESSAGE_METRICS.labels(metric_type="total_messages").observe(
            self.num_total_messages
        )
        MESSAGE_METRICS.labels(metric_type="truncated_messages").observe(
