...
def create_ordenes_financieras(df, month_to_predict, ordenes_aprobadas=None, service=None):
    """
    ...
    - month_to_predict (pd.DataFrame): Dataframe containing predicted values for month.
    ...
    if service == "SS":
        ...
        date_strings = [f'2024-07-{day}' for day in average_weight_by_day.index]
        ...
        average_weight_by_day.set_index(pd.date_range("2024-07-01","2024-07-31"), inplace=True)
        ...
        df_month_to_predict = month_to_predict['prediction']
        series_reindexed = df_month_to_predict.reindex(average_weight_by_day.index, fill_value=0)
        # if there are diferent numbers of approved orders then use it.
        if ordenes_aprobadas:
            ordenes_aprobadas = ordenes_aprobadas
            prop_ordenes_dias_mes = series_reindexed/series_reindexed.sum()
