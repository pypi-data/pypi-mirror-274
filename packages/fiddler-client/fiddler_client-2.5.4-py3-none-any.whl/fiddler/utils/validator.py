...
class ValidationModule:
    ...
    def create_mock_df(self) -> pd.DataFrame:
        df_dict = {}
        for column in self.model.inputs:  # type: ignore
            data: Any = None
...
class ModelValidator(ValidationModule):
    ...
    def _check_outputs_match(self) -> None:
        ...
        model_output_cols = [col.name for col in self.model.outputs]  # type: ignore
    ...
    def _check_classification_type_output_count(self) -> None:
        ...
        if task == ModelTask.BINARY_CLASSIFICATION and len(outputs) > 1:  # type: ignore
            self.append_error(
                'Binary classification task specified with more than one output column'
            )
        elif task == ModelTask.MULTICLASS_CLASSIFICATION and len(outputs) < 2:  # type: ignore
            self.append_error(
