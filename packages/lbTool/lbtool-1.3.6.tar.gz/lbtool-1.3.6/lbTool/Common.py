...
def base64_encode(raw_data, is_bytes=False, charset="utf-8"):
    # type: (str|bytes|bytearray, bool, str) -> str|bytes
    """
    Base64加密
    :param raw_data: 待加密数据
    :param is_bytes: 是否返回byte数据
...
def base64_decode(raw_data, is_bytes=False, charset="utf-8"):
    # type: (str|bytes|bytearray, bool, str) -> str|bytes
    """
    Base64解密
...
def to_hex(raw_str, ret_bytes=False, charset="utf8"):
    """
    转为16进制
    :param raw_str: 原始字符串
    ...
    :return:
    """
    hex_bytes = binascii.hexlify(raw_str.encode())
    if ret_bytes is True:
        return hex_bytes
    return hex_bytes.decode(charset)


def from_hex(hex_str, ret_bytes=False, charset="utf8"):
    """
    解析16进制为字符串
