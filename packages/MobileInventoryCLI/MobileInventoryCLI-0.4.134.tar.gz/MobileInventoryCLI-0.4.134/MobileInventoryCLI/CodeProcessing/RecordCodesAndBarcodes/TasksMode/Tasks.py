...
use of 'pint' as converter can be used with CVT(value,from,to)
...
{Fore.light_steel_blue}{'|'.join(replace_pallet)}{Style.reset}'''
                                    ...
                                    textO=ReParseFormula(formula=text,casecount=multipliers.get('#'),suffixes=replace_case)
                                    textO=ReParseFormula(formula=str(textO),casecount=multipliers.get('@'),suffixes=replace_unit)
                                    textO=ReParseFormula(formula=str(textO),casecount=multipliers.get('^'),suffixes=replace_load)
                                    textO=ReParseFormula(formula=str(textO),casecount=multipliers.get('%'),suffixes=replace_pallet)
                                    textO=ReParseFormula(formula=str(textO),casecount=multipliers.get('$'),suffixes=replace_shelf)
                                    textO=str(textO)
                                    print(textO)
                                    if MODE.startswith("+"):
...
{Fore.grey_50}InList - {color_2}{Style.bold}15{Style.reset}'''
                        ...
                        p=Prompt.__init2__(None,func=mkT,ptext="amount|+amount|-amount|a,+a,-a(advanced)|r,+r,-r(ReParseFormula) (Enter==1)",helpText=self.helpText_barcodes,data=code)
                        if p in [None,]:
                            return
                        if p:
                            value,text,suffix=p
