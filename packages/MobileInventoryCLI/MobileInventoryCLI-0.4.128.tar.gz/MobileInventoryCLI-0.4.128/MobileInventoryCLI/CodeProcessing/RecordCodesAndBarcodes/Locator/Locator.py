...
MatchMsg=f'''{Fore.light_green}
 ...
 ...   ...                      
 ...   ...          ...         
 ...   ...          ...         {Style.reset}'''.replace(" ",f"{Fore.grey_50},{Style.reset}").replace('.',f'{Fore.orange_red_1}{"H"*1}{Style.reset}')
...
not in the Entry Table{Style.reset}
	...
	def __init__(self,engine):
		self.engine=engine
		while True:	
			product_barcode=input(f"{Fore.cyan}[99]Barcode{Style.reset}: ")	
			if product_barcode.lower() in ['b','back']:
					return
			elif product_barcode.lower() in ['q','quit']:
				exit("user quit!")
			...
			pc.PossibleCodesEAN13(scanned=product_barcode)
			while True:
				shelf_code=input(f"{Fore.magenta}[99]Shelf Code{Fore.red}[or Barcode to exit]{Style.reset}: ")
				#if shelf_code == product_barcode and shelf_code not in ['?','help']:
				#	print(f"{Fore.green_yellow}Barcode == Shelf Code - exit action taken!{Style.reset}")
				#	return
				#el
