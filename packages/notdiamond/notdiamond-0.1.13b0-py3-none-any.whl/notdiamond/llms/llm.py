...
class NDLLM(LLM):
    ...
    def create(
        ...
        **kwargs,
        ...
        Parameters:
            ...
            preference_weights (Optional[Dict[str, float]], optional): Define the quality, cost and latency weights
                                                                        of importance for the router.
    ...
    async def acreate(
        self,
        messages: List[Dict[str, str]],
        model: List[NDLLMProvider] = None,
        max_model_depth: Optional[int] = None,
        preference_weights: Optional[Dict[str, str]] = None,
        **kwargs,
    ) -> tuple[Union[AIMessage, BaseModel], str, NDLLMProvider]:
        """
        ...
        Parameters:
            messages (Optional[List[Dict[str, str]], optional): Can be used instead of prompt_template to pass
                the messages OpenAI style.
            model (List[str]): List of models to choose from.
            max_model_depth (Optional[int]): If your top recommended model is down, specify up to which depth
                                                of routing you're willing to go.
            preference_weights (Optional[Dict[str, float]], optional): Define the quality, cost and latency weights
                                                                        of importance for the router.
        ...
        result = await self.ainvoke(messages=messages, **kwargs)
        return result
