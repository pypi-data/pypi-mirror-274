...
# def ThreadFuncUpdateCosSig(cos_client, region, build_path):
#     # update cos sig every 15 * 60 ms (15min)
#     waited_s = 0
#     # to_wait_s = 15
#     while not gUploadEnd:
#         if waited_s < kCosSigUpdateInternal:
#             time.sleep(3)
#             waited_s += 3
#             continue
#         waited_s = 0
#         cosAuth = pgos_backend_apis.RequestCosSig(region, build_path)
#         assetCredentials = cosAuth['BuildCredentials']
#         if "TmpSecretId" not in assetCredentials or "TmpSecretKey" not in assetCredentials or "Token" not in assetCredentials:
#             continue
#         secret_id = assetCredentials["TmpSecretId"]
#         secret_key = assetCredentials["TmpSecretKey"]
#         token = assetCredentials["Token"]
#         new_conf = config = CosConfig(Region=region, SecretId=secret_id, SecretKey=secret_key, Token=token)
#         cos_client.update_conf(new_conf)
...
def UploadAsset(command_name, region, res_path, new:bool):
    sig_api_list = [
    ...
    while True:
        exc_msg = ''
        try:
            cosAuth = pgos_backend_apis.RequestCosSig(sig_api_2023_09_15, region, res_path, command_name)
            assetCredentials = cosAuth['BuildCredentials']
