...
def format_pan_path(pan_path: str):
    ...
    if '/' in result:
        return '/'.join([tmp.strip() for tmp in result.split('/')])
    else:
        return result.strip()
...
def list_all_file(access_token: str, dir_path: str, recursion: int = 0,
    ...
    if _result and hasattr(_result, 'file_list') and _result.file_list:
        ...
        page_num = 1
        while _result and (_result.errno == 0 or _result.errno is None) and hasattr(_result, 'has_more') and _result.has_more:
            page_num = page_num + 1
