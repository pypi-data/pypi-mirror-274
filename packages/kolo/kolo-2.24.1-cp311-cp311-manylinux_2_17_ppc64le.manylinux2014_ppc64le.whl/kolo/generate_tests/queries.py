...
class AssertInsert:
    def __init__(
        self, variable_name, model, table, fields, import_path=None, query=None
    ):
    ...
    def get_template_parts(self, fields):
    ...
    def template_parts(self):
        fields, rest = self.get_fields()

        yield from self.get_template_parts(fields)
...
class AssertDelete:
    def __init__(self, model, fields, query=None):
        self.model = model
        self.fields = fields
        self.query = query
    ...
    @property
    def template_parts(self):
        yield f"self.assertFalse({self.model}.objects.filter("
        for name, fields in self.grouped_fields().items():
            if len(fields) == 1:
                yield f"{name}={fields[0].value_repr},"
            else:
                yield f"{name}__in=("
                for field in fields:
                    yield f"{field.value_repr},"
                yield "),"
        yield ").exists())"

    @property
    def pytest_template_parts(self):
        yield f"assert not {self.model}.objects.filter("
        for name, fields in self.grouped_fields().items():
            if len(fields) == 1:
                yield f"{name}={fields[0].value_repr},"
            else:
...
class DjangoField:
    ...
    def from_identifier(cls, identifier, value, table_schema, parser):
        ...
        if isinstance(value, sqlglot.expressions.Null):
            ...
            if field_schema["django_field"] == "django.db.models.fields.DecimalField":
                value = value.name
            else:
...
class SQLParser:
    ...
    def build_creates(self, query_dict, columns, row, seen_mutations, schema_data):
        ...
        return inserts, imports, skipped_inserts
    ...
    def parse_sql_queries(self, sql_queries, schema_data):
            ...
            if isinstance(parsed_query, sqlglot.exp.Select):
                ...
                if not any(
                    ...
                    if (
                        ...
                        if isinstance(where.this, sqlglot.expressions.EQ):
                            ...
                            new_inserts, new_imports, new_skips = self.build_creates(
                                query_dict,
                                [column],
                                [value],
                                seen_mutations,
                        ...
                        value = extract_value(where.this.expression)
                        new_inserts, new_imports, new_skips = self.build_creates(
                            query_dict, [column], [value], seen_mutations, schema_data
                        )
                        inserts.extend(new_inserts)
                        imports.update(new_imports)
                        skipped_inserts.extend(new_skips)
                ...
                if isinstance(parsed_query, sqlglot.expressions.Update):
                    ...
                    for fields in self.build_delete_lookup_fields(where, table_schema):
                        assert_delete = AssertDelete(model, fields, query_dict)
                        asserts.append(assert_delete)
                    import_path = f"from {module} import {model}"
        ...
        value_imports = build_value_imports(sql_fixtures, asserts)
        return sql_fixtures, imports, asserts, value_imports, skipped_inserts
...
def merge_creates(creates):
    ...
    for create in creates:
        for field in create.fields:
            if (field.name, field.value_repr) not in seen:
...
def build_value_imports(creates, asserts):
    imports = set()
    for query in chain(creates, asserts):
        for field in query.fields:
            if isinstance(field.value, Decimal):
                imports.add("from decimal import Decimal")
            elif isinstance(field.value, (date, datetime, time, timedelta, timezone)):
