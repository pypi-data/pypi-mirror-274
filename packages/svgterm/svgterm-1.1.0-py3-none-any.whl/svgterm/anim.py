...
# svgterm can distinguish FG_BG_256[0] (which defaults to black #000000 but
...
SVGTERM_NS = 'https://github.com/khulnasoft-lab/svgterm'
...
def render_still_frames(frames, geometry, directory, template,
    ...
    for frame_count, frame_root in enumerate(frame_generator):
        filename = os.path.join(directory, 'svgterm_{:05}.svg'.format(frame_count))
        with open(filename, 'wb') as output_file:
...
def _add_animation(root, timings, animation_duration):
    ...
    settings = root.find('.//{{{}}}defs/{{{}}}template_settings'
                         .format(SVG_NS, SVGTERM_NS))
    if settings is None:
        raise TemplateError('Missing "template_settings" element in definitions')

    animation = settings.find('{{{}}}animation[@type]'.format(SVGTERM_NS))
...
def validate_svg(svg_file):
    ...
    try:
        ...
        for bad in tree.xpath('/svg:svg/svg:defs/svgterm:template_settings',
