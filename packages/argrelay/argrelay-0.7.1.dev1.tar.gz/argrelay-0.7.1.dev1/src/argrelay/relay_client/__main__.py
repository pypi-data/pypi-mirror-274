...
from argrelay.misc_helper_common import get_config_path, eprint
...
def main():
    ...
    # TODO: clean up:
    if not shell_ctx.is_debug_enabled:
        eprint(TermColor.debug_output.value, end = "")
        eprint("child: after split", end = "")
    ...
    if client_config.use_local_requests:
        ...
        if not shell_ctx.is_debug_enabled:
            eprint(TermColor.debug_output.value, end = "")
            eprint("child: before request", end = "")
            eprint(TermColor.reset_style.value, end = " ")
        ...
        if not shell_ctx.is_debug_enabled:
            eprint(TermColor.debug_output.value, end = "")
            eprint("child: after response", end = "")
            eprint(TermColor.reset_style.value, end = " ")
