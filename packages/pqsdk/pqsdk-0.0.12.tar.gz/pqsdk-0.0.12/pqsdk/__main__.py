...
def handle_config_command():
    ...
    # 将客户端ID和密钥用冒号连接，并编码为Base64
    credentials = f"{client_id}:{client_secret}"
    encoded_credentials = base64.b64encode(credentials.encode()).decode()

    # 设置请求URL和参数
    host = 'portal.tickertrading.com'
...
def main():
    ...
    parser.add_argument('-save_results', action='store_true', help='是否保存结果到csv文件')
    ...
    if args.command == 'backtest':
        # 执行回测
        run_backtest(args.file, params, args.plot, args.save_results, args.save_orders)
