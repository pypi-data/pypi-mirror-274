...
class Chat:
    ...
    def initialize_new_chat(self, chat_name):
        if chat_name != "chat":
            chat_name = f"{chat_name}-chat"
        chat_name = f"{chat_name}.md"
        open(chat_name, 'a', encoding='utf-8').close()
        return chat_name

    def start(self):
        print("Start chatting (type 'GO' to exec last prompt, 'CONTINUE' to exec last prompt from chat *.md file or 'END' to exit chat mode):")
        while True: 
    ...
    def get_user_input(self):
        ...
        while True:
            line = input()
            if line in ["END", "GO", "CONTINUE"]:
                return f"{user_input.strip()}" if line == "GO" else line
            user_input += f"{line} "
