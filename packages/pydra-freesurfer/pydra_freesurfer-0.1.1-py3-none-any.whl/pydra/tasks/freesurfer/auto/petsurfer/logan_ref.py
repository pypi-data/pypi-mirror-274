...
LoganRef_input_spec = specs.SpecInfo(
...
LoganRef_output_spec = specs.SpecInfo(
    name="Output", fields=output_fields, bases=(specs.ShellOutSpec,)
)


class LoganRef(ShellCommandTask):
    """
    Examples
    -------
    ...
    >>> from fileformats.medimage import Nifti1
    >>> from pydra.engine.specs import MultiOutputType
    >>> from pydra.tasks.freesurfer.auto.petsurfer.logan_ref import LoganRef

    >>> task = LoganRef()
    >>> task.inputs.logan = ("ref_tac.dat", "timing.dat", 2600)
    >>> task.inputs.glm_dir = "logan"
    ...
    input_spec = LoganRef_input_spec
    output_spec = LoganRef_output_spec
