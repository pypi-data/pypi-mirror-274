...
#------------------------------------------------------- 
    ...
    def get_data(self, tickers, fields=None, start='', end='', freq='', kind=1):
        ...
        try:
            retName = False
            req, retName = self.post_user_request(tickers, fields, start, end, freq, kind)
...
#-------Helper Functions---------------------------------------------------
    ...
    def _format_Response(self, response_json):
        ...
        if (len(dates_converted) == len(dataframe.index)):
            ...
            if (len(dataframe.index) > len(dates_converted)):
                dataframe.index = dataframe['Dates']
                dataframe = dataframe.drop(columns='Dates', axis = 1)
