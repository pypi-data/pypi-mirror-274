...
def test_create_local_venv_success(tmp_path):
    ...
    with (
        ...
        patch(
            "qbraid_core.services.environments.create.update_install_status"
        ...
        scripts_path = venv_path / ("Scripts" if os.sys.platform == "win32" else "bin")
        activate_files = (
        ...
        mock_update_status.assert_called_once_with(slug_path, 1, 1)
...
def test_create_local_venv_failure(tmp_path):
    ...
    with (
        ...
        mock_update_status.assert_called_once_with(slug_path, 1, 0, message="Test Error")
