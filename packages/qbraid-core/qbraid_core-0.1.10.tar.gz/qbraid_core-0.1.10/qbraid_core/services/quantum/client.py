...
from .compat import load_local_result, qir_job_dispatch, transform_device_data
...
class QuantumClient(QbraidClient):
    ...
    def search_jobs(self, query: Optional[dict[str, Any]] = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            return [load_local_result(job) for job in jobs_data]
        except RequestsApiError as err:
    ...
    def get_job(
        ...
        if job_data.get("qbraidDeviceId") == "qbraid_qir_simulator":
            job_data = load_local_result(job_data)

        return job_data

    def create_job(self, data: dict[str, Any]) -> dict[str, Any]:
        """Creates a new quantum job with the given data."""
        if data.get("qbraidDeviceId") == "qbraid_qir_simulator":
            data = qir_job_dispatch(data)
