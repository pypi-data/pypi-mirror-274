...
@njit(fastmath=True)
def prdist_5s(tc, t0, p, x0, y0, vx, vy, ax, ay, jx, jy, sx, sy):
...
@njit(fastmath=True)
def prdist_4s(tc, t0, p, x0, y0, vx, vy, ax, ay, jx, jy):
    """Normalized planet-star center distance using Taylor series expansion.
    """
    epoch = floor((tc - t0 + 0.5 * p) / p)
    t = tc - (t0 + epoch * p)
    t2 = t * t
    t3 = t2 * t
    px = x0 + vx * t + 0.5 * ax * t2 + jx * t3 / 6.0
    py = y0 + vy * t + 0.5 * ay * t2 + jy * t3 / 6.0
    return sqrt(px ** 2 + py ** 2)
...
def find_contact_point(k: float, point: int, y0, vx, vy, ax, ay, jx, jy, sx, sy):
    ...
    z0 = prdist_5s(t0, 0.0, 1.0, y0, vx, vy, ax, ay, jx, jy, sx, sy) - zt
    z1 = prdist_5s(t1, 0.0, 1.0, y0, vx, vy, ax, ay, jx, jy, sx, sy) - zt

    i = 0
    while abs(t2 - t0) > 1e-6 and i < 100:
        if z0*z1 < 0.0:
            t1, t2 = 0.5*(t0 + t1), t1
            z1, z2 = prdist_5s(t1, 0.0, 1.0, y0, vx, vy, ax, ay, jx, jy, sx, sy) - zt, z1
        else:
            t0, t1 = t1, 0.5*(t1 + t2)
            z0, z1 = z1, prdist_5s(t1, 0.0, 1.0, y0, vx, vy, ax, ay, jx, jy, sx, sy) - zt
        i += 1
    return t1
...
def find_z_min(tc, p, y0, vx, vy, ax, ay, jx, jy, sx, sy):
    ...
    f1 = prdist_5s(x1, tc, p, y0, vx, vy, ax, ay, jx, jy, sx, sy)
    f2 = prdist_5s(x2, tc, p, y0, vx, vy, ax, ay, jx, jy, sx, sy)

    i = 0
    while abs(x3 - x0) > 1e-7 and i < 100:
        if f2 < f1:
            x0, x1, x2 = x1, x2, r*x2 + c*x3
            f1, f2 = f2, prdist_5s(x2, tc, p, y0, vx, vy, ax, ay, jx, jy, sx, sy)
        else:
            x3, x2, x1 = x2, x1, r*x1 + c*x0
            f2, f1 = f1, prdist_5s(x1, tc, p, y0, vx, vy, ax, ay, jx, jy, sx, sy)
        i += 1
