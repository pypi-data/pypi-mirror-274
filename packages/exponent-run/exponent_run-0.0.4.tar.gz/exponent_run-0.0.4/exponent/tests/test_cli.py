...
async def test_get_system_context(
    ...
    async with RemoteExecutionClient.session(
        ...
        request = requests[0]
        assert isinstance(request, SystemContextRequest)
        assert request == SystemContextRequest(
            correlation_id="123456",
        )

        response = get_system_context(request, client.working_directory)
...
async def test_get_file_contents(
    ...
    async with RemoteExecutionClient.session(
        ...
        response = files.get_file_contents(request, client.working_directory)
...
async def test_get_matching_files(
    ...
    async with RemoteExecutionClient.session(
        ...
        response1 = files.get_matching_files(request1, client.file_cache)
        assert response1 == GetMatchingFilesResponse(
            correlation_id="123456", file_paths=["test1.py", "test2.py", "exponent.txt"]
        )
        ...
        response2 = files.get_matching_files(request2, client.file_cache)
...
async def test_get_matching_files_nested(
    ...
    async with RemoteExecutionClient.session(
        ...
        # TODO: Bring back assertion that match on the matched files
        for search_term, _ in test_cases:
            ...
            response = files.get_matching_files(request, client.file_cache)
...
async def test_execute_code_diff(
    get_execution_requests: mock.Mock,
    tmpdir: Path,
) -> None:
    os.chdir(tmpdir)
    ...
    async with RemoteExecutionClient.session(
        ...
        assert response == CodeExecutionResponse(
