...
from labdiscoverylib.utils import create_token, _current_weblab, _current_backend, \
     _current_session_id, _to_timestamp, _current_timestamp

from labdiscoverylib.config import ConfigurationKeys
from labdiscoverylib.users import WebLabUser, AnonymousUser, ExpiredUser, CurrentUser, \
     get_weblab_user, weblab_user, socket_weblab_user, _set_weblab_user_cache
from labdiscoverylib.backends import RedisManager
from labdiscoverylib.tasks import WebLabTask, _TaskRunner, _TaskWrapper, current_task, current_task_stopping
from labdiscoverylib.ops import status_time, update_weblab_user_data, store_initial_weblab_user_data, dispose_user
from labdiscoverylib.views import weblab_blueprint
...
class WebLab(object):
    ...
    def init_app(self, app, backend=None):
        ...
        doc_link = 'https://developers.labsland.com/labdiscoverylib/'
        ...
        def weblab_poll_script(logout_on_close=False, callback=None):
            ...
            return Markup("""<script>
                ...
                if (window.jQuery !== undefined) {
                    ...
                    var msg = "labdiscoverylib error: jQuery not loaded BEFORE {{ weblab_poll_script() }}. Can't poll";
        ...
        @click.option('--back', default='https://developers.labsland.com/labdiscoverylib/en/stable/', help="URL to send the user back")
    ...
    def run_tasks(self):
        ...
        for task_id in task_ids:
            ...
            try:
                ...
                if hasattr(user.data, 'is_modified') and user.data.is_modified:
                    msg = "labdiscoverylib: you changed weblab_user.data inside a task. You need to call weblab_user.data.store() to upload the data to the server (tasks are long-running so it's risky to just rely on a modification in the end of the task)."
                    warnings.warn(msg)
