...
from finter.framework_model.submission.helper_poetry import copy_poetry_files
...
class NotebookSubmissionHelper:
    ...
    def process(
        ...
        if docker_submit:
            copy_poetry_files(self.model_name)

        # Submit the model if requested
        if submit:
            log_section("Model Submission")
            self.submit_result = submit_model(self.model_info, self.model_name, docker_submit)
