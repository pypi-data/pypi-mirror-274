...
class ModelModule(CoreModule):
    ...
    def execute(self, indir=None, outdir=None):
        ...
        self.origin = self.rupture_obj._origin
        ...
        if isinstance(self.rupture_obj, PointRupture):
            self.rff = RandomFiniteFault(self.origin, 100)
        ...
        if (
            ...
            self.ipe = VirtualIPE.__fromFuncs__(
                self.ipe_gmpe, self.gmice, rupture_obj=self.rupture_obj, rff=self.rff
            )
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            for imt_str in self.imt_out_set:
                ...
                gmpes[oqimt] = gmpe
            ffsim = FFSimmer(self.origin, self.rff)
    ...
    def get_point_amps(self, gmpe, imt_set, dfn):
        ...
        """
        if isinstance(self.rupture_obj, PointRupture):
            ffs = FFSimmer(self.origin, self.rff)
            do_mmi = False
            immi = imt.MMI()
            if immi in imt_set:
                imt_set.remove(immi)
