...
def gcs_multipart_upload(msg: Dict[str, Any]):
    ...
    _log.info(msg)
    post_url = msg["action"]["href"]

    upload_id = initiate_multipart_upload(gen_initiate_multipart_url(post_url))
        ...
        with multimap(cores) as pmap:
            for resp in pmap(
                ...
                if isinstance(resp, ProxyLFSException):
                    ...
                    parts.append(resp)
                    pass
        complete_multipart_upload(gen_upload_specific_url(post_url, upload_id), parts)
