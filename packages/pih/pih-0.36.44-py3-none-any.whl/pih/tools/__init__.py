...
class ListTool:
    ...
    @staticmethod
    def str_diff(a: list[str], b: list[str]) -> list[Any]:
        compare_function: Callable[[str, str], bool] = lambda a, b: StringTool.contains(
            a, b, False
        )
        return [
            i
            for i in a + b
            if not compare_function(i, j(a)) or not compare_function(i, j(b))
        ]
    ...
    def list_of_str_intersection(a: list[str], b: list[str]) -> bool:
        return ListTool.list_of_str_intersection_ratio(a, b) < (len(a) + len(b))

    @staticmethod
    def list_of_str_intersection_ratio(
        a: list[str],
        b: list[str],
    ) -> int:
        return len(ListTool.str_list_diff(a, b))

    @staticmethod
    def str_list_equal(a: list[str], b: list[str]) -> bool:
        return len(ListTool.str_list_diff(a, b)) == 0
...
class StringTool:
    ...
    @staticmethod
    def split(value: str, delimiters: tuple[str, ...]) -> list[str]:
    ...
    def _comparation_ratio_by_tokens(
        ...
        return (
            ListTool.list_of_str_intersection_ratio(a_list, b_list),
            a_list,
            b_list,
        )
