...
def check_item_key(key):
    """
     校验维度表标识符KEY，不能为空，长度小于100，只可以由字母数字下划线组成，且只能字母开头
    :param key: 字符串
    :return:
    """
    if key == '':
        logging.error("维度表标识符不能为空")
        exit(-1)
    elif len(key) > 100:
        logging.error("维度表标识符<{}>长度超限".format(key))
        exit(-1)
    elif key[0].isdigit() or is_chinese(key) or match(key):
        logging.error("维度表标识符<{}>不合法，只可以由字母数字下划线组成，且只能字母开头~".format(key))
        exit(-1)
...
def check_item_key_variables(key):
    ...
    """
    if key == '':
        logging.error("维度表字段不能为空")
        exit(-1)
    elif len(key) > 100:
        logging.error("维度表字段<{}>长度超限".format(key))
        exit(-1)
    elif key[0].isdigit() or is_chinese(key) or match(key):
        logging.error("维度表字段<{}>不合法，只可以由字母数字下划线组成，且只能字母开头~".format(key))
        exit(-1)
...
def check_item_var_exsit(token, key):
    """
     校验维度表标识符KEY是否存在
     :param token: 登录令牌
     :param key: 标识符
     :return:
     """
    item_variables = getdataCenterItemModels(token)
...
def check_item_key_exsit(token, key):
    """
     校验新维度表字段标识符KEY是否存在
     :param token: 登录令牌
     :param key: 标识符
...
def check_item_name_exsit(token, name):
    """
     校验新维度表名称name是否存在
     :param token: 登录令牌
     :param key: 标识符
...
def check_item_var_key_exsit(token, id, key):
    """
     校验维度表字段KEY是否存在
     :param token: 登录令牌
     :param key: 标识符
...
def check_item_var_key_data_exsit(token, old_id):
    """
     校验维度表字段KEY是否存在
     :param token: 登录令牌
     :param key: 标识符
