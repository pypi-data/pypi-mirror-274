...
def økonomi(
    ...
    if _debug_col == 3:
        for i in range(len(_pr_ls)):
            ...
            # Ramme > Single
            _col_ls[i] = str(_col_ls[i]).replace('─', f'{colo("r")}─{colo("u")}') # {Style.RESET_ALL}
            _col_ls[i] = str(_col_ls[i]).replace('┌', f'{colo("r")}┌{colo("u")}') # {Style.RESET_ALL}
            _col_ls[i] = str(_col_ls[i]).replace('│', f'{colo("r")}│{colo("u")}') # {Style.RESET_ALL}
            _col_ls[i] = str(_col_ls[i]).replace('┬', f'{colo("r")}┬{colo("u")}') # {Style.RESET_ALL}
            _col_ls[i] = str(_col_ls[i]).replace('└', f'{colo("r")}└{colo("u")}') # {Style.RESET_ALL}
            ...
            # Tittel
            _col_ls[i] = str(_col_ls[i]).replace("Eksamen",             f'{colo("t")}Eksamen') # {Style.RESET_ALL} Etter alle ord
