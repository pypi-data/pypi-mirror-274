...
import matematikk as mt # Eksamen
...
def økonomi(
    ...
    if _debug_col == 3:
        for i in range(len(_pr_ls)):
            ...
            # _col_ls.append(cc(_pr_ls[i], _col_ramme))
            _col_ls.append(f"{Fore.RED}{_pr_ls[i]}")
            ...
            # Ramme > Single
            _col_ls[i] = str(_col_ls[i]).replace('─', f'{colo("r")}─{Style.RESET_ALL}{colo("u")}')
            _col_ls[i] = str(_col_ls[i]).replace('┌', f'{colo("r")}┌{Style.RESET_ALL}{colo("u")}')
            _col_ls[i] = str(_col_ls[i]).replace('│', f'{colo("r")}│{Style.RESET_ALL}{colo("u")}')
            _col_ls[i] = str(_col_ls[i]).replace('┬', f'{colo("r")}┬{Style.RESET_ALL}{colo("u")}')
            _col_ls[i] = str(_col_ls[i]).replace('└', f'{colo("r")}└{Style.RESET_ALL}{colo("u")}')
            ...
            # Tittel
            _col_ls[i] = str(_col_ls[i]).replace("Eksamen",             f'{colo("t")}Eksamen{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Alle svar",           f'{colo("t")}Alle svar{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Økonomi",             f'{colo("t")}Økonomi{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Pris-funksjon",       f'{colo("t")}Pris-funksjon{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Uttrykk",             f'{colo("t")}Uttrykk{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Inntekt-funksjon",    f'{colo("t")}Inntekt-funksjon{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Størst inntekt",      f'{colo("t")}Størst inntekt{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Enheter",             f'{colo("t")}Enheter{Style.RESET_ALL}')
            _col_ls[i] = str(_col_ls[i]).replace("Pris",                f'{colo("t")}Pris{Style.RESET_ALL}')
