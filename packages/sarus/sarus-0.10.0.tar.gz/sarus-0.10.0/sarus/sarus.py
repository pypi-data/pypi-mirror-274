...
with warnings.catch_warnings():
    ...
    from sarus_data_spec.constants import SCHEMA_TASK, BIG_DATA_TASK
...
class Dataset(DataSpecWrapper[t.Iterator[pa.RecordBatch]]):
    ...
    def sql(self, query: str) -> Dataset:
        ...
        """
        select_ds = select_sql(query)(self._dataspec)
        attributes_ds = self._dataspec.attribute(name=srt.ATTRIBUTES_DS)
        assert attributes_ds is not None
        attach_properties(
            select_ds,
        ...
        push_dataspec_graph(self.client, referrables)
        # TODO: remove when we can compute the pup token of select SQL in SDK
        pull_dataspec_graph(self.client, select_ds.uuid())
