...
def print_virtualenv_path():
    if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
        venv_path = os.getenv('VIRTUAL_ENV')
        if venv_path:
            qtbx_lib_logger.logger_info(f"Virtual environment path: {venv_path}")
        else:
            qtbx_lib_logger.logger_warning(f"Virtual environment path: {venv_path}")
    else:
        qtbx_lib_logger.logger_warning("Not in a Python virtual environment")
