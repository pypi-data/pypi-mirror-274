...
class Connection:
    ...
    def __setup_events(self) -> None:
        ...
        self._reply_only = events.pipe(
            ...
            rxop.map(event_reply_only)
        )  # type: Observable[ReplyOnlyEvent]
...
def event_unknown_response(event: Event[dto.UnknownBinaryResponseEventWrapper]) -> UnknownResponseEvent:
    return event.data.unknown_response


def event_reply_only(event: Event[dto.BinaryReplyOnlyEventWrapper]) -> ReplyOnlyEvent:
