...
from bothpy.trader.constant import (
    Direction,
    Offset,
    Exchange,
    OrderType,
...
from bothpy.trader.gateway import BaseGateway
from bothpy.trader.object import (
    TickData,
    OrderData,
    TradeData,
    PositionData,
...
from bothpy.trader.utility import get_folder_path, ZoneInfo
from bothpy.trader.event import EVENT_TIMER
...
from bothpy_ctp.api import MdApi
...
class CtpMdApi(MdApi):
    ...
    def __init__(self, gateway: JeesGateway) -> None:
        ...
        self.gateway: JeesGateway = gateway
    ...
    def login(self) -> None:
        ...
        self.reqid += 1
        self.reqUserLogin(jees_req, self.reqid)
...
class JeesTdApi(TdApi):
    ...
    def onRspOrderInsert(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        order: OrderData = OrderData(
            ...
            offset=OFFSET_JEES2VT.get(data["CombOffsetFlag"], Offset.NONE),
        ...
        if contract:
            ...
            if not position:
                position = PositionData(
                    ...
                    exchange=contract.exchange,
                    direction=DIRECTION_JEES2VT[data["PosiDirection"]],
                    gateway_name=self.gateway_name,
                )
    ...
    def onRspQryTrade(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        trade: TradeData = TradeData(
            ...
            direction=DIRECTION_JEES2VT[data["Direction"]],
            offset=OFFSET_JEES2VT[data["OffsetFlag"]],
            price=data["Price"],
            volume=data["Volume"],
            datetime=dt,
            gateway_name=self.gateway_name,
    ...
    def onRspQryInstrument(
        ...
        if product:
            contract: ContractData = ContractData(
                symbol=data["InstrumentID"],
                exchange=EXCHANGE_JEES2VT[data["ExchangeID"]],
                name=data["InstrumentName"],
                product=product,
                size=data["VolumeMultiple"],
                pricetick=data["PriceTick"],
                gateway_name=self.gateway_name,
                productID=data.get("ProductID"),  # 合约品种
            )
            ...
            if contract.product == Product.OPTION:
                ...
                contract.option_type = OPTIONTYPE_JEES2VT.get(data["OptionsType"], None)
    ...
    def authenticate(self) -> None:
        ...
        self.reqid += 1
        self.reqAuthenticate(jees_req, self.reqid)
    ...
    def send_order(self, req: OrderRequest) -> str:
        """委托下单"""
        if req.offset not in OFFSET_VT2JEES:
            self.gateway.write_log("请选择开平方向")
            return ""

        if req.type not in ORDERTYPE_VT2JEES:
        ...
        jees_req: dict = {
            ...
            "Direction": DIRECTION_VT2JEES.get(req.direction, ""),
            "CombOffsetFlag": OFFSET_VT2JEES.get(req.offset, ""),
        ...
        self.reqid += 1
        n: int = self.reqOrderInsert(jees_req, self.reqid)
        if n:
    ...
    def cancel_order(self, req: CancelRequest) -> None:
        ...
        self.reqid += 1
        self.reqOrderAction(jees_req, self.reqid)
