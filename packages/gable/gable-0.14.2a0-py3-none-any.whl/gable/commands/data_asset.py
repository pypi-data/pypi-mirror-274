...
def register_data_asset(
    ...
    bucket: Optional[str],
    include: Optional[tuple[tuple[str]]],
    ...
    if source_type in SCHEMA_SOURCE_TYPE_VALUES:
        ...
        response, success, _status_code = client.post_data_asset_ingest(request)
        ...
        if (
            not spark_job_entrypoint
            or not project_root
            or not (connection_string or csv_schema_file)
        ):
            raise click.ClickException(
        ...
        if not library:
            raise click.ClickException(
                f"{EMOJI.RED_X.value} Missing required option --library for Typescript project registration. You can use the --debug or --trace flags for more details."
            )
        if not project_root:
            raise click.ClickException(
        ...
        if not bucket:
            raise click.ClickException(
                f"{EMOJI.RED_X.value} Missing required option --bucket for S3 file registration. You can use the --debug or --trace flags for more details."
            )

        # Flatten the list of tuples if include is not None, else set it to an empty list
        flattened_include = (
            [element for tupl in include for element in tupl] if include else []
        )
        if history:
        ...
        response_pydantic, success, _status_code = register_s3_data_assets(
            ctx, bucket, lookback_days, flattened_include, dry_run, skip_profiling
        )
        response = (
            response_pydantic.dict()
        )  # shim so we can use the same response handling below
    ...
    registered_assets = response["registered"]
...
def check_data_asset(
    ...
    if source_type == SourceType.PYTHON.value:
        ...
        results = check_compliance_s3_data_assets(
            ...
            lookback_days,
            (
                [element for tupl in include for element in tupl]
                if include is not None
                else None
            ),  # TODO - revisit this to see if we can avoid nested tuples
            response_type,  # type: ignore
            skip_profiling,
        )
    else:
