...
import click
import requests
from gable.data_asset import DataAssetResource
...
class GableClient:
    ...
    def __init__(
        ...
        # Resources
        self.data_asset = DataAssetResource(self)
    ...
    def post_data_asset_register_dataframe(
        self, request: RegisterDataAssetDataFrameRequest
    ) -> tuple[Union[IngestDataAssetResponse, ErrorResponseDeprecated], bool, int]:
        response, success, status_code = self._post(
            "v0/data-asset/register/dataframe",
            json=json.loads(request.json(by_alias=True)),
        )
        if isinstance(response, dict) and "registered" in response:
            result = IngestDataAssetResponse.parse_obj(response)
    ...
    def post_data_asset_check_typescript(self, request):
    ...
    def post_check_compliance_data_assets_dataframe(
        self, request: CheckComplianceDataAssetsDataFrameRequest
    ) -> Union[
        ErrorResponse,
        CheckDataAssetCommentMarkdownResponse,
        list[CheckDataAssetDetailedResponseUnion],
        ...
        response, _success, _status_code = self._post(
            "v0/data-assets/check-compliance/dataframe",
            data=request.json(by_alias=True),
        )
        if request.responseType == ResponseType.DETAILED.value:
            if isinstance(response, list):
                return [parse_check_data_asset_response(r) for r in response]
            else:
                return ErrorResponse.parse_obj(response)
