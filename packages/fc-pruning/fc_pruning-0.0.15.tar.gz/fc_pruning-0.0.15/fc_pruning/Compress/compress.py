...
class PruneBase(AppState):
    ...
    def prune(self, data):
        ...
        # Serialize the generator's state
        serialized_generator_state = pickle.dumps(binary_mask)

        # Get the weights of the model
        weights = pf.get_weights(model)

        # Append the serialized generator state to the list of weights
        weights.append(serialized_generator_state)
        data_with_mask = weights
    ...
    def gather_data(self, use_pruning=True, **kwargs):
        ...
        if use_pruning:
            ...
            for i in range(len(data_with_mask_list)):
                binary_mask_deserialized = pickle.loads(binary_mask[i])

                reconstructed_model = pf.reconstruct_model(binary_mask_deserialized[i][0],data[i], reference_model)
