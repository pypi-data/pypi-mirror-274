...
class TheOddsApiWrangler(Wrangler):
    ...
    _featured_markets: list[
        Literal["h2h", "spreads", "totals", "outrights", "h2h_lay", "outrights_lay"]
    ] = [
        "h2h",
        "spreads",
        "totals",
    ...
    def parse_event_data(
        ...
        teams = f'{response_data["away_team"]}@{response_data["home_team"]}'
        start_time = response_data["commence_time"]

        return [
            self.parse_bookmaker_data(
                bookmaker, event_id, teams, start_time, market_key_mapping
            )
            for bookmaker in response_data["bookmakers"]
        ]
    ...
    def parse_entity(outcome: dict, key: str):
        ...
        if "h2h" in key or "spreads" in key or key == "draw_no_bet" or key == "btts":
            return outcome["name"]
