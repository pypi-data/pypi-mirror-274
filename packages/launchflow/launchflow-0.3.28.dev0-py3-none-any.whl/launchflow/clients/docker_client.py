...
def hash_create_args(create_args: Dict[str, Any]) -> str:
    """
    ...
    Args:
    - `create_args`: The dictionary of create arguments.

    Returns:
    - The hash of the create args.
    """
    args_copy = deepcopy(create_args)
...
class DockerClient:
    def __init__(self):
        try:
            ...
            logging.exception("Error when connecting to docker: ", e)
    ...
    def get_container(self, name: str) -> Optional[Container]:
        ...
        try:
            ...
            logging.warning(f"No container with name {name} found.")
            return None
        except APIError as e:
            logging.error(
                f"Server returned error when trying to get container {name}: {e}"
            )
            return None
        ...
        **kwargs,
        ...
        try:
            ...
            container = self.client.containers.run(
                ...
                ports=ports,
                volumes=volumes,
                labels={"launchflow_managed": "true", "create_args_hash": hash_create_args(create_args)},
                **kwargs,
            )
    ...
    def stop_container(self, name: str):
        ...
        try:
            ...
            logging.warning(f"No container with name '{name}' to stop.")
        except APIError as e:
            logging.error(
                f"Server returned error when trying to stop container '{name}': {e}"
            )

    def remove_container(self, name: str, force=False):
        """
        Remove a Docker container by name. Can be forced to remove running containers.
        """
        try:
            container = self.get_container(name)
            if container:
            ...
            logging.warning(f"No container with name '{name}' to remove.")
        except APIError as e:
            logging.error(
                f"Server returned error when trying to remove container '{name}': {e}"
            )
