...
async def create(
    ...
    # TODO project can probably be removed as an argument, but need to check what this does
    ...
    try:
        if environment is None:
            # TODO: Update get_environment so they can create the environment by following prompts here like before
            raise ValueError("Environment is required for creating remote resources")
...
async def destroy(
    ...
    _set_global_project_and_environment(project, environment)
    if local_mode:
        await destroy_resources("local", local_only=True)
        return

    try:
        if environment is None:
            # TODO Look at the flow state and them choose an environment
            raise ValueError("Environment is required for destroying remote resources")

        await destroy_resources(environment)
