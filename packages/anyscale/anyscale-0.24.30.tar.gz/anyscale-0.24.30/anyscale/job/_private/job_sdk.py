...
class JobSDK(WorkloadSDK):
    ...
    def _job_model_to_status(self, model: ProductionJob) -> JobStatus:
        ...
        return JobStatus(name=model.name, id=model.id, state=state, config=config)
    ...
    def status(
        ...
        return self._job_model_to_status(job_model)
