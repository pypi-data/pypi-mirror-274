...
import os
from typing import Any, DefaultDict, Dict, Generator, IO, Optional, Set, Tuple
...
class TestClusterEnv:
    ...
    @pytest.mark.parametrize(
        "setup_anyscale_client",
        [{"inside_workspace": True, "workspace_dependency_tracking_disabled": False},],
        indirect=True,
    )
    def test_build_image_from_requirements(
        ...
        fake_external_client.set_cluster_env_build(
            FakeExternalAPIClient.WORKSPACE_CLUSTER_ENV_BUILD_ID,
            ClusterEnvironmentBuild(
                id=FakeExternalAPIClient.WORKSPACE_CLUSTER_ENV_BUILD_ID,
                cluster_environment_id="fake-cluster-env-id",
                revision=5,
                docker_image_name="anyscale/myworkspace:default",
                local_vars_configuration=OPENAPI_NO_VALIDATION,
            ),
        )

        build_id = anyscale_client.build_image_from_requirements(
            cluster_env_name="fake-cluster-env-name",
            base_build_id=FakeExternalAPIClient.WORKSPACE_CLUSTER_ENV_BUILD_ID,
        ...
        assert build.containerfile == "\n".join(
            [
                ...
                "FROM anyscale/myworkspace:default",
    ...
    def test_get_cluster_env_name(
        ...
        assert anyscale_client.get_cluster_env_build_image_uri(
            "fake-cluster-env-build-id"
        ) == ImageURI.from_str("anyscale/ray:2.9.3")
