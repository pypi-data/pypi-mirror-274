...
class TestGetComputeConfig:
    ...
    def test_version_info(
        ...
        sdk, fake_client = sdk_with_fake_client
        template = DecoratedComputeTemplate(
        ...
        fake_client.add_compute_config(deepcopy(template))

        v1: ComputeConfigVersion = sdk.get_compute_config(
            name="default-cloud-compute-config-name"
        )
        assert v1.name == "default-cloud-compute-config-name:1"
        assert v1.config.cloud == fake_client.DEFAULT_CLOUD_NAME

        template.version = 2
        fake_client.add_compute_config(deepcopy(template))

        v2: ComputeConfigVersion = sdk.get_compute_config(
            name="default-cloud-compute-config-name"
        )
        assert v2.name == "default-cloud-compute-config-name:2"
        assert v2.config == v1.config

        latest: ComputeConfigVersion = sdk.get_compute_config(
            name="default-cloud-compute-config-name"
        )
        assert latest.name == "default-cloud-compute-config-name:2"
        assert latest.config == latest.config
