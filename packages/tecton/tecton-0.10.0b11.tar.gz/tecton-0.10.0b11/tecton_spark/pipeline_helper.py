...
# However, the user-defined transforms take as input pandas.DataFrame and output pandas.DataFrame.
...
class _ODFVPipelineBuilder(_PipelineBuilder):
    def __init__(
        ...
        # this is needed because we need to know which pandas.Series that are inputs to this function correspond to the desired request context fields or dependent fv features that the customer-defined udf uses.
    ...
    def pandas_udf_wrapper(self, *args):
        ...
        # self.udf_arg_idx_map tells us which of these pandas.Series correspond to a given RequestDataSource or FeatureView input
