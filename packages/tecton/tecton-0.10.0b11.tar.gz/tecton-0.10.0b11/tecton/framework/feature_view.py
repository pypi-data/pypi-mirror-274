...
from tecton._internals.pandas_compat import pandas_to_spark
...
class MaterializedFeatureView(FeatureView):
    ...
    def _use_feature_param(self) -> bool:
        ...
        if len(aggregates):
            ...
            return has_attributes or has_embeddings
    ...
    def _build_feature_view_schema(self, entity_specs: List[specs.EntitySpec]):
        ...
        if attributes or embeddings:
            schema_dict.update(
                {attribute.column: data_types.data_type_from_proto(attribute.column_dtype) for attribute in attributes}
            )
            schema_dict.update(
    ...
    def _derive_schemas_with_specs(
        ...
        entity_specs: Optional[List[specs.EntitySpec]],
    ) -> _Schemas:
        ...
        if self._use_feature_param():
            explicit_schema = self._build_feature_view_schema(entity_specs).to_proto()
    ...
    def get_features_in_range(
        ...
        df = querytree_api.get_features_in_range(
            dialect=dialect,
        ...
        df._requestContext = GetFeaturesInRangeParams(
            self._spec,
            start_time,
            end_time,
            entities,
    ...
    def get_features_for_events(
        ...
        if save and self._is_local_object:
            raise errors.UNSUPPORTED_SAVE_IN_READ_API_WITH_LOCAL_OBJECT(self.name)
        ...
        df = querytree_api.get_features_for_events(
            dialect=dialect,
            compute_mode=compute_mode,
            feature_definition=feature_definition,
            spine=events,
        ...
        df._request_context = GetFeaturesForEventsParams(
            self._spec,
            events,
            timestamp_key,
            from_source,
    ...
    def _create_unvalidated_feature_definition(
        ...
        schemas = self._derive_schemas_with_specs(transformation_specs, data_source_specs, entity_specs)
    ...
    def __init__(
        ...
        *,
        ...
        if features and schema:
            msg = f"Feature View {name} can not have both `features` and `schema` set. Please only set the `features` parameter."
            raise TectonValidationError(msg)

        if features and aggregations:
            msg = f"Feature View {name} can not have both `features` and `aggregations` set. Please only set the `features` parameter."
            raise TectonValidationError(msg)
...
def batch_feature_view(
    *,
    ...
    environment: Optional[str] = None,
    features: Optional[
        Union[Sequence[feature.Aggregate], Sequence[Union[feature.Attribute, feature.Embedding]]]
    ] = None,
):
...
class FeatureTable(FeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        """
        if features and schema:
            msg = f"Feature Table {name} can not have both `features` and `schema` set. Please only set the `features` parameter."
    ...
    def _derive_schemas(self) -> _Schemas:
        if self._use_feature_param():
            ...
            schema_dict.update(
                {
                    attribute.column: data_types.data_type_from_proto(attribute.column_dtype)
...
class RealtimeFeatureView(FeatureView):
    ...
    The RealtimeFeatureView should not be instantiated directly and the :py:func:`tecton.realtime_feature_view`
    decorator is recommended instead.
    ...
    def get_features_for_events(
        ...
        Examples:
            A RealtimeFeatureView :py:mod:`fv` that expects request time data for the key :py:mod:`amount`.
            ...
            An RealtimeFeatureView :py:mod:`fv` the expects request time data for the key :py:mod:`amount` and has a feature view dependency with join key :py:mod:`user_id`.
    ...
    def get_historical_features(
        ...
        Examples:
            An RealtimeFeatureView :py:mod:`fv` that expects request time data for the key :py:mod:`amount`.
    ...
    def get_online_features(
        ...
        :param request_data: Dictionary of request context values used for RealtimeFeatureViews.
          ...
          Examples:
            ...
            An RealtimeFeatureView :py:mod:`fv` that has a feature view dependency with join key :py:mod:`user_id` and expects request time data for the key :py:mod:`amount`.
...
def realtime_feature_view(
    *,
    ...
    :param features: A list of features this feature view manages.
    :return: An object of type :class:`tecton.RealtimeFeatureView`.

    An example declaration of an realtime feature view using Python mode.
    ...
    An example declaration of an Realtime feature view using Pandas mode.
...
on_demand_feature_view = realtime_feature_view
