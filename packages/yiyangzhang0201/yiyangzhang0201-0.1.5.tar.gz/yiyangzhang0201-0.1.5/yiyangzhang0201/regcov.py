    ...
    if len(code_list) == 1:
        ...
        if len(fe_list) == 0:
            ...
            for code in code_list:
                ...
                fe = fe[:-2]
                output_convert = f"""
                outreg2 using results.tex, replace tex title("{title}") keep({kv}) addtext({fe})
                """
                ipython.run_cell_magic('stata', '', output_convert)
        ...
        for i in range(len(code_list)):
            if i == 0:
                ...
                if len(fe_list[i]) == 0:
                    output_convert = f"""
            outreg2 using results.tex, replace tex title("{title}") ctitle("Model 1") label
            """
                    ipython.run_cell_magic('stata', '', output_convert)
                else:
                    k = code_list[i].split()
                    ...
                    fe = fe[:-2]
                    output_convert = f"""
                    outreg2 using results.tex, replace tex title("{title}") ctitle("Model 1") keep({kv}) addtext({fe})
                    """
                    ipython.run_cell_magic('stata', '', output_convert)
            ...
            """
                    ...
                    fe = fe[:-2]
                    output_convert = f"""
                    outreg2 using results.tex, append ctitle("Model {i + 1}") keep({kv}) addtext({fe})
                    """
                    ipython.run_cell_magic('stata', '', output_convert)