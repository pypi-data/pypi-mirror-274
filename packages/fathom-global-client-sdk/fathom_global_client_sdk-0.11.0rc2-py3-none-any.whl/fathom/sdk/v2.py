...
class Client(BaseClient):
    ...
    Attributes:
        ...
        geojson: Client to talk to the geospatial data API using GeoJSON
...
class VectorFileClient:
    ...
    def get_polygon(
        ...
        return self.client.geojson.get_polygon(
            polygon_from_vector_file(vector_file_name),
            layer_ids=layer_ids,
...
class GeoJSONClient:
    ...
    Example: GeoJSON queries
        ...
        # With a path to a file. Note the use of pathlib.Path to wrap file paths
        geojson_path = pathlib.Path("/path/to/geo.json")
        polygon_resp = client.geojson.get_points(geojson_path, layer_ids)
