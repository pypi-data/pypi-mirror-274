...
class RegScaleModel(BaseModel, ABC):
    ...
    def show_changes(self) -> Dict[str, Any]:
        ...
        changes = {
            ...
            if current_data.get(key) != self._original_data.get(key)
        }
        return changes
    ...
    def _handle_list_response(
        ...
        :rtype: List[T]
        """
        logger.debug(f"Handling list response with status_code {response.status_code}")
        # regscale is sending ok with 400 status code for some reason
        if (not response and getattr(response, "status_code", None) != 400) or response.status_code in [204, 404]:
            logger.debug("No response or status code 204 or 404")
            return []
        if response.ok and response.status_code not in [400]:
            ...
            if cls._get_objects_for_list.default:
                return cast(List[T], [cls.get_object(object_id=o["id"]) for o in json_response])
    ...
    def get_all_by_parent(
        cls, parent_id: Optional[int] = None, parent_module: Optional[str] = None, search: Optional[Search] = None
    ) -> List[T]:
        """
        Get a list of objects by parent.

        :param Optional[int] parent_id: The ID of the parent
