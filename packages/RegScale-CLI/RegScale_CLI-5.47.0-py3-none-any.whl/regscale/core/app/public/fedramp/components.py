...
def create_component_based_on_type(
    ...
    """
    if component_data["type"] == "interconnection":
        if response := create_interconnection(component_data, user_id, ssp_id, api_handler):
...
def create_related_items(
    ...
    for item_type in ["properties", "props", "protocols", "links"]:
        if item_type in component_data and component_data[item_type]:
            ...
            logger.error(f"Creating {item_type} for component {component_data['uuid']} with {create_func}")
...
def create_component(component_data: Dict, ssp_id: int, user_id: str, api_handler: APIHandler) -> Optional[Any]:
    ...
    if resp_json:
        create_component_mapping(resp_json, ssp_id=ssp_id, user_id=user_id, api_handler=api_handler)
        return resp_json
    else:
...
def create_component_mapping(component_data: Dict, ssp_id: int, user_id: str, api_handler: APIHandler) -> None:
    """
...
def create_interconnection(component_data: Dict, user_id: str, ssp_id: int, api_handler: APIHandler) -> Optional[Any]:
    """
