...
from unionai._config import _UNIONAI_DEFAULT_CONFIG_PATH, _get_config_obj
...
def login():
    ...
    if not _UNIONAI_DEFAULT_CONFIG_PATH.exists():
        click.echo(f"Login at {_UNIONAI_DEFAULT_CONFIG_PATH} does not exist. No need to delete.")
        return

    try:
        _UNIONAI_DEFAULT_CONFIG_PATH.unlink()
