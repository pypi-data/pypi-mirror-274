...
vec3 traceDirectionalLights(vec3 pos, vec3 normal, vec3 diffuseColor, vec3 specularColor, float roughness, float transmissionWeight, float eta, out vec3 resultNoShadow) {
  vec3 result = vec3(0.0);
  resultNoShadow = vec3(0.0);
  ...
  for (uint i = 0; i < directionalLightCount; ++i) {
    ...
    vec3 intensity = evalDiffuse(dotNL, dotNV, diffuseColor) * emission;
    intensity += evalGGXReflection(dotNL, dotNV, dotNH, dotVH, roughness, specularColor) * emission;
    if (transmissionWeight > 1e-5) {
      intensity += evalGGXTransmission(normal, L, V, eta, roughness) * emission * transmissionWeight;
    }
...
vec3 traceParallelogramLights(vec3 pos, vec3 normal, vec3 diffuseColor, vec3 specularColor, float roughness, float transmissionWeight, float eta, out vec3 resultNoShadow) {
  vec3 result = vec3(0.0);
  resultNoShadow = vec3(0.0);
...
vec3 tracePointLights(vec3 pos, vec3 normal, vec3 diffuseColor, vec3 specularColor, float roughness, float transmissionWeight, float eta, out vec3 resultNoShadow) {
  vec3 result = vec3(0.0);
  resultNoShadow = vec3(0.0);
...
vec3 traceSpotLights(vec3 pos, vec3 normal, vec3 diffuseColor, vec3 specularColor, float roughness, float transmissionWeight, float eta, out vec3 resultNoShadow) {
  vec3 result = vec3(0.0);
  resultNoShadow = vec3(0.0);
  ...
  for (uint i = 0; i < spotLightCount; ++i) {
    ...
    if (visibility > 0.0) {
      ...
      if (visibility > 1e-5 && maxTexColor > 1e-5) {
...
void main() {
  ...
  vec3 plNoShadow, dlNoShadow, slNoShadow, alNoShadow, elNoShadow;

  ray.radiance = emission
    + tracePointLights(worldPosition, worldShadingNormal, diffuseColor, specularColor, roughness, transmission, eta, plNoShadow)
    + traceDirectionalLights(worldPosition, worldShadingNormal, diffuseColor, specularColor, roughness, transmission, eta, dlNoShadow)
    + traceSpotLights(worldPosition, worldShadingNormal, diffuseColor, specularColor, roughness, transmission, eta, slNoShadow)
    + traceParallelogramLights(worldPosition, worldShadingNormal, diffuseColor, specularColor, roughness, transmission, eta, alNoShadow);

  {
    ...
    if (envmap != 0) {
      ...
      elNoShadow = attenuation * ambientLight;
    }

    if (!shadowRay.shadowed) {
      ray.radiance += elNoShadow;
    ...
    if (obj.segmentation.x == 1 && inDepth == 0) {
      vec3 radianceNoShadow = plNoShadow + dlNoShadow + slNoShadow + alNoShadow + elNoShadow;
      float shadowness = 1.0 - (ray.radiance.x + ray.radiance.y + ray.radiance.z + 1e-6) / (radianceNoShadow.x + radianceNoShadow.y + radianceNoShadow.z + 1e-6);
      ray.radiance = vec3(0.0);
      ray.alpha = shadowness;
      ray.depth = maxDepth + 1;
    }
  }
}
