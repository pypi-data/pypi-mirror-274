...
def _add_data_to_pod_db(
    ...
    with contextlib.closing(
        ...
        datapoint_hash_column_placeholder = pd.DataFrame(
            {"datapoint_hash": [""] * len(data)}, index=data.index
        )
        ...
        existing_cols_without_index = set(
            sorted([i for i in existing_data.columns if i not in ["rowID"]])
        )
        # check if df is empty or if all columns are the same,
        ...
        if (
            ...
            data = pd.concat(
                [
                    data,
                    existing_data.drop(columns=["rowID"], errors="ignore"),
                ],
                join="outer",
                ignore_index=True,
            )
            ...
            cur.execute(
                f"""CREATE TABLE "{table_name}" ('rowID' INTEGER PRIMARY KEY)"""
            )
            for col in data.columns:
                try:
                    cur.execute(
            ...
            if len(column_list) == 1:
                ...
                for file in datasource.file_names:
                    ...
                    if file in pod_data[ORIGINAL_FILENAME_METADATA_COLUMN].tolist():
                        ...
                        # Get record from filename
                        new_record: pd.DataFrame = datasource._get_data(
                            file_names=[file],
                            skip_non_tabular_data=avoid_image_caching,
                        )
...
def _add_single_file_record_to_db(
    ...
    new_record["datapoint_hash"] = new_record[_get_cols_to_hash(new_record)].apply(
        _compute_hash, axis=1
    )
    # Remove ' from column names
...
def _update_records_in_db(
    ...
    for _, updated_record in updated_records.iterrows():
        updated_record["datapoint_hash"] = _compute_hash(
