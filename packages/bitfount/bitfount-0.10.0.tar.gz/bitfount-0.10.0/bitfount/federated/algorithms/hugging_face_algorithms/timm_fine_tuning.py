...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    def __init__(
        ...
        batch_transformations: Optional[
            Union[
                ...
                Dict[_TimmBatchTransformationStep, List[Union[str, _JSONDict]]],
            ]
        ] = None,
        **kwargs: Any,
    ):
        ...
        if isinstance(batch_transformations, dict):
            step = "train"
            if step in batch_transformations.keys():
                self.batch_transformations[step] = [
                    {
                        "albumentations": {
                            "arg": self.image_column_name,
                            "output": True,
                            "transformations": batch_transformations[step],
                            "step": step,
                        }
                    }
                ]
            ...
            for step in DataSplit:
                self.batch_transformations[step.value] = [
                    {
                        "albumentations": {
                            "arg": self.image_column_name,
                            "output": True,
    ...
    def initialise_data(self, datasource: BaseSource) -> None:
        ...
        if self.datastructure and self.schema:
            ...
            self.train_dataloader = databunch.get_train_dataloader(
                batch_size=self.args.batch_size
            )
            self.eval_dataloader = databunch.get_validation_dataloader(
            ...
            train_data_factory, train_dataset = get_data_factory_dataset(
                ...
                batch_transforms=self.batch_transformations["train"],
            ...
            val_data_factory, val_dataset = get_data_factory_dataset(
                ...
                selected_cols=[self.image_column_name, self.target_column_name],
                selected_cols_semantic_types=self.semantic_types,
                batch_transforms=self.batch_transformations["validation"],
                target=self.target_column_name,
                labels2id=self.labels2id,
            )
    ...
    def initialise(
        ...
        **kwargs: Any,
        ...
        if task_id is not None:
            self.save_path = Path(self.save_path / f"{task_id}")
    ...
    def run(self) -> float:
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            safe_append_to_file(
                lambda x: timm_utils.update_summary(
                    epoch,
                    train_metrics,
                    eval_metrics,
                    filename=x,
...
class TIMMFineTuning(BaseAlgorithmFactory):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.target_column_name = target_column_name
        self.labels = labels
        if not batch_transformations:
            self.batch_transformations = {}
            for step in ["train", "validation"]:
                self.batch_transformations[step] = DEFAULT_IMAGE_TRANSFORMATIONS
        else:
            self.batch_transformations = batch_transformations
        self.use_deprecated_args = self._process_args()
    ...
    fields_dict: ClassVar[T_FIELDS_DICT] = {
        ...
        "image_column_name": fields.Str(allow_none=True),
        "target_column_name": fields.Str(allow_none=True),
        "labels": fields.List(fields.Str(), allow_none=True),
        "batch_transformations": fields.Dict(
    ...
    def _process_args(self) -> bool:
        """Helper function for handling args passed to the algorithm."""
        enough_args = False
        use_deprecated_args = False
        if not self.image_column_name or not self.target_column_name or not self.labels:
            if (self.schema and not self.datastructure) or (
                self.datastructure and not self.schema
            ):
                raise ValueError("Both schema and datastructure must be passed.")
            elif self.schema and self.datastructure:
                enough_args = True
                use_deprecated_args = True
                warnings.warn(
                    "Instantiating this algorithm with `schema` and `datastructure` "
            ...
            if not enough_args:
                raise ValueError(
                    "Either `schema` and `datastructure` must be passed or `image_column_name`, `target_column_name` and `labels` must be passed."
                )
        return use_deprecated_args
