...
class DataStructure(BaseDataStructure, _BaseSerializableObjectMixIn):
    ...
    Args:
        ...
        schema_types_override: A mapping that defines the new data types that
            ...
            E.g. `{'Pod_id': {'categorical': [{'col1': {'value_1':0, 'value_2': 1}}], "continuous": ['col2']}` for remote training
            or `{'categorical':[{ "col1" : {'value_1':0, 'value_2': 1}}],'continuous': ['col2']}` for local training. Defaults to None.
        ...
        if target_table_name is None:
            ...
            logger.warning(
                (
                    ...
                    f"using first table in the schema: {table_schema.name}"
                )
            )
        else:
            for ts in schema.tables:
                if ts.name == target_table_name:
                    table_schema = ts
