...
class _InlineFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record_tags and isinstance(record_tags, dict):
            keywords |= record_tags
        record_vars = getattr(record, "vars", None)
        if record_vars and isinstance(record_vars, dict):
            keywords |= record_vars
        if record.levelno == logging.ERROR or record.levelno == logging.CRITICAL:
            keywords["caller"] = f"{record.module}:{record.lineno}"
        if keywords:
            if record.exc_info:
                return f"{time} {record.levelname[0]} | {record.msg} | {json.dumps(keywords)}\n{self.formatException(record.exc_info)}"
            else:
                return f"{time} {record.levelname[0]} | {record.msg} | {json.dumps(keywords)}"
        else:
            if record.exc_info:
