...
class MigrationHandler(object):
	...
	mig_script_re = re.compile(r'^(\d+)(_up|_down|_snap)?(_.+)?(\.py|\.sql)$', re.I)
	...
	def initialize_db(self):
		...
		try:
			try:
				if self.schema:
					c.execute(text('select * from %s.db_version;' % self.quote_ident(self.schema)))
				else:
					c.execute(text('select * from db_version;'))
...
##					logger.info('PGCode %s', e.orig.pgcode)
				...
				if self.schema:
					c.execute(text('insert into %s.db_version values (0);' % self.quote_ident(self.schema)))
				else:
					c.execute(text('insert into db_version values (0);'))
		except Exception:
			trans.rollback()
			raise
		else:
	...
	def migrate(self, target_rev=None):
		...
		trans = c.begin()
		try:
			current_rev = c.execute(text('select version from %sdb_version;' % (self.quote_ident(self.schema) + '.' if self.schema else ''))).scalar()
			if target_rev == current_rev:
				trans.rollback()
				return
			if self.engine.dialect.name in ('postgres', 'postgresql'):
				c.execute(text('lock db_version in exclusive mode nowait;'))
			...
			if current_rev == 0 and snapshots:
				...
				try:
					snapshot_rev = max(rev for rev in snapshots.keys() if rev <= target_rev)
				except ValueError:
...
##								c.execute(text(src.decode('utf8')).execution_options(no_parameters=True))
								# c.execution_options(no_parameters=True).execute(text(src.decode('utf8')))
								with c.connection.cursor() as cur:
									cur.execute(src.decode('utf8'))
						else:
							spec = importlib.util.spec_from_file_location('mig_script', script_fn)
						...
						c.execute(text('update %sdb_version set version = %i;' % (((self.quote_ident(self.schema) + '.') if self.schema else ''), to_rev)))
					except Exception:
						sub_trans.rollback()
						raise
					else:
