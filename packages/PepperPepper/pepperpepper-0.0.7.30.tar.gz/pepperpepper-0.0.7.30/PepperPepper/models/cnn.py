...
class YOLOv3_104(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        output104 = torch.reshape(output104, (-1, 3, 5+self.num_classes, output104.shape[2], output104.shape[3]))

        output26 = self.classifier_DarkNet53_26(output26)
        output26 = torch.reshape(output26, (-1, 3, 5+self.num_classes, output26.shape[2], output26.shape[3]))

        output13 = self.classifier_DarkNet53_13(output13)
        output13 = torch.reshape(output13, (-1, 3, 5+self.num_classes, output13.shape[2], output13.shape[3]))
