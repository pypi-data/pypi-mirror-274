...
class YOLOv3_104(torch.nn.Module):
    ...
    def get_loss(self, outputs, gtbox, gtlabel, gtscore=None, anchors=[[5,6,25,10,20,34],[35,79,52,43,59,115],[115,90,156,197,374,326]],ignore_thresh=0.7):
        ...
        for i, out in enumerate(outputs):
            label_objectness,label_location,label_classification, scale_location = self.get_objectness_label([outputs.shape,3,416,416], gtbox, gtlabel, iou_threshold=ignore_thresh, anchors=anchors[i],num_classes=self.num_classes, downsample=downsample[i])
            pred_boxes = get_yolo_box_xxyy(outputs[i].detach().numpy(),anchors[i],num_classes=self.num_classes, downsample=downsample[i])
            ...
            loss = self.loss(output=outputs[i], label_objectness=label_objectness, label_location=label_location, label_classification=label_classification, scales=scale_location, num_anchors=len(anchors[i]//2), class_num=self.num_classes)
