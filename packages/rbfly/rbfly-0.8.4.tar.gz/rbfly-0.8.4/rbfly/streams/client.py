...
from .types import PublisherInfo, SubscriptionInfo
...
class StreamsClient(RabbitMQClient):
    ...
    async def publisher(self, stream, *, name: str | None=None, cls=Publisher):  # type: ignore
        ...
        protocol = await self.get_protocol()
        info = PublisherInfo(stream, name)
        try:
    ...
    async def subscribe(
        ...
        try:
            ...
            info = SubscriptionInfo(stream, proto_offset, amqp, subscriber)
    ...
    async def _subscribe(
        ...
        logger.info(
            ...
            ' offset={}'.format(info.stream, subscription_id, info.offset)
        )
