...
def check_edits(original, corrected):
    ...
    edits.sort(key=lambda x: (x.original.start_char, x.original.end_char, x.corrected.start_char, x.corrected.end_char), reverse=True)
    new_text = original
    for edit in edits:
        print(edit)
        start, end, corrected_text = edit.original.start_char, edit.original.end_char, edit.corrected.text
