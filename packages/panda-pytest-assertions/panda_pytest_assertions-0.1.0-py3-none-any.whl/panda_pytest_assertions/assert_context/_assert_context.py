...
def assert_context(  # noqa: D418
    *,
    ...
    :param exception: type of exception, expected to be raised
...
def assert_context(  # noqa: D418
    *,
    ...
    If the `behaviour` parameter is set to an exception type, the `with` block is expected
    to raise an exception of provided type and the `set` method of yielded object is expected
    not to be called at all.
...
def assert_context(
    *,
    ...
    is set to an exception type, it is the same as if `exception` would be set to this value and `result`
    would not be set. If `behaviour` is set to any other value, its treated as the value of `result` and the
