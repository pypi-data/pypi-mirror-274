...
def clean_incidents_dict(data):
    ...
    for incident in data:
        ...
        for elem in incident["get_inclusion_criteria"]:
            incident_dic["get_inclusion_criteria"].append(safe_get(elem, "code", "data", "attributes", "title"))
        ...
        for elem in incident.get("receiver", {}).get("data", []):
            ...
            incident_dic["get_receivers"].append(receiver)
            i += 1

        incident_dic.update(number_of_attributions=len(incident["get_attributions"]))
        for attribution in incident["get_attributions"]:
            if attribution["settled_attribution"] is True:
                for initiator in attribution.get("get_initiators", {}).get("data", []):
            ...
            incident_dic["get_attributions"].append(attribution_dict)
        ...
        for elem in incident.get("get_mitre_initial_access", []):
            incident_dic["MITRE_initial_access"].append(safe_get(
                elem, "code", "data", "attributes", "title"))

        for elem in incident.get("get_mitre_impact", []):
        ...
        if len(incident["political_response"]) > 0:
            for elem in incident["political_response"]:
                ...
                incident_dic["get_political_responses"].append(pol_response)
...
def brexit_clean(data, receiver_region=None):
    ...
    if receiver_region == "EU":
        for incident in data:
            if all(receiver["receiver_country"] == "United Kingdom" for receiver in incident["get_receivers"]):
...
def clean_receivers(data):
    ...
    for incident in data:
        for receiver in incident["get_receivers"]:
...
def clean_dates(data):
    """Converts the year, month, and day values to a full date format for the get_attributions and political/legal responses"""
    for incident in data:
    ...
    for incident in data:
        if incident["number_of_political_responses"] > 0:
            for response in incident["get_political_responses"]:
                response_full_date = []
