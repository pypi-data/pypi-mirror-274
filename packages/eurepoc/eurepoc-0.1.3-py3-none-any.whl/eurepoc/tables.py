...
def process_inclusion_criteria(full_data):
    inclusion_criteria = []
    for incident in full_data:
        for criterion, subcode in zip(incident["get_inclusion_criteria"], incident["inclusion_criteria_subcode"]):
...
def process_receivers_data(full_data):
    receivers_data = []
    for incident in full_data:
        for i in range(len(incident["get_receivers"])):
            if isinstance(incident["get_receivers"][i], dict):
                for j in range(len(incident["get_receivers"][i]['receiver_category'])):
                    receivers_data.append(
                        Receiver(
                            receiver_id=str(incident["get_receivers"][i]['receiver_id']) + f"_{j}",
                            incident_id=int(incident["id"]),
                            receiver_name=incident["get_receivers"][i]['receiver_name'],
                            receiver_category=incident["get_receivers"][i]['receiver_category'][j],
                            receiver_subcategory=incident["get_receivers"][i]['receiver_category_subcode'][j],
                            receiver_country=incident["get_receivers"][i]['receiver_country'],
                            receiver_country_alpha_2_code=incident["get_receivers"][i]['receiver_country_alpha_2_code'],
                            receiver_country_alpha_3_code=incident["get_receivers"][i]['receiver_country_alpha_3_code'],
                            receiver_regions=incident["get_receivers"][i]['receiver_regions']
                        )
                    )
            else:
                receivers_data.append(
                    Receiver(
                        ...
                        receiver_name=incident["get_receivers"][i],
...
def process_attributions_data(full_data):
    attributions_data = []
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                attributions_data.append(
                    Attributions(
                        attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                        incident_id=int(incident["id"]),
                        settled_attribution=incident["get_attributions"][i]['settled'],
                        attribution_date=incident["get_attributions"][i]['attribution_full_date'][0] if incident["get_attributions"][i]['attribution_full_date'][0] != "Not available" else None
                    )
                )
            else:
                attributions_data.append(
...
def process_attributions_bases_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attribution_basis'])):
                    attribution_bases_data.append(
                        AttributionBases(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attribution_basis=incident["get_attributions"][i]['attribution_basis'][j]
                        )
                    )
...
def process_attributions_types_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attribution_type'])):
                    attribution_types_data.append(
                        AttributionTypes(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attribution_type=incident["get_attributions"][i]['attribution_type'][j],
                            attribution_subtype=incident["get_attributions"][i]['attribution_type_subcode'][j]
                        )
                    )
            else:
                attribution_types_data.append(
...
def process_attribution_countries_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attributing_country'])):
                    attribution_countries_data.append(
                        AttributionCountries(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attributing_country=incident["get_attributions"][i]['attributing_country'][j]
                        )
                    )
            else:
                attribution_countries_data.append(
...
def process_attribution_actors_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attributing_actor'])):
                    attribution_actors_data.append(
                        AttributionActors(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attributing_actor=incident["get_attributions"][i]['attributing_actor'][j]
                        )
                    )
            else:
                attribution_actors_data.append(
...
def process_attribution_companies_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attribution_it_company'])):
                    attribution_companies_data.append(
                        AttributionCompanies(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attributing_company=incident["get_attributions"][i]['attribution_it_company'][j]
                        )
                    )
            else:
                attribution_companies_data.append(
...
def process_attribution_legal_references_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['legal_attribution_reference'])):
                    attribution_legal_references_data.append(
                        AttributionLegalReferences(
                            attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                            attribution_legal_reference=incident["get_attributions"][i]['legal_attribution_reference'][j],
                            attribution_legal_reference_subcode=incident["get_attributions"][i]['legal_attribution_reference_subcode'][j]
                        )
                    )
            else:
                attribution_legal_references_data.append(
            ...
            if isinstance(incident["get_attributions"][i], dict):
                initiators_data.append(
                    Initiators(
                        ...
                        incident_id=int(incident["id"]),
                        attribution_id=str(incident["get_attributions"][i]['attribution_id']),
                        settled_initiator=incident["get_attributions"][i]['settled'],
                        initiator_name=incident["get_attributions"][i]['attributed_initiator_name'][0],
                        initiator_country=incident["get_attributions"][i]['attributed_initiator_country'][0],
                    )
                )
            else:
                initiators_data.append(
...
def process_initiators_categories_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident["get_attributions"])):
            if isinstance(incident["get_attributions"][i], dict):
                for j in range(len(incident["get_attributions"][i]['attributed_initiator_category'])):
                    initiators_categories_data.append(
                        InitiatorsCategories(
                            initiator_id="init_" + str(incident["id"]) + "_" + str(i),
                            initiator_category=incident["get_attributions"][i]['attributed_initiator_category'][j],
                            initiator_subcategory=incident["get_attributions"][i]['attributed_initiator_category_subcode'][j]
                        )
                    )
            else:
                initiators_categories_data.append(
...
def process_political_responses_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident['get_political_responses'])):
            if isinstance(incident['get_political_responses'][i], dict):
                political_responses_data.append(
                    PoliticalResponses(
                        political_response_id=str(incident['get_political_responses'][i]['political_response_id']),
                        incident_id=int(incident["id"]),
                        political_response_responding_country=incident['get_political_responses'][i]['political_response_country'][0],
                        political_response_responding_actor=incident['get_political_responses'][i]['political_response_actor'][0],
                        political_response_date=incident["get_political_responses"][i]['political_response_full_date'][0] if incident["get_political_responses"][i]['political_response_full_date'] != "Not available" else None
                    )
                )
            else:
                political_responses_data.append(
...
def process_political_responses_type_data(full_data):
    ...
    for incident in full_data:
        for i in range(len(incident['get_political_responses'])):
            if isinstance(incident['get_political_responses'][i], dict):
                for j in range(len(incident['get_political_responses'][i]['political_response_type'])):
                    political_responses_type_data.append(
                        PoliticalResponseTypes(
                            political_response_id=str(incident['get_political_responses'][i]['political_response_id']),
                            incident_id=int(incident['id']),
                            political_response_type=incident['get_political_responses'][i]['political_response_type'][j],
                            political_response_subtype=incident['get_political_responses'][i]['political_response_type_sub'][j]
                        )
                    )
            else:
                political_responses_type_data.append(
