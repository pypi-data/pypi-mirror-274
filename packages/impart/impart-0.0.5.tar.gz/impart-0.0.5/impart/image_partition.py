...
class ImagePartition:
    ...
    def __init__(self,
        ...
        self.image: Image = Image.new_from_file(image)
    ...
    def compute_image_subregion(self,
        ...
        """
        Compute the image subregion corresponding to the area of the indexed
        cell(s).
    ...
    def _compute_axis_from_slice(
        ...
        if index.start >= self.shape[axis]:
            raise IndexError(
                f"Index {index.start} is out of bounds for axis {axis} "
                f"with size {self.shape[axis]}")
        if index.stop > self.shape[axis]:
            index = slice(index.start, self.shape[axis])

        if index.start >= index.stop:
            raise IndexError(f"Empty or inverted slice: {index}")
