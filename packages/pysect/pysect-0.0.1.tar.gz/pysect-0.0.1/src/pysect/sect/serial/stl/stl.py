...
from primitives.coord import coord
from sect.primitives.stl import STL
...
def load(fh: IO, mode):
    if "b" in mode:
        return STL(_load_binary(fh))
    else:
        return STL(_load_ascii(fh))
...
def _load_binary(fh: IO):
    ...
    return np.fromfile(fh, dtype=STL.dtype(), count=count)
    ...
    def itter_facet(fh):
        while True:
            ...
            if facet is None:
                break
            yield facet
    return np.fromiter(itter_facet(fh), dtype=STL.dtype())
