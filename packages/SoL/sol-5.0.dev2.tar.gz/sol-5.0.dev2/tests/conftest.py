...
def tourney_second_replica(session):
    return session.query(Tourney).filter_by(description='Replica of Second test tournament').one()
...
def tourney_corona_all_against_all(session):
    return session.query(Tourney).filter_by(description='Corona Carrom All against All').one()


@pytest.fixture
def tourney_corona_all_against_all_odd(session):
    return session.query(Tourney).filter_by(description='Corona Carrom All against All Odd').one()
