...
def test_lele_insert_tourney_inaccessible_rating(lele_user,
                                                 championship_current,
                                                 rating_european):
    modified = [('idtourney', dict(idtourney=0,
    ...
    response = lele_user.post_route(dict(modified_records=JSON.encode(modified),
                                    ...
                                    'save_changes')
    assert response.json['success'] is False
    assert response.json['message'] == "Non ti è consentito di utilizzare quel livello di valutazioni"


def test_lele_modify_tourney_inaccessible_rating(lele_user,
                                                 tourney_first,
                                                 rating_european):
    modified = [('idtourney', dict(idtourney=tourney_first.idtourney,
                                   idrating=rating_european.idrating))]
    deleted = []
    response = lele_user.post_route(dict(modified_records=JSON.encode(modified),
                                         deleted_records=JSON.encode(deleted)),
...
def test_lele_create_rating(lele_user, club_scr, session):
    modified = [('idrating', dict(idrating=0,
    ...
    response = lele_user.post_route(dict(modified_records=JSON.encode(modified),
                                    ...
                                    'save_changes')
    assert response.json['success'] is False
    assert response.json['message'] == "Non ti è consentito aggiungere una valutazione globale, non associata ad uno specifico club"
    ...
    response = lele_user.post_route(dict(modified_records=JSON.encode(modified),
                                    ...
                                    'save_changes')
    assert response.json['success'] is False
    assert response.json['message'] == "Non ti è consentito aggiungere una valutazione di quel livello"
    ...
    newratingid = session.scalar(select(Rating.idrating)
                                 .where(Rating.description == 'Started ok...'))
    modified = [('idrating', dict(idrating=newratingid,
                                  level='1'))]
    response = lele_user.post_route(dict(modified_records=JSON.encode(modified),
                                         deleted_records=JSON.encode(deleted)),
                                    'save_changes')
