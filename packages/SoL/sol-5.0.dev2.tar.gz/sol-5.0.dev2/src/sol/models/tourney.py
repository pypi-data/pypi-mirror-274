...
class RankingStats:
    ...
    __slots__ = ('points', 'netscore', 'totscore', 'bucholz', 'real_points',
                 'played_matches', 'virtual_points', 'retired')
...
class Tourney(GloballyUnique, Base):
    ...
    def __table_args__(cls):
        return (GloballyUnique.__table_args__(cls) +
                (Index('%s_uk' % cls.__tablename__,
                       'date', 'description', 'idchampionship',
                       unique=True),))
    ...
    delaycompatriotpairing: Mapped[int] = mapped_column(
        ...
        info=dict(label=_('Delay compatriots pairing'),
                  hint=_('Number of rounds for which pairing of players belonging to the'
    ...
    finalkind: Mapped[str] = mapped_column(
        ...
        info=dict(label=_('Final kind'),
                  ...
                  dictionary=dict(simple=_('Single game'),
    ...
    matcheskind: Mapped[str] = mapped_column(
        ...
        nullable=False,
        default='simple',
        info=dict(label=_('Matches kind'),
    ...
    owner: Mapped[User | None] = relationship(
        'User', back_populates='owned_tourneys')
    ...
    def check_insert(
        ...
        if cmethod is None:
            raise OperationAborted(_('Invalid pairing method: $couplings',
                                     mapping=dict(couplings=repr(couplings))))
        if couplings == 'all':
            if fields.get('system') not in ('roundrobin', 'swiss'):
                cmethod = cls.couplings.info['dictionary'][couplings]
                raise OperationAborted(_('Invalid pairing method, $couplings is allowed'
                                         ' only in Swiss or Round-robin system tourney',
                                         mapping=dict(couplings=repr(_(cmethod)))))
        elif couplings == 'seeds':
            if fields.get('system') not in ('knockout', 'roundrobin'):
                cmethod = cls.couplings.info['dictionary'][couplings]
                raise OperationAborted(_('Invalid pairing method, $couplings is allowed'
                                         ' only in Knockout or Round-robin tourney',
                                         mapping=dict(couplings=repr(_(cmethod)))))
        elif couplings == 'circle':
            if fields.get('system') != 'roundrobin':
                cmethod = cls.couplings.info['dictionary'][couplings]
                raise OperationAborted(_('Invalid pairing method, $couplings is allowed'
        ...
        if mkind is None:
            raise OperationAborted(_('Invalid matches kind: $matcheskind',
                                     mapping=dict(couplings=repr(matcheskind))))
        if matcheskind == 'bestof3' and fields.get('system') != 'knockout':
            raise OperationAborted(_('Invalid matches kind, $matcheskind is allowed'
                                     ' only in Knockout tourney',
                                     mapping=dict(matcheskind=repr(_(mkind)))))

        if fields.get('system') != 'swiss' and fields.get('finals'):
            raise OperationAborted(_('Finals can be done only in Swiss system tourneys'))

        if user_id != 'admin':
            clubs = Club.__table__
            cships = Championship.__table__
            if not session.scalar(select(1)
                                  .where(exists()
            ...
            if idrating is not None:
                ...
                if session.scalar(select(ratings.c.level < users.c.maxratinglevel)
                                  .select_from(ratings.join(users, true()))
                                  .where(ratings.c.idrating == idrating)
                                  .where(users.c.iduser == user_id)):
                    raise OperationAborted(_('You are not allowed to use that level of'
                                             ' ratings'))
    ...
    def check_update(
        ...
        if fields.get('system', self.system) != 'swiss' and fields.get('finals', self.finals):
            raise OperationAborted(_('Finals can be done only in Swiss system tourneys'))
    ...
    def isAllAgainstAllTraining(self):
        return bool(self.system in ('roundrobin', 'swiss')
    ...
    def ranking(self):
        ...
        if self.rankedturn == 0:
            def key(c):  # type: ignore[reportRedeclaration]
                return (-c.position, c.rate or 0)
        else:
            if self.system == 'swiss':
                key = attrgetter('prize', 'points',
                                 'bucholz', 'netscore',
                                 'totscore')  # type: ignore[reportAssignmentType]
            elif self.system == 'roundrobin':
                def key(c):  # type: ignore[reportRedeclaration]
    ...
    def firstFinalTurn(self):
        ...
        if self.finalturns:
            ...
            session = object_session(self)
            assert session
            return session.scalar(select(func.min(matches.c.turn))
                                  .where(matches.c.idtourney == self.idtourney)
                                  .where(matches.c.final))

    @staticmethod
    def compute_elapsed_time(since: int | None, max: int) -> int | None:
        "Compute elapsed milliseconds since `since` timestamp, up to `max`."

        if since is None:
            elapsed = None
        else:
            elapsed = int(time() * 1000) - since
            if elapsed > max * 60 * 1000:  # pragma: no cover
                elapsed = None
        return elapsed

    @property
    def countdownElapsed(self) -> int | None:
        """
        The elapsed time since countdown started, in milliseconds, or ``None`` when
        the countdown is not active.
        """

        return self.compute_elapsed_time(self.countdownstarted, self.duration * 60 * 1000)
    ...
    def computeRanking(self, turn=None) -> list[tuple[Competitor, Rank]]:
        ...
        final_ranking = [(c, r.rank(c.position, c.rate))
    ...
    def _checkSeedsAmbiguities(self):
        ...
        if self.idrating is None:
            if not all(c.position for c in activecomps):
                logger.warning("Missing seeds position for some competitor: %s",
                               ', '.join(repr(c) for c in self.competitors
                                         if not c.retired and not c.position))
                raise OperationAborted(_("Missing seed for some competitor,"
            ...
            if len({c.position for c in activecomps}) != len(activecomps):
                ...
                for c in activecomps:
                    if c.position in seen:
                        ...
                        seen.add(c.position)
                logger.warning("Following competitors have duplicated seed: %s",
                               ', '.join(repr(c) for c in dups))
                raise OperationAborted(_("At least two competitors have the same seed,"
            ...
            if len({c.rate for c in activecomps}) != len(activecomps):
                ...
                for c in activecomps:
                    if c.rate in seen:
                        ...
                        seen.add(c.rate)
                logger.warning("Following competitors have duplicated rate: %s",
                               ', '.join(repr(c) for c in dups))
                raise OperationAborted(_("At least two competitors have the same rate,"
                                         " please disambiguate assigning explicit seeds"
                                         " in the tourney's “Competitors” window"))

            if any(c.position for c in activecomps):
                if not all(c.position for c in activecomps):
                    logger.warning("Missing seed for some competitor: %s",
                                   ', '.join(repr(c) for c in self.competitors
                                             if not c.retired and not c.position))
                    raise OperationAborted(_("Missing seed for some competitor,"
                                             " must be entered manually in the"
                ...
                if len({c.position for c in activecomps}) != len(activecomps):
                    ...
                    logger.warning("Following competitors have duplicated seed: %s",
                                   ', '.join(repr(c) for c in dups))
                    raise OperationAborted(_("At least two competitors have the same"
    ...
    def makeNextTurn(self) -> None:
        ...
        if self.championship.trainingboards:
            ...
            if noemail:  # pragma: no cover
                ...
                raise OperationAborted(_('Cannot proceed, following competitors do not have'
    ...
    def _addMatches(
            self,
            turn: int,
            pairings: Iterable[tuple[Competitor, Competitor | None]]
    ) -> None:
    ...
    def _makeNextRoundrobinCircleTurn(self) -> None:
        ...
        half = count // 2
        self._addMatches(self.currentturn + 1, zip(comps[:half], reversed(comps[half:])))
                ...
                if same_points < all_players and same_points % 2:
                    ...
                    if None not in self.competitors or same_points != (all_players - 1):
                        if logger.isEnabledFor(logging.DEBUG):  # pragma: nocover
                            border_c = self.competitors[same_points].caption(False, False)
                            logger.debug("Considering competitor “%s” as belonging to"
                                         " the group with %d points",
                                         border_c, self.pivot.points)
                        same_points += 1
    ...
    def _combine(
                ...
                for c1, c2 in done:
                    if c1:
                        ...
                        if c2 not in matches:
                            ...
                            matches.append(c1)
                logger.debug("Done matches:\n%s",
                             '\n'.join(f"  {C(c)}: {', '.join(C(o) for o in done_matches[c])}"
                                       for c in ranking))

            debug("Competitors to be paired:\n%s",
        ...
        try:
            visitor = getattr(self, self.system.capitalize() + self.couplings.capitalize() + "Visitor")
        ...
        if debug:  # pragma: nocover
            ...
            if remainingc:
                debug('Looking for a competitor for %s within\n%s', C(c1),
            ...
            if len(competitors) > 2:
                ...
                if remainings:
                    ...
                    if debug:  # pragma: nocover
                        if _level == 0:
                            debug("OK =>\n%s",
                                  '\n'.join(f"  {C(_c1)} vs {C(_c2)}" for _c1, _c2 in newturn))
                        else:
                            debug("OK")
                    return newturn
            else:
    ...
    def _assignBoards(self, matches) -> None:
        ...
        for match in matches:
            boards_usage = used_boards[match.competitor1]
            # Consider the second competitor only when there are more than
            # 6 boards/12 competitors: under that threshold it's better to
            # try making the first competitor to play an a different board
            # each time. See issue #34.
            if len(matches) > 6:
                boards_usage = boards_usage + used_boards[match.competitor2]
            board = sorted(available_boards, key=boards_usage.__getitem__)[0]
            available_boards = [b for b in available_boards if b != board]
            match.board = board
    ...
    def _makeNextTurn(self) -> None:
        ...
        if self.system == 'roundrobin':
            ranking = sorted(ranking,
                             key=lambda c: ((c.position, -(c.rate or 0), c.description)
                                            if c is not None
                                            else (1_234_567_890,)))
    ...
    def replay(self, date, newidowner=None):
        ...
        from . import Competitor, Championship, Rating, User
        ...
        if newidowner is not None and self.idrating is not None:
            ...
            if session.scalar(select(ratings.c.level < users.c.maxratinglevel)
                ...
                raise OperationAborted(_('You are not allowed to replicate that tourney,'
        ...
        if not self.championship.closed:
            ...
            session = object_session(self)
            assert session
            championship = session.query(Championship).filter_by(
    ...
    def sendTrainingURLs(self, request):
        ...
        if self.couplings == 'all':
            ...
            for m in self.matches:
                ...
                pm.append((m, 1, m.competitor2.player1 if m.competitor2 is not None else None))
            ...
            for m in self.matches:
                if m.turn == self.currentturn:
                    m.competitor1.player1.sendTrainingURL(request, m, 1,
                                                          m.competitor2.player1
                                                          if m.competitor2 is not None
                                                          else None)
                    if m.competitor2 is not None:
                        m.competitor2.player1.sendTrainingURL(request, m, 2,
                                                              m.competitor1.player1)
