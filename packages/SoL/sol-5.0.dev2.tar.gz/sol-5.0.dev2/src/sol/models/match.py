...
class Match(Base):
    ...
    boards: Mapped[list[Board]] = relationship(
        'Board', back_populates='match',
        cascade="all, delete-orphan",
        order_by="Board.number")
    """List of :py:class:`boards <.Board>`."""

    tourney: Mapped[Tourney] = relationship(
        'Tourney', back_populates='matches')
        ...
        if self.tourney.matcheskind == 'bestof3':
            for game in ('', '_2', '_3'):
                ...
                if s1fname in fields and s2fname in fields:
                    if fields[s1fname] == fields[s2fname]:
                        raise OperationAborted(_('Ties are not allowed in best-of-three'
                                                 ' games matches'))
    ...
    def _bestOf3Results(self) -> tuple[int, int]:
        assert self.tourney.matcheskind == 'bestof3'
        ...
        if self.score1 > self.score2:
            ...
            raise OperationAborted(
                _('How could game $game of "$match" end with a draw in a best of three'
                  ' tourney?!?', mapping=dict(game=1, match=self.description)))
        ...
        if not (self.score1_2 is None or self.score2_2 is None):
            if self.score1_2 > self.score2_2:
                ...
                raise OperationAborted(
                    _('How could game $game of "$match" end with a draw in a best of three'
                      ' tourney?!?', mapping=dict(game=2, match=self.description)))
            ...
            if not (self.score1_3 is None or self.score2_3 is None):
                if self.score1_3 > self.score2_3:
                    ...
                    raise OperationAborted(
                        _('How could game $game of "$match" end with a draw in a best of three'
                          ' tourney?!?', mapping=dict(game=3, match=self.description)))
    ...
    def results(self):
        ...
        if self.tourney.matcheskind == 'simple':
            ...
            if self.score1 == self.score2 and self.tourney.system == 'knockout':  # pragma: no cover
                raise OperationAborted(
            ...
            if won1 < 2 and won2 < 2:
                raise OperationAborted(
                    _('No one won enough games in match "$match"!',
                      mapping=dict(match=self.description)))
    ...
    def serialize(self,
                  ...
                  competitors: dict[int|None, int|None]) -> SerializedMatch:
