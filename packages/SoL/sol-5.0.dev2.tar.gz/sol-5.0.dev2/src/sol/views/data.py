    ...
    if 'metadata' in results:
        ...
        if not no_owners:
            caption_by_userid = {u.iduser: u.caption(html=False)
                                 for u in s.query(User).options(load_only(
                                         User.iduser,
                                         User.firstname,
                                         User.lastname,
                                         User.email,
...
def replace_ratings_description(request, results):
    if 'root' in results and results['root'] and 'idrating' in results['root'][0]:
        ...
        q = (select(_ratings_t.c.idrating,
             ...
             .select_from(_ratings_t.outerjoin(_clubs_t,
...
@expose(
    select(_federations_t.c.idclub,
...
_countplayed = (select(func.count(_tourneys_t.c.idtourney))
                .where(and_(or_(bindparam('played4club') == None,
        ...
        if played_for_club_id is not None:
            ...
            if 'filter' in args:
                ...
                if (len(filters) == 1
                    ...
                    conditions = or_(op.filter(_players_t.c.firstname, value),
...
@expose(
    ...
    .select_from(_players_t
                 .outerjoin(_clubs_t, _clubs_t.c.idclub == _players_t.c.idclub))
    .where(exists()
           .where(and_(_players_t.c.idplayer != _pdup_t.c.idplayer,
    ...
    if 'metadata' in args:
        ...
        if 'filter' in args:
            ...
            if (len(filters) == 1
                ...
                op = filters[0].operator
                value = filters[0].value
                conditions = or_(op.filter(_users_t.c.email, value),
...
@expose(
    select(_championships_t, _clubs_t.c.description.label('Club'),
           ...
           .scalar_subquery().label('Previous'),
           select(_ratings_t.c.description)
           ...
           .scalar_subquery().label('Tourneys'))
    ...
    metadata=dict(
        ...
        Previous=dict(label=_('Previous championship'),
                      ...
                      hidden=True,
                      nullable=True,
                      lookup=dict(url='/data/championships?only_cols=idchampionship,description'
...
@expose(
    select(_championships_t.c.idchampionship,
...
@expose(
    select(_tourneys_t,
           ...
           .scalar_subquery().label('Club'),
           select(_clubs_t.c.description)
           .where(_clubs_t.c.idclub == _tourneys_t.c.idhostingclub)
           .scalar_subquery().label('HostingClub'),
        ...
        Club=dict(label=_('Club'),
                  ...
                  lookup=dict(url='/data/clubsLookup',
                              ...
                              innerTpl=(
                                  ...
                                  '[values.nationality] ]}">{description}'
                                  '</div>'))),
        GeneratedTurns=dict(label=_('Turns'),
...
@expose(
    select(_ratings_t.c.idrating,
           _ratings_t.c.description,
           _ratings_t.c.level,
           _clubs_t.c.description.label('Club'))
    .select_from(_ratings_t.outerjoin(_clubs_t, _clubs_t.c.idclub == _ratings_t.c.idclub))
...
_countrates = (select(func.count(_ratesc_t.c.idrate))
               .where(and_(_ratesc_t.c.idrating == _rates_t.c.idrating,
