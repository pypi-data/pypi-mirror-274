...
@expose(
    Match,
    fields=('board,description,score1,score2,score1_2,score2_2,score1_3,score2_3'
...
def matches():
    ...
    if md:
        ...
        q = (select(_tourneys_t.c.matcheskind)
             .where(_tourneys_t.c.idtourney == int(request.params['filter_by_idtourney'])))
        ...
        if matcheskind != 'bestof3':
            ...
            for f in fields:
                if f['name'] == 'score1':
                    ...
                    f['label'] = t(_('S2 (1)'))
                    f['hint'] = t(_('Score of the second competitor in the first game.'))
...
def _new_turn(request, final):
    ...
    return dict(success=success, message=message,
                ...
                finalturns=tourney.finalturns,
                prized=tourney.prized,
                generated_turns=(0 if not tourney.matches
                                 else max(m.turn for m in tourney.matches)))
...
def countdown(request):
    ...
    return dict(
        ...
        elapsed=tourney.countdownElapsed,
...
def getBoardSelfEditURL(request):
    ...
    try:
        ...
        if (tourney is None
                or tourney.prized
                or tourney.currentturn == tourney.rankedturn):  # pragma: no cover
...
def getCompetitor1SelfEditURL(request):
    ...
    try:
        ...
        if (match is None
                ...
                or match.tourney.currentturn == match.tourney.rankedturn):  # pragma: no cover
...
@view_config(route_name='test_create_random_finals', renderer="json", )
...
@view_config(route_name='test_create_random_scores', renderer="json", )
