...
class Connection(NoSQLConnection):
    ...
    def _exists(self, key):
        ...
        qry = self._client.query(namespace=key.namespace, ancestor=key)
        qry.keys_only()
        qry.add_filter(filter=PropertyFilter("__key__", "=", key))
        return bool([x for x in qry.fetch(limit=1) if x])

    def _delete(self, key: Key):
        self._client.delete(self.wrapper.to_native_key(key))
    ...
    def _get(self, key_or_keys) -> List[Entity]:
        ...
        if isinstance(key_or_keys, Key):
            ...
            return [self.wrapper.from_native_entity(x) for x in results if x]

    def _put(self, key, entity, update: bool) -> Optional[Entity]:
        native_entity = self.wrapper.to_native_entity(entity)
        if self._txn:
            self._txn.put(native_entity)
            return entity
        else:
            self._client.put(native_entity)
            return entity
...
class Query(NoSQLQuery):
    ...
    def count(self, limit):
        # FIXME: When the local emulator supports aggregation queries, switch
        # to this code
        # agg = self.connection._client.aggregation_query(self._query)
        # return next(agg.count().fetch())
