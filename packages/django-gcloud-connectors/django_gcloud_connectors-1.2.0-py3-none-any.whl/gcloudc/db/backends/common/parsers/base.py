...
from ..helpers import has_concrete_parents, indexed_fields_on_model, get_top_concrete_parent
...
class BaseParser(object):
    ...
    def _sense_check(self, ret):
        ...
        model_indexed_fields = indexed_fields_on_model(ret.model)
        if projection and model_indexed_fields:
        ...
        for and_branch in ret.where.children if ret.where else []:
            ...
            for filter_node in filters:
                if (
                    model_indexed_fields
                    and filter_node.column != self.connection.key_property_name()
    ...
    def get_transformed_query(self, connection, force_keys_only=False) -> ORQuery:
        ...
        query = ORQuery(
            ...
            select=builder.columns or [x.column for x in self.model._meta.fields],
        ...
        query.order_by(builder.order_by)
