...
input_fields = [
    ...
    (
        ...
        Path,
        {
            "help_string": "output directory name",
            "argstr": "-o {out_dir}",
            "output_file_template": '"groupICA.out"',
        },
    ),
    (
        "mask",
...
output_fields = [("report_dir", ty.Any, {})]
...
class MELODIC(ShellCommandTask):
    ...
    >>> task.inputs.ICs = File.mock()
    >>> task.inputs.mix = File.mock()
    >>> task.inputs.smode = File.mock()
    >>> task.inputs.bg_image = File.mock()
    >>> task.inputs.tr_sec = 1.5
    >>> task.inputs.t_des = TextMatrix.mock("timeDesign.mat")
    >>> task.inputs.t_con = Con.mock("timeDesign.con")
    >>> task.inputs.s_des = TextMatrix.mock("subjectDesign.mat")
    >>> task.inputs.s_con = Con.mock("subjectDesign.con")
