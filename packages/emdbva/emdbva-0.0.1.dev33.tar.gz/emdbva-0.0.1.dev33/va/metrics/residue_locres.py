from va.utils.misc import floatohex, scale_values, scale_value, float_to_hex
...
def local_resolution_json(map, inmodels, radius, value_range):
    ...
    :param inmodels: Structure instance from TEMPy package mmcif parser
    ...
    for model in models:
        ...
        new_min = scale_value(residues_min, map_min, map_max, 0, 1)
        new_max = scale_value(residues_max, map_min, map_max, 0, 1)
...
def model_local_resolution_views(input_json, map_name, data_type, chimerax_bin_dir):
    ...
    viewer.get_views(map_name, root_data, data_type)

def map_local_resolution_views(map_name, data_type, chimerax_bin_dir, work_dir):
    """
