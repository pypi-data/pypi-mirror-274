...
class BuildSettings(BaseModel):
    raw_dockerfile: Optional[str] = Field(alias="dockerfile", default="")
...
class Service(BaseModel):
    ...
    def image_registry_url(self, run=False, region=None):
        ...
        if self.environment.application.provider_is_gcp:
            return f"{self.config.environment.collection.region}-docker.pkg.dev/{self.environment.collection.account_id}/{self.config.environment.application.name}/{self.gcr_image_name}{_run_string}"
