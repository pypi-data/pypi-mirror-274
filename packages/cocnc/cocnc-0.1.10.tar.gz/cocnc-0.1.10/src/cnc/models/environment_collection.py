...
class EnvironmentCollection(BaseModel):
    ...
    def cloud_resource_namespace(self):
        if self.cloud_resource_namespace_override:
            return self.cloud_resource_namespace_override

        _hash = hashlib.sha256()
        _hash.update(self.account_id.encode())
        _hash.update(self.application.name.encode())
    ...
    def base_domain(self):
        ...
        return f"{self.unique_id[:20]}.cncsites.com"
