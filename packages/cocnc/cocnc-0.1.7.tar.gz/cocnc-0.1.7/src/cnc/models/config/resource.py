...
class CacheResourceSettings(BaseResourceSettings):
    ...
    def toolbox_managed_environment_variables(self):
        ...
        _env.update(
            {
                f"{self.env_var_base}_PORT": self.local_port,
                f"{self.env_var_base}_IP": "host.docker.internal",
                f"{self.env_var_base}_URL": f"redis://host.docker.internal:{self.local_port}",
                "REDIS_URL": f"redis://host.docker.internal:{self.local_port}",
            }
        )
        return _env
