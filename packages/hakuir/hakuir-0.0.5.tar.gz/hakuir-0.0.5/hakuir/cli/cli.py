...
from .commands import (
    ...
    upscale_cli,
    restoration_cli,
    upscale_before_ir_cli,
    upscale_after_ir_cli,
)
...
def cli():
    ...
    models_command = subparsers.add_parser("models", help="Output model list")
    models_command.add_argument(
        "-a", "--available", action="store_true", help="Output available model list"
    )
    models_command.set_defaults(handle=available_model_list)

    download_command = subparsers.add_parser("download", help="Download model")
    download_command.add_argument(
        "-m", "--model", type=str, required=True, help="Model name"
    )
    download_command.set_defaults(handle=download_models)

    upscale_command = subparsers.add_parser("upscale", help="Just upscale")
    upscale_command.add_argument(
        "-r", "--resample", type=str, required=True, help="resample method name"
    )
    upscale_command.add_argument(
        "-i", "--input", type=str, required=True, help="input image path"
    )
    upscale_command.add_argument("-o", "--output", type=str, help="output image path")
    upscale_command.set_defaults(handle=upscale_cli)

    restoration_command = subparsers.add_parser("restoration", help="Just restoration")
    restoration_command.add_argument(
        "-m",
        "--model",
        default="NAFNet-REDS-width64",
        type=str,
    ...
    restoration_command.add_argument(
        "-o", "--output", type=str, help="output image path"
    )
