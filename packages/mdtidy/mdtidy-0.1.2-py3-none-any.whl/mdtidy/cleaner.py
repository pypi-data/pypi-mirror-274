...
def process_content_to_ipynb(input_string, output_filename):
    # Remove text blocks enclosed with triple backticks that start with ```text?code_stdout
        ...
        if line == "CODE_BLOCK_START":
            if markdown_content.strip():
                cells.append({
                    "cell_type": "markdown",
                    "metadata": {},
                    "source": markdown_content.strip().split('\n')
                })
                markdown_content = ""
            ...
            cells.append({
                ...
                "metadata": {},
                "outputs": [],
                "source": code_content.strip().split('\n')  # Ensure each line of code is a separate element in the list
            })
            code_block_index += 1
        else:
