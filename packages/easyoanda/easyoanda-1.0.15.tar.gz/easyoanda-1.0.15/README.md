...
1) ## <b>Verify Versions</b>
    #### <i>Windows:</i>
    ```python
    # verify `Python` version
    > python --version

    # verify `requests` and `pandas` version
    ...
    # verify `requests`` and `pandas` version
    $ pip3 show requests pandas
    ```

2) ## <b>Update Versions</b>
...
3) ## <b>Try Again</b>
    #### <i>Windows:</i>
    ```cmd
    > pip install easyoanda --no-cache-dir
    ```
    #### <i>MacOS / Unix / Linux:</i>
    ```bash
    $ pip3 install easyoanda --no-cache-dir
    ```
...
4) ## **Stream live quotes**
    ...
    # a few moments later, a new "EUR/USD" bid-ask quote (automatically populated)
    session.pricing.pricingStream
    ```
...
7) ## Managing Risk
    ...
    - ### Selecting an optimal stop-loss:
        ...
        # (2) get conversion factors and calculate target units for order
...
session.quit() -> None
    ...
    on exit, having not run this is likely the cause - simple press
...
- #### <i><u>Sizing a Position:</u></i>
    > Convert between a home currency and target instrument's base currency, evaluate a trade's price sensitivity
    <details>
        <summary><b>easyoanda.calc_units()</b></summary>

    ```python
    easyoanda.calc_units(currentQuotes : dict,
    ...
    <details>
        <summary><b>easyoanda.calc_home()</b></summary>

    ```python
    easyoanda.calc_home(currentQuotes : dict,
    ...
    <details>
        <summary><b>easyoanda.calc_pip_impact()</b></summary>

    ```python
    calc_pip_impact(currentQuotes : dict,
    ...
    <details>
        <summary><b>easyoanda.calc_price_impact()</b></summary>

    ```python
    calc_price_impact(currentQuotes : dict,
...
- #### <i><u>Managing Risk:</u></i>
    > Calculate optimal stop loss levels and trade sizes.
    <details>
        <summary><b>easyoanda.calc_stop_level()</b></summary>

    ```python
    calc_stop_level(currentQuotes : dict,
                    targetTradeSize : int,
    ...
    ```python
    calc_trade_size(currentQuotes : dict,
                    exitPrice : float,
                    maxLoss : float,
                    entryPrice : float | None = None) -> None
...
> Review a plethora of  &copy;OANDA transactional records.
<details>
...
### <i>Price Quotes:</i>
> Stream live quotes, get bid-ask spreads, view foreign-to-domestic conversion factors.
<details>
...
> View a session's order logs, view a session's error logs, configure streaming preferences.
<details>
