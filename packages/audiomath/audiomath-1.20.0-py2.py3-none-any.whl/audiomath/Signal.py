...
def GenerateWaveform(container=None,freq_hz=1.0,phase_rad=None,phase_deg=None,amplitude=1.0,dc=0.0,samplingfreq_hz=None,duration_msec=None,duration_samples=None,axis=None,waveform=cos,waveform_domain='auto',**kwargs):
	...
	# Are you here because of "ValueError: operands could not be broadcast together..." ?
	# To demonstrate how to reconstruct a waveform from its Spectrum(), use Reconstruct()
	# which wraps the necessary numpy dimension-fiddling. But don't use it for anything
	# other than small toy signals - space complexity is nsamp**2. For real applications
	# use InverseSpectrum() instead.
...
def Reconstruct(ap,**kwargs):
	...
	`fft2ap()` and hence also by `Spectrum()`.
	
	NB: this is for small toy/debugging examples only. Its space
	complexity is number-of-samples squared. For real applications,
	use `InverseSpectrum()`.
	"""
	ap = dict(ap) # makes a copy, at least of the container dict
	ap['duration_samples'] = len(ap.pop('fullfreq_hz'))
	ap.update(kwargs)
	axis=ap.pop('axis', -1)  # don't worry about negative values, that's handled in the loop below
	extra_axis = axis+1 
...
def _MakeFade(x, start, stop, direction, cosine=True):
	...
	stop  = float( 'nan' if stop  is None else stop  )
	scale = float(stop - start)
	if numpy.isnan( start ) or start == -numpy.inf:
		x = x * 0.0 # makes a copy
		if direction > 0: x += 1.0
	elif numpy.isnan( stop ) or stop == +numpy.inf:
		x = x * 0.0 # makes a copy
		if direction < 0: x += 1.0
	elif scale <= 0.0:
		out = x * 0.0 # makes a copy
		if   direction < 0: out[ x <= start ] = 1.0
		elif direction > 0: out[ x >= stop  ] = 1.0
		x = out
	else:
