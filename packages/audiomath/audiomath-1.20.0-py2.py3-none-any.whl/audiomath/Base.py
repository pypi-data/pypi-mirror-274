...
# 
# Copyright (c) 2008-2024 Jeremy Hill
# 
...
def TimeBase( durationInSeconds, samplesPerSecond=None ):
	if samplesPerSecond is None: samplesPerSecond = durationInSeconds.fs # if no sampling frequency supplied, assume first arg must be a Sound object
	if hasattr( durationInSeconds, 'nSamples' ): durationInSamples = durationInSeconds.nSamples
	elif hasattr( durationInSeconds, 'shape' ):  durationInSamples = durationInSeconds.shape[ ACROSS_SAMPLES ]
	else: durationInSamples = SecondsToSamples( durationInSeconds, samplesPerSecond, rounding=int )
	return numpy.arange( durationInSamples ) / samplesPerSecond
...
class Sound( object ):
	...
	def str2dat( self, raw, nSamples=None, nChannels=None, dtype=None ):
		...
		try:    y = numpy.frombuffer( raw, dtype=canonical_dtype )
		except: y = numpy.fromstring( raw, dtype=canonical_dtype )
	...
	def AutoScale( self, max_abs_amp=0.95, center='median' ):
		"""
		Remove the DC offset from each channel (see `.Center()`)
		and then rescale the waveform so that its maximum
		absolute value (across all channels) is `max_abs_amp`.
		
		Valid `center` options include `'median'`, `'mean'`,
		`'none'`, or numeric values (scalar, or one per channel).
	...
	def Center( self, center='median' ):
		...
		The array `self.y` is modified in place.
		"""
		if isinstance( center, str ) and center.lower() == 'none':
			return self
		elif isinstance( center, str ) and center.lower() == 'median':
			try:    center = numpy.median( self.y, axis=ACROSS_SAMPLES ) # newer numpy versions only
			except: center = numpy.median( self.y ) # older numpy versions only (grr!)
		elif isinstance( center, str ) and center.lower() == 'mean':
			center = self.y.mean( axis=ACROSS_SAMPLES )
		else:
			try: center = numpy.array( center, dtype=float ).ravel()
			except: raise ValueError( 'unrecognized `center` option' )
			if center.size not in [ 1, self.y.shape[ ACROSS_CHANNELS ] ]:
				raise ValueError( 'a numeric `center` should either be scalar or have exactly one value per channel' )
		self.y -= numpy.expand_dims( center, ACROSS_SAMPLES )
		return self._BumpRevision()
...
Sound.t = property( TimeBase )
Sound.TimeBase = TimeBase
