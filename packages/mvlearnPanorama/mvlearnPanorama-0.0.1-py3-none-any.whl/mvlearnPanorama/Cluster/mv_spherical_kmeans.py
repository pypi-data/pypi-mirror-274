...
from .mv_kmeans import mvKMeansCluster

class mvSphericalKMeans(mvKMeansCluster):
  r'''
  An implementation of multi-view spherical K-Means.
  Multi-view spherical k-means clustering extends the classic spherical 
  kmeans clustering technique to include two viewpoints of data. This 
  approach is similar to the mult-view k-means algorithm, but employs 
  cosine distance instead of Euclidean distance for calculating optimization 
  objectives and making assignments.
  ...
  def __init__(self, n_clusters=2,max_iter=5,patience=3,
               tol=0.0001,init='random',n_jobs=-1,n_init=1,random_state=None):
    super().__init__(n_clusters=n_clusters,max_iter=max_iter, 
                    patience=patience,tol=tol,init=init,
