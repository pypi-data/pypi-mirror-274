...
def data_converter_collnll2003(dirout="ztmp/data/ner/norm/conll2003",  nrows=100000000):
   ...
   def ner_merge_BOI_to_triplet_v1(tokens:list, ner_tags:list): 
      """Converts the NER tags and tokens into triplets based on the specified conditions.
   ...
   for dtype in ds0.keys():  
        ...
        df2["text_id"]      =  df2["text2"].apply(lambda x : hash_textid(x) )
...
def data_converter_DFKI_few_nerd(dirout="ztmp/data/ner/norm/conll2003",  nrows=100000000):
    # -*- coding: utf-8 -*-
    """fewnerd_data.ipynb
...
#     subsets = ['supervised', 'inter', 'intra']
#     for subset in subsets:
#         hf_ds_to_disk("DFKI-SLT/few-nerd", subset, output_dir)


def download_fewnerd(output_dir="./ztmp/fewnerd"):
    """
    Download and unzip the few-nerd datasets from the specified URLs and save them to the output directory.
    """
...
def fetchnorm_fewnerd(input_dir="./ztmp/fewnerd", output_dir="./ztmp/fewnerd"):
    """
    Normalize all files in the subdirectories of input_dir and save them as Parquet files in the corresponding output_dir subdirectories.
    """
...
def fetchnorm_gum(input_dir="./ztmp/gum", output_dir="./ztmp/gum"):
    """
    Dowload files using util function download_gum() then run this converter passing the file path.
    """
...
def fetchnorm_re3d(input_dir="./ztmp/re3d", output_dir="./ztmp/re3d"):
    """
    Dowload files using util function download_re3d() then run this converter passing the input directory.
    """
...
def fetchnorm_ewnertc(dirfile="./ztmp/ewnertc/ewnertc_ner.csv",  dirout="./ztmp/ewnertc/ewnertc_ner.parquet"):
    """
    Dowload file from kaggle from link below. then run this converter passing the file path
    """
    ...
    dfg                = dfg.iloc[:-1]
    dfg.loc[0, 'text'] = "XX Radawa is a village in the administrative district of Gmina Wizownica , within Jarosaw County , Subcarpathian Voivodeship , in south eastern Poland"
    pd_to_file(dfg,  dirout, show=1)
...
def zzz_run_convert(name="ag_news", diroot: str = "./ztmp/hf_datasets", 
    ...
    for key in splits:
        ...
        for k in range(0, n_batch):
            ...
            dfk = pd.DataFrame(data_k)
            dfk = convert_fun(dfk, meta_dict=cc)  # Assuming convert_fun is defined in the file
