...
class Metagenomics:
    ...
    def seqs_from_sra(self,accession:str,target_dir:str,container:str="None")-> tuple[str,dict]:
        ...
        if container=="None":
            ...
            prefetch_script=f""""""
            prefetch_script+=f"singularity exec prefetch {accession} -O {target_dir}\n"
            acc_folder=pathlib.Path(target_dir)/accession
            fasterq_dump_script=""
            sra_file=acc_folder/(accession+".sra")
            fasterq_dump_script+=f"singularity exec fasterq-dump {sra_file} -O {acc_folder} --split-files\n"
            fasterq_dump_script+=f"singularity exec rm {sra_file}"
            prefetch_script+=fasterq_dump_script
