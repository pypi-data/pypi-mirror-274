...
class BaseComputer(URL):
    ...
    def _link_dynamic_subs(self):
        ...
        for sub in self.substitution_objects:
            ...
            for char in default:
                ...
                if char == "}":  # "dedent" out of brackets
                    ...
                    if indent == 0:
                        ...
                        logger.debug("\tevaluted to: %s", evaluated)
                        main_cache.append(
                            evaluated
                        )  # This can be a DyanmicValue, which needs to be handled later
                        proc_cache = []
                    continue
            ...
            for char in main_cache:
                if isinstance(char, (DynamicValue, DynamicMixin)):
    ...
    def script(self, **kwargs) -> str:
        ...
        if self.template is None:
            ...
            script = self.apply_substitutions(self.template.split("\n"))
    ...
    def apply_substitutions(self, script: list) -> str:
        ...
        for sub in self.substitution_objects:
            logger.debug(f"processing Sub pointed at {sub.target}")
            if sub.target == "":
