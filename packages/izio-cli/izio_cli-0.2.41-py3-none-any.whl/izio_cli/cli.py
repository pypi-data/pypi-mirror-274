...
def izpay(
    ...
    # get root path of the app
    #   if path contains mb_izpay, split the path and get the first part
    if cd:
        root = path
    else:
    ...
    # change the flavor in the app
    # get env file data
    flavor, bundleId, appName, appIconPath, primaryColor = getEnv(
    ...
    with console.status(
        ...
        changeSplash(
            root, appIconPath.replace("/", os.sep), primaryColor, project="izpay"
        )
        status.update("Updating onesignal icons")
