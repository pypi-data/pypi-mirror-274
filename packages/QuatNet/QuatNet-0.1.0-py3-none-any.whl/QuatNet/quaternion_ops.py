...
def get_r(input):
    ...
    if input.dim() == 3:
        print("printing r input ",input.narrow(2, 0, nb_hidden // 4))
...
def get_i(input):
    ...
    if input.dim() == 3:
        print("print i input",input.narrow(1, nb_hidden // 4, nb_hidden // 4) )
...
def get_j(input):
    ...
    if input.dim() == 3:
        print("print j input", input.narrow(1, nb_hidden // 2, nb_hidden // 4))
...
def get_k(input):
    ...
    if input.dim() == 3:
        print("print k input ",input.narrow(1, nb_hidden - nb_hidden // 4, nb_hidden // 4) )
...
def quaternion_conv(input, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    """
    # print(" the input size in Q_CONV is ",input.size())
    # print("input in Q_CONV is ",input)
    ...
    if   input.dim() == 3:
        # print("input dimesion is 3 so CONV1D")
        convfunc = F.conv1d
    elif input.dim() == 4:
        # print("input dimesion is 4 so CONV2D")
        convfunc = F.conv2d
    ...
    # print("Kernal matrix after combining all 4 signals ",cat_kernels_4_quaternion)
    # print("Shape Kernal mAtrix", cat_kernels_4_quaternion.shape)
...
def quaternion_conv_rotation(input, zero_kernel, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    if quaternion_format:
        if scale is not None: #scale is r term
...
def complexKit_init(in_features, out_features, rng, kernel_size=None, criterion='glorot'):
...
def quaternion_init_ASM(in_features, out_features, rng, kernel_size=None, criterion='glorot'):
...
# def ifgram_torch ( y, sr, n_fft, hop_length, win_length,clip  ):
...
#         # fft_window_torch = pad_center_torch (torch.hann_window(window_length=win_length , periodic=True, dtype=None, layout=torch.strided, device=None, requires_grad=False), n_fft)
#         freq_angular_torch = torch.linspace(start = 0,end= 2*torch.pi,steps= n_fft )# torch linspace does not has endpoint false feature so i am just 
#         # excluding it manually
#         d_window_torch = torch.sin(-freq_angular_torch)*torch.pi/n_fft
        
#         stft_maxtrix_torch = torchaudio.functional.spectrogram(y, n_fft=n_fft, hop_length=hop_length,
#                                                      win_length=win_length, window=torch.hann_window(window_length=win_length), 
#                                                       power=None, normalized=False, pad=0) # power =None return complex tensor
        
#         diff_stft_torch =  torch.conj(torchaudio.functional.spectrogram(y, n_fft=n_fft, hop_length=hop_length,
#                                                      win_length=win_length, window = d_window_torch, 
#                                                       power=None, pad =0, normalized=False) ) # conjugate
        
        
#         mag_torch = stft_maxtrix_torch.abs()
        

#         mag_torch **= 1
        
#         phase_torch = torch.exp( 1.j * stft_maxtrix_torch.angle())
        
       
#         # print(mag_torch)
#         # print(phase_torch)

#         freq_angular_torch = freq_angular_torch.reshape((-1, 1))
#         bin_offset_torch = (-phase_torch * diff_stft_torch).imag / mag_torch
#         # print(bin_offset_torch)
#         ref_power_torch =1e-6 
#         bin_offset_torch[ mag_torch < ref_power_torch**0.5] = 0
#         if_gram_torch = freq_angular_torch[:n_fft//2 + 1] + bin_offset_torch

#         # if norm: # torch version w
#         #     stft_maxtrix_torch = stft_maxtrix_torch * 2.0 / fft_window_torch.sum()
    
#         if clip:
#             if_gram_torch = torch.clip(if_gram_torch,0, torch.pi)

#         if_gram_torch *= float(sr) * 0.5 / torch.pi
#         magnitude = stft_maxtrix_torch.abs()
        
#         if_gram_torch_delta = torchaudio.functional.compute_deltas(if_gram_torch)
#         magnitude_delta =  torchaudio.functional.compute_deltas(magnitude)

#         return if_gram_torch, if_gram_torch_delta, magnitude, magnitude_delta
# dont need this anyways!