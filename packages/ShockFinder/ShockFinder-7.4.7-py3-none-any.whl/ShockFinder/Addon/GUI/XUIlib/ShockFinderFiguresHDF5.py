    ...
    def is_valid_hdf5(self,filename=None):
        ...
        try:
            with h5py.File(filename, 'r') as f:
                if "HDF5LABEL" in f.attrs and f.attrs["HDF5LABEL"] == "ShockFinderSavedFile":
                    return True
        except:
            pass
        return False
        ...
        with h5py.File(self.filename, 'r') as f:
            ...
            if group_name:
                if group_name in units_group:
                    return read_subgroup(units_group[group_name])  # Return a one-level dictionary
        ...
        with h5py.File(self.filename, 'a') as f:
            ...
            if subgroup_name_to_delete:
                if subgroup_name_to_delete in f[group_path]:
                    ...
                    self.output(f"Configuration '{subgroup_name_to_delete}' does not exist.")
                    return {}  # Return an empty dictionary if the subgroup doesn't exist
            else:
        ...
        with h5py.File(self.filename, 'r') as f:
            ...
            if subgroup:  # If a specific subgroup is given
                if subgroup not in f[f"Data/{dimension}"]:
                    self.output(f"Data '{subgroup}' does not exist under 'Data/{dimension}'.")
                    return {}
                target_group = f[f"Data/{dimension}/{subgroup}"]
                for key in sorted(target_group.keys(), key=int):  # Sort keys as integers
            ...
            for subgroup_name in sorted(main_group.keys(), key=int):  # Sort keys as integers
        ...
        with h5py.File(self.filename, 'a') as f:
            ...
            if dataset_name_to_delete:
                if dataset_name_to_delete in f[group_path]:
                    ...
                    self.output(f"Data '{dataset_name_to_delete}' does not exist under '{group_path}'.")
            else:
