...
def get(Dataobj,args={},vargs={}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	quantities={"SoundSpeed":(Dataobj.quantities["gamma"]*Dataobj.quantities["prs"]/Dataobj.quantities["rho"])**0.5}
...
def result(quantity_name=None,anafname=None):
	return ("SoundSpeed","AvgTh_SoundSpeed")
		...
		if AvgTh_cal:
			...
			for i in newneed:
				if i not in Dataobj.quantities.keys() and i not in vargs.keys():
					...
					return Dataobj
			meanstr=""
			meanaxis=get_par(Dataobj,vargs,"Mean_axis",Dataobj.quantities["rho"].ndim-1)
			...
			quantities={"AvgTh_"+meanstr+"SoundSpeed":(Dataobj.quantities["gamma"]*Mean.Mean(Dataobj,"rho",vargs)/Mean.Mean(Dataobj,"prs",vargs))**0.5} #update here
			Dataobj.quantities.update(quantities)
		else:
			print("Warning: AvgTh mode is not opened:", "SoundSpeed") #update here
	except:
...
if __name__=="__main__":
	...
	print("Testing Result:", TestData.quantities["SoundSpeed"])
	if AvgTh_cal:
		TestData=get_AvgTh(TestData)
		print("Testing Result:", TestData.quantities["AvgTh_SoundSpeed"])