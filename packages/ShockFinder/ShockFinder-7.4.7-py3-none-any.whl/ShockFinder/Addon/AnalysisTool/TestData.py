...
def get(num=None,geometry="SPHERICAL",grids=grids_default_num,grids_map=grids_default["SPHERICAL"]):
	grid={}
	for i in range(3):
		try:
			grid.update({"x"+str(i+1):np.arange(grids_map[i][0],grids_map[i][1],(grids_map[i][1]-grids_map[i][0])/grids[i])})
		except:
			continue
