...
def get(Dataobj,quantity_name,args={},vargs={}):
	...
	fix_prs[np.where(fix_prs==0)]=np.min(np.abs(fix_prs))*get_par(Dataobj,vargs,"Minfix",0.01)
	fix_rho=copy.deepcopy(Dataobj.quantities[rho])
	fix_rho[np.where(fix_rho==0)]=np.min(np.abs(fix_rho))*get_par(Dataobj,vargs,"Minfix",0.01)
	quantities={
		"BVF_"+rho+"_"+prs:-1/fix_rho*Dataobj.quantities["Gradient_"+prs+"_x1"]*(1/Dataobj.quantities["gamma"]/fix_prs*Dataobj.quantities["Gradient_"+prs+"_x1"]-1/fix_rho*Dataobj.quantities["Gradient_"+rho+"_x1"])  if not np.all(fix_rho==0) and not np.all(fix_prs==0) else np.full(fix_prs.shape,0.0)
	}
...
def result(quantity_name=None,anafname=None):
	...
	return ["BVF_"+sstr[i]+"_"+sstr[i+1] for i in range(0,len(sstr),2)] #this function will return result types shown in GUI
