	...
	for i in meanaxis:
		try:
			...
			try:
				...
				meanaxisregion.append([Dataobj.grid["x"+str(i+1)][0],Dataobj.grid["x"+str(i+1)][-1]])
	return meanaxisregion

def GetMeanRegionIndex(Dataobj,meanaxis,meanaxisregion):
	index=[]
	for i in range(len(meanaxis)):
		index.append([np.where(Dataobj.grid["x"+str(meanaxis[i]+1)]>=meanaxisregion[i][0])[0][0],np.where(Dataobj.grid["x"+str(meanaxis[i]+1)]<=meanaxisregion[i][1])[0][-1]+1])
	return index
...
def GetMeanRadialIndex(Dataobj,meanaxis,radial):
	...
	for i in range(len(meanaxis)):
		...
		elif radial[i]>=min(Dataobj.grid["x"+str(meanaxis[i]+1)]) and radial[i]<=max(Dataobj.grid["x"+str(meanaxis[i]+1)]):
			if radial[i] in Dataobj.grid["x"+str(meanaxis[i]+1)]:
				ind=np.where(Dataobj.grid["x"+str(meanaxis[i]+1)]==radial[i])[0][0]
				index.append([ind,ind+1])
			else:
				index.append([np.where(np.where(Dataobj.grid["x"+str(meanaxis[i]+1)]<=radial[i])[0][-1],Dataobj.grid["x"+str(meanaxis[i]+1)]>=radial[i])[0][0]+1])
		else:
			index.append([-1,-1])
	return index
...
def Mean(Dataobj,quantity_name,vargs={"Mean_axis":(1,)}):
	meanaxis=get_par(Dataobj,vargs,"Mean_axis",Dataobj.quantities[quantity_name].ndim-1)
