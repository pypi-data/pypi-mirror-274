...
class ArrayOperator:
	...
	def add_argscut(self,args,default_args=None):
		...
		cal_n=0
		for i in args:
			if cal_n<self.cal_num:
				self.args[pnum][co].append(i)
			pnum+=1
			cal_n+=1
			if pnum==self.pnum:
				pnum=0
	...
	def result_combine(self):
		...
		for j in range(len(self.results[0])):
			for i in range(len(self.results)):
				try:
					new_result.append(self.results[i][j])
				except:
					pass
		self.results=tuple(new_result)
if __name__=="__main__":
	aoobj=ArrayOperator(9)
	aoobj.add_common_args("ca1")
	aoobj.add_argscut(range(15))
	aoobj.add_common_args(args_array=("ca2","ca3","ca4"))
	aoobj.add_argscut(range(5))
	print(aoobj.args)
	print(aoobj.results)