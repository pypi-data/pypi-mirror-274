...
def draw_aux_line(xmin,xmax,ymin,ymax,**Figureinfo):
	...
	for key,value in auxlines["vline"].items():
		...
		if "label" not in value.keys() and key!="":
			value["label"]=key
		plt.vlines(x=value["value"],ymin=ymin,ymax=ymax,**Basic.clean_keys(value,Basic.linekeys))
	for key,value in auxlines["hline"].items():
		...
		if "label" not in value.keys() and key!="":
			value["label"]=key
		plt.hlines(value["value"],xmin=xmin,xmax=xmax,**Basic.clean_keys(value,Basic.linekeys))
	for key,value in auxlines["linear"].items():
		...
		b=value["b"] if "b" in value.keys() else value["value"] if "value" in value.keys() else 0
		y=k*x+b
	...
	for key,value in auxlines["powerlaw"].items():
		...
		k=value["k"] if "k" in value.keys() else 1
		c=value["c"] if "c" in value.keys() else value["value"] if "value" in value.keys() else 0
		y=a*x**k+c
		plt.plot(x,y,**Basic.clean_keys(value,Basic.linekeys))
...
def line(*lines,**Figureinfo):
	...
	tt=np.array([np.array([min(i[0]["x"]),max(i[0]["x"]),min(i[0]["y"]),max(i[0]["y"])]) for i in lines]).T
