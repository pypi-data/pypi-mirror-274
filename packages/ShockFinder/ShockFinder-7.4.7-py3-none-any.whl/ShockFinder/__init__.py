		...
		for i in command.keys():
			...
			if i=="MultiprocessEngine":
				...
				if "Loader" in command.keys():
					databaseinfo.update({
							"FilePrefix":get_par(command["Loader"],"FilePrefix","data."),
							"FileFormat":get_par(command["Loader"],"FileFormat","04d"),
				...
				allfiles=LoaderLib["FPP"].GetFiles(get_par(values,"FileDir",os.getcwd()),get_par(values,"FilePrefix","data."),get_par(values,"FileFormat","04d"),get_par(values,"FileType","dbl"))
				if self.MPE!=None:
