...
LoaderPass=("__init__","Config","FileNamePreProcess","FilesPreProcess","LoaderModel")
AnalysisToolPass=("__init__","Config","AnalysisToolModel","Basic","TestData","Differential","Equilibrium","Mean","Radial","Harmonic","FastFourierTransform")
...
def get_header_end(modulename,main_fun_name="info"):
	Header="#File type: extension <Module> set\n#By Junxiang H., 2023/07/1\n#wacmk.com/cn Tech. Supp.\n\n#This script updates automaticly! Do not Modify!\n#Update time:"+now()+"\n\n"+modulename+"={}\n"
	End="if __name__==\"__main__\":\n\tprint(\"Testing Model:\",__file__)\n\tprint(\""+modulename+":\")\n\tfor i in "+modulename+".keys():\n\t\tprint(i,\":\","+modulename+"[i])\n\t\tprint(\"\\t\","+modulename+"[i]."+main_fun_name+")"
	return (Header,End)
def get_filesstr(modulename,moduledir,modulepass):
	FilesStr=""
	for i in tuple(set([FileNamePreProcess.FileNamePreProcess(i)[0] for i in FilesPreProcess.GetFiles(moduledir,"py")])-set(modulepass)):
		FilesStr+="try:\n\ttry:\n\t\timport ShockFinder.Addon."+modulename+"."+i+" as "+i+"\n\texcept Exception as err:\n\t\tprint(\""+i+"in ShockFinder dir importing error, reimporting in current dir\")\n\t\timport "+i+"\n\t"+modulename+"[\""+i+"\"]="+i+"\nexcept Exception as err:\n\tprint(\"Module: "+i+" import failure:\",err)\n\n" 
	return FilesStr
...
def Update(modulename,moduledir,modulepass,main_fun_name="info",cname="Config.py"):
	...
	Write(moduledir,cname,header,get_filesstr(modulename,moduledir,modulepass),end)
def Update_all():
