...
def get(Dataobj,quantity_name,args={},vargs={}):
	...
	fix_qt[np.where(fix_qt==0)]=np.min(np.abs(fix_qt))*get_par(Dataobj,vargs,"Minfix",0.01)
	quantities={
		"KHI_"+bvf+"_"+velo:Dataobj.quantities[bvf]/fix_qt**2 if not np.all(fix_qt==0) else np.full(fix_qt.shape,0.0)
	}
...
def result(quantity_name=None,anafname=None):
	...
	return ["KHI_"+sstr[i]+"_"+sstr[i+1] for i in range(0,len(sstr),2)] #this function will return result types shown in GUI
