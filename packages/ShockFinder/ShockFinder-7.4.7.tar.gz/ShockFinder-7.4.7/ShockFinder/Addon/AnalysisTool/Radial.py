...
def get(Dataobj,quantity_name,args={},vargs={"Radial_axis":(1,),"Radial_value":(None,)}):
	...
	for i in need:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys():
			...
			return Dataobj
	radialaxis=get_par(Dataobj,vargs,"Radial_axis",Dataobj.quantities[quantity_name].ndim-1)
	...
	quantities={
		"Radial_"+radial_str+quantity_name:Mean.GetMean(Dataobj.quantities[quantity_name],radialaxis,radialindex)
	}
...
def get_boundary(Dataobj,quantity_name):
	if Dataobj.quantities[quantity_name].ndim==3:#3d
		x1=(Dataobj.quantities[quantity_name][:1,:,:],Dataobj.quantities[quantity_name][-1:,:,:])
		x2=(Dataobj.quantities[quantity_name][:,:1,:],Dataobj.quantities[quantity_name][:,-1:,:])
		x3=(Dataobj.quantities[quantity_name][:,:,:1],Dataobj.quantities[quantity_name][:,:,-1:])
		return (x1,x2,x3)
		...
		x1=(Dataobj.quantities[quantity_name][0],Dataobj.quantities[quantity_name][-1])
		return (x1,)
