...
def get(Dataobj,quantity_name,args={},vargs={"ShockDeduct":0.1}):
	...
	for i in newneed:
		if i not in Dataobj.quantities.keys() and i not in vargs.keys() and "KHI" not in i: #KHI mode
	...
	if Dataobj.quantities[quantity_name].ndim>=2:
		...
		if Dataobj.quantities[quantity_name].ndim==3:
			...
			div=div[shockx1ind[0]:shockx1ind[1],shockx2ind[0]:shockx2ind[1],shockx3ind[0]:shockx3ind[1]]
		else:
	...
	if quantities["Shock_"+quantity_name].ndim==1:#1d
		...
		quantities["Shock_"+quantity_name]=np.array([div[i] if i in pc else 0 for i in range(len(div))])
		quantities["Shock_Outermost_x1_"+quantity_name]=0 if len(np.where(quantities["Shock_"+quantity_name]!=0)[0])==0 else quantities["Shock_x1"][np.where(quantities["Shock_"+quantity_name]!=0)[0][-1]]
	elif quantities["Shock_"+quantity_name].ndim==2:#2d
		quantities["Shock_Outermost_x1_"+quantity_name]=np.array([0 if len(np.where(i!=0)[0])==0 else quantities["Shock_x1"][np.where(i!=0)[0][-1]] for i in quantities["Shock_"+quantity_name].T]) #default
		quantities["Shock_Outermost_x2_"+quantity_name]=np.array([0 if len(np.where(i!=0)[0])==0 else quantities["Shock_x2"][np.where(i!=0)[0][-1]] for i in quantities["Shock_"+quantity_name]])
	elif quantities["Shock_"+quantity_name].ndim==3:
		quantities["Shock_Outermost_x1_"+quantity_name]=np.array([np.array([0 if len(np.where(j!=0)[0])==0 else quantities["Shock_x1"][np.where(j!=0)[0][-1]] for j in i.T]) for i in quantities["Shock_"+quantity_name].T])
		quantities["Shock_Outermost_x2_"+quantity_name]=np.array([np.array([0 if len(np.where(j!=0)[0])==0 else quantities["Shock_x2"][np.where(j!=0)[0][-1]] for j in i.T]) for i in quantities["Shock_"+quantity_name]])
		quantities["Shock_Outermost_x3_"+quantity_name]=np.array([np.array([0 if len(np.where(j!=0)[0])==0 else quantities["Shock_x3"][np.where(j!=0)[0][-1]] for j in i]) for i in quantities["Shock_"+quantity_name]])

	Dataobj.quantities.update(quantities)
	return Dataobj
def result(quantity_name=None,anafname=None):
	return ["Shock_"+i for i in quantity_name.split(",")]+["Shock_Outermost_x1_"+i for i in quantity_name.split(",")]
...
if __name__=="__main__":
	...
	TestData=get(TestData,"rho",vargs={"ShockDeduct":0.1,"Shock_bou_x1":(2,9),"Shock_bou_x2":(0.1*np.pi,0.9*np.pi)})
	print("Testing Result:", TestData.quantities["Shock_rho"]) #update here!
	from matplotlib import pyplot as plt
	plt.figure("imshow",facecolor="lightgray")
	plt.imshow(TestData.quantities["Shock_rho"],cmap="RdBu",extent=[TestData.quantities["Shock_x1"][0],TestData.quantities["Shock_x1"][-1],TestData.quantities["Shock_x2"][0],TestData.quantities["Shock_x2"][-1]])
	plt.colorbar()
