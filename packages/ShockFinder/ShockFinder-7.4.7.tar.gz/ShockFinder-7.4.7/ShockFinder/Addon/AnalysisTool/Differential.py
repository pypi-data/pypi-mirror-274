	...
	for i, surf in enumerate(surface):
		...
		if surf_dim == "x":
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(y_vals)):
					slice_data = f[boundary_val, y_vals[j][0]:y_vals[j][-1]] if f.ndim == 2 else f[boundary_val, y_vals[j][0]:y_vals[j][-1]][:,z_vals[j][0]:z_vals[j][-1]]
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(x_vals)):
					slice_data = f[x_vals[j][0]:x_vals[j][-1], boundary_val] if f.ndim == 2 else f[x_vals[j][0]:x_vals[j][-1], boundary_val][:,z_vals[j][0]:z_vals[j][-1]]
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(x_vals)):
					slice_data = f[x_vals[j][0]:x_vals[j][-1], : , boundary_val][:,y_vals[j][0]:y_vals[j][-1]]
					tmpl=np.trapz(slice_data, x[x_vals[j][0]:x_vals[j][-1]],axis=0)
					if f.ndim==3:
...
def integrate_pol_sur(f, r, phi, z=None, rr=(), pr=(), zr=(), surface=()):
	...
	for i, surf in enumerate(surface):
		...
		if surf_dim == "r":
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(phi_vals)):
					slice_data = f[boundary_val, phi_vals[j][0]:phi_vals[j][-1]] if f.ndim == 2 else f[boundary_val, phi_vals[j][0]:phi_vals[j][-1]][:,z_vals[j][0]:z_vals[j][-1]]
					tmpl=np.trapz(slice_data*r[boundary_val], phi[phi_vals[j][0]:phi_vals[j][-1]],axis=0)
					if f.ndim==3:
						tmpl=np.trapz(tmpl, z[z_vals[j][0]:z_vals[j][-1]])
					elif len(z_vals)>0:
						tmpl*=z_vals[j][-1]-z_vals[j][0]
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(r_vals)):
					slice_data = f[r_vals[j][0]:r_vals[j][-1], boundary_val] if f.ndim == 2 else f[r_vals[j][0]:r_vals[j][-1], boundary_val][:,z_vals[j][0]:z_vals[j][-1]]
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(r_vals)):
					slice_data = f[r_vals[j][0]:r_vals[j][-1], : , boundary_val][:,phi_vals[j][0]:phi_vals[j][-1]]
					tmpl=np.trapz(slice_data*r[r_vals[j][0]:r_vals[j][-1]], r[r_vals[j][0]:r_vals[j][-1]],axis=0)
					if f.ndim==3:
...
def integrate_sph_sur(f, r, th, phi=None, rr=(), tr=(), pr=(), surface=()):
	...
	for i, surf in enumerate(surface):
		...
		if surf_dim == "r":
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(th_vals)):
					slice_data = f[boundary_val, th_vals[j][0]:th_vals[j][-1]] if f.ndim == 2 else f[boundary_val, th_vals[j][0]:th_vals[j][-1]][:,phi_vals[j][0]:phi_vals[j][-1]]
					tmpl=np.trapz(slice_data*r[boundary_val]**2*np.sin(th[th_vals[j][0]:th_vals[j][-1]]), th[th_vals[j][0]:th_vals[j][-1]],axis=0)
					if f.ndim==3:
						tmpl=np.trapz(tmpl, phi[phi_vals[j][0]:phi_vals[j][-1]])
					elif len(phi_vals)>0:
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(r_vals)):
					slice_data = f[r_vals[j][0]:r_vals[j][-1], boundary_val] if f.ndim == 2 else f[r_vals[j][0]:r_vals[j][-1], boundary_val][:,phi_vals[j][0]:phi_vals[j][-1]]
					tmpl=np.trapz(slice_data*np.sin(th[boundary_val])*r[r_vals[j][0]:r_vals[j][-1]], r[r_vals[j][0]:r_vals[j][-1]],axis=0)
					if f.ndim==3:
						tmpl=np.trapz(tmpl, phi[phi_vals[j][0]:phi_vals[j][-1]])
					elif len(phi_vals)>0:
						tmpl*=phi_vals[j][-1]-phi_vals[j][0]
			...
			for boundary_val in [boundaries[boundary]] if boundary != "both" else [0, -1]:
				for j in range(len(r_vals)):
					slice_data = f[r_vals[j][0]:r_vals[j][-1], : , boundary_val][:,th_vals[j][0]:th_vals[j][-1]]
					tmpl=np.trapz(slice_data*r[r_vals[j][0]:r_vals[j][-1]], r[r_vals[j][0]:r_vals[j][-1]],axis=0)
					if f.ndim==3:
