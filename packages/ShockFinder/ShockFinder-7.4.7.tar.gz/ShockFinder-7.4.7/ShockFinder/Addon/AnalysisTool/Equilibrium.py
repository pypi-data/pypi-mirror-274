...
def Equilibrium(rb,lam,mach,gamma=4/3,rs=1,eng=0,theta_inj=0,steps=10000):
	def cal_vR(sintheta):
		return ((2*mach**2*(gamma-1)/(mach**2*(gamma-1)*sintheta**2+2))*(eng+rs/(2*(rb-rs))-lam**2*math.cos(theta_inj)**2/2/rb**2/sintheta**2))**0.5
	def cal_theta(i):
		sintheta=i[0]
		return rb**2*(1-sintheta**2)-2*rb*sintheta*(rb*sintheta-rs)**2/rs**2*cal_vR(sintheta)**2/mach**2*gamma
	quantities={}
	...
	psc=np.array([cal_vR(math.sin(i)) for i in np.arange(quantities["Accretion_x2_beg"],quantities["Accretion_x2_end"],(quantities["Accretion_x2_end"]-quantities["Accretion_x2_beg"])/steps)])
	quantities["Accretion_vR"]=np.array([psc for i in range(steps)])
	quantities["Accretion_InjFlux"]=Differential.integrate_sph_sur(quantities["Accretion_vR"],np.arange(0,rb,rb/steps),th=np.arange(quantities["Accretion_x2_beg"],quantities["Accretion_x2_end"],(quantities["Accretion_x2_end"]-quantities["Accretion_x2_beg"])/steps),rr=("max",),surface=("r",))[0]
	quantities["Accretion_Min_Tem"]=cal_vR(math.sin(math.pi/2))**2*c**2*mu*mp/mach**2/gamma/k
	quantities["rs"]=rs
	return quantities
...
def SphericalGrid(rbeg,rend,tbeg,tend,rgnum):
	def cal_detr(i):
		detr=i[0]
		return [
		detr/rbeg-2*((rend/(rbeg+detr))**(1/(rgnum-1))-1)/((rend/(rbeg+detr))**(1/(rgnum-1))+1)
		]
	def cal_ptr(detr,detphi):
		return abs(math.log(rend/(rbeg+detr),10)-rgnum*math.log((2+detphi)/(2-detphi),10))
...
def Unit(mass,injflux,injrho,hequantities,basic_m=Msun,basic_r=2*G/c**2,basic_v=c):
	quantities={}
	quantities["unit_r"]=mass*basic_r*basic_m/hequantities["rs"]
	quantities["unit_t"]=quantities["unit_r"]/basic_v
	quantities["unit_rho"]=injflux/injrho/hequantities["Accretion_InjFlux"]/quantities["unit_r"]**2/basic_v
