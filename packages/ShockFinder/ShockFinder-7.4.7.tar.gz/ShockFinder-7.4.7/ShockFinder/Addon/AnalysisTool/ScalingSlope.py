...
def get(Dataobj,quantity_name,args={},vargs={}):
	...
	quantities={
		"ScalingSlope_"+quantity_name:Dataobj.grid[axis].reshape(*[Dataobj.quantities[quantity_name].shape[i] if i==(0 if axis=="x1" else 1 if axis=="x2" else 2) else 1 for i in range(Dataobj.quantities[quantity_name].ndim)])/fix_qt*Dataobj.quantities["Gradient_"+quantity_name+"_"+axis] if not np.all(fix_qt==0) else np.full(fix_qt.shape,0.0)
	}
