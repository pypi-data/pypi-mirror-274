...
def FormatSurface(Surface,Surfaceinfo,Surfaceid):
	...
	if "z" in newsurface.keys():
		newsurface["v"]=[str(tuple([str(tuple(j.tolist())).replace(" ","") for j in i])).replace(" ","").replace("\"","").replace("\'","") for i in newsurface["v"] ]
	else:
		newsurface["v"]=[str(tuple(i.tolist())).replace(" ","") for i in newsurface["v"] ]
...
def DecodeSurface(Surfaceframe,Surfaceinfoframe):
	data=dict(zip(["x","y","v","z"],[np.array(list(Surfaceframe.to_dict()[i].values())) for i in Surfaceframe.keys()]))
	if "z" in data.keys():
		data["v"]=np.array([np.array([np.array([float(k) for k in j.replace("(","").replace(")","").split(",")]) for j in i.split("),(")]) for i in data["v"]])
	else:
		data["v"]=np.array([np.array([float(j) for j in i.replace("(","").replace(")","").split(",")]) for i in data["v"]])
...
if __name__=="__main__":
	x=np.arange(0,100)
	y=np.arange(0,100)
	sur0,surinfo0=CreateSurface(x=x,y=x,z=x,v=x.reshape(len(x),1,1)*(y.reshape(1,len(y),1)-100)*x.reshape(1,1,len(x)))
	#print(sur0)
