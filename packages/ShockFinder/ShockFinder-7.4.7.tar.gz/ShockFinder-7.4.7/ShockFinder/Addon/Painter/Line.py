...
try:
	...
	print("Warning: The \"lmfit\" is not installed, auto-Lorentzian-fitting is not supported!")
...
def lorentzian_fit(x, y, init_gamma_rate="10%",method="lm"):
	...
	init_gamma_rate=float(init_gamma_rate[:-1])/100 if type(init_gamma_rate)==str and init_gamma_rate[-1]=="%" else float(init_gamma_rate)
	initial_guess = [np.max(y), x[np.argmax(y)], init_gamma_rate*(np.max(x)-np.min(x))]  # A, f0, gamma
	fit_params, covariance = curve_fit(lorentzian, x, y, p0=initial_guess,method=method)
...
def dfp(x, amplitude=1, exponent=1.0, center=1.0,factor=1.0):
	return amplitude*(1+factor*((x-center+abs(x-center))/2)**exponent)
			"""
		return ExpressionModel("dfp(x,amplitude,exponent,center,factor)",init_script=script,independent_vars=["x"],nan_policy="omit")
...
def dfexp(x, amplitude=1, factor=1, center=1.0):
	return amplitude*exp((x-center+abs(x-center))/2*factor)
			"""
		return ExpressionModel("dfexp(x,amplitude,factor,center)",init_script=script,independent_vars=["x"],nan_policy="omit")
	BuiltinModels={
		"l":LorentzianModel,
		...
		for i in range(len(peak_data)):
			...
			for j in mode:
				...
				if j in "PE":
					...
					for k in peak_data[i].keys():
						try:
							params[peak_data[i]["name"]+"_"+j+k].set(**peak_data[i][k])
