...
def get_save_sql(data,index,tablename,*quantities_name):
	dt={}
	sql="insert into `"+tablename+"` (`index`) values ("+str(index)+");\nupdate `"+tablename+"` set "
...
def PreSave(filename,*quantities_name,**databaseinfo):
	...
	sql+="insert into `mapping_database`(`name`) values (\""+filename+"\");\nupdate `mapping_database` set `parameters`=\""
	for i in databaseinfo.keys():
		sql+=i+"="+str(databaseinfo[i]).replace("[","").replace("]","").replace("(","").replace(")","").replace("\\","\\\\")+";"
	sql=sql[:-1]+"\" where `name`=\""+filename+"\"\n"
	sql+="create table if not exists `"+filename+"`(`index` int NOT NULL, PRIMARY KEY (`index`));\n"
	for i in quantities_name:
...
def Formatdata(dt): #max 2d
	if dt.ndim==1:#grid or 1d data:
		strc=str(tuple(dt.tolist())).replace(" ","").replace("\"","").replace("\'","")
	elif dt.ndim==2:#2d data:
		strc=str(tuple([tuple(i.tolist()) for i in dt])).replace(" ","").replace("\"","").replace("\'","")
	elif dt.ndim==3:#3d data:
		strc=str(tuple([tuple([tuple(j.tolist()) for j in i]) for i in dt ])).replace(" ","").replace("\"","").replace("\'","")
	return strc
def Decodedata(strc):
	...
	if dt.ndim==1:#1d
		...
		dt=np.array([np.array([np.array([float(k) for k in j]) for j in i]) for i in dt])
	return dt
...
def str_to_array(strc):
	if type(strc) is not str:
		return strc
	strc=strc.replace(" ","")
	if strc=="" or not char_num(strc,"]")==char_num(strc,"[") or not char_num(strc,")")==char_num(strc,"(") or not (strc[0]=="[" and strc[-1]=="]" or strc[0]=="(" and strc[-1]==")"):
		return strc
	pt=0
	ptb=[]
	pte=[]
	...
	tmp=strc.split(",")
	for i in tmp:
		if pt<=1 and (i[0]=="[" and i[-1]=="]" and char_num(i,"]")==char_num(i,"[") or i[0]=="(" and i[-1]==")" and char_num(i,"(")==char_num(i,")")):
...
class MySQL:
	...
	def connect(self):
		if not self.is_connect:
			try:
				self.conn=pymysql.connect(host=self.host,user=self.user,passwd=self.password,port=self.port,db=self.database,charset=self.charset)
