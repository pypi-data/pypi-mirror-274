	...
	def analysis_data(self,analyze_fun,*index,**vargs):
		...
		for ind in index:
			...
			if "vargs" in analyze_fun.__code__.co_varnames:
				kwargs["vargs"]=vargs
			self.printfun("Analyzing \"",info,"\" with \"",resultname,"\" in index:",self.tindex[ind],"...")
	...
	def analyze(self):
		if self.loader==None:#test mode
			...
			for i in self.filesname.keys():
				...
				self.input_default()
				for j in range(len(self.anaf)):
					self.analysis_data(self.anaf[j],len(self.data)-1,**self.vargs[j]) 
		...
		for quantity in quantities:
			...
			for ind in index:
				try:
					...
					try:
						self.printfun("Error: Quantity: \"",quantity,"\" not exists...",color="red")
					except:
