...
class EnvironmentMetaData:
    ...
    def get_column_list(
        ...
        with tracer.start_as_current_span("get_column_list"):
            try:
                ...
                df_results = cls.read_column_csv_with_pandas_return_spark(file_path, first_row_is_header, delimiter, ingesttimestamp, data_product_id, logger)
    ...
    def read_column_csv_with_pandas_return_spark(csv_file_path, first_row_is_header, delimiter, ingesttimestamp, data_product_id,  logger):
        ...
        pandas_df = pd.read_csv(csv_file_path, header=0 if first_row_is_header else None, delimiter=delimiter,  quoting=csv.QUOTE_ALL,  # Ensures multiline support
        ...
        df_results = df_results.withColumn("row_id", concat_ws("-", col("data_product_id"), col("dataset_name"), col("column_name")))
    ...
    def get_dataset_list(cls, 
        ...
        with tracer.start_as_current_span("get_dataset_list"):
            try:
                ...
                csv_file_path = cls.get_csv_file_path(config)
                data_product_id = config["data_product_id"]
                ingesttimestamp = datetime.now()
    ...
    def get_pipeline_list(
        ...
        with tracer.start_as_current_span("get_pipeline_list"):
            try:
                ...
                ingress_folder_sps = ingress_folder_sps.rstrip("/") + "/"
                pipeline_file_path = (
                    f"{ingress_folder_sps}bronze_sps_config_workflows.csv"
                )
                logger.info(f"pipeline_file_path: {pipeline_file_path}")
                ...
                pd_df = pd.read_csv(pipeline_file_path, sep=delimiter, header=0 if first_row_is_header else None)
