...
# Define the parser and request arguments
parser = reqparse.RequestParser()
parser.add_argument('data_product_id', type=str, required=False, default='lava_core', help='ID of the data product (optional)')
parser.add_argument('environment', type=str, required=False, default='dev', help='Environment parameter (optional)')
...
class AltmetricDownload(Resource):
    ...
    def get(self, altmetric_id=None):
        ...
        Returns:
            A JSON response containing the downloaded Altmetric data, or an error message if the download fails.
        """

        args = parser.parse_args()
        data_product_id = args['data_product_id']  # Default value is handled by the parser
        ...
        with tracer.start_as_current_span("__init__"):
            try:
                ...
                if not data_product_id:
                    return jsonify({'error': 'data_product_id parameter is required'}), 400
                
                obj_altmetric_downloader = AltmetricDownloader()
                results = obj_altmetric_downloader.download_altmetric_data(altmetric_id, data_product_id, environment)
                
                if results is None:
                    return jsonify({'error': 'Failed to download Altmetric data'}), 500
...
ns_jira.add_resource(Task, "/task/<string:jira_project>/<string:jira_component>/', defaults={'jira_fields': None}")
ns_jira.add_resource(
    Issue, 
    "/issue/<string:jira_project>/<string:jira_issue_id>/<string:jira_fields>"
)
ns_jira.add_resource(
    Issue, 
     '/issue/<string:jira_project>/<string:jira_issue_id>/', defaults={'jira_fields': None})
...
ns_altmetric.add_resource(
    AltmetricDownload,  "/download_altmetric_data/<string:altmetric_id>", endpoint="altmetric_download"
)
