...
AMPLAPI Environment* AMPL_Environment_Create_WithBin(
    const char* binaryDirectory, const char* binaryName,
    ErrorInformation* errorInfo);
...
AMPLAPI void AMPL_Environment_setBinaryDirectory(Environment* env,
                                                 ...
                                                 ErrorInformation* errorInfo);
AMPLAPI void AMPL_Environment_setBinaryName(Environment* env,
                                            const char* binaryName,
                                            ErrorInformation* errorInfo);
AMPLAPI const char* AMPL_Environment_getBinaryDirectory(
    Environment* env, ErrorInformation* errorInfo);
AMPLAPI const char* AMPL_Environment_getBinaryName(Environment* env,
                                                   ErrorInformation* errorInfo);
AMPLAPI std::size_t AMPL_Environment_size(Environment* env,
                                          ErrorInformation* errorInfo);
...
class EnvironmentIterator {
 public:
  ...
  static ValueType getPointer(ReferencePointer it) {
    ...
    internal::AMPL_Environment_iterator_getPointer(it, &first, &second,
                                                   internal::ErrorInfo());
