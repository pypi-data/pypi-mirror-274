...
class Transcribe:
    ...
    def transcribe_process(self, upload_id: UUID, webhook: Optional[str] = None):
        ...
        json_data = {"webhook": webhook} if webhook else {}
    ...
    def transcribe(self, file_path: str, webhook: Optional[str] = None):
        ...
        Parameters: file_path (str): The path to the file that needs to be transcribed. webhook (Optional[str]): The
        URL to which transcription completion notifications should be sent. If provided, the server will send a POST
        request to this URL upon completion of the transcription.

        Returns: dict: The response from the transcription process, typically containing details about the
