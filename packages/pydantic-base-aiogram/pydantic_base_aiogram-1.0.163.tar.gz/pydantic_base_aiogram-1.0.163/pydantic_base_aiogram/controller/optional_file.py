...
class OptionalFileController(FileController):
    def __init__(self, *args, single_batch=False, **kwargs) -> None:
    ...
    async def format_data(self, self_: "THandler", event: Event[ProxyAlbumMessage], state):
        ...
        if not issubclass(self.field.type_, OptionalAlbum):
            if len(event._event.album._items) > 1:
                raise DataValidationError(self.dialects.REQUIRED_ONLY_ONE_FILE)

            if (file := (event._event.album._items and event._event.album[-1])):
...
class SOptionalFileController(OptionalFileController):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, single_batch=True, **kwargs)
