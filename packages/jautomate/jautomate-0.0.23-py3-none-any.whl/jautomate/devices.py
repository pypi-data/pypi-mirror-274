...
def build_computer_payload(asset: Asset) -> Dict:
    ...
    if getattr(asset, 'build', None):
        payload["extensionAttributes"].append(
            build_computer_extension_attribute("7", "Build", [asset.build])
        )
...
def get_computer_record_by_serial_number(serial_number: str) -> Dict:
    ...
    j_logger.error("Computer with serial number %s not found in Jamf", serial_number)
