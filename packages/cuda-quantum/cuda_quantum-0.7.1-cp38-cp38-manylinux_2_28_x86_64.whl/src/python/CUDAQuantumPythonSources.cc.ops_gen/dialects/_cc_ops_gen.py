...
class ExtractValueOp(_ods_ir.OpView):
  ...
  def __init__(self, result, container, position, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
...
@_ods_cext.register_operation(_Dialect)
@_ods_extend_opview_class(_ods_ext_module)
class GetConstantElementOp(_ods_ir.OpView):
  OPERATION_NAME = "cc.get_const_element"

  _ODS_REGIONS = (0, True)

  def __init__(self, result, constantArray, offset, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(constantArray))
