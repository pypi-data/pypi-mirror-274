"""Main module for the cherrygpt package."""
...
from cherrygpt.chat import ChatManager
from cherrygpt.conversations import ConversationManager
from cherrygpt.encryption import EncryptionManager
from cherrygpt.printer import Printer, PrinterFactory, PrintUtils


@click.group()
@click.version_option(prog_name="cherrygpt", message="%(prog)s %(version)s")
@click.option(
    "--model",
    "-m",
...
def cli(ctx, model, style: str, token_limit: int = 0):
    """*~ cherrygpt - Your Personal Terminal Assistant ~*"""
...
def install():
    """Install the cherrygpt openai api key and create app directories."""
...
def new(ctx):
    ...
    client = OpenAI(
        api_key=enc_manager.get_api_key(), base_url="http://g.manaai.cn:8082/v1"
    )
