...
class RunInfo(_MLflowObject):
    ...
    def to_dto(self) -> RunInfoDto:
        return RunInfoDto(
            ...
            artifact_uri=self.artifact_uri if self.artifact_uri else None,
    ...
    def from_dto(cls, dto):
        ...
        # The default scalar value of zero indicates that the run's end time is absent.
        # An absent end time is represented with a NoneType in the `RunInfo` class
        if end_time == 0:
            end_time = None
        return cls(
            run_uuid=dto.run_uuid,
            run_id=dto.run_id,
