...
from pydra.tasks.nireports.reportlets.utils import _bbox, _get_limits, get_parula
import numpy as np
...
def plot_mosaic(
    ...
    if bbox_mask_file is not None:
        bbox_data = (
            np.asanyarray(nb.as_closest_canonical(nb.load(bbox_mask_file)).dataobj)
            > 1e-3
        )
    ...
    for ii, row_slices in enumerate(main_mosaic_idx):
        for jj, z_val in enumerate(row_slices):
            ...
            if overlay_mask:
                ...
                msk_cmap = cm.Reds  # @type(attrs.NOTHING)Variable
                msk_cmap._init()
