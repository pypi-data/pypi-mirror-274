...
class GATConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj, scale_f=1,
        ...
        if tied_attention == None:
            ...
            alpha_src = alpha_src / (math.sqrt(self.out_channels) * scale_f)
            alpha_dst = None if x_dst is None else (x_dst * self.att_dst).sum(-1)
            alpha_dst = None if x_dst is None else alpha_dst / (math.sqrt(self.out_channels) * scale_f)
