...
class Reader(object):
    ...
    def get_annoation_fileuris(self, data_uri) -> list():
        ...
        for filename in filenames:
            if not filename.lower().endswith(annotation_extensions):
                continue
    ...
    def get_imagenet_annoation_fileuris(self, data_uri) -> list():
        ...
        for image_uri in image_uris:
            if not image_uri.endswith(image_extensions):
                continue
            # 获取相对路径
            ...
            if label_name.endswith("-thumbnail") or label_name.endswith("-webp"):
                continue
    ...
    def get_cityscapes_annotation_fileuris(self, data_uri) -> list():
        ...
        file_uris = list()
        for filename in filenames:
            if "-thumbnail" in filename or "-webp" in filename:
                continue
