...
class CocoFormatter(object):
    ...
    def merge(self, rows: DataFrame) -> DataFrame:
        ...
        categories = rows['categories'].sum()
        results = [{
            "images": images,
    ...
    def _group_by_image_id(self, group: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(ids)):
            ...
            anno = {
                ...
                "iscrowd": iscrowd,
                "rle": (rle if len(rle) > 0 else np.NAN)
    ...
    def to_vistudio_v1(self, ds: "Dataset") -> "Dataset":
        ...
        union_df = group_anno_ds.union(fill_image_ds).to_pandas().drop_duplicates(subset=['image_id', 'data_type'])
        return ray.data.from_pandas(union_df)
