...
class VistudioFormatterPreprocessor(Preprocessor):
    ...
    def _fit_images(self, ds: "Dataset") -> "Dataset":
        ...
        ds = ds.filter(lambda row: row["data_type"] == 'Image' and row['image_id'] not in self.exist_images)
        ...
        vistudio_formatter = VistudioFormatter(
            ...
            tag=self.tag)
        ds = vistudio_formatter.to_vistudio_v1(ray.data.from_pandas(df))
        return ds
    ...
    def _fit_annotations(self, ds: "Dataset") -> "Dataset":
        ...
        filter.drop_duplicates(source=source_df, cols=['image_name', 'data_type', 'artifact_name'], inplace=True)
        return ray.data.from_pandas(source_df)
