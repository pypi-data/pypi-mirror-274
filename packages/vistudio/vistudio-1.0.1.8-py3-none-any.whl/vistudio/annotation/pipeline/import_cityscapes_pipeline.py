...
from vistudio.annotation.processor.importer.annoation.cityscapes_preprocessor import CityscapesFormatPreprocessor
...
class ImportCityscapesPipeline(BaseImportPipline):
    """
    ImportCocoPipeline
    """
    ...
    def _import_image(self):
        ...
        # 读取图片
        cityscapes_reader = Reader(filesystem=self.config.filesystem, annotation_set_id=self.annotation_set_id)

        if self.file_format == 'zip':
            zip_file_uris = cityscapes_reader.get_zip_file_uris(data_uri=self.data_uri)
        ...
        file_uris = cityscapes_reader.get_file_uris(data_uri=self.data_uri, data_types=self.data_types)
        ds = ray.data.from_items(file_uris)
        logit.info("import cityscapes image from json.dataset count = {}".format(ds.count()))
    ...
    def _build_labels(label_names: list(), max_label_id: int) -> dict():
        ...
        for index, label_name in enumerate(label_names):
            labels[str(max_label_id + index)] = label_name
        return labels
    ...
    def _import_annoation(self):
        ...
        file_uris = cityscapes_reader.get_file_uris(data_uri=self.data_uri, data_types=self.data_types,
                                                    import_type='cityscapes')
        ds = ray.data.from_items(file_uris)
        logit.info("import cityscapes annotation .dataset count = {}".format(ds.count()))
        ...
        logit.info(
            "format cityscapes annotation.need_import_annotation_labels:{}".format(need_import_annotation_labels))

        cityscapes_preprocessor = CityscapesFormatPreprocessor(config=self.config,
                                                               annotation_set_id=self.annotation_set_id,
                                                               annotation_set_name=self.annotation_set_name,
                                                               labels=self._get_labels(),
                                                               tag=self.tag)
        final_ds = cityscapes_preprocessor.fit(ds).stats_
