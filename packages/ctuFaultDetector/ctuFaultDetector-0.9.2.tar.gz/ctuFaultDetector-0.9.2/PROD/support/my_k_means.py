...
class GaussianClassifier():
    ...
    def set_confidence_interval(self, data_c, data_w, metric = "TPR", value = 0.95, n_steps=100, report_result = True):
        
        confidence_intervals : np.ndarray = np.linspace(0.8, 1, n_steps)
        ...
        skewed_accuracy = (value * score_data[:, 0] + score_data[:, 1])/ ((2/(value + 1)) * len(data))
        ...
        if metric == "TPR":
            ...
            print(np.sum(score_data[:, :2], axis=1)*(tnr >= value))
            print(max_idx, confidence_intervals[max_idx])
        elif metric == "ACC":
