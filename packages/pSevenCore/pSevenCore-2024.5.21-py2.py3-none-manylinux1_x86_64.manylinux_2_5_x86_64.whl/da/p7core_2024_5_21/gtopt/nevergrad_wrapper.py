...
from .problem import _get_gtopt_negative_infinity, _get_gtopt_positive_infinity, _limited_evaluations
...
class _Blackbox(object):
  ...
  def __init__(self, archive, problem, max_iterations=None, scalability=1, catvars=None):
  ...
  def set_preprocess_linear_response(self, _preprocess_linear_response):
    ...
    for i in np.arange(self.problem.size_full()):
      ...
      if linearity.lower() == "linear":
        ...
        resp_type = self.problem.elements_hint(self.problem.size_x() + i, "@GT/ObjectiveType")
        resp_type = 'minimize' if resp_type is None else resp_type.lower()
        if resp_type in ['minimize', 'maximize']:  # Only responses that to be calculated (objectives and constraints)
  ...
  def _exclude_linear_mask(self, inputs, mask):
    ...
    for category in categories:
      ...
      with self.problem._solve_as_subproblem(vars_hints, objs_hints, cons_hints, doe_mode=False):
        ...
        # TODO Do not allow additional points evaluation, wait until the sample size is sufficient for linears reconstruction
        hints, evaluations = self._preprocess_linear_response(problem=self.problem, sample_x=sample_x, sample_f=sample_f, sample_c=sample_c)
  ...
  def _evaluate_linear_outputs(self, inputs, mask, outputs, archive_idx):
    ...
    for i, input in enumerate(inputs):
      ...
      if category is not None and self.n_category_hits[category] <= 1:
        ...
        continue
      for output_i, resp_i in enumerate(np.where(mask)[0]):
        weights = self.linear_responses_weights[category].get(resp_i, [])
        if len(weights):
  ...
  def _prepare_mask(self, inputs, mask, restore_linears=True):
    ...
    if restore_linears and mask[self.linear_responses].any():
      ...
      outputs_mask *= self._exclude_linear_mask(inputs, mask)
  ...
  def evaluate_batch(self, inputs, mask):
    ...
    if mask[self.linear_responses].any() and self.linear_responses_weights:
      outputs = self._evaluate_linear_outputs(inputs, mask, outputs, inputs_archive_idx)
    return outputs
...
class Objectives(object):
  ...
  def calc(self, *args, **kwargs):
    ...
    if not cache_only:
      values = self.blackbox.evaluate_batch(input=[args], mask=self.mask)[0]
    else:
...
class Constraints(object):
  ...
  def calc(self, args_kwargs, cache_only=False):
    ...
    if not cache_only:
      value = self.blackbox.evaluate_batch(input=[input], mask=self.mask)[0]
    else:
      value = self.blackbox.evaluate_from_cache(input=input, mask=self.mask)
      if value is None:
        return None
...
class ObjectivesCSP(Constraints):
  ...
  def __init__(self, blackbox, problem, tolerance=1.e-5, cache_size=100, csp_objective_type=None, csp_stop_on_feasible=True):
    ...
    if csp_objective_type == "Psi":
      self._calc_objective = lambda inputs, cache_only=False: self._max_psi(inputs, cache_only)
      ...
      self._calc_objective = lambda inputs, cache_only=False: self._max_psi_distance_normed(inputs, max_dist, cache_only)
      self._n_objectives = 1
    elif csp_objective_type == "PsiDistanceInverse":
      self._calc_objective = lambda inputs, cache_only=False: self._max_psi_distance_inverse(inputs, cache_only)
      self._n_objectives = 1
    else:
  ...
  def _distance(self, inputs):
    return np.hypot.reduce(self._init_guess - inputs, axis=1)

  def _max_psi(self, inputs, cache_only=False):
    if not cache_only:
      batch_values = self.blackbox.evaluate_batch(inputs=inputs, mask=self.mask)
    else:
      assert len(inputs) == 1  # Does not support batch mode!!!
      value = self.blackbox.evaluate_from_cache(input=inputs[0], mask=self.mask)
      if value is None:
        return [None]
      batch_values = [value]
    return self._calc_batch_violations(batch_values=batch_values)

  def _max_psi_distance_normed(self, inputs, max_dist, cache_only=False):
    ...
    batch_violations = self._max_psi(inputs=inputs, cache_only=cache_only)
  ...
  def _max_psi_distance_inverse(self, inputs, cache_only=False):
    if self.csp_stop_on_feasible and self._feasible_point_found:
      raise errors.NevergradEarlyStopping("Feasible point found")
    # Set constraint violation to 0 in feasible area
    batch_violations = self._max_psi(inputs=inputs, cache_only=cache_only)
  ...
  def calc(self, *args, **kwargs):
    cache_only = kwargs.get("cache_only", False)
    return self._calc_objective(inputs=np.atleast_2d(args), cache_only=cache_only)[0]
...
def estimate_budget(effective_dim, catvars, max_iterations, responses_scalability, batch_size):
  if max_iterations == 0:
  ...
  # Reduce the budget if it is a pure-categorical problem
  if max_iterations == 0 and effective_dim == len(catvars):
    n_categories = 1
    for levels in catvars.values():
      n_categories *= len(levels)
      if n_categories > np.iinfo(int).max:
        n_categories = np.iinfo(int).max
        break
    if scaled_budget >= n_categories:
      scaled_budget = n_categories
...
def validate(solver, problem, sample_x=None, sample_f=None, sample_c=None):
  ...
  scaled_budget = estimate_budget(effective_dim=effective_dim,
                                  catvars=catvars,
    ...
    def _evaluate_with_archive(inputs, full_mask, *args, **kwargs):
      ...
      if np.any(inputs_mask):
        ...
        if new_masks.shape != batch_shape:
          raise ValueError("Wrong shape of outputs mask. Expected %s, got %s." % (str(batch_shape), str(new_masks.shape)))
        for i, archive_index in enumerate(inputs_archive_idx[inputs_mask]):
          blackbox.archive._update(archive_index, new_outputs[i], new_masks[i])
        # Reduce the limit w.r.t. to scalability, e.g. with scalability N any number K<N of evaluated points counts as N.
      ...
      if mask[blackbox.linear_responses].any() and blackbox.linear_responses_weights:
        outputs = blackbox._evaluate_linear_outputs(inputs, mask, outputs, inputs_archive_idx)
...
def solve(solver, problem, sample_x=None, sample_f=None, sample_c=None):
  ...
  if restore_linear:
    blackbox.set_preprocess_linear_response(
      partial(solver._preprocess_linear_response, mode=api.GTOPT_SOLVE, return_evaluations=True)
    )
  ...
  batch_functions = {objectives.calc: objectives.calc_batch, constraints.calc: constraints.calc_batch}
  ...
  with shared.sigint_watcher(solver):
    ...
    try:
      ...
      optimizer.minimize(objectives.calc,
                         executor=executor,
                         batch_mode=True,
                         verbosity=verbosity,
