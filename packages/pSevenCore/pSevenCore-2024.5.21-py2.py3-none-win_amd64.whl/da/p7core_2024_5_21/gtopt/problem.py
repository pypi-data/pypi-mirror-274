...
def _evaluate_with_time_check(batch_x, size_r, response, timecheck):
...
class ProblemGeneric(with_metaclass(_ProblemMetaclass, object)):
  ...
  * :class:`~da.p7core.gtopt.ProblemMeanVariance`
  ...
  def _validate(self):
    ...
    try:
      ...
      linear_parameters_vector_hint = _backend.normalize_option_name("@GTOpt/LinearParameterVector")
      ...
      for i, resp in enumerate(self._objectives):
        ...
        if objective_type == "Payload":
          ...
          if resp.hints.get(linear_parameters_vector_hint):
            raise _ex.InvalidProblemError('Hint ' + linear_parameters_vector_hint + ' cannot be used for ' + objective_type + ' objective.')
      ...
      for resp in (self._objectives + self._constraints):
        ...
        if linear_parameters:
          try:
            linear_parameters = _numpy.array(linear_parameters, dtype=float, copy=False)
          except:
            exc_info = _sys.exc_info()
  ...
  def _regression_string(self, weights, alt_expression, ndig=None):
    try:
      ...
      terms = [(("+" if weight > 0 else "-"), (fmt % abs(weight)), var.name) for weight, var in zip(weights[:-1], self._variables) if weight]
      if not terms:
  ...
  * In some cases you may prefer to evaluate objectives and constraints separately.
  ...
  def _evaluate(self, queryx, querymask, timecheck):
    ...
    mask = _shared.as_matrix(mask, dtype=bool, shape=querymask.shape, name="responses mask")
    resp, mask = self._translate_user_response(resp, mask, (self._regular_responses_mask() if self._payload_objectives else None), "evaluate")
      ...
      if kind == "continuous":
        ...
        if kind == "integer":
          ...
          if _numpy.not_equal(bounds, rounded_bounds).any() or (sample_x[:, i][valid_points] != rounded_input[valid_points]).any():
            ...
            if sample_x is original_sample_x:
              sample_x = sample_x.copy() # make a copy if we have not already
            sample_x[point_index, i] = _numpy.array(bounds, copy=False)[value_index] # assign original, unmodified values from allowed set
  ...
  def _refill_analytical_history(self, history_records=None, history_fields=None):
    ...
    linear_weights = [(i, _numpy.array(weights_i, copy=False).reshape(-1)) \
    ...
    for idx in _numpy.where(lin_mask)[0]:
      ...
      try:
        ...
        if x.shape[0] <= (x.shape[1] + 1 - n_consts):
          ...
          weights, _ = _utils._linear_rsm_fit(x, resp[:, _numpy.newaxis])
          #rrmse[0] = _numpy.nan
          rrmse = None
        else:
          weights, rrmse = _utils._linear_rsm_stepwise_fit(x, resp[:, _numpy.newaxis])
...
class ProblemConstrained(ProblemGeneric):
  ...
  * :meth:`~da.p7core.gtopt.ProblemConstrained.define_constraints()` (or :meth:`~da.p7core.gtopt.ProblemConstrained.define_constraints_batch()`)
  ...
  def evaluate(self, queryx, querymask):
    ...
    """
    queryx = _numpy.array(queryx, dtype=float, ndmin=2, copy=False)
    querymask = _numpy.array(querymask, dtype=bool, ndmin=2, copy=False)
    size_f = self.size_f()
  ...
  * ``None`` value indicates that evaluation of a constraint was skipped.
  ...
  def define_objectives_batch(self, x):
    ...
    * ``None`` value indicates that evaluation of an objective was skipped.
    ...
    Note that skipped and failed evaluations may stop optimization prematurely.
    """
    return _evaluate_with_time_check(x, self.size_f(), self.define_objectives, self._timecheck)
  ...
  def define_constraints_batch(self, x):
    ...
    * ``None`` value indicates that evaluation of a constraint was skipped.
    ...
    Note that skipped and failed evaluations may stop optimization prematurely.
    """
    return _evaluate_with_time_check(x, self.size_c(), self.define_constraints, self._timecheck)
  ...
  * ``None`` value indicates that evaluation of a gradient was skipped.
  ...
  def define_objective_batch(self, x):
    ...
    * ``None`` value indicates that objective evaluation was skipped.
    ...
    Note that skipped and failed evaluations may stop optimization prematurely.
    """
    return _evaluate_with_time_check(x, self.size_f(), self.define_objective, self._timecheck)
  ...
  def define_objective_gradient_batch(self, x):
    ...
    * ``None`` value indicates that evaluation of a gradient was skipped.
    ...
    Note that skipped and failed evaluations may stop optimization prematurely.
    """
    return _evaluate_with_time_check(x, _grad_size(self)[0], self.define_objective_gradient, self._timecheck)
  ...
  def define_constraints_gradient_batch(self, x):
    ...
    * ``None`` value indicates that evaluation of a gradient was skipped.
    ...
    Note that skipped and failed evaluations may stop optimization prematurely.
    """
    return _evaluate_with_time_check(x, _grad_size(self)[1], self.define_constraints_gradient, self._timecheck)
...
class _LimitedEvaluations(object):
  ...
  def _update_limits(self, calcs, masks):
    try:
      ...
      obtained = (_numpy.array(masks, dtype=bool, copy=False).sum(axis=0)[:len(self._limits)] + self._scalability - 1) // self._scalability
...
class _SlicedEvaluationsBlackbox(object):
  def __init__(self, evaluate, input_size, response_size, batch_size, watcher):
