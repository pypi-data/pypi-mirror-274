...
class AutoMLBaseSettings:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        :type track_child_runs: bool
        :param n_best_runs: Number of best runs to upload per partition for a Many Models Run. Defaults to 1.
        :type n_best_runs: int
        ...
        if isinstance(featurization, FeaturizationConfig):
            ...
            # If the featurization config is a dictionary, we have to make sure it is valid
            # We will use the FeaturizationConfig mechanism for that.
            temp_confog = FeaturizationConfig()
            temp_confog._from_dict(featurization)
            self.featurization = temp_confog.__dict__
        else:
        ...
        self._ignore_package_version_incompatibilities = (
            "AUTOML_IGNORE_PACKAGE_VERSION_INCOMPATIBILITIES" in os.environ
        )
        ...
        if (
            self.task_type == constants.Tasks.CLASSIFICATION
            and self.enable_dnn
            and self.featurization == FeaturizationConfigMode.Off
        ):
            self.featurization = FeaturizationConfigMode.Auto
            logger.info(
        ...
        if (
            is_feature_sweeping_possible
            and self.enable_dnn
            and self.task_type == constants.Tasks.CLASSIFICATION
        ...
        if is_timeseries:
            ...
            self.features_unknown_at_forecast_time = forecasting_parameters.formatted_unknown_features
            ...
            # In future, we will detect, if we have multiple time series in the data set.
            # Here we set it to True, because we do not support data sets with one time series
            # in the distributed runs and we will detect this value in the non distributed ones.
            self.has_multiple_series = True

            # set enabling future regressors
            self._enable_future_regressors = forecasting_parameters._enable_future_regressors

            # If cv is enabled while cv_step_size is not set by user, set it automatically.
            if self.n_cross_validations is not None and self.cv_step_size is None:
                self.cv_step_size = TimeSeries.AUTO
        # pop all the forecasting parameters if any.
        ...
        if is_timeseries:
            ...
            if self.features_unknown_at_forecast_time is not None:
                if self.enable_dnn:
                    self._enable_future_regressors = True
                else:
                    logging.warning(
                        "Automatic support of features unknown at forecast time is only available for \
                        dnn models."
                    )
    ...
    def _verify_settings(self):
        ...
        if (
            self.max_cores_per_iteration is not None
            and self.max_cores_per_iteration != -1
            and (
                self.max_cores_per_iteration < 1
                or self.max_cores_per_iteration > AutoMLBaseSettings.MAX_CORES_PER_ITERATION
            )
        ):
            raise ConfigException._with_error(
                AzureMLError.create(
        ...
        if self.iteration_timeout_minutes is not None and (
            self.iteration_timeout_minutes < 1
            or self.iteration_timeout_minutes > AutoMLBaseSettings.MAXIMUM_ITERATION_TIMEOUT_MINUTES
        ):
            raise ConfigException._with_error(
                AzureMLError.create(
                    ArgumentOutOfRange,
        ...
        if self.experiment_timeout_minutes is not None and (
            self.experiment_timeout_minutes < AutoMLBaseSettings.MIN_EXPTIMEOUT_MINUTES
            or self.experiment_timeout_minutes > AutoMLBaseSettings.MAXIMUM_EXPERIMENT_TIMEOUT_MINUTES
        ):
        ...
        if not self.track_child_runs and self.n_best_runs is not None:
            if not isinstance(self.n_best_runs, int):
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidInputDatatype,
                        target="n_best_runs",
                        input_type=type(self.n_best_runs),
                        supported_types=int.__name__,
                    )
                )

            if self.iterations < self.n_best_runs:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        ArgumentOutOfRange,
                        target="n_best_runs",
                        argument_name="n_best_runs",
                        min=1,
                        max="Number of Iterations or 10, whichever is smaller",
                    )
                )

            if self.n_best_runs < 1 or self.n_best_runs > 10:
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidValuesInData,
                        target="n_best_runs",
    ...
    def _validate_timeseries_featurization_settings(self, featurization: Dict[str, Any]) -> None:
        ...
        warning_msg_template = (
            "Only {0} is supported in blocked_transformer list for forecasting task. "
            "Others are not supported and all the settings related to them will be ignored by AutoML."
        )
        blocked_transformers = featurization.get("_blocked_transformers")
        if hasattr(SupportedTransformers, 'TimeIndexFeaturizer') and \
                isinstance(blocked_transformers, list) and \
                SupportedTransformers.TimeIndexFeaturizer not in blocked_transformers:
            self._warning(warning_msg_template.format(SupportedTransformers.TimeIndexFeaturizer))
