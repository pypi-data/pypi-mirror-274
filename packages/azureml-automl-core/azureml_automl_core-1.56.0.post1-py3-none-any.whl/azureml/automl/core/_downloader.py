...
class Downloader:
    ...
    @classmethod
    def get_download_path(cls, file_name: str, target_dir: str,
                          prefix: Optional[str] = None, md5hash: Optional[str] = None) -> str:
        """
        ...
        """
        if os.environ.get('AZUREML_AUTOML_OPTIMIZED_DOWNLOADS') and md5hash:
            tmpDir = tempfile.gettempdir()
            os.makedirs(os.path.join(tmpDir, 'automl_downloads'), exist_ok=True)
            result = os.path.join(tmpDir, 'automl_downloads', md5hash)
            print(f"Downloading ({cls},{file_name},{target_dir},{prefix}  to {result}", file=sys.stderr)
            return result
    ...
    def download(cls, download_prefix: str, file_name: str, target_dir: str, md5hash: Optional[str] = None,
                 prefix: Optional[str] = None, retries: int = 4) -> Optional[str]:
        """
    ...
    def _download(cls, download_url: str, download_path: str, md5hash: Optional[str] = None) -> bool:
        ...
        tmp_download_path = download_path + ".tmp"
        try:
            download_hash = cls._download_file(download_url, tmp_download_path)
            ...
            has_download_failed = True
            logger.warning("Exception when trying to download model: {}".format(ose.strerror))
            ...
            if has_download_failed:
                Downloader.remove_file(tmp_download_path)
        ...
        if not has_download_failed:
            try:
                os.rename(tmp_download_path, download_path)
            except (FileExistsError, FileNotFoundError):
                pass  # the file can be downloaded in a parallel thread or process
