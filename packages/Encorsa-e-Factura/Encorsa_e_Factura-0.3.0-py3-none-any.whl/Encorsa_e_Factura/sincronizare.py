...
def get_token_with_refresh(refresh_token, clientID, clientSecret, parameters):
    ...
    try:
        ...
        error_message = f"Error at getting ANAF aceess token. Error message: {str(e)}"
        print(error_message)
        raise Exception(error_message)
...
def xml_to_pdf_to_base64(xml_data, parameters):
    ...
    while attempts < max_retries:
        try:
            ...
            print(f"Error when converting the XML to PDF: {str(e)}")
            attempts += 1

    print("Maximum retries reached, PDF content could not be retrieved.")
    raise Exception(
        "Maximum retries reached, PDF content could not be retrieved.")
...
def read_json_parameters(file_path):
    ...
    try:
        ...
        print(f"Error: The file {file_path} cannot opened/used. " + str(ex))
        raise Exception(
            f"Error: The file {file_path} cannot opened/used. " + str(ex))
...
def startRunning(json_file_path, xml_template_file_path, xml_nota_template_file_path=None):
    ...
    try:
        ...
        print("Error in main function: " + str(ex))
