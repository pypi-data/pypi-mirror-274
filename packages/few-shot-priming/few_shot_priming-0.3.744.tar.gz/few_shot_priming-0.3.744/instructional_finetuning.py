...
def run_instructiona_fine_tuning_similar(config, params, experiment, validate, few_shot_size, logger, model_path_fine_tuned,
    ...
    if fine_tune:
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune)
        if validate:
            test_data = val_data
    ...
    for _, record in dataset[experiment_type].iterrows():
        ...
        if fine_tune:
            ...
            train_data, _, _ = prepare_dataset(new_dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune)
            val_data = train_data
        else:
            new_dataset["test"] = dataset["test"]
            new_dataset["validation"] = dataset["validation"]
            train_data, val_data, test_data = prepare_dataset(new_dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune)
            if validate:
                test_data = val_data
...
def run_instructional_fine_tuning(config, params, experiment, validate, few_shot_size, logger, model_path_fine_tuned, debug,
    ...
    # unk. we want this to be different from the eos token
        ...
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len=cutoff_len,
                                                          few_shot_size=few_shot_size, fine_tune=fine_tune, diverse=diverse, )
