...
def prepare_dataset(dataset, prompter, label_mapper, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False, alpaca_model=False):
    huggingface_dataset = {}
    ...
    if not fine_tune:
        examples = format_prompt(dataset["training"], tokenizer, label_mapper,  cutoff_len, few_shot_size, logger, alpaca_examples=alpaca_model)
...
def evaluate(experiment, model, model_name, validate, test_data, tokenizer, prompter, logger):
...
def run_instructiona_fine_tuning_similar(config, params, experiment, validate, few_shot_size, logger, model_path_fine_tuned,
                                         debug=False,
                                         topic_count=None, similarity=None, fine_tune=True, analyze_primes=False, path_similar_examples=None ):
    all_test_preds = []
    all_test_labels = []
    ...
    if fine_tune:
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune, alpaca_model=alpaca_model)
        if validate:
            test_data = val_data
    ...
    for _, record in dataset[experiment_type].iterrows():
        ...
        if fine_tune:
            ...
            train_data, _, _ = prepare_dataset(new_dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune, alpaca_model=alpaca_model)
            val_data = train_data
        else:
            new_dataset["test"] = dataset["test"]
            new_dataset["validation"] = dataset["validation"]
            train_data, val_data, test_data = prepare_dataset(new_dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=few_shot_size, fine_tune=fine_tune, alpaca_model=alpaca_model)
            if validate:
                test_data = val_data
...
def run_instructional_fine_tuning(config, params, experiment, validate, few_shot_size, logger, model_path_fine_tuned, debug,
    ...
    if sampling_strategy =="similar":
        return run_instructiona_fine_tuning_similar(config, params, experiment, validate, few_shot_size, logger, model_path_fine_tuned, debug, topic_count, similarity,
                                                    fine_tune=fine_tune, analyze_primes=analyze_primes, path_similar_examples=path_similar_examples)
    else:
    ...
    # unk. we want this to be different from the eos token
        ...
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len=cutoff_len,
                                                          few_shot_size=few_shot_size, fine_tune=fine_tune, alpaca_model=alpaca_model )
        ...
        metrics = evaluate(experiment, model, model_name, validate, test_data, tokenizer, prompter, logger)
        log_metrics(logger, metrics, level=logging.WARNING)
        return metrics
