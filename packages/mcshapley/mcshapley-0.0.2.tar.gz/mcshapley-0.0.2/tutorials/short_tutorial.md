...
All of the Non-temporal Functions have the invp flag set to False by default. The invp flag when set proportionally inverses the scores inputted into the function. A proportional inverse takes the maximum of the scores and replaces every score with it's difference with the max, basically making every lower score much larger while mainting their proportion to other scores. The proportional inverse is used to attribute scores to members correctly when smaller scores are better than larger scores. 
