...
class Output(BaseAbstraction):
    ...
    def save(self) -> None:
        def stream_request(path: Path) -> Generator[OutputSaveRequest, None, None]:
        ...
        if self.path.is_dir():
            ...
            path = shutil.make_archive(self.path.name, "zip", self.path)
            path = Path(path)

        res: OutputSaveResponse = self.run_sync(self.stub.output_save_stream(stream_request(path)))
        if not res.ok:
