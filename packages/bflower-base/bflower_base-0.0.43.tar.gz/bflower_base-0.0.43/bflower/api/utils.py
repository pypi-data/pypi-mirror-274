...
from bflower.services.store.schema import StoreComponentCreate
from bflower.services.store.utils import get_lf_version_from_pypi
...
def get_file_path_value(file_path):
    ...
    if not path.exists() or not str(path).startswith(user_cache_dir("bflower", "bflower")):
        return ""
    return file_path
...
async def check_langflow_version(component: StoreComponentCreate):
    from bflower.version.version import __version__ as current_version  # type: ignore
