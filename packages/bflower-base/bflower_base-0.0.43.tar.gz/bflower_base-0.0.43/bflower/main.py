...
from bflower.api import router
from bflower.initial_setup.setup import create_or_update_starter_projects
from bflower.interface.utils import setup_llm_caching
from bflower.services.plugins.langfuse_plugin import LangfuseInstance
from bflower.services.utils import initialize_services, teardown_services
from bflower.utils.logger import configure
...
def get_lifespan(fix_migration=False, socketio_server=None):
    from bflower.version import __version__  # type: ignore
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            if "bflower migration --fix" not in str(exc):
                logger.error(exc)
            raise
        # Shutdown message
...
if __name__ == "__main__":
    ...
    from bflower.__main__ import get_number_of_workers

    configure()
    uvicorn.run(
        "bflower.main:create_app",
        host="127.0.0.1",
        port=7860,
