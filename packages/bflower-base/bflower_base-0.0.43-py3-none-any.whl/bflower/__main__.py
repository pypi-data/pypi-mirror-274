...
from bflower.main import setup_app
from bflower.services.database.utils import session_getter
from bflower.services.deps import get_db_service
from bflower.services.utils import initialize_services
from bflower.utils.logger import configure, logger
from bflower.utils.util import update_settings
...
def run(
    ...
    log_file: Path = typer.Option("logs/bflower.log", help="Path to the log file.", envvar="LANGFLOW_LOG_FILE"),
...
def build_new_version_notice(current_version: str, package_name: str):
    ...
    if version_is_prerelease(current_version):
        # curl -s "https://pypi.org/pypi/bflower/json" | jq -r '.releases | keys | .[]' | sort -V | tail -n 1
...
def print_banner(host: str, port: int):
    ...
    try:
        from bflower.version import __version__ as langflow_version  # type: ignore

        is_pre_release |= is_prerelease(langflow_version)  # Update pre-release status
        notice = build_version_notice(langflow_version, "bflower")
        notice = stylize_text(notice, "bflower", is_pre_release)
        if notice:
            notices.append(notice)
        package_names.append("bflower")
        package_name = "Bflower"
    except ImportError:
        langflow_version = None

    # Attempt to handle bflower-base similarly
    if langflow_version is None:  # This means bflower.version was not imported
        try:
            from importlib import metadata

            langflow_base_version = metadata.version("bflower-base")
            is_pre_release |= is_prerelease(langflow_base_version)  # Update pre-release status
            notice = build_version_notice(langflow_base_version, "bflower-base")
            notice = stylize_text(notice, "bflower-base", is_pre_release)
            if notice:
                notices.append(notice)
            package_names.append("bflower-base")
    ...
    info_text = "Collaborate, and contribute at our [bold][link=https://github.com/bflower-ai/bflower]GitHub Repo[/link][/bold] :rocket:"
...
def run_langflow(host, port, log_level, options, app):
    ...
    try:
        if platform.system() in ["Windows"]:
            ...
            from bflower.server import LangflowApplication
...
def superuser(
    ...
    with session_getter(db_service) as session:
        from bflower.services.auth.utils import create_super_user
