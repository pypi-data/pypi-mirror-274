...
from bflower.services.store.exceptions import APIKeyError, FilterError, ForbiddenError
...
from bflower.services.store.utils import (
...
def get_id_from_search_string(search_string: str) -> Optional[str]:
    ...
    if "www.bflower.store/store/" in search_string:
...
class StoreService(Service):
    """This is a service that integrates bflower with the store which
