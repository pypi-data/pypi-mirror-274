...
class BaseStrategy(ABC):
    ...
    def subscribe_strikes(self):
        if LiveFeeds.price_feed_connected():
            default_strikes = LiveFeeds.price_feed.default_strike_range
            for index in self.indices_to_trade:
                logger.info(
                    f"Tracking default {default_strikes} strikes for {index.name} in {self.__class__.__name__}"
                )
                LiveFeeds.price_feed.command_queue.put(
                    (
                        ...
                        {
                            "underlyings": [index],
                            "range_of_strikes": default_strikes,
