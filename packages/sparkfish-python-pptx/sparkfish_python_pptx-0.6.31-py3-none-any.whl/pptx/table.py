...
class _RowCollection(Subshape):
    ...
    def add(self, copyIndex=None, insertIndex=None):
        """
        Adds a new row to the table, possibly copying formatting from an existing row.
        - copyIndex: index of the row to copy formatting from (None for default formatting)
        - insertIndex: position to insert the new row (None for append at end)
        """
        if copyIndex is not None:
            row_to_copy = self._tbl.tr_lst[copyIndex]
            new_row = copy.deepcopy(row_to_copy)
        else:
            new_row = OxmlElement('a:tr')  # Create a new empty row element if no row to copy

        if insertIndex is None or insertIndex >= len(self):
            self._tbl.append(new_row)
        else:
            # Remove rows from insertIndex to end, then add them back after new row
            rows_to_reinsert = [self._tbl.tr_lst.pop(i) for i in range(insertIndex, len(self))]
            self._tbl.append(new_row)
            for row in rows_to_reinsert:
                self._tbl.append(row)

        return _Row(new_row, self)  # Return the new row instance
