                                                   ...
                                                   const std::vector<double>& derivatives )
{
    ...
    {
        if( x <= positions.front( ) )
        {
            auto f = values.front( ) + derivatives.front( ) * ( x - positions.front( ) );

            return { f, derivatives.front( ) };
        }

        if( x >= positions.back( ) )
        {
            auto f = values.back( ) + derivatives.back( ) * ( x - positions.back( ) );

            return { f, derivatives.back( ) };
        }
                                                    ...
                                                    bool constantExtrapolation )
{
    ...
    return [=]( double x ) MLHP_PURE -> std::array<double, 2>
    {
        if( constantExtrapolation )
        {
            if( x <= positions.front( ) ) return { values.front( ), 0.0 };
            if( x >= positions.back( ) ) return { values.back( ), 0.0 };
        }
                                                     ...
                                                     size_t degree )
{
    ...
    MLHP_CHECK( npoints > 1, "At least twi interpolation points needed." );
    MLHP_CHECK( npoints == values.size( ), "Inconsistent sizes." );
