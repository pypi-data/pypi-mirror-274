...
struct Kinematics
{
    using Evaluate = void( const BasisFunctionEvaluation<D>& shapes,
...
//                                            std::span<const double, N> totalStrainIncrement,
...
{
    using Evaluate = void( const BasisFunctionEvaluation<D>& shapes,
                           std::span<const double> strainResult,
                           std::span<double> stressResult,
                           size_t nstrainvectors );

    std::function<Evaluate> evaluate;
};

template<size_t D>
using PostprocessingMeshCreator = std::function<bool( const MeshMapping<D>& mapping,
...
using VectorFunction = std::function<std::array<double, D2>( std::array<double, D1> )>;
...
template<size_t D, typename... Args> 
using ScalarFunctions = std::vector<ScalarFunction<D, Args...>>;
...
using LinearOperator = std::function<void( const double* vector, double* target )>;
