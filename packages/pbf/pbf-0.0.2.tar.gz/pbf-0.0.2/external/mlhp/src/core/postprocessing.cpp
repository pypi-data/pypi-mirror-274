                                           ...
                                           const std::string& name )
{
    auto evaluate = []( auto&&, auto&& stress, auto&& target )
    {
        ...
        else if constexpr ( D == 2 )
        {
            ...
            target[0] = std::sqrt( S11 * S11 - S11 * S22 +
                S22 * S22 + 3 * S12 * S12 );
        }
        else if constexpr ( D == 3 )
        {
            auto [S11, S22, S33, S23, S13, S12] = stress;
                                        ...
                                        const std::string& name )
{
    ...
    {
        MLHP_CHECK( mesh.ncells( ) == ncells, "Inconsistent cell data "
            "vector size in cell data processor." );
...
ElementProcessor<D> convertToElementProcessor( CellProcessor<D>&& processor_ )
{
    ...
    return ElementProcessor<D>
    {
        ...
        .evaluateCell = [=]( auto& anyCache, auto targets, 
            auto elementIndex, auto&, auto& mapping )
        { 
            auto& cache = anyCache.template cast<typename CellProcessor<D>::Cache>( );

            processor->evaluateCell( cache, targets, elementIndex, mapping );
        },
                                         ...
                                         const AbsBasis<D>& basis )
{
    struct Cache
    {
        typename ElementProcessor<D>::Cache pcache;
    ...
    return CellProcessor<D>
    {
        ...
        .evaluateCell = [=, &basis]( auto& anyCache, auto targets, auto cellIndex, auto& )
        {
            ...
            processor->evaluateCell( cache.pcache, targets, cellIndex,
...
struct BasisOutputEvaluator : public OutputEvaluator<D, BasisOutputEvaluator<D>>
{
    ...
    void evaluateCell( CellIndex elementIndex, auto& spans, auto& rst, auto isGrid )
    {
        ...
        processor->evaluateCell( processorCache, spans, elementIndex, locationMap, *mapping );
    }
...
template<size_t D>
void writeOutput( const PostprocessingMeshCreator<D>& meshCreator,
...
void writeOutput( const AbsMesh<D>& mesh,
                  const PostprocessingMeshCreator<D>& meshCreator,
                  const CellProcessor<D>& processor,
                  const MeshWriter& writer )
{
...
template<size_t D> 
PostprocessingMeshCreator<D> createGridOnCells( std::array<size_t, D> resolution,
                                                PostprocessTopologies topologies )
{
...
PostprocessingMeshCreator<3> createMarchingCubesVolume( const ImplicitFunction<3>& function,
...
{
    return createMarchingCubesVolume( function, uniformResolution( resolution ) );
}
...
MLHP_EXPORT
PostprocessingMeshCreator<3> createMarchingCubesBoundary( const ImplicitFunction<3>& function,
                                                          const ResolutionDeterminor<3>& determinor,
...
PostprocessingMeshCreator<3> createMarchingCubesBoundary( const ImplicitFunction<3>& function,
                                                          ...
                                                          bool recoverMeshBoundaries )
{
    return createMarchingCubesBoundary( function, uniformResolution( resolution ), recoverMeshBoundaries);
}
