...
CoordinateList<3> readStl( const std::string& filename, bool flipOnOppositeNormal )
{
    ...
    if( !ascii )
    {
        ...
        constexpr auto facetsize = 3 * 4 * 4 + 2; // 3 * 4 * sizeof( float ) + sizeof( int16_t )
        ...
        for( size_t itriangle = 0; itriangle < ntriangles; ++itriangle )
        {
            ...
            { 
                return *( reinterpret_cast<std::float_t*>( buffer ) + index++ );
            };
...
TriangulationDomain::TriangulationDomain( const std::string& stlfile ) :
    TriangulationDomain( std::make_shared<Triangulation<3>>( createTriangulation<3>( readStl( stlfile) ) ) )
{ }

MLHP_EXPORT
TriangulationDomain::TriangulationDomain( const TriangulationPtr& triangulation ) :
    TriangulationDomain( triangulation, std::make_shared<KdTree<3>>( buildKdTree( *triangulation ) ) )
{ }
...
std::any TriangulationDomain::initialize( ) const
{
    return std::vector<size_t> { };
}

bool TriangulationDomain::inside( std::array<double, 3> xyz, std::any& cache ) const
{
    if( auto state = kdtree_->stateFull( kdtree_->fullIndexAt( xyz ) ); state != 0 )
    {
        return state > 0;
    }

    auto& target = std::any_cast<std::vector<size_t>&>( cache );

    auto ray = std::array { 0.3123, -0.423, 0.8323 };

    return countIntersections( *kdtree_, *triangulation_, xyz, ray, target ) % 2;
}
                          ...
                          std::any& anyCache )
{
    ...
    nd::execute( resolution, [&]( std::array<size_t, 3> ijkCell )
    {
        ...
        auto createCorner = [&]( std::array<size_t, 3> ijk ){ return array::extract( rstGrid, ijk ); };
        ...
        if( index == 255 )
        {
            for( size_t ivertex = 0; ivertex < 8; ++ivertex )
            {
                connectivity.push_back( vertexMap.vertexIndex( vertexIjk( ivertex ), createCorner ) );
            }
        ...
        {
            for( size_t itet = 0; itet < marchingcubes::tetrahedra[index].size( ) / 4; ++itet )
            {
                for( size_t ivertex = 0; ivertex < 4; ++ivertex )
                {
                    if( auto id = marchingcubes::tetrahedra[index][4 * itet + ivertex]; id >= 8 )
                    {
                        connectivity.push_back( vertexMap.edgeIndex( vertexIjk( marchingcubes::numbering[id - 8][0] ),
                                                                     vertexIjk( marchingcubes::numbering[id - 8][1] ), createEdge ) );
                    }
                    else
                    {
                        connectivity.push_back( vertexMap.vertexIndex( vertexIjk( id ), createCorner ) ); 
                    }
                }
...
template<size_t D>
CellAssociatedTriangulation<D>::operator Triangulation<D>& ( )
{
    return triangulation;
}

template<size_t D>
CellAssociatedTriangulation<D>::operator const Triangulation<D>& ( ) const
{
    return triangulation;
}
                                                      ...
                                                      std::array<size_t, 3> resolution )
{
    ...
    CellAssociatedTriangulation<3> result { Triangulation<3> { }, { }, { 0 }, { } };

    auto vertexOffsets = std::vector<size_t> { 0 };
    auto mask = std::vector<std::uint8_t> ( mesh.ncells( ) );
    auto map = std::vector<size_t> { };
    auto ncellsboundary = size_t { 0 };
    ...
    #pragma omp parallel
    {
        ...
        auto localTriangleOffsets = std::vector<size_t>{ 0 };
        auto localVertexOffsets = std::vector<size_t>{ 0 };
        ...
        for( std::int64_t ii = 0; ii < ncells; ++ii )
        {
            ...
            if( mask[icell] )
            {
                ...
                for( auto ivertex = localVertexOffsets.back( ); ivertex < localRst.size( ); ++ivertex )
                {
                    localXyz[ivertex] = mapping( localRst[ivertex] );
                }

                localVertexOffsets.push_back( localRst.size( ) );
                localTriangleOffsets.push_back( localTriangles.size( ) / 3 );
            }
        }
        ...
        #pragma omp single
        {
            ncellsboundary = std::accumulate( mask.begin( ), mask.end( ), size_t { 0 } );
            map = algorithm::backwardIndexMap<size_t>( mask );
        
            vertexOffsets.resize( ncellsboundary + 1 );
            result.cells.resize( ncellsboundary );
            result.triangleOffsets.resize( ncellsboundary + 1 );
        }

        for( size_t ilocal = 0; ilocal < localCells.size( ); ++ilocal )
        {
            result.cells[map[localCells[ilocal]]] = localCells[ilocal];
            vertexOffsets[map[localCells[ilocal]] + 1] = localVertexOffsets[ilocal + 1] - localVertexOffsets[ilocal];
            result.triangleOffsets[map[localCells[ilocal]] + 1] = localTriangleOffsets[ilocal + 1] - localTriangleOffsets[ilocal];
        }
        ...
        #pragma omp single
        {
            ...
            std::partial_sum( result.triangleOffsets.begin( ), result.triangleOffsets.end( ), result.triangleOffsets.begin( ) );

            result.rst.resize( vertexOffsets.back( ) );
            result.triangulation.vertices.resize( vertexOffsets.back( ) );
            result.triangulation.triangles.resize( result.triangleOffsets.back( ) );
        }

        for( size_t ilocal = 0; ilocal < localCells.size( ); ++ilocal )
        {
            auto iglobal = map[localCells[ilocal]];

            for( size_t ivertex = 0; ivertex < localVertexOffsets[ilocal + 1] - localVertexOffsets[ilocal]; ++ivertex )
            {
                auto index1 = vertexOffsets[iglobal] + ivertex;
                auto index2 = localVertexOffsets[ilocal] + ivertex;

                result.rst[index1] = localRst[index2];
                result.triangulation.vertices[index1] = localXyz[index2];
            }
        
            for( size_t itriangle = 0; itriangle < localTriangleOffsets[ilocal + 1] - localTriangleOffsets[ilocal]; ++itriangle )
            {
                auto& triangle = result.triangulation.triangles[result.triangleOffsets[iglobal] + itriangle];
        
                for( size_t ivertex = 0; ivertex < 3; ++ivertex )
                {
                    auto localIndex = localTriangles[3 * localTriangleOffsets[ilocal] + 3 * itriangle + ivertex];
           
                    triangle[ivertex] = ( localIndex - localVertexOffsets[ilocal] ) + vertexOffsets[iglobal];
                }
            }
        }
    } // omp parallel
...
auto interpolateImpl( auto&& function,
                      ...
                      std::array<double, 3> c2, bool )
{
    std::array<double, 3> m;

    for( size_t it = 0; it < 8; ++it )
    {
        m = array::multiply( array::add( c1, c2 ), 0.5 );
