...
{
    auto& impl = cache.template cast<EvaluationImpl>( );
...
{
    return cache.template cast<EvaluationImpl>( ).mapping;
}
...
{
    auto& cache = anyCache.template cast<UnstructuredBasisCache<D>>( );
...
void UnstructuredBasis<D>::prepareGridEvaluation( const CoordinateGrid<D>& rst,
                                                  BasisEvaluationCache<D>& anyCache ) const
{
    anyCache.template cast<UnstructuredBasisCache<D>>( ).rst = &rst;
}
...
{
    return anyCache.template cast<UnstructuredBasisCache<D>>( ).mapping;
}
                                ...
                                std::array<size_t, D> pLeaf )
{
    ...
    AnsatzTemplateVector ansatzTemplates( { grid.nleaves( ), numberOfFieldComponents, D } );

    for( CellIndex iLeaf = 0; iLeaf < grid.nleaves( ); ++iLeaf )
    {
        for( size_t field = 0; field < numberOfFieldComponents; ++field )
        {
            double t = maxLevel == 0 ? 0.0 : levels[grid.fullIndex( iLeaf )] / static_cast<double>( maxLevel );
...
template<size_t D>
LinearizedVectors<CellIndex> findSupportingElements( const AbsBasis<D>& basis,
...
AnsatzTemplateVector PerLevelGrading::operator()( const AbsHierarchicalGrid<D>& grid, size_t nfields )
{
    ...
    AnsatzTemplateVector ansatzTemplates( { grid.nleaves( ), nfields, D } );
