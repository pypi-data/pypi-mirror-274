...
def compute( D, refinementDepth ):
    ...
    # Vtk Postprocessing
    processors = [mlhp.makeSolutionProcessor( D, dofs ),
                  mlhp.makeFunctionProcessor( analytical, "Analytical" )]

    postmesh = mlhp.createGridOnCells( [refinementDepth + 1] * D )
    writer = mlhp.VtuOutput( f'outputs/singular_L2_{D}D_{refinementDepth}' )
            
    mlhp.writeBasisOutput( basis, postmesh, writer, processors )
        
    # Error computation
    l2ErrorIntegrand = mlhp.makeL2ErrorIntegrand( dofs, analytical );
