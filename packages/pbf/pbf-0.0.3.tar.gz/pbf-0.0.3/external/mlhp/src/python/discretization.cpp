...
void defineGrid( pybind11::module& m )
{
    ...
    {
        auto matrix = std::array<std::array<double, D>, D + 1> { };
    ...
    m.def( "implicitTransformation", []( const ImplicitFunctionWrapper<D>& function,
        ...
        { return ImplicitFunctionWrapper<D> { implicit::transform( function.get( ), transformation ) }; },
        pybind11::arg( "function" ), pybind11::arg( "transformation" ) );

    auto implicitF = []( std::vector<ImplicitFunctionWrapper<D>>&& functions, auto&& op, bool initial )
    {
...
template<size_t D>
void defineKDTree( pybind11::module& m )
{
...
{    
    ...
    auto boundingBoxF = []( Triangulation<D>& triangulation, size_t itriangle )
    { 
        return triangulation.boundingBox( itriangle );
    };
    ...
    if constexpr( D == 3 )
    {
        ...
        auto implicitTriangulationF = []( std::shared_ptr<Triangulation<D>> triangulation )
        {
