...
{
    return Cache { basis_->createEvaluationCache( ), MeshMapping<D> { } };
}
...
{
    basis_->prepareGridEvaluation( rst, cache.template cast<Cache>( ).otherCache );
}
                                               ...
                                               BasisEvaluationCache<D>& cache ) const
{
    ...
    basis_->evaluateGridPoint( ijk, shapes, cache.template cast<Cache>( ).otherCache );
                                                 ...
                                                 BasisEvaluationCache<D>& cache ) const
{
    ...
    basis_->evaluateSinglePoint( rst, shapes, cache.template cast<Cache>( ).otherCache );
