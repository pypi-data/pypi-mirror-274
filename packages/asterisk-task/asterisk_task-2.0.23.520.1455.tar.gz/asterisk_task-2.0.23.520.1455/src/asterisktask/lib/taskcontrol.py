...
from asterisktask.util.tool import AsteriskContext, dprint, error_print,iprint,wprint,print_prompt,classproperty
...
class TaskManager():
    ...
    def __init__(self) -> None:
        ...
        # 自动导入tasks模块下的task类
        # from asterisktask.lib.task import TaskEngine  
        try:   
    ...
    def start_default_task(self) -> None:
        ...
        iprint('启动默认任务')
        try:
            self.start_task(AppConfig['default_task'])
        except KeyError:
            wprint('没有默认任务。')
                ...
                if AppConfig['tasks'][task_item].get('is_loop'):                        
                    ...
                    if int(AppConfig['tasks'][task_item]['timer']['period']) > 0 :
                        schedule.every(AppConfig['tasks'][task_item]['timer']['period']).seconds.do(\
                            self.__threaded_schedule_task,self.start_task,task_item)                            
                    else:
            ...
            for task_name in TaskPool.items:
                ...
                if task_class.__dict__.get('is_loop'):
                    if task_class.timer.get('period') > 0:
                        schedule.every(task_class.timer.get('period')).seconds.do(self.__threaded_schedule_task,self.exec_task,task_class)
                    else:
        ...
        try:
            if AppConfig['tasks'][task_item]['timer'].get('fixed_time'):
                for timing in AppConfig['tasks'][task_item]['timer'].get('fixed_time'):
                    schedule.every().day.at(timing).do(self.__threaded_schedule_task,self.start_task,task_item)
            ...
            if task_class.timer.get('fixed_time'):
                for timing in task_class.timer.get('fixed_time'):
                    schedule.every().day.at(timing).do(self.__threaded_schedule_task,self.exec_task,task_class)
    ...
    def __init_weekly_task(self,timing:tuple,task_item:str):
        ...
        '''
        if timing[0] == 0:
            schedule.every().monday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 1:
            schedule.every().tuesday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 2:
            schedule.every().wednesday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 3:
            schedule.every().thursday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 4:
            schedule.every().friday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 5:
            schedule.every().saturday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        elif timing[0] == 6:
            schedule.every().sunday.at(timing[1]).do(self.__threaded_schedule_task,self.start_task,task_item)
        else:
    ...
    def exec_task(self,t:AsteriskTask,context_id:str = '') -> None:
        '''
        执行任务（第二代）
        ...
        if task.__dict__.get('next_context_id'):
            for next_task in t.next_tasks:
                self.exec_task(TaskPool.get_task(next_task),task.next_context_id)
    ...
    def start_threading_task(self,name:str,context_id:int):
        ...
        '''
        t = Thread(target=self.start_task,args=[name,context_id],name=f'threading_task_{name}')
        t.daemon = True
        t.start()
