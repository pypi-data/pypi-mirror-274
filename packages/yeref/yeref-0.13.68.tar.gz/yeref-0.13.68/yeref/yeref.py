...
<body>
    ...
    <script>
        ...
        let owner = "{10}"
        let stateInit = "{11}"
        console.log(addressContractFriendly)
        console.log(addressContractFriendly == '')
        console.log(addressContractFriendly === '')
        if (addressContractFriendly == '') {{
            document.querySelector('.happy-container').style.display = 'none'
        }}
...
<body>
    ...
    <script>
        ...
        async function sendRequest(method, url, body = null) {{
            try {{  const response = await fetch(url, {{ method, body: JSON.stringify(body), headers: {{ 'Content-Type': 'application/json' }} }})
...
<body>
    ...
    <script>
        ...
        async function sendRequest(method, url, body = null) {{
            let res = {{}}
            try {{  
                let response = await fetch(url, {{ method, body: JSON.stringify(body), headers: {{ 'Content-Type': 'application/json' }} }})
    ...
    <style>
        ...
        *::after {{
        ...
        .upload-media {{
            ...
            padding: 16px;
            margin: 5px 0;
            border: 2px dashed rgba(85, 149, 242, 0.99);
            border-radius: 50%;
            cursor: pointer;
            position: relative;
            overflow: hidden;
...
<body>
    <div class="container">
        ...
        <div class="choose-address">
            <div class="subscribe-datalist">
                <input list="address-datalist-list" id="address-datalist-id" placeholder="EQ: адрес жетона">
                <datalist id="address-datalist-list">
                    <option value="+">создать новый жетон</option>
                </datalist>
            </div>
            <div id="choose-address-id" class="hint">выбери адрес</div>
        </div>
        <div class="jetton">
            <div class="upload-media">
                <label for="image-upload">
                    <img id="logo" src="photo.svg" alt="logo">
                </label>
                <input type="file" id="image-upload" accept=".jpg, .jpeg, .webp, .png, .svg">
            </div>
            <button id="btn">push</button>

            <input type="text" id="input-field-image" class="input-field" maxlength="65536"
                placeholder="https: загрузи лого или вставь ссылку/base64">
            <div id="hint-field-image" class="hint">&lt; 5 Mb .jpg,&nbsp;
                <a href="https://tether.to/images/logoCircle.png" id="link-png" class="hint">.png,</a>&nbsp;
                <a href="https://cache.tonapi.io/imgproxy/cOMlJuViiVXDCkAghnyNj7plX8pAZ9pv3WhklvebpTY/rs:fill:200:200:1/g:no/aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3RvbmtlZXBlci9vcGVudG9uYXBpL21hc3Rlci9wa2cvcmVmZXJlbmNlcy9tZWRpYS90b2tlbl9wbGFjZWhvbGRlci5wbmc.webp"
                    id="link-webp" class="hint">.webp,</a>&nbsp;
                <div href="https://ipfs.io/ipfs/Qmcd7PGHxEYSfmSBvL3jVkWVoS78yvZ7oPVDxfi3R6sypX" id="link-svg"
                    class="hint">.svg</div>&nbsp;256х256
            </div>

            <input type="text" id="input-field-name" class="input-field" placeholder="Jetton: название жетона"
                maxlength="128">
            <div id="hint-field-name" class="hint">1-3 слова с заглавной буквы</div>

            <input type="text" id="input-field-symbol" class="input-field" placeholder="JTN: обозначение жетона"
                maxlength="64">
            <div id="hint-field-symbol" class="hint">3-5 заглавных букв</div>

            <input type="number" id="input-field-decimals" class="input-field" placeholder="9: десятичная точность"
                maxlength="3">
            <div id="hint-field-decimals" class="hint">9 - точность по умолчанию для TON</div>

            <input type="number" id="input-field-total" class="input-field" placeholder="1000000: количество JTN"
                maxlength="64">
            <div id="hint-field-total" class="hint">начальное количество на кошельке</div>

            <input type="text" id="input-field-description" class="input-field" placeholder="DESC: описание жетона"
                maxlength="512">
            <div id="hint-field-description" class="hint">история в 1 предложении</div>

            <input type="text" id="input-field-social" class="input-field" placeholder="https: информационные ресурсы"
                maxlength="512">
            <div id="hint-field-social" class="hint">ссылки: вебсайт/социальные сети</div>

            <div id="ft-send">Создать жетон</div>
            <div id="ft-edit">Редактировать JTN</div>
            <a href="#" id="gen-rand" class="hint">или сгенерируй рандом</a>
        </div>
        <div class="copy-media">
            <div class="copy-div">Скопировано</div>
        </div>
        <div class="sticker">
            <tgs-player id="lottie" autoplay loop mode="normal"
                src="https://raw.githubusercontent.com/fereysitnerya/ferey/main/AnimatedSticker.tgs">
        ...
        <div class="edit">
            <div class="both">
                <a id="total-title" class="hint" href="https://tonviewer.com">Общий баланс:</a>
                <div id="total-text"></div>
            </div>
            <div class="both">
                <a id="balance-title" class="hint" href="https://tonviewer.com">Баланс кошелька:</a>
                <div id="balance-text"></div>
            </div>
            <div class="both">
                <a id="owner-title" class="hint" href="https://tonviewer.com">Ваш адрес:</a>
                <div id="owner-text">....</div>
            </div>
            <div class="both">
                <a id="admin-title" class="hint" href="https://tonviewer.com">Адрес админа:</a>
                <div id="admin-text">....</div>
            </div>
            <div class="both">
                <a id="wallet-title" class="hint" href="https://tonviewer.com">Адрес кошелька:</a>
                <div id="wallet-text">....</div>
            </div>
            <div class="both">
                <a id="master-title" class="hint" href="https://tonviewer.com">Адрес мастера:</a>
                <div id="master-text">....</div>
            </div>

            <div id="ft-revoke">Отозвать права</div>
            <div class="hint">рекомендуется отозвать права админа на: отчеканивание новых монет и передачу прав
                владения, а также на редактирование жетона: ссылка на лого, название, обозначение, описание</div>

            <div class="block">
                <input type="number" id="input-mint" class="input-field" placeholder="100: количество JTN"
                    maxlength="256">
                <div id="ft-mint">Отчеканить</div>
            </div>
            <div id="hint-mint" class="hint">создать еще больше монет</div>

            <div class="block">
                <input type="text" id="input-admin" class="input-field" placeholder="EQ: адрес нового админа"
                    maxlength="256">
                <div id="ft-admin">Передать права</div>
            </div>
            <div id="hint-owner" class="hint">передать права на редактирование новому админу</div>

            <div class="block">
                <input type="number" id="input-burn" class="input-field" placeholder="1000: количество JTN"
                    maxlength="256">
                <div id="ft-burn">Сжечь</div>
            </div>
            <div id="hint-burn" class="hint">удалить несколько монет из оборота</div>

            <div class="block-transfer">
                <div class="block-outer">
                    <div class="block-inner">
                        <input type="number" id="input-amount" class="input-field-second" placeholder="1: количество JTN"
                            maxlength="256" value="1">
                        <input list="symbol-dt-list" id="symbol-dt-id" placeholder="JTN: жетон" value="TON">
                        <datalist id="symbol-dt-list">
                            <option value="TON">TON</option>
                        </datalist>
                    </div>
                    <input type="text" id="input-address" class="input-field-second" placeholder="EQ: адрес кошелька"
                        maxlength="256">
                </div>
                <input type="text" id="input-comment" class="input-field-second" placeholder="TXT: комментарий"
                    maxlength="256">
                <div id="isCipher">
                    <input id="chkCipher" type="checkbox">
                    <label for="chkCipher" class="label">Зашифровать комментарий</label>
                </div>
                <div id="ft-transfer">Отправить</div>
                <a id="hint-transfer" class="hint" href="https://tonviewer.com">отправить несколько монет другому пользователю</a>
            </div>
            
            <a id="footer" class="hint" href="https://t.me/FereyTONestBot">@FereyTONestBot</a>
        </div>
    </div>
    <script>
        let tg = window.Telegram.WebApp
        ...
        tonConnectUI.onStatusChange(async () => {{
            ...
            if (tonConnectUI.connected) {{
                console.log('2')
                jettons = await sendRequest('POST', '/download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                console.log(jettons)
                if (!Object.keys(jettons).length) {{ return }}
                if (activeMaster === "") {{ activeMaster = Object.keys(jettons)[0] }}
                console.log('activeMaster', activeMaster)

                document.getElementById('address-datalist-list').innerHTML = ''
                const createOption = document.createElement('option')
                createOption.value = "+"
                createOption.text = "создать новый жетон"
                document.getElementById('address-datalist-list').appendChild(createOption);

                for (const key in jettons) {{
                    let jtn = jettons[key]
                    const option = document.createElement('option')
                    option.value = jtn['master']
                    option.text = jtn['symbol']
                    document.getElementById('address-datalist-list').appendChild(option)
                }}
            }}
        }})
        document.addEventListener("DOMContentLoaded", async () => {{
            document.getElementById("address-datalist-id").placeholder = "EQ: адрес жетона"
            document.querySelector('option[value="+"]').textContent = 'создать новый жетон';
            document.getElementById("choose-address-id").textContent = 'выбери адрес'
            document.getElementById("input-field-image").placeholder = "https: загрузи лого или вставь ссылку/base64"
            document.getElementById("input-field-name").placeholder = "Jetton: название жетона"
            document.getElementById("hint-field-name").textContent = "1-3 слова с заглавной буквы"
            document.getElementById("input-field-symbol").placeholder = "JTN: обозначение жетона"
            document.getElementById("hint-field-symbol").textContent = "3-5 заглавных букв"
            document.getElementById("input-field-decimals").placeholder = "9: десятичная точность"
            document.getElementById("hint-field-decimals").textContent = "9 - точность по умолчанию для TON (0-255)"
            document.getElementById("input-field-decimals").value = 9
            document.getElementById("input-field-description").placeholder = "DESC: описание жетона"
            document.getElementById("hint-field-description").textContent = "история в 1 предложении"
            document.getElementById("input-field-social").placeholder = "https: информационные ресурсы"
            document.getElementById("hint-field-social").textContent = "ссылки: вебсайт/социальные сети"
            document.getElementById("gen-rand").textContent = "или сгенерируй рандом"
            document.getElementById("ft-edit").style.display = "none"

            document.getElementById("link-png").addEventListener("click", async () => {{ tg.openLink(document.getElementById("link-png").href, {{ try_instant_view: true }}) }})
            document.getElementById("link-webp").addEventListener("click", async () => {{ tg.openLink(document.getElementById("link-webp").href, {{ try_instant_view: true }}) }})
            document.getElementById("link-svg").addEventListener("click", async () => {{ tg.openLink('https://ipfs.io/ipfs/Qmcd7PGHxEYSfmSBvL3jVkWVoS78yvZ7oPVDxfi3R6sypX', {{ try_instant_view: true }}) }})

            document.getElementById("total-title").textContent = 'Общий баланс:'
            document.getElementById("balance-title").textContent = 'Баланс кошелька:'
            document.getElementById("admin-title").textContent = 'Адрес админа:'
            document.getElementById("wallet-title").textContent = 'Адрес кошелька:'
            document.getElementById("owner-title").textContent = 'Ваш адрес:'
            document.getElementById("master-title").textContent = 'Адрес мастера:'

            document.getElementById("total-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("total-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("balance-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("balance-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("admin-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("admin-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("wallet-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("wallet-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("owner-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("owner-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("master-title").addEventListener("click", async () => {{ tg.openLink(document.getElementById("master-title").href, {{ try_instant_view: true }}) }})
            document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink(document.getElementById("hint-transfer").href, {{ try_instant_view: true }}) }})
        }})
        // ----------------------------------------------------
        document.getElementById('address-datalist-id').addEventListener('change', async () => {{
            dlAddress = document.getElementById('address-datalist-id').value.trim()
            console.log('Выбрано или введено: ' + dlAddress)
            if (dlAddress === "" || /\s/.test(dlAddress) ) {{ return }}
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            console.log('next')

            if (dlAddress === "+") {{
                document.getElementById("logo").src = 'photo.svg'
                document.getElementById("logo").style.position = 'static'
                document.getElementById("input-field-image").value = ""
                document.querySelector('.upload-media').style.border = '2px dashed rgba(85, 149, 242, 0.99)'

                document.getElementById("input-field-name").value = ""
                document.getElementById("input-field-symbol").value = ""
                document.getElementById("input-field-decimals").value = 9
                document.getElementById("input-field-total").value = ""
                document.getElementById("input-field-description").value = ""
                document.getElementById("input-field-social").value = ""

                document.getElementById('ft-send').style.display = 'flex'
                document.getElementById('ft-edit').style.display = 'none'

                document.getElementById("total-text").textContent = ""
                document.getElementById("balance-text").textContent = ""
                document.getElementById("owner-text").textContent = "...."
                document.getElementById("admin-text").textContent = "...."
                document.getElementById("wallet-text").textContent = "...."
                document.getElementById("master-text").textContent = "...."
                return
            }}

            let options = document.getElementById("address-datalist-list").getElementsByTagName("option")
            for (let i = 0; i < options.length; i++) {{
                if (options[i].text === dlAddress) {{ console.log("Такое значение уже существует в datalist"); return }}
            }}
            console.log('after loop')

            console.log('a тут datalist')
            jettons = await sendRequest('POST', '/download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
            console.log(jettons)
            if (!Object.keys(jettons).length) return;
            console.log(jettons[dlAddress]['name'])
            activeMaster = dlAddress

            let newOption = document.createElement("option")
            newOption.value = dlAddress
            newOption.text = jettons[dlAddress]['name']
            document.getElementById("address-datalist-list").appendChild(newOption)
            console.log('end')

            document.getElementById("input-field-image").value = jettons[dlAddress]['image']
            document.getElementById("input-field-image").dispatchEvent(new Event('input', {{ bubbles: true }}))
            document.getElementById("input-field-name").value = jettons[dlAddress]['name']
            document.getElementById("input-field-symbol").value = jettons[dlAddress]['symbol']
            document.getElementById("input-field-decimals").value = jettons[dlAddress]['decimals']
            document.getElementById("input-field-total").value = jettons[dlAddress]['total']
            document.getElementById("input-field-description").value = jettons[dlAddress]['description']
            document.getElementById("input-field-social").value = jettons[dlAddress]['social']

            document.getElementById("total-text").textContent = `${{jettons[activeMaster]['total']}} ${{jettons[activeMaster]['symbol']}}`
            document.getElementById("balance-text").textContent = `${{jettons[activeMaster]['amount']}} ${{jettons[activeMaster]['symbol']}}`
            document.getElementById("owner-text").textContent = jettons[activeMaster]['owner_str'].slice(0, 4) + "...." + jettons[activeMaster]['owner_str'].slice(-4)
            document.getElementById("admin-text").textContent = jettons[activeMaster]['admin_str'].slice(0, 4) + "...." + jettons[activeMaster]['admin_str'].slice(-4)
            document.getElementById("wallet-text").textContent = jettons[activeMaster]['wallet_str'].slice(0, 4) + "...." + jettons[activeMaster]['wallet_str'].slice(-4)
            document.getElementById("master-text").textContent = jettons[activeMaster]['master_str'].slice(0, 4) + "...." + jettons[activeMaster]['master_str'].slice(-4)

            document.getElementById("total-title").href = `https://tonviewer.com/${{activeMaster}}`
            document.getElementById("balance-title").href = `https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`
            document.getElementById("admin-title").href = `https://tonviewer.com/${{jettons[activeMaster]['admin']}}`
            document.getElementById("wallet-title").href = `https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`
            document.getElementById("owner-title").href = `https://tonviewer.com/${{jettons[activeMaster]['owner']}}`
            document.getElementById("master-title").href = `https://tonviewer.com/${{activeMaster}}`

            document.getElementById("total-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{activeMaster}}`, {{ try_instant_view: true }}) }})
            document.getElementById("balance-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("admin-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['admin']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("wallet-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("owner-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['owner']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("master-title").addEventListener("click", async (event) => {{ event.preventDefault(); tg.openLink(`https://tonviewer.com/${{activeMaster}}`, {{ try_instant_view: true }}) }})

            if (jettons[dlAddress]['admin'] === jettons[dlAddress]['owner'] && jettons[dlAddress]['admin'] !== nullAddress) {{
                document.getElementById('ft-send').style.display = 'none'
                document.getElementById('ft-edit').style.display = 'flex'
                document.getElementById('ft-edit').textContent = `Редактировать ${{jettons[dlAddress]['symbol']}}`

                document.getElementById('ft-mint').style.backgroundColor = 'var(--tg-theme-button-color)'
                document.getElementById('ft-admin').style.backgroundColor = 'var(--tg-theme-button-color)'
            }}
            else {{
                document.getElementById('ft-send').style.display = 'flex'
                document.getElementById('ft-edit').style.display = 'none'

                document.getElementById('ft-mint').style.backgroundColor = 'var(--tg-theme-subtitle-text-color)'
                document.getElementById('ft-admin').style.backgroundColor = 'var(--tg-theme-subtitle-text-color)'
            }}

            document.getElementById('input-burn').placeholder = `1000: количество ${{jettons[activeMaster]['symbol']}}`
            document.getElementById('input-mint').placeholder = `100: количество ${{jettons[activeMaster]['symbol']}}`
            ...
            document.getElementById('symbol-dt-id').value = jettons[activeMaster]['symbol']
        }})

        document.getElementById('image-upload').addEventListener('change', async () => {{
            let arr = document.getElementById('image-upload').files
            ...
            if (file['size'] > 5242880) {{ tg.showAlert('no size'); return }}
            if (!isValidExtension) {{ tg.showAlert('no ext'); return }}
            let fd = new FormData()
            fd.append('initData', tg.initData)
            fd.append('FOO', file)

            try {{
                const response = await fetch("/image", {{ method: 'POST', body: fd }})
                const res = await response.json()
                console.log(res)

                if (res) {{
                    document.getElementById("logo").src = res['link']
                    document.getElementById("logo").style.position = 'absolute'
                    document.getElementById("input-field-image").value = document.getElementById("logo").src
                    document.querySelector('.upload-media').style.border = 'none'
                }}
                else {{
                    document.getElementById("logo").src = 'photo.svg'
                    document.getElementById("logo").style.position = 'static'
                ...
                console.error('/import_stage err', err)
                tg.showAlert('Ошибка файла загрузи другой или попробуй позже')
            }}

            document.getElementById('image-upload').value = ''
        }})

        document.getElementById('input-field-image').addEventListener('input', async () => {{
            console.log('change')
            let inputValue = document.getElementById('input-field-image').value.trim()
            if (inputValue === "") {{ return }}
            let isCorrect = (/^(http|https):\/\/[^ "]+$/.test(inputValue)) ? true : false

            if (isCorrect) {{
                document.getElementById("logo").src = inputValue
                document.getElementById("logo").style.position = 'absolute'
                document.getElementById("input-field-image").value = document.getElementById("logo").src
                document.querySelector('.upload-media').style.border = 'none'
            }}
                ...
                tg.showAlert("Не корректная ссылка")
            }}
        }})

        document.getElementById('owner-text').addEventListener('click', async () => {{
            await navigatorClipboard('owner')
        }})
        document.getElementById('admin-text').addEventListener('click', async () => {{
            await navigatorClipboard('admin')
        }})
        document.getElementById('wallet-text').addEventListener('click', async () => {{
            await navigatorClipboard('wallet')
        }})
        document.getElementById('master-text').addEventListener('click', async () => {{
            await navigatorClipboard('master')
        }})
        async function navigatorClipboard(field) {{
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}

            if (Object.keys(jettons).length) {{
                if (activeMaster === "") {{ activeMaster = Object.keys(jettons)[0] }}

                navigator.clipboard.writeText(jettons[activeMaster][field])
                document.querySelector('.copy-media').classList.add('window--open')
                await sleep(1000).then(() => {{ document.querySelector('.copy-media').classList.remove('window--open') }})
            }}
        }}

        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (isEdit) {{
                let res = await sendRequest('POST', '/random', {{ 'initData': tg.initData }})
                console.log(res)

                document.getElementById("input-field-image").value = res['image']
                document.getElementById("input-field-name").value = res['name']
                document.getElementById("input-field-symbol").value = res['symbol']
                document.getElementById("input-field-decimals").value = res['decimals']
                document.getElementById("input-field-total").value = res['total']
                document.getElementById("input-field-description").value = res['description']
                document.getElementById("input-field-social").value = res['social']
                ...
                let params = await sendRequest('POST', '/params', body)
                console.log('/params', params['master'])
                document.getElementById("address-datalist-id").value = params['master']

                wallet = params['wallet']
                owner = params['owner']
                master = params['master']

                document.getElementById("owner-text").textContent = params['owner_str'].slice(0, 4) + "...." + params['owner_str'].slice(-4)
                document.getElementById("admin-text").textContent = params['owner_str'].slice(0, 4) + "...." + params['owner_str'].slice(-4)
                document.getElementById("wallet-text").textContent = params['wallet_str'].slice(0, 4) + "...." + params['wallet_str'].slice(-4)
                document.getElementById("master-text").textContent = params['master_str'].slice(0, 4) + "...." + params['master_str'].slice(-4)

                document.getElementById('ft-send').style.display = 'flex'
                document.getElementById('ft-edit').style.display = 'none'
            }}
        }})
        document.getElementById('input-field-symbol').addEventListener('input', async () => {{
            await editHandle()
        }})
        async function editHandle() {{
            symbol = document.getElementById('input-field-symbol').value.toUpperCase()
            let total = document.getElementById('input-field-total').value ? document.getElementById('input-field-total').value : 0
            document.getElementById('balance-text').textContent = `${{total}} ${{symbol}}`
            document.getElementById('input-field-symbol').placeholder = `1000000: количество ${{symbol}}`

            document.getElementById('ft-edit').textContent = `Редактировать ${{symbol}}`
            document.getElementById('input-burn').placeholder = `1000: количество ${{symbol}}`
            document.getElementById('input-mint').placeholder = `100: количество ${{symbol}}`
        }}
        // ----------------------------------------------------
        document.getElementById('ft-send').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let image = document.getElementById("input-field-image").value.trim()
            let name = document.getElementById("input-field-name").value.trim()
            let symbol = document.getElementById("input-field-symbol").value.trim()
            let decimals = document.getElementById("input-field-decimals").value.trim() ? document.getElementById("input-field-decimals").value.trim() : "9"
            let total = document.getElementById("input-field-total").value.trim()
            let description = document.getElementById("input-field-description").value.trim()
            let social = document.getElementById("input-field-social").value.trim() ? document.getElementById("input-field-social").value.trim() : ""

            if (image === "") {{ tg.showAlert("image"); return }}
            if (name === "") {{ tg.showAlert("name"); return }}
            if (symbol === "") {{ tg.showAlert("symbol"); return }}
            if (total === "") {{ tg.showAlert("total"); return }}
            ...
            let res = await sendRequest('POST', '/params', body)
            console.log(res)
            ...
            let success = await sendRequest('POST', '/success', body)
            console.log(success)
            isSuccess = 1
        }})
        document.getElementById('ft-edit').addEventListener('click', async (event) => {{
            console.log(event.target.id)
        }})
        document.getElementById('ft-revoke').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeMaster === "") {{ tg.showAlert("Выбери адрес мастера, чтобы отозвать права у админа"); return }}
        }})

        document.getElementById('ft-mint').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeMaster === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера для отчеканивания еще большего количества монет"); return }}

            // if (isSuccess === 0) {{ tg.showAlert("Необходимо создать жетон или выбрать существующий"); return }}
            let inputMint = document.getElementById('input-mint').value.trim()
            if (inputMint === "") {{ tg.showAlert("Введи количество монет для отчеканивания1"); return }}
            if (!Number.isInteger(Number(inputMint))) {{ tg.showAlert("Введи количество монет для отчеканивания"); return }}

            body = {{
                'owner': tonConnectUI.account.address,
                'master': jettons[activeMaster]['master'],
                'wallet': jettons[activeMaster]['wallet'],
                'coins': inputMint.toString(),
                'decimals': jettons[activeMaster]['decimals'],
                'symbol': document.getElementById('input-field-symbol').value,

                'initData': tg.initData,
            }}
            let mint = await sendRequest('POST', '/mint', body)
            console.log(mint)
        ...
        document.getElementById('ft-admin').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeMaster === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера для передачи прав другому админу"); return }}

            // if (isSuccess === 0) {{ tg.showAlert("Необходимо создать жетон или выбрать существующий"); return }}
            let inputAdmin = document.getElementById('input-admin').value.trim()
            if (inputAdmin === "") {{ tg.showAlert("Введи адрeс новго админа"); return }}

            body = {{
                'master': jettons[activeMaster]['master'],
                'admin': inputAdmin.toString(),

                'initData': tg.initData,
            }}
            let admin = await sendRequest('POST', '/admin', body)
            console.log(admin)
            ...
            const result = await tonConnectUI.sendTransaction({{ validUntil: Math.round(Date.now() / 1000) + 5 * 60, messages: message }})
            console.log('result', result)
            jettons[activeMaster]['admin'] = admin['admin']
            jettons[activeMaster]['admin_str'] = admin['admin_str']
            document.getElementById("admin-text").textContent = jettons[activeMaster]['admin_str'].slice(0, 4) + "...." + jettons[activeMaster]['admin_str'].slice(-4)
        }})
        document.getElementById('ft-burn').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeMaster === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера/кошелька для сжигания монет"); return }}

            // if (isSuccess === 0) {{ tg.showAlert("Необходимо создать жетон или выбрать существующий"); return }}
            let inputBurn = document.getElementById('input-burn').value.trim()
            if (inputBurn === "") {{ tg.showAlert("Введи количество монет для сжигания1"); return }}
            if (!Number.isInteger(Number(inputBurn))) {{ tg.showAlert("Введи количество монет для сжигания"); return }}

            body = {{
                'owner': tonConnectUI.account.address,
                'master': jettons[activeMaster]['master'],
                'wallet': jettons[activeMaster]['wallet'],
                'coins': inputBurn.toString(),
                'decimals': jettons[activeMaster]['decimals'],

                'initData': tg.initData,
            }}
            let burn = await sendRequest('POST', '/burn', body)
            console.log(burn)
            ...
            const result = await tonConnectUI.sendTransaction({{ validUntil: Math.round(Date.now() / 1000) + 5 * 60, messages: message }})
            console.log('result', result)
            jettons[activeMaster]['amount'] = parseInt(jettons[activeMaster]['amount'], 10) - parseInt(inputBurn, 10)
            document.getElementById("balance-text").textContent = `${{jettons[activeMaster]['amount']}} ${{jettons[activeMaster]['symbol']}}`
        }})
        document.getElementById('ft-transfer').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeMaster === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера/кошелька для пересылки монет другому пользователю"); return }}

            let inputAddress = document.getElementById('input-address').value.trim()
            if (inputAddress === "") {{ tg.showAlert("Введи адрес получателя"); return }}
            let inputAmount = document.getElementById('input-amount').value.trim()
            let symbolDatalist = document.getElementById('symbol-dt-id').value
            let inputComment = document.getElementById('input-comment').value.trim()
            ...
            body = {{
                ...
                'amount': inputAmount,
                'decimals': jettons[activeMaster]['decimals'],
                'symbol': symbolDatalist,
                'recipient': inputAddress,
                'comment': inputComment,
                'isCipher': document.getElementById('chkCipher').checked,
                
                'initData': tg.initData,
            }}
            let transfer = await sendRequest('POST', '/transfer', body)
            ...
            document.getElementById("hint-transfer").href = `https://tonviewer.com/${{transfer['wallet']}}`
            document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{transfer['wallet']}}`, {{ try_instant_view: true }}) }})
        }})
        // ----------------------------------------------------
        async function sendRequest(method, url, body = null) {{
            let res = {{}}
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTON, ENT_TYPE,
                     ...
                     address_contract_friendly=None, state_init=None):
    result = None
    try:
        ...
        html_web = html_template.format(m_html, txt_html, btn_html, msg_views, ENT_LINK, ENT_USERNAME, ENT_TID, POST_ID,
                                        POST_LNKS, address_contract_friendly, address_owner_raw, state_init)
        if POST_ID:
