...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            if (tonConnectUI.connected && Object.keys(jettons).length) {{
                ...
                document.getElementById("owner-text").textContent = jettons[activeMaster]['owner'].slice(0, 4) + "...." + jettons[activeMaster]['owner'].slice(-4)
                document.getElementById("admin-text").textContent = jettons[activeMaster]['admin'].slice(0, 4) + "...." + jettons[activeMaster]['admin'].slice(-4)
                document.getElementById("wallet-text").textContent = jettons[activeMaster]['wallet'].slice(0, 4) + "...." + jettons[activeMaster]['wallet'].slice(-4)
                document.getElementById("master-text").textContent = jettons[activeMaster]['master'].slice(0, 4) + "...." + jettons[activeMaster]['master'].slice(-4)
            }}
        }})
        // ----------------------------------------------------
        document.getElementById('subscribe-datalist-id').addEventListener('change', async () => {{
            ...
            if (dlAddress === "+") {{
                ...
                document.getElementById('ft-edit').textContent = `Создать жетон`
                document.getElementById('ft-edit').style.backgroundColor = 'var(--tg-theme-button-color)'
            ...
            // пытаемся вызвать по этому контракту все данные 
            // но только сжигать можно будет есил человек введет адрес волета а не минтера
            ...
            document.getElementById("total-text").textContent = `${{jettons[dlAddress]['total']}} ${{jettons[dlAddress]['symbol']}}`
            document.getElementById("balance-text").textContent = `${{jettons[dlAddress]['amount']}} ${{jettons[dlAddress]['symbol']}}`
            document.getElementById("admin-text").textContent = jettons[dlAddress]['admin'].slice(0, 4) + "...." + jettons[dlAddress]['admin'].slice(-4)
            document.getElementById("wallet-text").textContent = jettons[dlAddress]['wallet'].slice(0, 4) + "...." + jettons[dlAddress]['wallet'].slice(-4)
            document.getElementById("owner-text").textContent = jettons[dlAddress]['owner'].slice(0, 4) + "...." + jettons[dlAddress]['owner'].slice(-4)
            document.getElementById("master-text").textContent = jettons[dlAddress]['master'].slice(0, 4) + "...." + jettons[dlAddress]['master'].slice(-4)
        ...
        document.getElementById('wallet-text').addEventListener('click', async () => {{
            await navigatorClipboard('await navigatorClipboard('owner')')
        }})
        ...
        document.getElementById('gen-rand').addEventListener('click', async () => {{
            if (isEdit) {{                
                ...
                document.getElementById("subscribe-datalist-id").value = params['address']
                
                wallet = params['wallet']
                owner = params['owner']
                master = params['address']

                document.getElementById("admin-text").textContent = admin.slice(0, 4) + "...." + admin.slice(-4)
                document.getElementById("wallet-text").textContent = wallet.slice(0, 4) + "...." + wallet.slice(-4)
                document.getElementById("owner-text").textContent = owner.slice(0, 4) + "...." + owner.slice(-4)
                document.getElementById("master-text").textContent = master.slice(0, 4) + "...." + master.slice(-4)
            }}
        }})
        ...
        document.getElementById('ft-mint').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{1}"); return }}
            if (activeMaster === "") {{ tg.showAlert("Выбери адрес мастера, чтобы отчеканить еще больше монет"); return }}
        ...
        document.getElementById('ft-owner').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{1}"); return }}
            if (activeMaster === "") {{ tg.showAlert("Выбери адрес мастера, чтобы передать права другому админу"); return }}
        }})
        document.getElementById('ft-burn').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{1}"); return }}
            if (activeMaster === "") {{ tg.showAlert("Выбери адрес мастера или кошелька, чтобы сжечь на нем монеты"); return }}
        ...
        document.getElementById('ft-transfer').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{1}"); return }}
            if (activeMaster === "") {{ tg.showAlert("Выбери адрес мастера или кошелька, чтобы переслать монеты другому пользователю"); return }}
        }})
        // ----------------------------------------------------
        async function sendRequest(method, url, body = null) {{
            let res = {{}}
