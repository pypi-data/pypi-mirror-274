...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink('https://tonviewer.com', {{ try_instant_view: true }}) }})
        ...
        document.getElementById('ft-mint').addEventListener('click', async (event) => {{
            ...
            if (activeMaster !== "" && (jettons[activeMaster]['admin'] !== jettons[activeMaster]['owner'] || jettons[activeMaster]['admin'] !== nullAddress)) {{ tg.showAlert("👩🏽‍💻 Отсутствуют права на редактирование жетона"); return }}
