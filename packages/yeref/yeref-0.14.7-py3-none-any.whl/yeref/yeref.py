...
<body>
    <div class="container">
        ...
        <div class="jetton">
            ...
            <div href="#" id="gen-rand" class="hint">или сгенерируй рандом</div>
        </div>
        ...
        <div class="edit">
            ...
            <div class="both">
                <div id="balance-title" class="hint">Баланс кошелька:</div>
                <div id="balance-text"></div>
            </div>
            <div class="both">
                <div id="owner-title" class="hint">Ваш адрес:</div>
                <div id="owner-text">....</div>
            </div>
            <div class="both">
                <div id="admin-title" class="hint">Адрес админа:</div>
                <div id="admin-text">....</div>
            </div>
            <div class="both">
                <div id="wallet-title" class="hint">Адрес кошелька:</div>
                <div id="wallet-text">....</div>
            </div>
            <div class="both">
                <div id="master-title" class="hint">Адрес мастера:</div>
            ...
            <div id="hint-admin" class="hint">передать права на редактирование новому админу</div>
            ...
            <div id="hint-burn" class="hint">удалить монеты из оборота со своего кошелька</div>
            ...
            <div class="block-transfer">
                ...
                <div id="instruction" class="hint"></div>
                <div id="footer" class="hint"></div>
            </div>
        </div>
    </div>
    <script>
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{tonConnectUI.account.address}}`, {{ try_instant_view: true }}) }})
                ...
                if (jettons[activeMaster]['admin'] === jettons[activeMaster]['owner'] && jettons[activeMaster]['admin'] !== nullAddress) {{
                    document.getElementById('ft-send').style.display = 'none'
                    document.getElementById('ft-edit').style.display = 'flex'
                    document.getElementById('ft-edit').textContent = `Редактировать ${{jettons[activeMaster]['symbol']}}`
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById("hint-mint").textContent = 'создать еще больше монет'
            document.getElementById("hint-admin").textContent = 'передать права на редактирование новому админу'
            document.getElementById("hint-burn").textContent = 'удалить монеты из оборота со своего кошелька'
            document.getElementById("hint-transfer").textContent = 'отправить несколько монет другому пользователю'
        
            document.getElementById("instruction").textContent = 'Инструкция продвижения жетона'
            document.getElementById("instruction").addEventListener("click", async () => {{ tg.openLink('https://telegra.ph/FereyTONestBot-05-20', {{ try_instant_view: true }}) }})
            document.getElementById("footer").textContent = '@FereyTONestBot'
            document.getElementById("footer").addEventListener("click", async () => {{ tg.openLink('https://t.me/FereyTONestBot', {{ try_instant_view: true }}) }})
        }}) 
        // ----------------------------------------------------
        document.getElementById('address-datalist-id').addEventListener('change', async () => {{
            ...
            document.getElementById("balance-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("admin-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['admin']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("wallet-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['wallet']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("owner-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{jettons[activeMaster]['owner']}}`, {{ try_instant_view: true }}) }})
            document.getElementById("master-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{activeMaster}}`, {{ try_instant_view: true }}) }})
        ...
        document.getElementById('ft-mint').addEventListener('click', async (event) => {{
            ...
            console.log(jettons[activeMaster]['admin'])
            console.log(jettons[activeMaster]['owner'])
            if (jettons[activeMaster]['admin'] !== jettons[activeMaster]['owner'] || jettons[activeMaster]['admin'] === nullAddress) {{ tg.showAlert("👩🏽‍💻 Отсутствуют права на редактирование жетона"); return }}
            ...
            body = {{
                ...
                'symbol': jettons[activeMaster]['symbol'],
            ...
            jettons[activeMaster]['balance'] = (parseInt(jettons[activeMaster]['balance'], 10) + parseInt(inputMint, 10)).toString()
            document.getElementById("balance-text").textContent = `${{jettons[activeMaster]['balance']}} ${{jettons[activeMaster]['symbol']}}`
            
            jettons[activeMaster]['total'] = (parseInt(jettons[activeMaster]['total'], 10) + parseInt(inputMint, 10)).toString()
            document.getElementById("total-text").textContent = `${{jettons[activeMaster]['total']}} ${{jettons[activeMaster]['symbol']}}`
        }})
        ...
        document.getElementById('ft-burn').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let tmp = document.getElementById('address-datalist-id').value.trim()
            if (activeMaster === "" && tmp === "") {{ tg.showAlert("Создай жетон или выбери адрес мастера/кошелька для сжигания монет"); return }}
            ...
            if (Object.keys(jettons).length && tmp in jettons) {{
                ...
                console.log('ключа не существует')
                wallet = tmp
                decimals = "9"
                symbol = "TON"
                isWallet = true
        ...
        document.getElementById('ft-transfer').addEventListener('click', async (event) => {{
            ...
            let inputAmount = document.getElementById('input-amount').value.trim()
             if (inputAmount === "") {{ tg.showAlert("Введи сумму перевода"); return }}
        ...
        let argument = window.location.href.split("/").pop().split("?")[0]
        if (argument === 'web' && tg.initData) {{
            let startUrl = `/web?tgWebAppStartParam=ft_all&${{tg.initData}}`
            console.log('startUrl = ', startUrl)
            fetchData(startUrl)
        }}
    </script>
