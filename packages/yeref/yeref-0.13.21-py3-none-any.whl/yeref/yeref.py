...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            // document.getElementById("hint-field-image").placeholder = "5 Mb .jpg, .png,"
            ...
            document.getElementById("total-text").textContent = `${{total}} ${{symbol}}`
            document.getElementById("balance-title").textContent = 'Баланс кошелька:'
            document.getElementById("balance-text").textContent = `??? ${{symbol}}`
        ...
        document.getElementById('subscribe-datalist-id').addEventListener('change', async () => {{
            let inputValue = document.getElementById('subscribe-datalist-id').value.trim()
            console.log('Выбрано или введено: ' + inputValue)
            if (inputValue === "") {{ return }}
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{1}"); return }}
            dlAddress = inputValue
            console.log('next')
            ...
            for (let i = 0; i < options.length; i++) {{
                if (options[i].value === inputValue) {{ console.log("Такое значение уже существует в datalist"); return }}
                if (options[i].text === inputValue) {{ console.log("Такое значение уже существует в datalist"); return }}
            }}
            ...
            for (const key in jettons) {{
                ...
                option.value = jtn['symbol']
                option.text = jtn['master']
