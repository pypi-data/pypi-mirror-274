...
from sqlalchemy import create_engine, URL # SQL Server
...
class WA:
    ...
    def sql_connection(self, uid, pwd, server, database, driver='ODBC Driver 17 for SQL Server'):
        ...
        try:
            ...
            enviar_email(str(f'String de Conexão: {url} \n Erro: {erro}'))
            return str(erro)
    ...
    def criar_imagem_SQL(self, consulta, conn, arquivo = 'dist/temp.png'):
...
if __name__ == '__main__':
    w = WA()
    engine = w.sql_connection("guilherme.breve","84584608-Gui","10.56.6.56","Vista_Replication_PRD")
    with engine.connect() as conn:
        w.criar_imagem_SQL('SELECT TOP 1 NOME FROM TAREFA', conn)