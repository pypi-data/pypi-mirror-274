...
class D4RLDataset(Dataset):
    def __init__(
        ...
        hdf5_url = DATASETS_URLS[d4rl_name]
        random_score = REF_MIN_SCORE[d4rl_name] if normalize_rewards else None
        expert_score = REF_MAX_SCORE[d4rl_name] if normalize_rewards else None
    ...
    def normalize_rewards(self, episodes: List[Dict[str, Any]]) -> None:
        ...
        """Normalize the rewards in each episode."""
        for episode in episodes:
            episode["rewards"] = [
                100
                * (reward - self.random_score)
