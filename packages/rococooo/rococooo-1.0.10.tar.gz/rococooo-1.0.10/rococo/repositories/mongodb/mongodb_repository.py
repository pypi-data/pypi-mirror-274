...
class MongoDbRepository(BaseRepository):
    ...
    def create(self, instance: VersionedModel, collection_name: str):
        return self.update(instance, collection_name)
    ...
    def update(self, instance: VersionedModel, collection_name: str, updated_by=None):
        with self.client.start_session() as session:
            with session.start_transaction():
                if data:
                    instance.prepare_for_save(changed_by_id=updated_by)
                    data = self._process_data_before_save(instance)
