...
class Workspace(Page_BM):
    def __init__(self, window, THEFrame, parent):
        ...
        self.Reset            =    self.c_wgts.section_unit(self.confirmation, title="", widget="button", default="Reset", command= self.create_page)
        self.Reset.winfo_children()[-1].configure(height=50, font=(FONT_B, 17), fg_color = (hvr_clr_g(LIGHT_MODE["primary"], "l", -20), hvr_clr_g(DARK_MODE["primary"], "d", -20)), hover_color = (LIGHT_MODE["primary"], DARK_MODE["primary"]))
