...
class large_tabs(ctk.CTkFrame):
    ...
    def constructor(self, text=None, image=None, button_icon=None, icon_size=20, button_command=None):
        ...
        expander = ctk.CTkFrame(master=None, fg_color="transparent")
        ...
        if button_icon != None:
            ...
            image = ctk.CTkImage(image, size=s)
            actbtn = ctk.CTkButton(content, text="", image=image, fg_color="transparent", hover_color=(LIGHT_MODE["primary"], DARK_MODE["primary"]), 
    ...
    def update(self):
        if self.autofit:
            self.available_tab_spaces = int(self.rows[0].winfo_width() / (self.image_width+self.padx))
            # print(self.available_tab_spaces)
    ...
    def page_function_calls(self):
        self.page.updating_call_list.append(self.update)
