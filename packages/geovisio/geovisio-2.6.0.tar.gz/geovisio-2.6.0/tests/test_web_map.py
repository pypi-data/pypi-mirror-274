...
def test_getStyle(client):
    response = client.get(f"/api/map/style.json")
        ...
        "layers": [
            ...
            },
            {
                ...
                "paint": {
                    ...
                    "fill-opacity": ["interpolate", ["linear"], ["zoom"], 0, 1, 4, 1, 6, 0.8, 6.5, 0],
                },
            },
        ],
    }
...
def test_getMyStyle(app, client, defaultAccountToken):
    response = client.get("/api/users/me/map/style.json", headers={"Authorization": f"Bearer {defaultAccountToken(app)}"})
    assert response.status_code == 200
    assert response.json == {
        "version": 8,
        "name": "GeoVisio Vector Tiles",
...
def test_getTile(datafiles, initSequence, dburl, z, x, y, layersCount):
...
def test_get_user_Tiles(app, client, dburl, bobAccountToken, bobAccountID, defaultAccountID, defaultAccountToken):
    ...
    conftest.uploadSequenceFromPics(
        ...
        wait=True,
        jwtToken=defaultAccountToken(app),
        pics=[
            datadir / "4.jpg",
            datadir / "5.jpg",
        ],
    ...
    def _get_user_tiles_features(id, token_func=None, query_params=""):
        headers = {"Authorization": f"Bearer {token_func(app)}"} if token_func else {}
    ...
    response = client.patch(
        ...
        data={"visible": "false"},
        headers={"Authorization": f"Bearer {defaultAccountToken(app)}"},
    )
    assert response.status_code == 200
    ...
    response = client.get(f"/api/users/me/map/15/16558/11252.mvt", headers={"Authorization": f"Bearer {defaultAccountToken(app)}"})
    assert response.status_code == 200
    assert _get_tiles_data(response)[1] == {"nb_pic": 2, "nb_seq": 1, "nb_grid": 0}
    response = client.get(
        f"/api/users/me/map/15/16558/11252.mvt{only_hidden}", headers={"Authorization": f"Bearer {defaultAccountToken(app)}"}
    )
    assert response.status_code == 200
...
def test_get_user_Tiles_higher_zoom(app, client, dburl, bobAccountToken, bobAccountID):
    ...
    seq_location = conftest.createSequence(client, "seq1", jwtToken=bobAccountToken(app))
    initial_lon = 1.7
    initial_lat = 46
    ...
    sequence = conftest.getPictureIds(dburl)[0]
    response = client.get("/api/map/5/16/11.mvt", headers={"Authorization": f"Bearer {bobAccountToken(app)}"})
    assert response.status_code == 200
    ...
    response = client.get("/api/users/me/map/5/16/11.mvt", headers={"Authorization": f"Bearer {bobAccountToken(app)}"})
    assert response.status_code == 200
    t, stats = _get_tiles_data(response)
    # high zoom tile should not contains picture info
