...
from .conftest import redirect_history, get_keycloak_authenticate_form_url
from geovisio import tokens, create_app
...
def test_get_jwt_token(server, keycloak, auth_client):
    ...
    assert user_info.json["links"] == [
        {
            "href": f"http://localhost:5000/api/users/{user_info.json['id']}/catalog/",
            "rel": "catalog",
            "type": "application/json",
        },
        {
            "href": f"http://localhost:5000/api/users/{user_info.json['id']}/collection",
            "rel": "collection",
            "type": "application/json",
        },
        {
            "href": f"http://localhost:5000/api/users/{user_info.json['id']}" + "/map/{z}/{x}/{y}.mvt",
            "rel": "user-xyz",
