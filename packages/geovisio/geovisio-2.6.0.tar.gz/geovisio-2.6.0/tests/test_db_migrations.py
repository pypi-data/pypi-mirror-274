...
def test_db_update_pictures_sequences(datafiles, initSequence, dburl):
    ...
    with psycopg.connect(dburl, autocommit=True) as db:
        ...
        # If we move too far points, it might lead to the geometry beeing splited (not to have too far strings in the map)
        db.execute(
...
@conftest.SEQ_IMGS
def test_db_delete_all_pics(datafiles, initSequence, dburl):
