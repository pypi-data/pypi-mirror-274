...
def _download_zoo_args(parser):
    ...
    parser.add_argument(
        ...
        choices=["ORCA", "ORCA1", "CPU", "GPU", "EDGETPU", "DLA", "DLA_FALLBACK", "MYRIAD", "NPU"],
    )
    parser.add_argument(
        "--runtime",
        help="runtime agent type filter",
        choices=["N2X", "TFLITE", "ONNX", "OPENVINO", "TENSORRT"],
    )
    parser.add_argument(
        "--precision",
...
def _server_run(args):
    ...
    if args.protocol == "asio" or args.protocol == "both":
        servers.append({"protocol": "asio", "port": port})
        port += 1
    if args.protocol == "http" or args.protocol == "both":
        servers.append({"protocol": "http", "port": port})
    ...
    if args.command == "start":
        ...
        with ExitStack() as stack:
            for server in servers:
                server["server"] = stack.enter_context(
            ...
            def keyboard_handler():
                try:
                    ...
                    input(
                        msg
                        + f"Local model zoo is server from '{args.zoo}' directory.\n"
                        + "Press Enter to stop the server"
                    )

                    for server in servers:
                        server["server"].stop(False)  # no wait
