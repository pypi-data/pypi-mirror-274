...
from collections.abc import AsyncGenerator, Sequence, Awaitable
...
async def get_organization_aws_accounts(request: web.Request) -> web.Response:
    ...
    if account_ids:
        result = [a.to_dict() async for a in client.get_all(request.app, URL(account_url).with_path('accounts').with_query(query), AccountView, headers=headers)]
    else:
        result = []
    return await response.get_all(request, result)
...
async def _update_permissions(request: web.Request):
    ...
    if old_dict is not None:
        ...
        coros = []
        old_org_user_ids = set(chain(old_org.admin_ids, old_org.manager_ids, old_org.member_ids))
        new_org_user_ids = set(chain(new_org.admin_ids, new_org.manager_ids, new_org.member_ids))
...
async def _update_volumes_and_credentials(request: web.Request, changed: _OrganizationPermissionsChanged):
    ...
    app = request.app
    sub = request.headers.get(SUB, NONE_USER)
    group_url = URL(await client.get_resource_url(app, Group))
    person, groups, account_view_url_str, volume_url_str = await gather(
        client.get(app, URL(await client.get_resource_url(app, Person)) / changed.user_id,
                    type_or_obj=Person,
                    headers=_sub_header),
        gather(*[client.get(app, group_url / group_id) for group_id in changed.group_ids]),
    ...
    if deleted_accounts := set(changed.old_account_ids).difference(changed.new_account_ids):
        async def deleted_accounts_coro():
            logger.debug('Deleting volumes and credentials for accounts %s for user %s', deleted_accounts, person.id)
            volumes_to_delete = await client.get_all_list(app, volume_url, Volume,
                                                          query_params=_account_query_params(deleted_accounts),
                                                          headers=_sub_header)
            volumes_for_current_user = filter(lambda v: v.has_permissions(person.id, RESTPermissionGroup.GETTER_PERMS), volumes_to_delete)
...
async def _update_volume_and_credentials(request: web.Request,
    ...
    credential, account_view = await gather(
        client.get(app,
                   ...
                   type_or_obj=Credentials, headers=_sub_header),
        client.get(app,
    ...
    actual_account_url = URL(await client.get_resource_url(app, account_view.actual_object_type_name))
    actual_account = await client.get(app,
                                    actual_account_url / account_view.actual_object_id,
                                    type_or_obj=Account,
                                    headers={SUB: sub, hdrs.AUTHORIZATION: request.headers[hdrs.AUTHORIZATION]})
    ...
    if credential is None:
        ...
        credentials_url_ = URL(await client.get_resource_url(app, type(new_credentials)))
        ...
        volume.credential_id = (await client.get(app, credentials_url_ / id_, type(new_credentials), headers=_sub_header)).id
    else:
        ...
        credential.name = f'{person.id}_{credential.type}_{credential.id}'
...
async def _delete_volume_and_credentials(app: web.Application,
    ...
    try:
        ...
        credential_url = URL(await client.get_resource_url(app, volume.credential_type_name))
