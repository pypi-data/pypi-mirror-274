...
def localres_histogram(mapone, primary_mapname, resolution=None):
    ...
    if resmapname and os.path.isfile(resmapname):
        with mrcfile.open(resmapname, permissive=True) as inmap:
            ...
            localres_dict['values'] = bin_edges.tolist()
            localres_dict['counts'] = hist.tolist()
            localres_dict['minmax'] = {'min': keep_three_significant_digits(min(bin_edges).astype(float)),
                                       'max': keep_three_significant_digits(max(bin_edges).astype(float))}

            # Plot the histogram
            plt.figure(figsize=(8, 6))
            plt.hist(map_data, bins=50, color='blue', edgecolor='black')
            if resolution:
