...
class SklearnObject(ObjectSpec):
    ...
    def prepare_kwargs(self, cls, problem_info,
        ...
        # In the scikit-learn >= 0.24.0 the class signature contains asterisk and
        # argspec.args remains empty. In this case we will use inspect.signature
        if argspec.args or cls_parameters:
            if 'random_state' in argspec.args or 'random_state' in cls_parameters:
                self.prepared_kwargs['random_state'] = random_state

            if 'n_jobs' in argspec.args or 'n_jobs' in cls_parameters:
                self.prepared_kwargs['n_jobs'] = num_threads

            if 'problem_info' in argspec.args or 'problem_info' in cls_parameters:
            ...
            if ('presort' in self.prepared_kwargs
               and 'presort' not in argspec.args
               and 'presort' not in cls_parameters):
                del self.prepared_kwargs['presort']

            # In scikit-learn 1.0.0 min_impurity_split was replaced by min_impurity_decrease
            if "min_impurity_split" in self.prepared_kwargs:
                if self.prepared_kwargs["min_impurity_split"] is None:
                    self.prepared_kwargs["min_impurity_decrease"] = 0.0
                else:
                    self.prepared_kwargs["min_impurity_decrease"] = self.prepared_kwargs["min_impurity_split"]
                del self.prepared_kwargs["min_impurity_split"]

            # In scikit-learn 1.1.0 mae was removed and we need to use loss = "lad" instead.
            if ("criterion" in self.prepared_kwargs
               and self.prepared_kwargs["criterion"] == "mae" and cls.__name__.startswith("GradientBoosting")):
                del self.prepared_kwargs["criterion"]
