from openai import OpenAI as OriginalOpenAI, OpenAIError, Timeout, APITimeoutError
    ...
    def create(
        ...
        if model.startswith("openpipe:"):
            ...
            try:
                ...
                if "fallback" in openpipe_options and "model" in openpipe_options["fallback"]:
                    kwargs["model"] = openpipe_options["fallback"]["model"]
                    kwargs["timeout"] = openpipe_options["fallback"].get("timeout", default_timeout)
                    try:
                        chat_completion = self.fallback_client.chat.completions.create(**kwargs)
                        return self._handle_response(chat_completion, kwargs, openpipe_options, requested_at)
                    except Exception as e:
...
class ChatWrapper(Chat):
    def __init__(
        ...
        self.completions = CompletionsWrapper(
            client, openpipe_reporting_client, openpipe_completions_client, fallback_client
        )
...
class OpenAIWrapper(OriginalOpenAI):
    ...
    def __init__(
        ...
        *,
        ...
        self.fallback_client = openpipe.get("fallback_client") if openpipe else OriginalOpenAI(
            api_key=api_key,
            organization=organization,
            base_url=base_url,
            timeout=timeout,
        ...
        self.chat = ChatWrapper(
            self, self.openpipe_reporting_client, self.openpipe_completions_client, self.fallback_client
        )
