...
def locate_google_drive(os_type: OSDefinition, max_depth: int, max_workers: int) -> Optional[Path]:
    ...
    common_names = ['Google Drive', 'GoogleDrive', 'googledrive', 'google_drive', 'Google_Drive']

    # First check common likely locations
    likely_locations = [Path.home() / name for name in common_names]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(_search_directory_for_drive, root, common_names, max_depth) for root in search_roots]
