...
if sys.version_info > (3, 10):
    ...
    Self = Any  # just a workaround for now
...
class AsyncStreamingAction(Action, abc.ABC):
    ...
    async def run(self, state: State, **run_kwargs) -> dict:
        ...
        while True:
            try:
                result = await anext(gen)  # if we just run through, we do nothing with the result
...
class AsyncStreamingResultContainer(typing.AsyncIterator[dict]):
    def __init__(self,
                 streaming_result_generator: typing.AsyncGenerator[typing.Tuple[dict, Optional[State]], None],
                 initial_state: State,
                 process_result: Callable[[dict, State], tuple[dict, State]],
                 callback: Callable[[Optional[dict], State, Optional[Exception]], typing.Coroutine[None, None, None]]):
        self.streaming_result_generator = streaming_result_generator
        self._initial_state = initial_state
        self._process_result = process_result
        self._callback = callback
    ...
    async def __anext__(self):
        ...
        result, state = await anext(self.streaming_result_generator)
