...
from burr.core.action import (
    ...
    create_action,
    default, AsyncStreamingResultContainer, SingleStepStreamingActionAsync, AsyncStreamingAction,
)
...
async def _run_single_step_streaming_action_async(
        action: SingleStepStreamingActionAsync,
        state: State,
    ...
    async for item in generator:
        if not isinstance(item, tuple):
            ...
            raise ValueError(f"Action {action.name} must yield a tuple of (result, state_update). "
        ...
        if state_update is None:
            yield result, None
    if state_update is None:
        raise ValueError(f"Action {action.name} did not return a state update. For async actions, the last yield "
