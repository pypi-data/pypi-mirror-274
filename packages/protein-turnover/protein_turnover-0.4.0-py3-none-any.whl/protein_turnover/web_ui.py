...
def web_options(f):
    ...
    f = click.option("-g", "--gunicorn", is_flag=True, help="run webapp with gunicorn")(
        f,
    )
    f = click.option(
        "--port",
...
`turnover web -g --workers=4 -- --workers=2`
...
@click.option(
    "--email",
    is_flag=True,
    help="allow background jobs to email user when done (requires a MAIL_SERVER configured)",
)
...
def view(
    ...
    jobsddir = jf.parent
    if job.auto_generated_jobid:
        results = ResultsResourceFile(job.jobid, jobsddir)
        if not results.has_result():
            raise ValueError(f"can't find output data file {results.cache_result()}")
    dataid = jf.stem
    defaults = {"JOBSDIR": str(jobsddir), "DATAID": dataid}
...
@click.option("--with-website", is_flag=True, help="add website configuration too")
def init_config(force: bool, with_website: bool, out: str | None):
    ...
    if configfile.exists() and not force:
        ...
        return
    if with_website and not has_package("protein_turnover_website"):
        click.secho(
    ...
    dump_config(configfile, with_website)
