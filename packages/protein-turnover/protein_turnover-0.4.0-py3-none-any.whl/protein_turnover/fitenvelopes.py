...
from .utils import get_abundance
...
from .utils import theoretical_dist
...
@dataclass
class HeavyEnvelope:
    heavyDistribution: np.ndarray  # 1-D
    heavyFitParams: np.ndarray  # 1-D
    heavy_adj_r_squared: float
...
@dataclass
class AllEnvelopes(Envelope, LabelledEnvelope):
    fail: int = 0
...
def normalIntensityFunction2(
    rt: float | np.ndarray,
    mu: float,
    sigma: float,
    k: float,
) -> float | np.ndarray:
    return normalIntensityFunction(rt, mu, sigma, k, 0.0)
...
def fitIsotopeEnvelopes(
    ...
    if eicIds.sum() == 0:
        return None, fail | (1 << FAIL_E_BOUNDARY)
    ...
    if USE_QUAD and INTERPOLATE_INTENSITY:
        ...
        # if not fail2:
        #     monoPeakArea2 = base * (rBoundary - lBoundary) + kscale * sigma * RPI2 * (
        #         erf((rBoundary - mu) / sigma / R2) - erf((lBoundary - mu) / sigma / R2)
        #     )
        #     a_ = np.abs(monoPeakArea2 - monoPeakArea) / monoPeakArea2

        #     if a_ > 1e-2:
        #         logger.debug(
        #             "can't integrate: %s != %s (%s)", monoPeakArea, monoPeakArea2, a_
        #         )
    ...
    a = alpha * (rBoundary - lBoundary)
    isotopeEnvelopes = ensure_pos(a + monoPeakArea * slopes)
    ...
    return (
        Envelope(
            # isotopeRegressions=isotopeRegressions,  # only used for filtering in export
            ...
            monoFitParams=popt.astype(np.float32),  # for plotting the fitted gaussian
            inv_ratio=inv_ratio,
            monoPeakArea=monoPeakArea,
...
def heavy_dist(peptide: str, isotopeEnvelopeInfo: Envelope) -> np.ndarray:
    ndist = isotopicDistribution(peptide)

    isotopeEnvelopes = isotopeEnvelopeInfo.isotopeEnvelopes
    monoPeakArea = isotopeEnvelopes[1]
...
def fitHeavyEnvelope(
    peptide: str,
    isotopeEnvelopeInfo: Envelope,
) -> tuple[HeavyEnvelope, int]:
    heavyDistribution = heavy_dist(peptide, isotopeEnvelopeInfo)

    nh = len(heavyDistribution)

    idx = np.array(list(range(0, nh)), dtype=np.float32)
    i = heavyDistribution.argmax()
    scale, mu = heavyDistribution[i], idx[i]

    try:

        def fit() -> np.ndarray:
            popt, *_ = curve_fit(
                normalIntensityFunction2,
                idx,
                heavyDistribution,
                p0=(mu, 1.0, scale),
                maxfev=1000,
        ...
        popt, fail = catch_warnings(
            fit,
            peptide,
            "fitHeavyEnvelope::curve_fit[%s]: %s",
            quiet=not LOG_ERRORS,
        )
        fail = fail << FAIL_W_H_CURVE

    except (RuntimeError, TypeError) as e:
        if LOG_ERRORS:
            logger.error("fitHeavyEnvelope::curve_fit[%s]: %s", peptide, e)
        return (
            HeavyEnvelope(
                heavyDistribution=heavyDistribution,
                heavyFitParams=np.zeros(shape=(5,), dtype=np.float32),
                heavy_adj_r_squared=0.0,
            ),
            1 << FAIL_E_H_CURVE,
        )
    ...
    # HEAVYFITPARAMS = ('mu','sigma','scale','r_squared', 'adj_r_squared')
    heavyFitParams = np.array([*popt, r.r_squared, r.adj_r_squared], dtype=np.float32)

    return (
        HeavyEnvelope(
            heavyDistribution=heavyDistribution,
            heavyFitParams=heavyFitParams,
            heavy_adj_r_squared=r.adj_r_squared,
        ),
        fail,
    )


def get_enrichments(peptide: str, settings: PeptideSettings) -> np.ndarray:
    """elementCount + 1 array of enrichments"""
    naturalLabelledElement = get_abundance(settings)
    formula = peptideFormula(peptide)
    elementCount = formula[NAMES == settings.labelledElement][0]
...
def labelledEnvelopeCalculation(
    ...
    maxIso: int,
    isotopeEnvelopeInfo: Envelope,
    # heavyEnvelopeInfo: HeavyEnvelope,
    settings: PeptideSettings,
) -> tuple[LabelledEnvelope, int]:
    enrichments = get_enrichments(peptide, settings)
    ...
    for i, elementEnrichmentLevel in enumerate(enrichments):
        abundances = labelledAtomicAbundances(
    ...
    labelledEnvelope: np.ndarray

    # area = isotopeEnvelopeInfo.monoPeakArea

    labelledEnvelope, nnls_residual = nnls(isotopeEnvelopeBasis, isotopeEnvelopes)

    nnls_residual = np.sqrt(nnls_residual) if nnls_residual > 0 else 0.0

    totalNNLSWeight = labelledEnvelope.sum()
    ...
    if totalNNLSWeight > 0:
        ...
        relativeIsotopeAbundance = 1.0 - labelledEnvelope[0] / totalNNLSWeight

        labelEnrichment2 = enrichments @ labelledEnvelope / totalNNLSWeight
        labelEnrichment2 = min(
            max(0, labelEnrichment2),
            settings.maximumLabelEnrichment,
        )

    theoreticalMaxEnrichment = elementCount * labelledEnvelope[1:].sum()
    if theoreticalMaxEnrichment > 0:
        labelEnrichment = (
    ...
    if totalNNLSWeight > 0:
        # x /= totalNNLSWeight
        theoreticalDist = isotopeEnvelopeBasis @ labelledEnvelope
        # nnls_residual /= totalNNLSWeight
    else:
        theoreticalDist = theoretical_dist(peptide, labelEnrichment2, settings)
...
def fitEnvelopes(
    ...
    return AllEnvelopes.make_all(faile | faill, iso, labelled)
