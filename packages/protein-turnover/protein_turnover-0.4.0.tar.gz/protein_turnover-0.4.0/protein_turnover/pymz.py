...
class MzML:
    ...
    def eics_from_df(
        ...
        *,
        ...
        if columns:
            df = df.rename(
                columns=dict(zip(columns, cols)),
            )

        def findeic(row: pd.Series) -> np.ndarray | float:
            ret = self._getEIC(row[cols[0]], row[cols[1]], row[cols[2]], row[cols[3]])
            if ret is None:
                return zeroi() if zero_length else np.nan
            return ret.T

        return df[list(cols)].apply(findeic, axis=1)
...
def mzml_calc_pep_envelopes(
    ...
    start = end
    e = fitEnvelopes(row, eic, settings)
    if e is None:
        return None
    logger.debug("fitEnvelopes: %s %s", row.peptide, datetime.now() - start)
    d = dict(eics=eic)
    d.update(asdict(e))
    return d
