...
def turnover_job_run_cmd(
    ...
    from .config import NSPECTRA, INSPECT_URL, MAIL_TEXT
    ...
    def maybeemail(template: str | None, url: str | None = None) -> None:
        if not no_email and jobby.email and template:
            ...
            try:
                sendmail(msg, jobby.email, mimetype="html")
            except OSError as e:
...
def turnover_job_prepare(
    ...
    jobby = TurnoverJob.restore(job)
    if out is not None:
        jobby = replace(jobby, cache_dir=out)
...
def hash(filenames: list[str], verbose: bool) -> None:
    "Calculate sha256 hashes from files"
    from datetime import datetime
    from .utils import hash256, human
