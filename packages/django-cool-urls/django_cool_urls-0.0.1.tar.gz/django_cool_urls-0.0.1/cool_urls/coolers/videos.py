...
class VideoCooler(ABC, Cooler):
    ...
    def remote_markup(self) -> str:
        return format_html(
            '<p>Waiting on download of <a href="{}">{}</a></p>',
            self.cool_url.url,
            self.cool_url.url,
        )
    ...
    def local_markup(self) -> str:
        return format_html(
                ...
                """
                    <div class="cool-urls-video-container-local">
                      <video controls poster="{}" preload="none">
                        ...
                        {}
                        <p>
                          It looks like your browser doesn't do HTML video.
                          Maybe you'll have better luck with <a href="{}">the original source</a>.
                        </p>
                      </video>
                      <p class="origin">Via <a href="{}">{}</a></p>
                    </div>
                """  # NOQA: E501
            ),
            self.poster,
            ...
            self.cool_url.url,
            self.cool_url.url,
            self.__class__.__name__.replace("Cooler", ""),
        )
