...
def eagle_complete_before_cit_society_required(scout):
    ...
    if not all(
        (
            scout["Ranks"]["Eagle"]["Requirements"].get(requirement)
    ...
    if any(
        (
            scout["Ranks"]["Eagle"]["Requirements"][requirement]["Date"]
            >= date(2022, 7, 1)
            for requirement in ["01", "02", "04", "05", "06"]
        )
    ):
    ...
    if not all(
        (
            scout["Ranks"]["Eagle"]["Requirements"]["03"].get(code)
    ...
    if any(
        (
            scout["Ranks"]["Eagle"]["Requirements"]["03"][code].get("Date")
            >= date(2022, 7, 1)
            for code in "abcefghijklmnopqrstu"
        )
    ):
