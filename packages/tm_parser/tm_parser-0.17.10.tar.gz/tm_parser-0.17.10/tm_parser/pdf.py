...
from .merit_badge_allocation import (allocate_merit_badges,
                                     record_eagle_badges, record_life_badges,
                                     record_palm_badges, record_star_badges)
...
def parse_scout(data):
    ...
    if "Ranks" in scout:
        scout["Ranks"] = get_rank_advancement(scout["Ranks"])

    if "Merit Badges" in scout:
        scout["Merit Badges"] = get_full_merit_badges(scout["Merit Badges"])
    ...
    if "Partial Merit Badges" in scout:
        scout["Merit Badge Requirements"] = get_partials(scout["Partial Merit Badges"])
