...
INVALID_PROJECT_NAME = "Could not create experiment, common cause for this is an invalid project_name. Project_name must be an alphanumeric string, but got %r"
...
UPLOAD_IMAGES_LIMIT_REACHED = "Experiment has reached the maximum number of images allowed. No more images would be uploaded. Please contact us if you would like to increase the limit."
...
LOG_PARAMS_MAX_DEPTH_REACHED = "%r passed to Experiment.log_parameters exceeded the nested limit of 10, Comet will convert the sub layers into strings."
...
CONFIG_CLAIMED_ANONYMOUS_USER = "Could not start anonymous mode. If you would like to log data to your Comet account, call `comet_ml.init()`. If you would like to log data to an anonymous account, delete the Comet config file located at %s and call `comet_ml.init(anonymous=True)."
...
MAX_NUMBER_OF_EXPERIMENTS_PER_WORKSPACE_REACHED = "Maximum number of experiment reached for project %s of workspace %s. Archive experiments or use a different project. Data will not be logged."
...
CONFIG_SETTING_FAILED_WARNING = "config setting %r failed with value %r: %s"

CONFIG_SETTINGS_FILE_EXISTS_ERROR = (
    "'%s' exists and force is not True; refusing to overwrite"
)
...
CONNECTION_UPLOAD_THUMBNAIL_FAILED_ERROR = "Thumbnail could not be uploaded: %r"

CONNECTION_UPLOAD_ASSET_ITEM_FAILED_ERROR = "Asset item could not be uploaded: %r"
...
CONVERT_UTILS_CONVERT_USER_INPUT_TO_METRIC_VALUE_LOG_STR_WARNING = (
...
EXPERIMENT_START_FAILED_LOG_COLAB_NOTEBOOK_URL_ERROR = (
    "Failed to log colab notebook_url"
)
...
EXPERIMENT_LOG_LLM_PROJECT_ERROR_MSG = (
    "You are trying to log an experiment to an LLM project."
)
...
EXPERIMENT_LOG_IMAGE_NAME_LONG_ERROR = (
    "Image was not uploaded to Comet, name should be less than 100 characters."
)
...
FILE_UPLOADER_SAVE_MATPLOTLIB_FAILED_WARNING = "Failing to save the matplotlib figure"

FILE_UPLOADER_IMAGE_COVERT_ERROR = "Could not convert image_data into an image; ignored"
...
COMET_START_FAILED_TO_END_RUNNING_EXPERIMENT_WARNING = (
    "Failed to end running experiment, reason: %r"
)
