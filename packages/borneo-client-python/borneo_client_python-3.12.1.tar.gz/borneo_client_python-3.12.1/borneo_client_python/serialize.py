...
async def _serialize_scan_results_export(
    input: ScanResultsExportInput, config: Config
) -> HTTPRequest:
    if not input.scan_id:
        raise ServiceError("scan_id must not be empty.")

    path = "/scan/{scan_id}/export".format(
        scan_id=urlquote(input.scan_id, safe=""),
    )
    query: str = f""
...
def _serialize_data_subject(
    input: DataSubject, config: Config
) -> dict[str, DocumentValue]:
    result: dict[str, DocumentValue] = {}
...
def _serialize_data_subject_collection(
    input: list[DataSubject], config: Config
) -> list[DocumentValue]:
    return [_serialize_data_subject(e, config) for e in input]
