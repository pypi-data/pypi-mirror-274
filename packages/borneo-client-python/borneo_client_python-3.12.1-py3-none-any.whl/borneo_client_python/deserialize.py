...
async def _deserialize_scan_results_export(
    http_response: HTTPResponse, config: Config
) -> ScanResultsExportOutput:
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_scan_results_export(http_response, config)
    ...
    return ScanResultsExportOutput(**kwargs)


async def _deserialize_error_scan_results_export(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
async def _deserialize_untag_resources(
    ...
    if http_response.status != 200 and http_response.status >= 300:
        raise await _deserialize_error_untag_resources(http_response, config)
    ...
    return UntagResourcesOutput(**kwargs)


async def _deserialize_error_untag_resources(
    http_response: HTTPResponse, config: Config
) -> ApiError:
    code, message, parsed_body = await parse_rest_json_error_info(http_response)
...
def _deserialize_data_plane_ids(output: DocumentValue, config: Config) -> list[str]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [expect_type(str, e) for e in output]
...
def _deserialize_data_subject(output: DocumentValue, config: Config) -> DataSubject:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
...
def _deserialize_data_subject_collection(
    ...
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_data_subject(e, config) for e in output]


def _deserialize_data_type(output: DocumentValue, config: Config) -> DataType:
    if not isinstance(output, dict):
        raise ServiceError(f"Expected dict, found {type(output)}")
...
def _deserialize_data_type_collection(
    output: DocumentValue, config: Config
) -> list[DataType]:
    if not isinstance(output, list):
        raise ServiceError(f"Expected list, found {type(output)}")
    return [_deserialize_data_type(e, config) for e in output]
...
def _deserialize_recipient_list_member(
    ...
    if (_last_modification := output.get("lastModification")) is not None:
        kwargs["last_modification"] = expect_type(str, _last_modification)
