...
class ContactGroups(GenericMapping, metaclass=MetaLogger, object=CONTACT_GROUP_PYTHON_PACKAGE_CODE_LOGGER_OBJECT):
    ...
    def insert_contact_and_link_to_existing_or_new_group(
            self, *, contact_id: int, groups: list, lang_code: LangCode = None,
        ...
        groups_array = [self.add_update_group_and_link_to_contact(
            ...
            parent_group_id=parent_group_id, lang_code=lang_code, image=image) for group_name in groups]
        return groups_array
    ...
    def link_existing_groups(self, groups_to_link: list, contact_id: int, title: str, lang_code: LangCode,
    ...
    def add_update_group_and_link_to_contact(
        ...
        if len(groups_array) == 0:
            groups_linked = self.create_and_link_new_group(
                normalized_entity_name, contact_id, lang_code, is_interest)
        else:
            groups_linked = self.link_existing_groups(
                groups_array, contact_id, title, lang_code,
                parent_group_id, is_interest, image)
    ...
    def insert_link_contact_group_with_group_local(
            self, contact_id: int,
            groups: list, lang_code: LangCode = None, parent_group_id: int = None,
        ...
        for group_num, group in enumerate(groups):
            ...
            if not group_id:
                ...
                group_dict = ContactGroups.__create_group_dict(
                    group_title=group,
    ...
    @staticmethod
    def __create_group_dict(*, group_title: str, parent_group_id: int = None,
