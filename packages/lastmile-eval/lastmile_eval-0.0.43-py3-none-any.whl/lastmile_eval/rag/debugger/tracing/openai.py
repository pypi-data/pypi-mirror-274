...
def _json_serialize_anything(obj: Any) -> str:
    try:
        return json.dumps(
...
def flatten_json(obj: Mapping[str, Any]):
    return {k: _json_serialize_anything(v) for k, v in obj.items()}
    ...
    def create(self, *args, **kwargs):
        ...
        with self.tracer.start_as_current_span("chat-completion-span") as span:
            self.tracer.mark_rag_query_trace_event(
                PromptResolved(
                    fully_resolved_prompt=_json_serialize_anything(params)
                )
            )
                ...
                span.set_attributes(
                    {
                        ...
                        "choices": _json_serialize_anything(
