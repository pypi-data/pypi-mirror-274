...
# refresh JWT token at this interval
TOKEN_REFRESH_INTERVAL = 60 * 60  # 1hr
...
class APIClient:
    ...
    def token(self):
        curr_time = time.time()
        if (curr_time - self.last_token_refresh_time) > TOKEN_REFRESH_INTERVAL:
            self.refresh_token()
    ...
    def api_request(
        ...
        try:
            ...
            if j:
                if "detail" in j:
                    text = j["detail"]
            print(f"Got HTTP Error Code {e.response.status_code}: {text}")
            exit(1)
    ...
    def launch_job(
        ...
        payload = {
            "name": name,
            "setup_script": job_config.setup_script,
            "run_script": job_config.run_script,
            "num_nodes": job_config.num_nodes,
            "env": job_config.env,
            "resources": job_config.resources,
            "storage": job_config.storage,
            "cloud": job_config.cloud.value if job_config.cloud else None,
            "docker_image": job_config.docker_image,
        }
    ...
    def print_job_logs(self, job_id: str, node_index: int, follow: bool):
        ...
        if job.status in [JobStatus.PENDING, JobStatus.INITIALIZING]:
            raise Exception(f"Job {job_id} has not started")
    ...
    def get_job_host_info(self, job_id: str):
        host_info = self.api_request(
            "GET", f"{KOMODO_API_URL}/api/v1/jobs/{job_id}/host-info"
        )
        return host_info
    ...
    def launch_machine(
        ...
        payload = {
            "name": name,
            "setup_script": machine_config.setup_script,
            "env": machine_config.env,
            "resources": machine_config.resources,
            "storage": machine_config.storage,
            "cloud": machine_config.cloud.value if machine_config.cloud else None,
            "docker_image": machine_config.docker_image,
            "start_notebook": machine_config.notebook,
        }
    ...
    def get_machine_host_info(self, machine_id: str):
        host_info = self.api_request(
            "GET", f"{KOMODO_API_URL}/api/v1/machines/{machine_id}/host-info"
        )
        return host_info
    ...
    def print_machine_setup_logs(self, machine_id: str, follow: bool):
        ...
        live = machine.status == MachineStatus.INITIALIZING
    ...
    def mark_machine_as_initializing(self, machine_id: str):
        self.api_request(
            "POST",
            f"{KOMODO_API_URL}/api/v1/machines/{machine_id}/initializing",
        )
    ...
    def launch_service(
        ...
        payload = {
            "name": name,
            "setup_script": service_config.setup_script,
            "run_script": service_config.run_script,
            "num_nodes": service_config.num_nodes,
            "env": service_config.env,
            "resources": service_config.resources,
            "storage": service_config.storage,
            "cloud": service_config.cloud.value if service_config.cloud else None,
            "docker_image": service_config.docker_image,
            "readiness_path": service_config.service.readiness_probe_path,
            "post_data": service_config.service.readiness_probe_post_data,
            "initial_delay_seconds": service_config.service.readiness_probe_initial_delay_seconds,
            "min_replicas": service_config.service.min_replicas,
            "max_replicas": service_config.service.max_replicas,
            "target_qps_per_replica": service_config.service.target_qps_per_replica,
            "upscale_delay_seconds": service_config.service.upscale_delay_seconds,
            "downscale_delay_seconds": service_config.service.downscale_delay_seconds,
        }
