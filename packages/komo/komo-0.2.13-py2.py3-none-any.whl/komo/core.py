...
from komo.types import (Cloud, Job, JobConfig, Machine, MachineConfig,
                        MachineStatus, ReplicaStatus, Service, ServiceConfig,
                        ServiceReplica, ServiceStatus)
...
def ssh_machine(machine_name):
    ...
    machine = api_client.get_machine(machine_name, is_name=True)
    host_info = api_client.get_machine_host_info(machine.id)
    host_name = host_info["host_name"]
    user = host_info["user"]
