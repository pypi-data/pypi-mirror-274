...
class ChatCompletionDataset(AzuremlDataset, AzuremlValidatorMixin):
    ...
    @staticmethod
    def apply_chat_template(tokenizer: PreTrainedTokenizerBase, dialog: List[Dict[str, str]]) -> str:
        """
    ...
    def _apply_chat_template_validation(self, input_ds):
        ...
        def _helper(example: Dict[str, Any]):
            try:
                self.apply_chat_template(self.tokenizer, example[self.dataset_args["chat_key"]])
