...
class IdentityVerification:
    ...
    def load_and_process_image_deepface(self, image, country = None):
        from deepface import DeepFace

        front_face_locations, front_face_encodings, face_objs = [], [], []
        
        image_np = np.frombuffer(base64.decodebytes(image), np.uint8)
        ...
        try:
            ...
            # if country == 'LBN':
            #     try:
            #         face_objs = DeepFace.extract_faces(image, detector_backend = 'yolov8')
            #     except:
            face_objs = []
        ...
        if face_objs:
            ...
            front_face_locations = [(y, x + w, y + h, x)]
            front_face_encodings = face_recognition.face_encodings(image, front_face_locations)

        return front_face_locations, front_face_encodings
    ...
    def get_ocr_results(self, processed_image, country=None, side=None):
        ...
        # print(f'Time taken to compress image: {time.time() - ct}')

        if country == 'QAT' and side == 'front':
            image = vision_v1.types.Image(content=processed_image)
        
        else:
            compressed_image = BytesIO()
            processed_image.save(compressed_image, format="JPEG", quality=50, optimize=True)
    ...
    def extract_front_id_info(self, front_id, country):
        if country == 'UAE':
                ...
                from idvpackage.common import detect_id_card_uae
                front_data = {
                    'error': '',
                ...
                try:
                    ...
                    front_face_locations, front_face_encodings = self.load_and_process_image_deepface(bytes(front_id, "utf-8"))
        ...
        if country == 'IRQ':
                ...
                try:
                    ...
                    combined_pattern = r'(Ministry|National Card|Passports and Residence)'
                    ...
                    required_keys = ["front_face_locations", "front_face_encodings", "id_number", "name", "card_number_front"]
        ...
        if country == 'QAT':
                ...
                try:
                    ...
                    processed_front_id = self.image_conversion(front_id)
                    print(f'Conversion time: {time.time() - conversion_time}')
                    ...
                    print(f'Vision time: {time.time() - vt}')
                    front_id_text_desc = front_id_text[0].description
                    ...
                    print(f'Pattern time: {time.time() - pattern_time}')

                    image_handling = time.time()
                    ...
                    print(f'Image handling time: {time.time() - image_handling}')

                    face = time.time()
                    front_face_locations, front_face_encodings = self.load_and_process_image_deepface(bytes(front_id, "utf-8"))
                    print(f'Face time: {time.time() - face}')
                    # front_face_locations, front_face_encodings = self.load_and_process_image_fr(front_id)
                    
                    dumt_time = time.time()
                    front_face_locations_str = json.dumps([tuple(face_loc) for face_loc in front_face_locations])
                    front_face_encodings_str = json.dumps([face_enc.tolist() for face_enc in front_face_encodings])
                    print(f'Dump encodings time: {time.time() - dumt_time}')

                    extracttion_time = time.time()
                    front_data_fields = qatar_front_id_extraction(self.client, compressed_image_data, front_id_text, front_id_text_desc)
                    print(f'QATAR front id Extraction time: {time.time() - extracttion_time}')
                    ...
                    print(f'Expiry check time: {time.time() - expiry_check_time}')
    ...
    def extract_back_id_info(self, back_id, country):
        ...
        if country == 'LBN':
                ...
                try:
                    ...
                    text = id_infos[0].description
                    translated_id_text = self.translator.translate(text, src='ar', dest='en').text
                    pattern4 = r'(Marital status|Family)'
                    k= re.search(pattern4, translated_id_text, re.IGNORECASE)
    ...
    def exract_passport_info(self, passport, country):
        ...
        if country.upper() == 'IRQ':
                ...
                try:
                    ...
                    if not passport_details.get('full_name') and passport_details.get('full_name_generic'):
    ...
    def extract_ocr_info(self, data, video, country, report_names, face_match_threshold=0.59):
