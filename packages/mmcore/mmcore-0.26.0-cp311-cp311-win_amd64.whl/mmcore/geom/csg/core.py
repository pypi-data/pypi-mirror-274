...
class ConeProps(TypedDict):
    """ Returns a cone properties.
           ...
           """
    start: typing.Union[tuple[float, float, float], list[float]]
    end: typing.Union[tuple[float, float, float], list[float]]
    radius: float
    slices: int


class CubeProps(TypedDict):
    """
           ...
           """
    center: typing.Union[tuple[float, float, float], list[float]]
    radius: typing.Union[float, list[float], tuple[float, float, float]]
...
class CSG(object):
    ...
    def mesh_data(self):
        vertices, poly, count = self.toVerticesAndPolygons()


        return MeshData(vertices=vertices, indices=poly)
    ...
    @classmethod
    def cube(cls, props: CubeProps = None):
        """
        Construct an axis-aligned solid cuboid. Optional parameters are `center` and
        `radius`, which default to `[0, 0, 0]` and `[1, 1, 1]`. The radius can be
        specified using a single number or a list of three numbers, one for each axis.
    ...
    @classmethod
    def cone(cls, props: ConeProps = None):
        """ Returns a cone.
