...
if selected_type == "Regression Models":
	...
	selected_models = st.multiselect("Select models to train", [key for key, model in models.items() if isinstance(model, (LogisticRegression, GradientBoostingClassifier, BaggingClassifier, MultinomialNB, AdaBoostClassifier, RandomForestClassifier, SVC, KNeighborsClassifier, DecisionTreeClassifier))])
	...
	with columns[i]:
		if i < len(selected_models):
			...
			try:
				...
				# y_train = y_train.map({1: "Yes", 0: "No"})
				# print("hehe", y_test)
				# Train the model and get evaluation metrics                                train_X, train_y, test_X, test_y
				model, train_metrics, test_metrics, test_pred_y, train_pred_y = train_gradient_boosting_classifier(X_train, y_train, X_test, y_test)
				st.success(f"{model_name} trained successfully!")
				# Display evaluation metrics in Streamlit UI
