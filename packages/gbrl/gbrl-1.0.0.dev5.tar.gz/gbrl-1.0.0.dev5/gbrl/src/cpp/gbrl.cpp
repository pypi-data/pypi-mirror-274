...
float* GBRL::predict(const float *obs, const char *categorical_obs, const int n_samples, const int n_num_features, const int n_cat_features, int start_tree_idx, int stop_tree_idx){
    ...
    dataSet dataset{obs, categorical_obs, nullptr, nullptr, nullptr, n_samples};
...
void GBRL::_step_gpu(dataSet *dataset){
    ...
    size_t alloc_size = obs_size*2 + cat_obs_size + grads_size*2 + grads_norm_size + cand_indices_size + cand_float_size + cand_cat_size + cand_numerical_size;
    ...
    dataSet cuda_dataset{trans_obs, gpu_categorical_obs, gpu_grads, gpu_build_grads, gpu_grads_norm, n_samples};
...
float GBRL::_fit_gpu(dataSet *dataset, float *targets, const int n_iterations){
    ...
    dataSet cuda_dataset{gpu_obs, gpu_categorical_obs, gpu_grads, gpu_build_grads, gpu_grads_norm, n_samples};
...
#endif
void GBRL::step(const float *obs, const char *categorical_obs, float *grads, const int n_samples, const int n_num_features, const int n_cat_features){
    if (this->metadata->iteration == 0){
        this->metadata->n_num_features = n_num_features;
        this->metadata->n_cat_features = n_cat_features;
    }
    ...
    dataSet dataset{obs, categorical_obs, grads, nullptr, nullptr, n_samples};
#ifdef USE_CUDA
    if (this->device == gpu)
...
float GBRL::fit(float *obs, char *categorical_obs, float *targets, int iterations, const int n_samples, const int n_num_features, const int n_cat_features, bool shuffle, std::string _loss_type){
    ...
    dataSet dataset{training_obs, training_cat_obs, nullptr, nullptr, nullptr, n_samples};
