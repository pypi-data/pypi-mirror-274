...
class Asset:
    ...
    def __init__(self, stream):
        ...
        while more_sections_to_read:
            ...
            self._read_section(section_type, stream)
    ...
    def _read_section(self, section_type, stream):
        if Asset.SectionType.BYTECODE_TRIGGER == section_type: # TMR, MOV
            ...
            duplicate_asset_id = Datum(stream).d
            ...
            if self.type == Asset.AssetType.MOVIE:
                ...
                chunk_reference = Datum(stream).d.chunk_id
            ...
            self.mouse_active_area_polygon = Polygon(stream)
            ...
            self.text = Text.Text()
            self.text.font_asset_id = Datum(stream).d

        elif section_type == 0x0259: # TXT
            self.text.initial_text = Datum(stream).d

        elif section_type == 0x025a: # TXT
            self.text.maximum_width_in_pixels = Datum(stream).d
            ...
            [Datum(stream) for _ in range(3)]

        elif section_type == 0x0266:
            [Datum(stream) for _ in range(2)]
            ...
            self.mouse["frames"].append(
                {"id": Datum(stream).d, "x": Datum(stream).d, "y": Datum(stream).d}
            )

        elif section_type == 0x03ea: # SPR
            self.mouse["first"] = Datum(stream).d

        elif section_type == 0x03eb: # IMG, SPR, TXT, CVS
            self.editable = bool(Datum(stream).d)

        elif section_type == 0x03ec:
            # This should only occur in version 1 games.
            self.cursor = Datum(stream).d
            ...
            self.mouse = {
                "timers": {
                    Datum(stream).d: [Datum(stream).d, Datum(stream).d] for _ in range(5)
                }
            }

        elif section_type == 0x03ee:
            self.mouse.update({"unk": [Datum(stream).d, Datum(stream).d]})
            ...
            self.y = Datum(stream).d

        elif section_type == 0x060e: # PTH
            self.start = [Datum(stream).d]
