...
def re22(var_iv: np.array = np.array([7.2, 10, 20])) -> MOProblem:
    ...
    objective_1 = ScalarObjective(
        name="minimize the total cost of concrete and reinforcing steel of the beam", evaluator=f_1, maximize=[False]
    )
...
def re23(var_iv: np.array = np.array([50, 50, 100, 120])) -> MOProblem:
    ...
    objective_1 = ScalarObjective(
        name="minimize to total cost of a clyndrical pressure vessel", evaluator=f_1, maximize=[False]
    )
    objective_2 = ScalarObjective(name="the sum of the four constraint violations", evaluator=f_2, maximize=[False])
...
def re25(var_iv: np.array = np.array([35, 15, 0.207])) -> MOProblem:
    ...
    def g_4(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        return -(300 / ((11.5 * 10 ** 6 * x[:, 2] ** 4) / (8 * np.round(x[:, 0]) * x[:, 1] ** 3))) + 6
    ...
    def g_6(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        x = feas_val(x)
        return -1.25 + ((1000 - 300) / ((11.5 * 10 ** 6 * x[:, 2] ** 4) / (8 * np.round(x[:, 0]) * x[:, 1] ** 3)))
...
def re31(var_iv: np.array = np.array([50.0, 50.0, 2.0])) -> MOProblem:
    ...
    def f_1(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return x[:, 0] * np.sqrt(16 + x[:, 2] ** 2) + x[:, 1] * np.sqrt(1 + x[:, 2] ** 2)

    def f_2(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return 20 * np.sqrt(16 + x[:, 2] ** 2) / (x[:, 2] * x[:, 0])
    ...
    def g_3(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        return 10 ** 5 - ((80 * np.sqrt(1 + x[:, 2] ** 2) / (x[:, 2] * x[:, 1])))
...
def re32(var_iv: np.array = np.array([2.5, 5.0, 5.0, 2.5])) -> MOProblem:
    ...
    def tau(x: np.ndarray) -> np.ndarray:
        ...
        return np.sqrt(
            ...
            + (
                ((6000 * (14 + (x[:, 1] / 2))) * (np.sqrt(((x[:, 1] ** 2) / 4.0) + ((x[:, 0] + x[:, 2]) / 2) ** 2)))
                / (2 * (np.sqrt(2) * x[:, 0] * x[:, 1] * ((x[:, 1] ** 2) / 12 + ((x[:, 0] + x[:, 2]) / 2) ** 2)))
            )
            ** 2
        )
    ...
    def sigma(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return (6 * 6000 * 14) / (x[:, 3] * x[:, 2] ** 2)

    def p_c(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return ((4.013 * 30 * 10 ** 6 * np.sqrt((x[:, 2] ** 2 * x[:, 3] ** 6) / 36)) / (14 ** 2)) * (
            1 - (x[:, 2] / (2 * 14)) * np.sqrt((30 * 10 ** 6) / (4 * 12 * 10 ** 6))
        )
    ...
    def f_2(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return (4 * 6000 * 14 ** 3) / (30 * 10 ** 6 * x[:, 3] * x[:, 2] ** 3)
...
def re33(var_iv: np.array = np.array([67.5, 92.5, 2000, 15])) -> MOProblem:
    ...
    def g_2(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        return 0.4 - (x[:, 2] / (3.14 * (x[:, 1] ** 2 - x[:, 0] ** 2)))

    def g_3(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        return 1 - ((2.22 * 10 ** -3 * x[:, 2] * (x[:, 1] ** 3 - x[:, 0] ** 3)) / (x[:, 1] ** 2 - x[:, 0] ** 2) ** 2)

    def g_4(x: np.ndarray, _=None) -> np.ndarray:
        x = np.atleast_2d(x)
        return (
            (2.66 * 10 ** -2 * x[:, 2] * x[:, 3] * (x[:, 1] ** 3 - x[:, 0] ** 3)) / (x[:, 1] ** 2 - x[:, 0] ** 2)
        ) - 900

    # Objective functions
    def f_1(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return 4.9 * 10 ** -5 * (x[:, 1] ** 2 - x[:, 0] ** 2) * (x[:, 3] - 1)

    def f_2(x: np.ndarray) -> np.ndarray:
        x = np.atleast_2d(x)
        return 9.82 * 10 ** 6 * ((x[:, 1] ** 2 - x[:, 0] ** 2) / (x[:, 2] * x[:, 3] * (x[:, 1] ** 3 - x[:, 0] ** 3)))
