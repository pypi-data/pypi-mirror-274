...
class Bot:
        ...
        try:
            ...
            for question, question_tokens in self.preprocessed_questions:
                ...
                if similarity > max_similarity:
                    ...
                    best_response = random.choice(next(entry["answers"] for entry in self.conversations if entry["question"] == question))
            if best_response:
    ...
    async def edit_username(self, username, new_name):
        if username in self.users:
    ...
    async def view_chat(self, username):
        chat_log_file = os.path.join("chat_logs", f"{username}.json")
        if os.path.exists(chat_log_file):
            async def read_chat_log():
                async with aiofiles.open(chat_log_file, 'r') as file:
                    return json.loads(await file.read())
            chat_log = await read_chat_log()
    ...
    def load_conversations(self, file_path):
        try:
            ...
            print(f"Error loading conversations from {file_path}: {e}")
            return []
        ...
        while True:
            if not bot.current_user:
                ...
                if user_choice.lower() == "login":
                    asyncio.run(bot.login())
                elif user_choice.lower() == "signup":
                    asyncio.run(bot.signup())
                else:
