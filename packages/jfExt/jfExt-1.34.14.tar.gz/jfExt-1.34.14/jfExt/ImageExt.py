...
def image_detect_image_border(in_file, direction):
    ...
    if direction == "horizontal":
        # 检测水平边框
        diff = np.diff(pixels, axis=1)
        left_border_width = np.argmax(np.any(diff, axis=0))
        right_border_width = np.argmax(np.any(np.flip(diff, axis=1), axis=0))
        return left_border_width, right_border_width
    elif direction == "vertical":
        # 检测垂直边框
        diff = np.diff(pixels, axis=0)
        top_border_height = np.argmax(np.any(diff, axis=1))
        bottom_border_height = np.argmax(np.any(np.flip(diff, axis=0), axis=1))
