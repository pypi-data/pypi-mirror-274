...
def image_detect_image_border(in_file, direction, color_threshold=30, border_threshold=0.1):
    """
    >>> 图片检测边框
    :param {String} in_file: 图片读取地址
    ...
    if direction == "horizontal":
        start, end, step = 0, image_width, 1
        pixel_index = lambda x, y: x + y * image_width
    elif direction == "vertical":
        start, end, step = 0, image_height, 1
        pixel_index = lambda x, y: y + x * image_height
    else:
        raise ValueError("Invalid direction")
    border_width = 0
    for i in range(start, end, step):
        pixel_values = [pixels[pixel_index(i, j)] for j in range(image_height)] if direction == "horizontal" else [pixels[pixel_index(j, i)] for j in range(image_width)]
        different_pixels_count = sum(1 for pixel in pixel_values if color_similarity(pixel, pixel_values[0]) > color_threshold)
        if (different_pixels_count * 1.0 / image_height) < border_threshold:
            border_width += 1
        else:
            break
    return border_width
