...
def image_detect_image_border(in_file, direction):
    """
    >>> 图片检测边框
    :param {String} in_file: 图片读取地址
    ...
    if direction == "horizontal":
        # 从左侧开始，逐列扫描像素，找到左边框的位置
        left_border_width = 0
        for x in range(image_width):
            column_pixels = [pixels[x, y] for y in range(image_height)]
            if all(pixel == column_pixels[0] for pixel in column_pixels):
                left_border_width += 1
            else:
                break
        # 从右侧开始，逐列扫描像素，找到右边框的位置
        ...
        # 从顶部开始，逐行扫描像素，找到上边框的位置
        top_border_height = 0
        for y in range(image_height):
            row_pixels = [pixels[x, y] for x in range(image_width)]
            if all(pixel == row_pixels[0] for pixel in row_pixels):
                top_border_height += 1
            else:
                break
        # 从底部开始，逐行扫描像素，找到下边框的位置
...
def image_fit_border_ratio(in_file, out_file, target_ratio=0.8, target_size=(1024, 1024), background_color=(255, 255, 255)):
    ...
    if cur_ratio > target_size:
        # 进行缩小
        width, height = img.size
        percent = cur_ratio / 0.8
    ...
    if cur_ratio < target_ratio:
        ...
        # 创建目标大小的白色背景图（正方形）
        target_img = Image.new('RGB', (target_width, target_width), background_color)
        # 将原图片粘贴到目标图中心
...
def image_calc_border_ratio(image_path, background_color=(255, 255, 255), threshold=30):
    ...
    for x in range(width):
        for y in range(height):
            ...
            if color_similarity(pixel_color, background_color) > threshold:
                min_x = min(min_x, x)
                min_y = min(min_y, y)
                max_x = max(max_x, x)
                max_y = max(max_y, y)
...
def image_center_object(in_file, out_file, horizontal_border_widths, vertical_border_heights):
    """
    >>> 图片居中
    :param {String} in_file: 图片读取地址
    :param {String} out_file: 图片输出地址
    ...
    ic(move_pixels_horizontal, move_pixels_vertical)
    # 创建一个新的图像，大小与原图相同
