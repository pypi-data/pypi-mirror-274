...
def image_detect_image_border(in_file, direction, color_threshold=30, border_threshold=0.1):
    ...
    if direction == "horizontal":
        ...
        for x in range(image_width):
            ...
            different_pixels_count = sum(1 for pixel in column_pixels if color_similarity(pixel, column_pixels[0]) > color_threshold)
        ...
        for y in range(image_height):
            ...
            different_pixels_count = sum(1 for pixel in row_pixels if color_similarity(pixel, row_pixels[0]) > color_threshold)
            if (different_pixels_count * 1.0 / image_height) < border_threshold:
                top_border_height += 1
            else:
                break
