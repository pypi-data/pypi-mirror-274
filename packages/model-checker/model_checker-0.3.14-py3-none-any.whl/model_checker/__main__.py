...
def parse_file_and_flags():
    ...
    return args #module_name, module_path, cons_bool, save_bool, imposs_bool, upgrade_bool

def check_update(program_name):
    # Get the installed version of the package
    installed_version = None
    try:
        installed_version = __version__
        installed_version = subprocess.check_output(['pip', 'show', program_name]).decode().split('\n')[1].split(': ')[1]
    ...
    latest_version = None
    try:
        latest_version = subprocess.check_output(['pip', 'search', '--disable-pip-version-check', program_name]).decode().split('\n')[0].split(' (')[1][:-1]
    except ValueError as e:
    ...
    if installed_version == latest_version:
        print(f"{program_name} is already up to date (version {installed_version})")
        return True
    else:
        print(f"{program_name} is not up to date (installed version: {installed_version}, latest version: {latest_version})")
        return False
