...
class CentralReachRequestsCore:
    def __init__(
        ...
        self._login_to_central_reach(self.__login, self.__password, self.__auth_totp_key)

        # validate with the new API auth method if needed
        if client_id and client_secret:
            # exchange credentials for JWT
            jwt_url = "https://login.centralreach.com/connect/token"
            jwt_headers = {"Content-Type": "application/x-www-form-urlencoded"}
            jwt_data = {
            ...
            jwt_response = self.session.post(jwt_url, data=jwt_data, headers=jwt_headers)
            JWT = jwt_response.json()
            # exchange jwt for session cookies
            cookies_url = "https://members.centralreach.com/api/?framework.authtoken"
            cookies_headers = {
            ...
            cookies_response = self.session.post(cookies_url, headers=cookies_headers, json=cookies_json_data)
    ...
    def check_response(
        ...
        if unauthorized_request:
            if re_authorize:
                self._login_to_central_reach(self.__login, self.__password, self.__otp_code, self.__auth_totp_key)
    ...
    def _login_to_central_reach(self, login: str, password: str, auth_totp_key: str) -> None:
        self.central_reach_core = CentralReachCore(login, password, auth_totp_key, headless=True)
