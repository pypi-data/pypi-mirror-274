...
class IPCCGlossary:
    ...
    def add_anchor(cls, div_entry):
        ...
        a_elem = ET.SubElement(div_entry, f"{H_A}")
    ...
    def create_confidences(cls, div):
        ...
        for span in spans:
            ...
            if current_div is None:
                current_div = ET.SubElement(parent, H_DIV)
            if "confidence" in span.text and len(span.text) < 50:  # because confidence can occur elsewhere
                ...
                if match:
                    span = ET.SubElement(current_div, "span")
...
class IPCCTargetLink:
    ...
    def _follow_ipcc_target_link(
        ...
        if anchor_link is not None:
            anchor = ET.SubElement(anchor_link, "a")
...
class IPCCDict:
    ...
    @classmethod
    def get_ipcc_dict(cls):

        ipcc_dict = {
            # NOT CHECKED - THIS HAS ERRORS
            "out_top": Path(Resources),
            IP_WG1: {
                ...
                CHAPTER_INFO: {
                    ...
                    CHAPTERS: [
                        {TITLE: "Chapter 1: Framing, Context and Methods",
                         PDF: f"{AR6_URL}/wg1/downloads/report/IPCC_AR6_WGI_Chapter01.pdf",
                         URL: f"{AR6_URL}/wg1/chapter/chapter-1/"
                         },
                        {TITLE: "Chapter 2: Changing State of the Climate System",
                         PDF: f"{AR6_URL}/wg1/downloads/report/IPCC_AR6_WGI_Chapter02.pdf",
                         URL: f"{AR6_URL}/wg1/chapter/chapter-2/"
                         },
                    ]
                },
                FR: {
                    PDF: {
                        URL: f"{AR6_URL}/wg1/IPCC_AR6_WGI_FullReport.pdf",
                        COUNT: 2409,
                    },
                },
                SPM: {TITLE: "summary",
                    PDF: {
                        URL: f"{AR6_URL}/wg1/downloads/report/IPCC_AR6_WGI_SummaryForPolicymakers.pdf",
                        TITLE: SPM_FULL,
                    },
                },
                TS: {
...
class TargetExtractor:
    ...
    def extract_node_dict_lists_from_file(self, xml_inpath, div_xp=None, regex_dict=None):
        ...
        for div in divs_with_text:
            if type(div) is not ET._Element:
