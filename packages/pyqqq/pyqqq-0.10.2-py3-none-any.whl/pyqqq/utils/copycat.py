...
class RabbitmqPubSub:
    ...
    def __init__(self, host=None):
        self.rabbitmq = pika.BlockingConnection(
            pika.ConnectionParameters(host=host or os.getenv('RABBITMQ_HOST') or 'localhost')
        )
    ...
    def declare_exchange(self, exchange_name):
        ...
        self.channel.exchange_declare(exchange=exchange_name, exchange_type='fanout') # fanout 은 broadcast 방식. 즉, 모ㄷ든 큐에 메시지를 전달한다.
...
class CopyCatSubscriber(RabbitmqPubSub):
    ...
    def _on_message(self, ch, method, properties, body):
        ...
        print(f" [x] Received {command} at {dt.isoformat()}")
        data = message.get('order_form')
        if command == OrderRequestType.NEW:
            print(f" [x] New order: {data}")
            # TODO: 따라하기
        elif command == OrderRequestType.MODIFY:
            print(f" [x] Modify order: {data}")
            # TODO: 어떤 주문에서 내용을 수정할 것인지 알아내는 것은 쉽지 않다.
        elif command == OrderRequestType.CANCEL:
            print(f" [x] Cancel order: {data}")
...
class CopyCatMessage:
    @staticmethod
    def make_for_create(asset_code: str, side: OrderSide, order_type: OrderType, price: int = None, quantity: int = None):
        command = OrderRequestType.NEW
        order_form = OrderForm(asset_code=asset_code, side=side, order_type=order_type, price=price, quantity=quantity)
        return pickle.dumps({
    ...
    def make_for_update(asset_code: str, order_type: OrderType, price: int = None, quantity: int = None):
        command = OrderRequestType.MODIFY
    ...
    def make_for_cancel(asset_code: str, quantity: int = None):
        command = OrderRequestType.CANCEL
