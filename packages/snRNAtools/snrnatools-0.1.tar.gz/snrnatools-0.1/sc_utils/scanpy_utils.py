...
## qc
def qc(data, name, mtprefix, basedir, order=None, batch_key=None):
    """\
        Parameters
        -------
        data
...
def norm_hvg(adata, name, target_sum, n_top_genes=None):
    """\
        Parameters
        -------
        adata
    ...
    for key in adata.uns.keys():
        if key.endswith('colors'):
            if type(adata.uns[key]) == dict:
                adata.uns[key] = {k: np.array_str(np.array(
                [(v if len(v) <= 7 else v[:-2]) for k,v in adata.uns[key].items()]), precision = 6, suppress_small = True)}
            else:
