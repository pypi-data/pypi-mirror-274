...
from lightningchart.charts import GeneralMethods, TitleMethods, ChartWithXYAxis, ChartWithSeries
...
from lightningchart.series.area_series import AreaSeries, AreaSeriesPositive, AreaSeriesNegative, AreaSeriesBipolar
...
from lightningchart.series.heatmap_scrolling_grid_series import HeatmapScrollingGridSeries
from lightningchart.series.box_series import BoxSeries
from lightningchart.ui.axis import Axis
from lightningchart.ui.chart_marker import ChartMarker
...
class ChartXY(GeneralMethods, TitleMethods, ChartWithXYAxis, ChartWithSeries):
    ...
    def set_title_position(self, position: str = 'center-top'):
        ...
        """
        title_positions = (
            "right-top", "left-top", "right-bottom", "left-bottom", "center-top",
            "center-bottom", "series-center-top", "series-right-top", "series-left-top",
            "series-center-bottom", "series-right-bottom", "series-left-bottom"
        )
        if position not in title_positions:
            raise ValueError(
                f"Expected position to be one of {title_positions}"
                f", but got '{position}'."
            )
    ...
    def add_x_axis(
        ...
        """
        return Axis(self, 'x', opposite, axis_type, base)
    ...
    def add_y_axis(
        ...
        Returns:
            Reference to Axis class.
        """
        return Axis(self, 'y', opposite, axis_type, base)

    def set_cursor_mode(self, mode: str = "disabled"):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        cursor_modes = ("disabled", "onHover", "snapToClosest")
        if mode not in cursor_modes:
            raise ValueError(
                f"Expected mode to be one of {cursor_modes}"
                f", but got '{mode}'."
            )

        self.instance.send(self.id, 'setAutoCursorMode', {'mode': mode})
        return self

    def set_mouse_interaction_pan(self, enabled: bool):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'setMouseInteractionPan', {'enabled': enabled})
        return self

    def set_mouse_interaction_rectangle_fit(self, enabled: bool):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'setMouseInteractionRectangleFit', {'enabled': enabled})
        return self

    def set_mouse_interaction_rectangle_zoom(self, enabled: bool):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'setMouseInteractionRectangleZoom', {'enabled': enabled})
        return self

    def set_mouse_interaction_wheel_zoom(self, enabled: bool):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'setMouseInteractionWheelZoom', {'enabled': enabled})
        return self
    ...
    def zoom(self, location: tuple[int, int], amount: tuple[int, int]):
        ...
        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'zoom', {
            'location': {
                'x': location[0],
                'y': location[1]
            },
    ...
    def set_auto_cursor_enabled_during_axis_animation(self, enabled: bool):
        """Disable/Enable AutoCursor during Axis Animations. Axis Animations are Axis Scale changes that are animated,
        such as Zooming and Scrolling done by using API (such as Axis.setInterval)
        or by using the mouse to click & drag on the Chart.

        Args:
            enabled (bool): Boolean value to enable or disable AutoCursor during Axis Animations.

        Returns:
            The instance of the class for fluent interface.
        """
        self.instance.send(self.id, 'setAutoCursorEnabledDuringAxisAnimation', {'enabled': enabled})
        return self
...
class ChartXYDashboard(ChartXY):
    ...
    def __init__(
        ...
        ChartWithSeries.__init__(self, instance)
        self.instance.send(self.id, 'createChartXY', {
            'db': dashboard_id,
            'column': column,
            'row': row,
            'colspan': colspan,
