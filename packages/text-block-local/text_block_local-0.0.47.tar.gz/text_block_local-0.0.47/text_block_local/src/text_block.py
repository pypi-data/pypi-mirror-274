...
class TextBlocks(GenericCRUD, metaclass=MetaLogger, object=object1):
    ...
    def __clean_text(text_without_empty_lines: str, text: str) -> str:
        return (text_without_empty_lines or text).replace("\n", " ")
    ...
    def process_and_update_field(self, fields_dict: dict, field_id: int, index: int) -> int or None:
        ...
        self.logger.info(object=dict_record)
        ...
        if not all((table_name, view_name, profile_mapping_table_name, profile_mapping_view_name)):
            self.logger.warning("table_name or view_name missing, so I will guess the value", object=dict_record)
            ...
            profile_mapping_view_name = profile_mapping_view_name or profile_mapping_table_name.replace("_table",
                                                                                                        "_view")
        ...
        if profile_id and profile_mapping_table_id:
            ...
            if table_name == PERSON_TABLE:
                entity_id = self.select_one_value_by_column_and_value(
                    schema_name="profile", view_table_name="profile_view", select_clause_value="person_id",
        ...
        if "Email" in fields_dict:
            ...
            if not phone_id:
                phone_id = self.select_one_value_by_column_and_value(
                    ...
                    view_table_name="phone_view",
                    select_clause_value="phone_id",
                    column_name="full_number_normalized",
                    column_value=phone_number
                )
            if phone_id:
                # get contact_id
