...
class NotifyProxy(BaseServiceProxy):
    ...
    def send_email(
        ...
        """Send an email by the viqi1

        Args:
           recipients: [a comma seperated] list of emails
           subject :   subject of email
           body:       text  of email
           attachments:  [ a comma seperated] list of viqi paths files to attach (must be on server)
           sender: overide the default sender (must be an email address  allowed by the system )
        Returns:
           a metaodc <result>[ok|fail]</result>
    ...
    def send_message(self, channel: str, message: str, attachments: list[str] = None):
        """Send an a slack message by the viqi1
