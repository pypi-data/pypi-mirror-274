...
def v_html_clparameter(self, node):
    ...
    if node.hasattr("lambda_keyword"):
        self.body.append('<span class="lambda_keyword k">')
...
def specializer_qualify_symbols(
    ...
    def qualify(symbol):
        ...
        if symbol.startswith(":"):
            return ["EQL", "KEYWORD%s" % symbol.upper()]
...
def clos_solt_name_xref(
    ...
    """Generate a link to a CLOS slot.

    The output of this function is the partner to the output of
    CLClosSlot.get_index_name
    """
    target = "{} <{} {}>".format(
        local_atom(package, clos_slot.lower()),
        clos_class.lower(),
        clos_slot.lower(),
    )

    xref = ":cl:clos-slot:`{}`".format(target)
    node = CLXRefRole()("cl:clos-slot", xref, target, lineno, inliner)
    return nodes.inline("", "", node[0][0])
...
def symbol_exists(package, name):
    """Return true if the symbol exists in the exported data."""
    symbol = ("%s:%s" % (package, name)).upper()
    return symbol in SYMBOL_DATA
...
def get_lisp_object(package, name, objtype):
    ...
    try:
        if objtype in ["function", "macro", "generic", "method"]:
            data = SYMBOL_DATA[symbol]["function"]
        elif objtype == "clos-class":
            data = SYMBOL_DATA[symbol]["closClass"]
        else:
            data = SYMBOL_DATA[symbol][objtype]
    except KeyError:
        pass
    if not data:
        if symbol_exists(package, name):
            raise LispDataError(
                """Missing Lisp data for `%s' with name `%s:%s'.

    This indicates that possibly the objtype `%s' is wrong, possible objtypes are %s.
                """
                % (objtype, package, name, objtype, SYMBOL_DATA[symbol].keys())
            )
        elif package.upper() in PACKAGE_DATA:
            raise LispDataError(
                """Missing Lisp data for symbol `%s:%s'.

    The symbol is not exported from the package `%s'.
                """
                % (package, name, package)
            )
        else:
            raise LispDataError(
                """Missing Lisp data for package `%s' while trying to find symbol `%s:%s'

    Please see https://sphinxcontrib-cldomain.russellsim.org/gettingstarted.html#configuration
    for details about how to add package `%s' to the `cl_systems' variable in the
...
class CLsExp(ObjectDescription):
    ...
    def handle_signature(
        ...
        if not self.env.temp_data.get("cl:package"):
            raise RuntimeError(
                "No package declared please add a declaration like .. cl:package:: <package>"
            )
    ...
    def get_index_text(self, name, type):
        otype = self.env.get_domain(self.domain).object_types.get(type)
        return _("%s (Lisp %s)") % (name.lower().split(":")[-1], otype.lname)

    def get_index_name(self, name, type):
        return type + ":" + self.cl_get_ref_name()

    def cl_get_ref_name(self):
        # TODO this should deal with internal vs external symbols better.
        return self.cl_package + ":" + self.cl_symbol_name

    def get_signature_prefix(self):
        otype = self.env.get_domain(self.domain).object_types.get(self.objtype)
    ...
    def add_target_and_index(
        ...
        objtype, name = name_obj

        # Exit if there was already on archor of the same name in this
        # document.
        if name in self.state.document.ids:
            self.state_machine.reporter.warning(
                "duplicate %s description of `%s' in this file, "
                % (objtype, name),
                line=self.lineno,
            )
            return

        indexname = self.get_index_name(name, objtype)
        refname = self.cl_get_ref_name()
        ...
        # Initialize hash for all symboles of this type
        if objtype not in self.env.domaindata["cl"]:
            self.env.domaindata["cl"][objtype] = {}

        # Initialize the list of all symbols of this name
        if refname not in self.env.domaindata["cl"][objtype]:
            self.env.domaindata["cl"][objtype][refname] = []
        else:
            self.state_machine.reporter.warning(
                "duplicate %s description of `%s', " % (objtype, name)
                + "other instance in "
                + self.env.doc2path(
                    self.env.domaindata["cl"][objtype][refname][0][0]
                ),
                line=self.lineno,
            )

        # Add current page to the list of pages with this item in it.
        self.env.domaindata["cl"][objtype][refname].append(
            (self.env.docname, objtype)
        )

        indextext = self.get_index_text(name, objtype)
        if indextext:
    ...
    def cl_doc_string(self, objtype=None):
        ...
        return self.cl_lisp_data()["documentation"]
...
class CLClass(CLsExp):
    ...
    def transform_content(self, contentnode: addnodes.desc_content) -> None:
        ...
        contentnode.insert(0, fields)

        lispobj = self.cl_lisp_data()
        slots = lispobj["slots"]

        fields.append(
            Field("superclass", label="Superclass").make_field(
                {},
                self.domain,
                (
                    None,
                    addnodes.desc_inline(
                        self.domain, "", lispobj["directSuperclass"]
                    ),
                ),
                env=self.env,
            )
        ...
        fields.append(
            Field("metaclass", label="Metaclass").make_field(
                {},
                self.domain,
                (
                    None,
                    addnodes.desc_inline(
                        self.domain, "", lispobj["metaclass"].lower()
                    ),
                ),
                env=self.env,
            )
        ...
        if slots and "noinitargs" not in self.options:
            ...
            slots_initargs = tuple(
                ...
                for slot in slots
                for initarg in slot["initargs"]
                if initarg.lower() != "nil"
            )
            fields.append(
                GroupedField("initargs", label="Initargs").make_field(
                    {}, self.domain, slots_initargs, env=self.env
                )
            )

    def before_content(self):
        super(CLClass, self).before_content()
        if "nodoc" in self.options:
            return

        lispobj = self.cl_lisp_data()
        lines = []
        if not lispobj["slots"]:
            return

        for slot in lispobj["slots"]:
            lines.append(
                ".. cl:clos-slot:: %s %s"
                % (
                    self.cl_symbol_name,
                    local_atom(self.cl_package, slot["name"].lower()),
                )
            )

        self.content = self.content + StringList(lines)


class CLFunction(CLsExp):
    def cl_handle_signature(self, sig: str, signode: desc_signature):
        """Perform Function specific signature additions."""
...

class CLClassSlot(CLFunction):
    def cl_get_ref_name(self):
        return "%s %s" % (
            qualify_atom(self.cl_package, self.cl_class_name).lower(),
            qualify_atom(self.cl_package, self.cl_symbol_name).lower(),
        )

    def cl_handle_signature(self, sig: str, signode: desc_signature):
        """Perform CLOS specific signature additions."""
        self.cl_class_name = sig.split(" ")[0].lower()
        self.cl_symbol_name = sig.split(" ")[1].lower()
        lispobj = self.cl_lisp_data()

        # If the symbol is private then preserve that

        if "::" in lispobj["name"]:
            self.cl_symbol_name = lispobj["name"].lower()

        name = addnodes.desc_name(
            self.cl_symbol_name,
            local_atom(self.cl_package, self.cl_symbol_name, private=False),
        )
        signode.append(name)
    ...
    def cl_lisp_data(self):
        ...
        package = self.cl_package
        symbol = self.cl_class_name
        objtype = "clos-class"

        def find_slot(data, name):
            """Find the first slot and return it."""
            slot = [
                s for s in data["slots"] if s["name"].lower().endswith(name)
            ]
            if len(slot) < 1:
                raise LispDataError(
                    """Missing Lisp data for %s `%s:%s', slot `%s''
                    """
                    % (objtype, package, symbol, name)
                )
            return slot[0]

        self.lispobj = find_slot(
            get_lisp_object(package, symbol, objtype), self.cl_symbol_name
        )

        return self.lispobj

    def cl_slot_initarg(self, initarg):
        return Field("initarg", label="Initarg").make_field(
            {},
            self.domain,
            (
                None,
                addnodes.desc_inline(self.domain, "", initarg.lower()),
            ),
            env=self.env,
        )

    def cl_slot_reader(self, reader):
        signode = addnodes.desc_signature("", "")
        sexp = SEXP(
            "(object)",
            types=["%s:%s" % (self.cl_package, self.cl_class_name)],
            package=self.env.temp_data.get("cl:package"),
        )
        signode.append(
            sexp.as_parameterlist(addnodes.desc_name("", reader.lower()))
        )

        return Field("reader", label="Reader").make_field(
            {},
            self.domain,
            (None, (addnodes.desc("", signode, objtype="method"),)),
            env=self.env,
        )

    def cl_slot_writer(self, writer):
        signode = addnodes.desc_signature("", "")
        sexp = SEXP(
            "(new-value object)",
            types=[
                "COMMON-LISP:T",
                "%s:%s" % (self.cl_package, self.cl_class_name),
            ],
            package=self.env.temp_data.get("cl:package"),
        )
        signode.append(sexp.as_parameterlist(writer))

        return Field("writer", label="Writer").make_field(
            {},
            self.domain,
            (None, (addnodes.desc("", signode, objtype="method"),)),
            env=self.env,
    ...
    def transform_content(self, contentnode: addnodes.desc_content) -> None:
        ...
        contentnode.insert(0, fields)
        fields.append(
            Field("type", label="Type").make_field(
                {},
                self.domain,
                (
                    None,
                    addnodes.desc_inline(
                        self.domain, "", lispobj["type"].lower()
                    ),
                ),
                env=self.env,
            )
        )
        for initarg in lispobj["initargs"]:
            fields.append(self.cl_slot_initarg(initarg))

        for reader in lispobj["readers"]:
            fields.append(self.cl_slot_reader(reader))

        for writer in lispobj["writers"]:
            fields.append(self.cl_slot_writer(writer))
...
class CLMethod(CLFunction):
    ...
    def cl_method(self, sig: str, package: str):
        """Convert find methods using the sig and package.
    ...
    def cl_setf_methods(self, symbol_name: str, package: str):
        """Convert find setf methods using the sig and package.
...
class CLCurrentPackage(Directive):
    """This directive tells Sphinx that we're documenting stuff in a
    specific package.  So that each symbol won't require a package
    specifier.
    ...
    def _find_package(self, name_or_nickname):
        for package in PACKAGE_DATA.values():
            if package["name"] == name_or_nickname or name_or_nickname in (
                package["nicknames"] or []
            ):
                return package["name"]

    def run(self):
        env = self.state.document.settings.env
        name_or_nickname = self.arguments[0].upper()

        package = self._find_package(name_or_nickname)
        if package:
            env.temp_data["cl:package"] = package
        else:
            load_packages(
                self.state.document.settings.env.app, [name_or_nickname]
            )
        ...
        if not package:
            ...
            if package:
                ...
                raise RuntimeError(
                    """Missing Lisp data for package `%s'

    Please see https://sphinxcontrib-cldomain.russellsim.org/gettingstarted.html#configuration
    for details about how to add package `%s' to the `cl_systems' variable in the
    project configuration.  Symbols were loaded from %s packages.
                    """
                    % (
                        name_or_nickname,
                        name_or_nickname,
                        list(PACKAGE_DATA.keys()),
                    )
                )
...
class CLDomain(Domain):
    ...
    object_types = {
        ...
        "clos-class": ObjType(_("CLOS class"), "clos-class"),
        "clos-slot": ObjType(_("CLOS slot"), "clos-slot"),
    ...
    roles = {
        ...
        "macro": CLXRefRole(),
        "variable": CLXRefRole(),
        "clos-class": CLXRefRole(),
        "clos-slot": CLXRefRole(),
        "method": CLXRefRole(),
    }
    ...
    def find_obj(
        self, env: BuildEnvironment, type: str, name: str
    ...
    def resolve_xref(
        ...
        if type.lower() == "symbol":
            for type in [
                ...
                matches = self.find_obj(env, type, target.upper())
                if matches:
                    break
        else:
            matches = self.find_obj(env, type, target.upper())
        ...
        name = first_match[0]  # the symbol name
        filename = first_match[1][0][0]  # the first filename
        type = first_match[1][0][1]  # the first type
        link = type + ":" + name
        return make_refnode(
...
def ros_command(
    ...
    return (
        ros_arguments
        + cldomain_exe
        + command_arguments(systems, system_paths, packages)
    )
...
def command_arguments(
    ...
    return cldomain_exe + cldomain_args
...
def index_packages(
    ...
    if app.config.cl_custom_command:
        command = app.config.cl_custom_command + command_arguments(
            systems, system_paths, packages
        )
    else:
        command = ros_command(systems, system_paths, packages)
    if cl_debug:
        logger.info("Running: " + " ".join(command))
    env = os.environ.copy()
    raw_output = subprocess.check_output(command, env=env)
    try:
    ...
    if not lisp_data["result"] in ["success", "error"]:
        raise Exception("Invalid return value from Lisp `%s`" % lisp_data)
    if lisp_data["result"] == "error":
        error_type = lisp_data["error"]["type"]
        logger.error(lisp_data["error"]["value"])
        raise Exception("%s" % error_type)

    PACKAGE_DATA.update(lisp_data["packages"])
    SYMBOL_DATA.update(lisp_data["symbols"])


def init_cldomain(app: Sphinx) -> None:
    packages = app.config.cl_packages or []
    ...
    load_packages(app, packages)


def load_packages(app: Sphinx, packages: List[str]) -> None:
    systems = []
    system_paths = []
    if app.config.cl_systems:
        for system in app.config.cl_systems:
    ...
    logger.info(
        "Loading Lisp systems %s..." % ", ".join(str(x) for x in systems)
    )
    logger.info(
        "Collecting Lisp docstrings from packages %s..."
