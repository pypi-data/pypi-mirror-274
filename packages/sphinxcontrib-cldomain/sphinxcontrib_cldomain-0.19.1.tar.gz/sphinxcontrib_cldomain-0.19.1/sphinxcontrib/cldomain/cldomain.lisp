...
#+sbcl
(defmethod documentation ((slotd sb-pcl::condition-direct-slot-definition) (doc-type (eql 't)))
...
(defclass cldomain-slot (cldomain-base)
  ((initargs
    :initarg :initargs
    :accessor cldomain-slot-initargs)
   (readers
    :initarg :readers
...
(defclass cldomain-clos-class (cldomain-base)
  ((direct-superclass
    :initarg :direct-superclasses
    :accessor cldomain-clos-class-direct-superclasses)
   (metaclass
    :initarg :metaclass
    :accessor cldomain-clos-class-metaclass)
   (slots
    :initarg :slots
    :accessor cldomain-clos-class-slots)))
...
(defclass cldomain-symbol ()
  ((name
   ...
   (clos-class
    :initarg :clos-class
    :accessor cldomain-symbol-clos-class)))

(defclass cldomain-package ()
  ((name
    :initarg :name
    :accessor cldomain-name)
   (nicknames
    :initarg :nicknames
    :accessor cldomain-package-nicknames)
   (use-list
    :initarg :use-list
    :accessor cldomain-package-use-list)
   (used-by-list
    :initarg :used-by-list
    :accessor cldomain-package-used-by-list)))

(defmethod print-object ((object cldomain-symbol) stream)
  (print-unreadable-object (object stream :type t :identity t)
    (format stream "~a" (cldomain-symbol-name object))))

(defmethod print-object ((object cldomain-base) stream)
  (print-unreadable-object (object stream :type t :identity t)
    (format stream "~a" (cldomain-name object))))
...
default values from the arglist."
  ;; This is split up to support macros lambda lists like (foo bar
  ;; . baz) which is equivalent to (foo bar &rest baz)
  (let ((last (last arglist)))
    (append
     (loop :for e :in (butlast arglist)
           :if (listp e)
           :collect (car e)
           :else
           :collect e)
     ;; If the last element is a list, then unwrap it
     (if (listp (car last))
         (cons (caar last) (cdr last))
         last))))
...
is a string then just return the string."
  (flet ((external-symbol-p (symbol)
    ...
    (if (stringp symbol)
        ...
        (format nil ftm-string
                (concatenate
                 'string
                 (package-name (symbol-package symbol))
                 (if (external-symbol-p symbol) ":" "::")
                 (symbol-name symbol))))))
...
(defun encode-specializer (atom)
  ...
  (cond ((eq (type-of atom) 'eql-specializer)
         (concatenate
          ...
          "(EQL "
          (cond
            ((symbolp (eql-specializer-object atom))
             (encode-symbol (eql-specializer-object atom)))
            ((typep (eql-specializer-object atom) 'standard-class)
             (concatenate 'string "(FIND-CLASS '"
                          (encode-class (eql-specializer-object atom)) ")"))
            (t (format nil "~s" (eql-specializer-object atom))))
          ")"))
        ((classp atom)
         (encode-class atom))
...
(defmethod encode-value-documentation (symbol (type (eql 'class)))
  (when (class-p symbol)
    (make-instance
     ...
     :direct-superclasses
     (mapcar (lambda (class) (prin1-to-string (class-name class)))
    ...
    (dolist (slot (class-direct-slots (find-class sym)) slots)
      (push
       (make-instance
        ...
        :initargs (mapcar 'write-to-string (slot-definition-initargs slot))
        :readers (mapcar 'write-to-string (slot-definition-readers slot))
        :writers (mapcar 'write-to-string (slot-definition-writers slot))
...
(defun find-package1 (package-name)
  (let ((package (find-package (intern (string-upcase package-name)))))
    (unless package
      (error 'package-not-found-error :package-name package-name))
    package))
...
(defun symbol-to-domain-model (symbol)
  (make-instance
   'cldomain-symbol
   :name (encode-symbol symbol)
   ...
   :clos-class (encode-value-documentation symbol 'class)))

(defun package-symbols-as-domain-models (package)
  (loop :for symbol :in (external-symbols package)
        :collect (symbol-to-domain-model symbol)))

(defun encode-package-symbols (package-names &key (stream *json-output*))
  (let ((*json-output* stream))
    (cl-json:with-object ()
      (dolist (package-name package-names)
        (let ((*current-package* (find-package1 package-name)))
          (dolist (sym (package-symbols-as-domain-models *current-package*))
            (cl-json:as-object-member ((encode-symbol (cldomain-symbol-name sym)))
              (cl-json:encode-json sym))))))))


(defun package-to-domain-model (package)
  (make-instance
   'cldomain-package
   :name (package-name package)
   :nicknames (package-nicknames package)
   :use-list (mapcar #'package-name (package-use-list package))
   :used-by-list (mapcar #'package-name (package-used-by-list package))))

(define-condition cldomain-error (error) ())

(define-condition package-not-found-error (cldomain-error)
  ((package-name :initarg :package-name :reader package-missing-name))
  (:report (lambda (condition stream)
             (format stream
                     "Package ~S could not be found"
                     (package-missing-name condition)))))

(define-condition no-packages-specified (cldomain-error) ())

(defun encode-package-details (package-names &key (stream *json-output*))
  (let ((*json-output* stream))
   (json:with-object ()
     (dolist (package-name package-names)
       (let ((package (find-package1 package-name)))
         (if (not (null package))
             (json:as-object-member ((package-name package))
               (json:encode-json (package-to-domain-model package)))))))))

(defun encode-error (condition &optional (stream *json-output*))
  "Encode CONDITION into JSON-STREAM."
  (let ((*json-output* stream))
   (json:with-object ()
     (json:encode-object-member "result" "error")
     (json:as-object-member ("error")
       (json:with-object ()
         (json:encode-object-member "type" (princ-to-string (type-of condition)))
         (json:encode-object-member "value" (princ-to-string condition))
         (json:encode-object-member "package" (princ-to-string (package-name (symbol-package (type-of condition))))))))))

(defun render-result (&rest package-names)
  (handler-case
      (with-output-to-string (output)
        (when (null package-names)
          (error 'no-packages-specified))
        (let ((*json-output* output))
          (json:with-object ()
            (json:encode-object-member "result" "success")
            (json:as-object-member ("packages")
              (encode-package-details package-names))
            (json:as-object-member ("symbols")
              (encode-package-symbols package-names)))))
    (cldomain-error (e)
      (with-output-to-string (output)
        (encode-error e output)))))
