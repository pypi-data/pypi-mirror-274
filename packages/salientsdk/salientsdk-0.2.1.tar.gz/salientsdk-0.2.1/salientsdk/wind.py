...
def celsius_to_kelvin(temp: float | xr.DataArray) -> float | xr.DataArray:
    ...
    """
    C2K = 273.15
    MIN_TEMP = -90 + C2K  # The coldest temperature ever on earth
    UNITS_K = "degK"
...
def shear_wind(
    ...
    shear = shear.where(~shear.isnull(), 1.0)  # Protect vs divide by zero
    wnd_hh = wnd_hi * (hgt_hh / hgt_hi) ** shear
    wnd_hh = wnd_hh.where(wnd_hh >= 0, 0)
...
def get_power_curve(
    ...
    """
    try:
        # Attempt to find the CSV file within the installed package
        csv_path = resource_filename(__name__, "../data/power_curves.csv")
    except FileNotFoundError:
        csv_path = "./data/power_curves.csv"
    ...
    power_table = pd.read_csv(csv_path, index_col=WSPD, usecols=usecols)
