...
class PyFrameApp:
    ...
    def test_session(self):
        s = requests.Session()
        s.mount("http://testserver", wsgiadapter.WSGIAdapter(self))
        return s
    
    def template(self, temp_name, context=None):
        if context is None:
            context = {}
        
        return self.template_env.get_template(temp_name).render(**context)    # ** dictionary ni ochib bizga parameter sifatida bervoradi, .encode -> text object ni binary ga o'zgartirib beradi
