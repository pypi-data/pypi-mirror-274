...
class _FireDucksOptions:
    ...
    def __init__(self):
        self._default_eval_options = EvalOptions()
    ...
    def _configure(self):
        ...
        _, unknowns = parse_fireducks_flags(flags, self)

        opts = fireducks_ext.FireDucksCompileOptions()
        opts.target = self.target
        if fireducks_ext.ParseFireDucksFlags(" ".join(unknowns), opts) != 0:
            raise RuntimeError("fireducks flags parse error")
        self._default_eval_options.compile_options = opts
...
def _evaluate(values: List[fire.Value], options: EvalOptions = None):
    for value in values:
    ...
    def wrapper(ir, input_values, output_values):
        ...
        return fireducks_ext.execute(
            context().ext_context, options.compile_options, fi
        )
...
def evaluate(values: List[fire.Value], options=None):
    with tracing.scope(tracing.Level.DEFAULT, "fireducks.core.evaluate"):
        ret = _evaluate(values, options)
    return ret
