...
def compute( D, refinementDepth ):
    ...
    solution = mlhp.makeSingularSolution( D )
    source = mlhp.makeSingularSolutionSource( D )
    derivatives = mlhp.makeSingularSolutionDerivatives( D )

    # Domain and discretization
    nelements = [2] * D
    lengths = [1.0] * D

    sphere = mlhp.makeImplicitSphere( [0.0] * D, 0.0 )
    ...
    kappa = mlhp.makeConstantFunction( D, 1.0 )

    integrand = mlhp.makePoissonIntegrand( kappa, source )

    mlhp.integrateOnDomain( basis, integrand, [matrix, vector], 
        dirichletDofs=dirichlet, orderDeterminor=determinor )

    # Solve system
    interiorDofs = mlhp.makeCGSolver( 1e-8 )( matrix, vector )
    ...
    processors = [mlhp.makeSolutionProcessor( D, allDofs)]

    postmesh = mlhp.createGridOnCells( nsamples )
    writer = mlhp.VtuOutput( f'outputs/singular_Poisson_{D}D_{refinementDepth}' )

    mlhp.writeBasisOutput( basis, postmesh, writer, processors )
        
    # Error computation
    l2ErrorIntegrand = mlhp.makeL2ErrorIntegrand( allDofs, solution );
    energyErrorIntegrand = mlhp.makeEnergyErrorIntegrand( allDofs, derivatives );
