...
class PackageTests(TestCase):
    ...
    def test_get_attachments(self):
        """
        It tests if the get_attachments function works.
        """
        attachments = self.package.get_attachments()
        self.assertEqual(len(attachments), 2)
        for attachment in attachments:
            self.assertTrue(attachment.is_file())
            self.assertTrue(attachment.name in ('abc.in', 'abc.out'))

    def test_check_source_with_zip(self):
        """
        It tests if the check_source function works.
        """
        zip_dir = base_dir() / 'zip_src'
        ok = [zip_dir / 'z_cpp_h.zip', zip_dir / 'z_src_cpp.zip']
        for zip_file in ok:
            self.assertIsNone(self.package.check_source(zip_file))
        wr = zip_dir / 'z_src_cpp_py.zip'
        with self.assertRaises(InvalidFileExtension):
            self.package.check_source(wr)
