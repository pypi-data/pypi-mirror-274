...
class ErrorStrings:
    ...
    TIMESERIES_DF_PROPHET_RESTRICTED_COL = (
        "The input table contains columns {columns}, which are not allowed "
        "when the Prophet model is enabled. Please rename these columns or disable "
        "Prophet by removing it from allowed_training_algorithms or by adding it "
        "to blocked_training_algorithms in set_training method of ForecastingJob and "
    ...
    TIMESERIES_DISTRIBUTED_PARTITION_SPECIAL_CHARACTERS = (
        "For distributed training for forecasting, grain columns cannot have values that are "
        "strings that contain forward slashes or backward slashes, or contain only dot characters. "
        "Kindly remove such characters and then run the AutoML job again."
    )
    ...
    UNIQUE_VALUE_CROSS_VALIDATION = (
        "The cross validation fold contains only unique target values in the training portion. "
        "It usually means that the training set contains mostly the same values for each time "
        "series. Please decrease forecast_horizon, cv_step_size and n_cross_validations "
