...
def config_to_docker(config_path: pathlib.Path, config: Config):
    ...
    pip_pkgs_str = (
        f"RUN pip install -c constraints.txt {' '.join(pypi_deps)}" if pypi_deps else ""
    )
    faux_pkgs_str = "\n\n".join(
        f"ADD {relpath} /deps/{fullpath.name}/{fullpath.name}\n                RUN touch /deps/{fullpath.name}/pyproject.toml"
        + (
            f"\n                RUN pip install -c constraints.txt -r /deps/{fullpath.name}/{fullpath.name}/requirements.txt"
...
RUN pip install -c constraints.txt /deps/*
