...
class CRUDFastAPI(
    ...
    Examples:
        ...
        ----------------------
        Create a CRUDFastAPI instance for a User model and perform basic CRUD operations.
        ```python
        ...
        CRUDUser = CRUDFastAPI[User, UserCreateInternal, UserUpdate, UserUpdateInternal, UserDelete]
        user_crud = CRUDUser(User)

        # If you don't care about typing, you can also just ignore the CRUDUser part
        # Straight up define user_crud with CRUDFastAPI
        user_crud = CRUDFastAPI(User)
        ...
        ```python
        product_crud = CRUDFastAPI(Product)
        ...
        ```python
        comment_crud = CRUDFastAPI(Comment)
        ...
        ----------------------------------------------------
        If your model uses different column names for indicating a soft delete and its timestamp, you can specify these when creating the CRUDFastAPI instance.
        ```python
        custom_user_crud = CRUDFastAPI(
            User,
    ...
    async def get_multi_joined(
        ...
        **kwargs: Any,
        ...
        Examples:
            ...
            ```python
            from CRUDFastAPI import JoinConfig, CRUDFastAPI, aliased

            # Aliasing for self-joins or multiple joins on the same table
            owner_alias = aliased(ModelTest, name="owner")
            user_alias = aliased(ModelTest, name="user")

            # Initialize your CRUDFastAPI instance for BookingModel
            crud = CRUDFastAPI(BookingModel)
