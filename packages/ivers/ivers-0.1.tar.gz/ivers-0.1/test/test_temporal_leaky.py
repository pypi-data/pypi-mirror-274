...
class TestEndpointSplits(unittest.TestCase):
    ...
    def test_leaky_endpoint_split(self):
        ...
        train_df, test_df = leaky_endpoint_split(self.df, self.split_size, self.smiles_column, self.endpoint_date_columns)


    def test_leaky_endpoint_split_folds(self):
        # Test the fold split function for a single DataFrame with multiple date columns
        num_folds = 2
        results = leaky_folds_endpoint_split(self.df, num_folds, self.smiles_column, self.endpoint_date_columns)
