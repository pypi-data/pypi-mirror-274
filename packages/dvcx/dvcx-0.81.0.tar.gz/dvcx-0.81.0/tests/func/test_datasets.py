...
def test_registering_dataset_source_dataset_with_multiple_versions(
    ...
    dogs_dataset = catalog.create_new_dataset_version(dogs_dataset, 2)
...
def test_registering_dataset_invalid_source_version(
    ...
    assert str(exc_info.value) == (
        f"Dataset {dogs_dataset.name} does not have version 5"
    )
...
def test_ls_dataset_rows_with_custom_columns(cloud_test_catalog, dogs_dataset):
    ...
    for r in catalog.ls_dataset_rows("dogs_custom_columns", 1, custom_columns=True):
        assert r.custom == {
            "int_col": 5,
            "int_col_32": 5,
            "int_col_64": 5,
