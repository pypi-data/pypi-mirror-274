...
from . import messages, constants, transports, exceptions
...
class Iec6205621Client:
    ...
    def read_single_value(self, address, additional_data="1"):
        ...
        logger.info(f"[Read Request Response]: {response}")
    ...
    def startup(self):
        ...
        logging.debug(f"Reading identification...")
    ...
    def send_password(self, password=None):
        ...
        logger.info(f"[Password Message Send] {cmd.to_representation()}")
    ...
    def send_init_request(self):
        ...
        logger.info(f"[Init Request]: {request}")
        self.transport.send(request.to_bytes())
        self.rest()
    ...
    def with_serial_transport(
        ...
        :return:
        """
        transport = transports.SerialTransport(port=port, parity=parity)
        return cls(
            transport, device_address,
            password,
            battery_powered,
            error_parser_class
        )
    ...
    def with_tcp_transport(
        ...
        :return:
        """
        transport = transports.TcpTransport(address=address, parity=parity)
        return cls(
            transport,
            device_address,
            password, battery_powered,
            error_parser_class
        )
