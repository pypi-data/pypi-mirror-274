...
import serial
import socket
from . import utils, exceptions, constants
...
class MeterUnavailableError(Exception):
    """ Raise when meter isn't available"""
...
class BaseTransport:
    ...
    def simple_read(self, start_char, end_char, timeout=None):
        ...
        in_data = b""
        complete_data = b""
        meter_not_available_frame=b"available"
        start_char_received = False
        timeout = timeout or self.timeout
        duration = 0
        start_time = time.time()
        logging.debug(f"Starting the received process, looking for {_start_char}")
        while True:
            b = self.recv(1)
            complete_data += b
            duration = time.time() - start_time
        ...
        if meter_not_available_frame in complete_data:
            raise MeterUnavailableError(complete_data.decode(constants.ENCODING).replace('\r\n', ' '))
        return in_data
...
class SerialTransport(BaseTransport):
    ...
    def __init__(self, port, timeout=10, parity: str=serial.PARITY_EVEN, stopbits: int=serial.STOPBITS_ONE, bytesize: int=serial.SEVENBITS):
...
class TcpTransport(BaseTransport):
    ...
    def __init__(self, address, timeout=30, parity: str="N"):
    ...
    def connect(self):
        ...
        logging.debug(f"Connected to socket : {self.socket}")
    ...
    def _send(self, data: bytes):
        ...
        def add_parity(data: bytes):
            ...
            result = b''
            for byte in data:
                nb_1 = bin(byte).count('1')
                if nb_1 % 2 == remainder:
                    modified_byte = byte | mask            
                else:
                    modified_byte = byte

                result += bytes([modified_byte])
            return result
        
        data = add_parity(data)
        logging.debug(f"Sending bytes over socket : {data}")
