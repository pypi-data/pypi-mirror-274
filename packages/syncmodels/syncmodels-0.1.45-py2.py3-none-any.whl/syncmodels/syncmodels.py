...
from .storage import Storage, iCRUD, WaveStorage
...
def apply_fqui(item: BaseModel, force=False):
    ...
    if hasattr(item, 'id'):
        if isinstance(item.id, str):
            if m := re.match(r"(?P<table>[^:]+):(?P<uid>[^:]+)$", item.id):
                return item.id
        ...
        if ":" in uid:
            ...
            log.warning(
                "uid [%s] must be escaped to be inserted into surreal"
            )
        fqid = f"{kind}:{uid}"
        item.id = fqid
...
class SyncModel(iCRUD):
    ...
    def __init__(
        ...
        **kw,
        ...
        if storage is None:
            ...
            sur = SurrealistStorage(url=surreal_url, mode=mode)
            storage = [
                sur,
            ]
    ...
    async def put(self, item: BaseModel) -> bool:
        ...
        if item:
            ...
            if self.mode in (self.MODE_SNAPSHOT,):
                fqid = apply_fqui(item, force=True)
                # any other code here ...
            else:
                fqid = apply_fqui(item, force=False)
                # any other code here ...
    ...
    async def put_old(self, item: BaseModel) -> bool:
        """Try to create / update an item of `type_` class from raw data
        ...
        if item:
            ...
            # create/update save data into all storages
            for storage in self.storage:
                # TODO: `tube` or `snapshot` is delegated to storages
                # TODO: don't make this logic here
                ...
                # TODO: insert in `swarmtube` mode (*always* insert)
                # TODO: or normal mode (keep only the last snapshot)

                # TODO: consider not using `id` as object `id`
                # TODO: but `fquid`
                if current:
                    # data.pop('id')
                    # current.pop('id')
