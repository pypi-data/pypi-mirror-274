...
class iSession(iContext):
    ...
    async def authenticate(self):
        "resolve authentication (i.e 403 alike situations)"
    ...
    async def new(cls, url):
        def score(item):
            ...
            uri, factory = item
            sc = len([_ for _ in uri.values() if _ is not None])
...
class iBot(iAgent):
    ...
    async def _get_data(self, path="", **context):
        ...
        if self._is_already(url):
            ...
            try:
                # authentication process can be done here or by 403
                proxy = await self._get_session(url)

                # provide extra params to the proxy before calling get
                params = await proxy.update_params(url, params, context)
