...
class UpHill(Node):
    """
    A class used to represent a up hill terrain (Node)

    An up hill terrain is a Node that inherits the following attributes a ``Position(x,y)`` that is a position on a
    grid and a ``weight`` that represents the weight for a vehicle to move through different terrains, that affects its
    fuel efficiency. in this case the default weight is 2.
