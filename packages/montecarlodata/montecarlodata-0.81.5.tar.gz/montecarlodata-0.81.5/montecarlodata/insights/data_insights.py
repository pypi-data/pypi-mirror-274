...
class InsightsService:
    ...
    def echo_insights(
        ...
        for insight in self._request_wrapper.make_request_v2(
            query=GET_INSIGHTS, operation=EXPECTED_GET_INSIGHTS_FIELD
        ).data:
            table.append(
                [
    ...
    def get_insight(
        ...
        if scheme == S3_SCHEME and not aws_profile:
            raise ValueError(
                "Cannot use an s3 destination without specifying an AWS profile."
            )

        netloc_with_path = parsed_destination.geturl().replace(
            f"{scheme}{SCHEME_DELIM}", "", 1
        )
    ...
    def _save_insight_to_disk(
        self, insight_url: str, destination: str, **kwargs
    ) -> None:
        """
