...
from pycarlo.common.retries import ExponentialBackoffJitter, Backoff
...
class CollectorManagementService:
    ...
    def echo_template(self, **kwargs) -> None:
        ...
        """
        click.echo(
            self._get_template_url_from_launch(self._get_template_launch_url(**kwargs))
        )
    ...
    def echo_collectors(
        ...
        for collector in self._user_service.collectors or []:
            ...
            last_updated = (
                datetime.fromisoformat(collector.get("lastUpdated")).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            ...
            table.append(
                [
                    ...
                    collector["uuid"],
                    collector.get("codeVersion")
                    or template_version
                    or self._DEFAULT_COLLECTOR_VAL,
                    template,
                    last_updated,
    ...
    def add_collector(self, no_prompt: Optional[bool] = False) -> None:
        ...
        self.launch_quick_create_link(
            dry=False, dc_id=dc_id, collection_region=collection_region
        )
    ...
    def upgrade_template(
        ...
        collector_details = self._generate_collector_template(
            update_infra=update_infra, **kwargs
        )
        try:
            ...
            complain_and_abort(
                f"Missing expected property ({err}). The collector may not have been deployed before"
            )
        else:
            click.echo(f"Updating '{stack_arn}'")
            update_variant = (
                update_infra and template_variant != self._JANUS_TEMPLATE_VARIANT
            )
            self._upgrade(
                stack_arn=stack_arn,
    ...
    def ping_dc(self, dc_id: Optional[str] = None, trace_id: Optional[str] = None):
        ...
        click.echo(
            f"Pinging data collector to verify it is operational (trace ID: {trace_id})"
        )
        ...
        try:
            ...
            click.echo(
                f"Data collector responded to ping (trace ID: {result.trace_id})"
            )
    ...
    def _generate_collector_template(
        self, update_infra: bool = False, **kwargs
    ) -> Optional[Dict]:
        """
    ...
    def _get_template_url_from_launch(launch_url: str) -> str:
        ...
        """
        return parse.parse_qs(parse.urlparse(launch_url).fragment).get(
            "templateURL", []
        )[0]
