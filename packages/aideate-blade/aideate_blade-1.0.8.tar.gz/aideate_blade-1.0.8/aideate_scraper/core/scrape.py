...
async def parse_html(
    ...
    """
    web_content = WebScrapeContent(content_type=WebContentType.HTML, content=html_text, content_signature=content_signature)
    ...
    if image_url:
        try:
            image = await image_manager.http_get_image(image_url, 1)
    ...
    if not web_content.thumbnail and thumbnails:
        ...
        get_image_tasks = [
            image_manager.async_http_get_image(thumbnail_url, 1)
            for thumbnail_url in thumbnails
        ]
...
async def aget_scrape_api(url: str, enable_javascript: bool = False):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            if need_enable_javascript(text) and not enable_javascript:
                ...
                return await aget_scrape_api(url, enable_javascript=True)
            return text
