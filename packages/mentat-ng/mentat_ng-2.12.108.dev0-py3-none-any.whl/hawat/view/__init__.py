...
class ItemActionView(RenderableView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def changelog_log(self, item, json_state_before = '', json_state_after = ''):
        ...
        return (author, chlog.diff) if chlog else (None, None)
    ...
    def inform_about_filter_change(self, _filter, author, diff, managers, maintainers=None):
        """
        ...
        """
        informed_users = {author}
        def _send_email(recipients, template_path):
            for recipient in recipients:
                if recipient in informed_users:
                    continue
                mail_locale = recipient.locale
                ...
                with force_locale(mail_locale):
                    ...
                    email_body = flask.render_template(
                        ...
                        _filter = _filter,
                        diff = diff,
                        author = author.fullname if author else None
                    )
                    flask.current_app.mailer.send(email_headers, email_body)
                    flask.current_app.logger.debug(
                        "Sent information about the change of filter '{}' to user '{}'.".format(
                            _filter.name, recipient.email
                        )
                    )
                informed_users.add(recipient)
        ...
        try:
            _send_email(recipients=maintainers, template_path='filters/email_maintainers.txt')
        ...
        try:
            _send_email(recipients=managers, template_path='filters/email_managers.txt')
        except ConnectionRefusedError:
            # Mail service is probably not configured.
        ...
        if form.validate_on_submit():
            ...
            if form_data[hawat.const.FORM_ACTION_SUBMIT]:
                try:
                    ...
                    author, diff = self.changelog_log(item, '', item.to_json())

                    if isinstance(item, FilterModel) and author and diff:
                        self.inform_about_filter_change(item, author, diff, item.group.managers)
        ...
        if form.validate_on_submit():
            ...
            if form_data[hawat.const.FORM_ACTION_SUBMIT]:
                try:
                    ...
                    author, diff = self.changelog_log(item, item_json_before, item.to_json())
        ...
        if form.validate_on_submit():
            ...
            if form_data[hawat.const.FORM_ACTION_SUBMIT]:
                try:
                    ...
                    author, diff = self.changelog_log(item, item_json_before, '')
