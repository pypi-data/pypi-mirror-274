...
] + [chart_const.CHART_SECTION_MAP[key] for key, _, _ in AGGREGATIONS if key in chart_const.CHART_SECTION_MAP]

TIMELINE_CHART_SECTION_MAP = {chs.key: chs for chs in TIMELINE_CHART_SECTIONS}
...
class AbstractSearchView(PsycopgMixin, CustomSearchView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def custom_search(self, form_args):
        ...
        if 'aggregations' in form_args and form_args['aggregations']:
            ...
            self.response_context['chart_sections'] = TIMELINE_CHART_SECTIONS
    ...
    def _add_chart(self, key, chsection):
        timeline_data_iter = takewhile(
            lambda x: x.bucket is not None,
            self.response_context['search_result'][key]
        )

        response_chart_dict = self.response_context.setdefault(chart_const.KEY_CHARTS, {})[chsection.key] = {}
        table_data = self.response_context.setdefault(chart_const.KEY_TABLE_DATA, {}).setdefault(chsection.key, {})
        data_format = chart_const.InputDataFormat.LONG_SIMPLE if chsection.key == mentat.stats.idea.ST_SKEY_CNT_EVENTS else chart_const.InputDataFormat.LONG_COMPLEX

        response_chart_dict[chart_const.KEY_CHART_TIMELINE], table_data[chart_const.KEY_CHART_TIMELINE] = charts.get_timeline_chart_and_table_data_frame(
            timeline_data_iter,
            chsection,
        ...
        if chsection.key != mentat.stats.idea.ST_SKEY_CNT_EVENTS:
            response_chart_dict[chart_const.KEY_CHART_SECONDARY], table_data[chart_const.KEY_CHART_SECONDARY] = charts.get_secondary_chart_and_table_data_frame(
    ...
    def do_after_search(self, **kwargs):
        ...
        for chsection in self.response_context['chart_sections']:
...
class TabView(HTMLMixin, AbstractSearchView):
    ...
    def do_before_response(self, **kwargs):
        ...
        self.response_context.update(
            chart_section=TIMELINE_CHART_SECTION_MAP[self.response_context['aggregations'][0]]
        )
