...
DASHBOARD_CHART_SECTIONS = [
    chart_const.ChartSection(
        ...
        data_keys=[
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_REPS_S, lazy_gettext('summaries')),
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_REPS_E, lazy_gettext('extras'))
        ]
    ),
    chart_const.ChartSection(
    ...
    chart_const.ChartSection(
        ...
        data_keys=[
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_EVTS_N, lazy_gettext('new')),
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_EVTS_R,  lazy_gettext('relapsed'))
        ]
    ),
    chart_const.ChartSection(
        ...
        data_keys=[
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_EVENTS, lazy_gettext('reported')),
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_EVTS_F, lazy_gettext('filtered')),
            chart_const.DataKey(mentat.stats.idea.ST_SKEY_CNT_EVTS_T, lazy_gettext('thresholded'))
        ]
    )
] + [chart_const.CHART_SECTION_MAP[key] for key in (
...
def adjust_query_for_groups(query, groups):
    ...
    if not flask_login.current_user.has_role(hawat.const.ROLE_ADMIN):
        # Naive approach.
...
class AbstractDashboardView(SQLAlchemyMixin, BaseSearchView):  # pylint: disable=locally-disabled,too-many-ancestors
    ...
    def _add_charts(self, stats, timeline_cfg):
        for chsection in DASHBOARD_CHART_SECTIONS:
            ...
            if chsection.data_complexity != chart_const.DataComplexity.NONE:
    ...
    def do_after_search(self, items):
        ...
        if items:
            ...
            if self.always_include_charts:
                ...
                self._add_charts(stats, timeline_cfg)
                self.response_context.update(
                    chart_sections=DASHBOARD_CHART_SECTIONS
                )
