...
def read_opus_file_bytes(filepath):
...
def get_block_type(type_int: int):
...
def parse_header(filebytes: bytes):
...
def parse_directory(filebytes: bytes, directory_start: int, num_blocks: int):
    ...
    for i in range(num_blocks):
        type_int, size_int, start = struct.unpack_from('<3i', filebytes, directory_start + i*12)
...
def parse_data_block(filebytes: bytes, size: int, start: int, dpf=1):
...
def parse_3d_data_block(filebytes: bytes, start: int, dpf: int = 1):
    ...
    '''
    header = struct.unpack_from('6l', filebytes, start)
    data = {
        'version': header[0],
        'num_blocks': header[1],
        'offset': header[2],
    ...
    data['store_table'] = [struct.unpack_from('<2l', filebytes, start + 24 + i*8) for i in range(header[5])]
    ...
    for i in range(data['num_blocks']):
        data['y'][i] = np.frombuffer(filebytes[offset:], dtype=dtype, count=count)
        offset = offset + data['data_size']
        info_vals = struct.unpack_from('<' + ''.join([e['fmt'] for e in STRUCT_3D_INFO_BLOCK]), filebytes, offset)
...
def parse_file_log(filebytes: bytes, size: int, start: int):
    ...
    been performed on the file.  This function parses the file log as a list of strings using b'\x00' as a seperator,
