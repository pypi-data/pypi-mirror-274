...
def get_figsize(portrait: bool = True) -> tuple[float, float]:
    if portrait:
        return current_app.config["FIGSIZE"]
    return current_app.config["LANDSCAPE"]
...
def mkplot(dataid: str, rowid: int, fmt: str = "png") -> BytesIO:
    if rowid < 0:
        abort(404)
    figsize = get_figsize(portrait=False)
    mp = make_plot_table(dataid)
    pdata = mp.plot(rowid, figsize=figsize, image_format=fmt, portrait=False)
    if pdata is None:
        abort(404)
    plotio, _ = pdata
    return plotio
...
def pdf(dataid: str) -> Response:
    ...
    return make_plot_table(dataid).send_pdf(rowid)
...
@inspect.route("/about-orig.html")
def about_orig() -> str:
    return render_template("about-orig.html")
...
def init_view(app: Flask, dataid: str, url_prefix: str = "/") -> None:
    ...
    @inspect.route("/")
    def index():
        return redirect(url_for("inspect.data", dataid=dataid))

    # @inspect.route("/configuration.html")
