...
from protein_turnover.plotting import ALL_COLUMNS
...
from protein_turnover.sqla.model import file2engine
...
from protein_turnover.sqla.utils import rehydrate_peptides
...
class EngineMixin:
    ...
    def get_engine(self) -> Engine | None:
        ...
        db = self.locate()  # type: ignore
        if not db.is_file():
            return None

        engine = g.engines[self.dataid] = file2engine(db, echo=current_app.debug)
        # engine.echo = False
...
class SQLDataTable(DataTable, EngineMixin):
    ...
    def full_query(
        ...
        search_columns = [c.name for c in self.columns if c.search]
        send_columns = [c.name for c in self.columns if c.send and c.read]
        aggregates = [c.aggregate for c in self.columns if c.aggregate is not None]
        pq = ProteinQuery(
            sqlafilter,
            rows,
            query=dtquery,
            search_columns=search_columns,
            protein_columns=send_columns,
...
class SQLPeptideTable(PeptideTable, EngineMixin):
    ...
    def get_datatable_df(self, rows: RowFilter | None = None) -> pd.DataFrame | None:
        ...
        peptide_columns = [c.name for c in self.columns if c.read]
        pdq = PeptideQuery(
            filt,
            rows,
    ...
    def peptide_data(self) -> DTPepData | None:
        ...
        if "nnls_residual" in df.columns and "totalNNLSWeight" in df.columns:
            df["nnls_residual"] = df["nnls_residual"] / df["totalNNLSWeight"]

        # send_columns = list({str(c) for c in df.columns} & set(send_columns))
        # df = df[send_columns].copy()
        # # df = dedup_peptides(df)
    ...
    def get_lpf(self, df: pd.DataFrame) -> str | None:
        ...
        if "relativeIsotopeAbundance" not in df.columns:
            return None
        return to_image_url(plotLPF(df).get_figure())  # pylint: disable=no-member
...
class SQLSimplePeptideQueryTable(DataTable, EngineMixin):
    ...
    def get_datatable_df(
        ...
        if engine is None:
            return None
        columns: list[str] = [c.name for c in self.columns if c.read]
        start = datetime.now()
        # engine.echo = True
        filt = self.get_filter() if self.want_filter else None
        spd = SimplePeptideQuery(rows, filt, peptide_columns=columns)  # type: ignore
        df = spd.query(engine)
...
PROTEIN_COLUMNS = [
    ...
    Column(
        "num_peptides",
        title="#Peptides",
        aggregate=Aggregate("peptideid", "count"),
    ),  # virtual
...
PEPTIDE_COLUMNS = [
    ...
    Column("nnls_residual", title="NNLS Residual"),
...
def make_enrichment_plot(
    ...
    fig = ax.get_figure()  # pylint: disable=no-member
    out = BytesIO()
...
def make_nnls_plot(
    ...
    dt = SQLSimplePeptideQueryTable(
        dataid,
        columns=[
            Column("nnls_residual"),
            Column("totalNNLSWeight"),
        ],
        want_filter=True,
    )
    df = dt.get_datatable_df()
    ...
    if figsize is None:
        dpi = 96
        figsize = (600 / dpi, 400 / dpi)
    ax = nnls_plot(df, figsize=figsize, percent=percent)
    fig = ax.get_figure()  # pylint: disable=no-member
    fig.tight_layout()
    out = BytesIO()
    fig.savefig(out, format="png")
...
def make_plot_table(dataid: str) -> SQLSimplePeptideQueryTable:
    return SQLSimplePeptideQueryTable(
        dataid,
        columns=[Column(c) for c in ALL_COLUMNS],
        want_filter=False,
        index_col="peptideid",
    )
