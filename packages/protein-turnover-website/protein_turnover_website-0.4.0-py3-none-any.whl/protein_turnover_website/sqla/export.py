...
class SQLExportDataTable(SQLDataTable):
    ...
    def send_df(self, df: pd.DataFrame, filename: str, fmt: str = "tsv") -> Response:
        # last_modified = self.locate().stat().st_mtime
        ...
        if fmt == "tsv":
            if "gzip" in request.accept_encodings:
                compression = "gzip"
            elif "zip" in request.accept_encodings:
                compression = "zip"
...
EXPORT_COLUMNS = [
    ...
    Column(
        "Filtered STD LPF",
        aggregate=Aggregate("relativeIsotopeAbundance", function="std"),
    ),
    Column("Enrichment Avg", aggregate=Aggregate("enrichment", function="avg")),
    Column("lpf_median", title="Median LPF", search=False),
    Column(
        "Filtered Peptides",
        aggregate=Aggregate("peptide", function="group_concat", args=(", ",)),
    ),  # virtual
    Column("unique_stripped_peptides", title="All Peptides", search=False),
    Column(
        "PP Probability Avg",
        aggregate=Aggregate("peptideprophet_probability", function="avg"),
    ),
]
