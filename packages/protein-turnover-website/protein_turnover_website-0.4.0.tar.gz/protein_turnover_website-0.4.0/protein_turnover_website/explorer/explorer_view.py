...
from ..flask_utils import error_resp
...
def change_directory() -> str:
    ...
    template = current_app.jinja_env.get_template("explorer.html")
    return template.module.explorer(mountpoints, explorer)  # type: ignore


@exp.route("/change_directory_json")
def change_directory_json() -> Response:
    mountpoints = get_mountpoints()
    explorer = find_directory_from_request(mountpoints)
    if explorer is None:
