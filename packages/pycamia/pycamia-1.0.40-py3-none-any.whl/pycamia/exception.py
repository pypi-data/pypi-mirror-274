...
def touch(target, *args, default=None, error_type: (tuple, Exception)=Exception, print_error_message=False, **kwargs): 
    """
    ...
    Examples::
        ...
        failed
        >>> touch(lambda x: 1/x, a, default = lambda e, x: f'failed for {x}')
        failed for 0
    """
    if not callable(default): default = const_function(default)
    if print_error_message: default = lambda *a, **k: (print(e), default(e, *a, **k))[1]
