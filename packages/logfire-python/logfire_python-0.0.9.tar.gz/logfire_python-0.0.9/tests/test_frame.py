...
from logfire.frame import create_frame
...
class TestLogfireLogEntry(unittest2.TestCase):
    def test_create_frame_happy_path(self):
        handler = LogfireHandler(source_token="some-source-token")
        log_record = logging.LogRecord("logfire-test", 20, "/some/path", 10, "Some log message", [], None)
        frame = create_frame(log_record, log_record.getMessage(), LogfireContext())
    ...
    def test_create_frame_with_extra(self):
        ...
        frame = create_frame(log_record, log_record.getMessage(), LogfireContext(), include_extra_attributes=True)
