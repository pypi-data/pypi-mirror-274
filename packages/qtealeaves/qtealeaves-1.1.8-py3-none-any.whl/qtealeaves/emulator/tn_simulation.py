    ...
    for ii in range(state.convergence_parameters.max_iter):
        ...
        for jj, pos in enumerate(sweep_order):
            ...
            if load_stat_from_checkpoint and (jj == 0):
                ...
                stat_checkpoint_file = os.path.join(
                    folder_name_output, f"TTN_stat_{int_str_i}"
                )
                stat_checkpoint_json = stat_checkpoint_file + ".json"
                stat_checkpoint_file_pkl = (
                    stat_checkpoint_file + ".pkl" + ansatz.extension
                )
                if os.path.isfile(stat_checkpoint_file_pkl):
                    # Found file for the end of the ii-th sweep
                    state.log_print(f"Loading checkpoint from sweep {ii + 1}.")
            ...
            if load_stat_from_checkpoint:
                ...
                stat_checkpoint_file = os.path.join(
                    folder_name_output, f"TTN_stat_{int_str_ij}"
                )
                stat_checkpoint_json = stat_checkpoint_file + ".json"
                stat_checkpoint_file_pkl = (
                    stat_checkpoint_file + ".pkl" + ansatz.extension
                ...
                if os.path.isfile(stat_checkpoint_file_pkl):
                    # Found file for intrasweep of the ii-th sweep
                    state.log_print(
                        f"Loading checkpoint at sweep {ii + 1}, tensor {jj + 1}."
                    )
            ...
            if store_checkpoints and intrasweep_checkpoint:
                # Long computation time, store intra-sweep checkpoint
        ...
        if store_checkpoints and (not intra_sweep_last):
            stat_checkpoint_file = state.checkpoint_store(
                folder_name_output,
                stat_checkpoint_file,
                int_str_i,
