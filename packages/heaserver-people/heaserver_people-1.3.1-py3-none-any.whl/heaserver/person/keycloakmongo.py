...
class KeycloakMongo(Mongo):
                ...
                try:
                    async with session_get(role_base_url / 'groups' / group.id / 'role-mappings') as response_:
                        for role_dict in await response_.json():
                            group.add_role_id(self.__new_role(sub, role_dict).id)
    ...
    async def get_all_roles(self, request: Request) -> list[Group]:
        logger = logging.getLogger(__name__)
        sub = request.headers.get(SUB, NONE_USER)
    ...
    async def get_current_user_group_by_group(self, request: Request, group: str) -> Group:
        try:
            return next(g for g in await self.get_current_user_groups(request) if g.group == group)
        except StopIteration:
            return None

    async def get_group_by_group(self, request: Request, group: str) -> Group:
        try:
            return next(g for g in await self.get_all_groups(request) if g.group == group)
        except StopIteration:
            return None

    async def add_current_user_to_group(self, request: Request, id_: str) -> bool:
        logger = logging.getLogger(__name__)
        sub = request.headers.get(SUB, NONE_USER)
    ...
    def __keycloak_user_to_person(self, user: Mapping[str, Any], groups: Sequence[Mapping[str, Any]]) -> Person:
        ...
        person.group_ids = [self.__new_group(ALL_USERS, group).id for group in groups]
        return person
