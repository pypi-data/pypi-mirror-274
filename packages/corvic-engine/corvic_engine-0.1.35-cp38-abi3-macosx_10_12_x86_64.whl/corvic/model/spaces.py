...
@dataclasses.dataclass(kw_only=True)
class SpaceSourceColumnRenames(DataclassAsTypedMetadataMixin):
    """Metadata attached to space-source tables to remember renamed columns."""
...
class Space(WrappedOrmObject[SpaceID, orm.Space]):
    ...
    def sources(self) -> list[Source]:
        return [space_source.source for space_source in self._space_sources.values()]
    ...
    def _find_start_end_columns(rels: list[Relationship]) -> tuple[str, str]:
        ...
        if rels[0].fkey_relationship.source_with_fkey == rels[0].from_source_id:
            start_field = rels[0].from_space_source.table.schema.get_primary_key()
            if not start_field:
                raise BadArgumentError(
                    "configuration requires column to have a primary key",
                    source_name=rels[0].from_space_source.source.name,
                )
            start_col = start_field.name
        else:
            start_col = rels[0].new_column_name

        if rels[-1].fkey_relationship.source_with_fkey == rels[-1].to_source_id:
            end_field = rels[-1].to_space_source.table.schema.get_primary_key()
            if not end_field:
                raise BadArgumentError(
                    "configuration requires source to have primary key",
                    source_name=rels[-1].to_space_source.source.name,
                )
        ...
        for path in paths_between_outputs:
            ...
            for rel in path_itr:
                ...
                table = table.join(
                    ...
                    suffix=f"_{rel.to_space_source.source.name}",
                )
                update_renames(rel, column_renames)
        ...
        *,
        ...
        if renames:
            new_table = new_table.rename_columns(renames).update_typed_metadata(
                SpaceSourceColumnRenames(column_renames=renames)
            )
    ...
    def _check_or_infer_foreign_keys(  # noqa: C901
        ...
        if from_foreign_key:
            frm_renames = (
                frm.table.get_typed_metadata(SpaceSourceColumnRenames).column_renames
                if frm.table.has_typed_metadata(SpaceSourceColumnRenames)
                else dict[str, str]()
            )
        ...
        if to_foreign_key:
            to_renames = (
                to.table.get_typed_metadata(SpaceSourceColumnRenames).column_renames
                if to.table.has_typed_metadata(SpaceSourceColumnRenames)
                else dict[str, str]()
            )
