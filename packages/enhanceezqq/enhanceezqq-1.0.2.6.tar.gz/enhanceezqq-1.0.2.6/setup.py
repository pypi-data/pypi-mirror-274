...
def init_data_files() -> list:
    ...
    originconfigdir = 'config/enhanceezqq'
    configfilename = 'enhanceezqq_config.json'
    nonconfigfile = []
...
setup(
    name='enhanceezqq',
    version='1.0.2.6',
    description='用于对ezqq模块增强功能，能够获取历史数据保存到数据库，并从数据库读取历史数据',
    author='Frank Gong',
    author_email='583983716@qq.com',
    packages=find_packages(),        # 表示你要封装的包，find_packages用于系统自动从当前目录开始找包
    license="BSD",
    py_modules=['enhanceezqq', 'basefund', 'datasource_ths', 'datasource_ttjj', 'datasource_akpack'],
    install_requires=['pandas~=1.5.2', 'numpy~=1.24.1', 'openpyxl', 'argparse', 'pathlib',
                      'selenium', 'requests', 'xlrd', 'bottle', 'dftrans',
                      'chardet', 'nuitka', 'PyQt5', 'pycryptodomex', 'datacompy', 'akshare',
                      'shortuuid', 'sqlalchemy==1.4.46', 'mysql-connector-python', 'ipy',
                      'pathos', 'pymysql', 'colorlog', 'matplotlib~=3.7.2', 'pypinyin',
                      'pyxlsb', 'jsonlines', 'easyquotation', 'Jinja2', 'bs4'],
