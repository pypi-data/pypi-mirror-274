...
class CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    **Note**: The velocity that is reduced or increased by the applied force is not fixed and it depends on the angle
    ...
    The sensor is a `ndarray` with shape `(4,)` with the values corresponding to the following positions and velocities:
    ...
    **Note:** While the ranges above denote the possible values for sensor space of each element,
    ...
    ## Starting State
    All sensors are assigned a uniformly random value in `(-0.05, 0.05)`
    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        # Angle limit set to 2 * theta_threshold_radians so failing sensor
        ...
        self.sensor_space = spaces.Box(-high, high, dtype=np.float32)
...
class CartPoleVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self.single_sensor_space = spaces.Box(-high, high, dtype=np.float32)
        self.sensor_space = batch_space(self.single_sensor_space, num_envs)
