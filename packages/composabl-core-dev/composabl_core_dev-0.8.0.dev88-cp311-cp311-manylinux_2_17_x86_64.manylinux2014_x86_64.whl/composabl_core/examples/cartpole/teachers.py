...
from composabl_core.agent.skill.goals import (
    CoordinatedGoal,
    DriveGoal,
...
class CartpoleTeacher(CoordinatedGoal):
    def __init__(self):
        pole_goal = DriveGoal("pole_theta", "Drive pole to upright", target=0, stop_distance=0.418)
        cart_goal = DriveGoal("cart_pos", "Drive cart to center", target=0, stop_distance=2.4)
...
class BalanceTeacher(DriveGoal):
    def __init__(self, *args, **kwargs):
        super().__init__("pole_theta", "Drive pole to upright", target=0, stop_distance=0.418)
