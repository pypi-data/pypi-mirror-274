...
class _CatboxClient:
    ...
    def clear_cookies(self):
        pass
        # self.__client.cookies.clear(domain=".catbox.moe")

    async def get_video_content(self, link_id: str):
        video_url = await self.get_video_url(link_id)
        res = await self.__client.get(video_url)
        res.raise_for_status()
        return BytesIO(await res.read())

    async def get_video_url(self, link_id: str):
        res = await self.__client.get(f"{_CatboxClient.api_url}?action=info&key={link_id}")
        res.raise_for_status()
    ...
    async def is_video_available(self, filename: str):
        res = await self.__client.get(f"{self.file_base_url}/{filename}")
    ...
    async def upload_video(
        ...
        data = {
            "reqtype": "fileupload",
            "userhash": "",
            "fileToUpload": video_io
        }
        headers = {"Content-Type":"video/mp4"}
        try:
            print(f"_CatboxClient API URL: {_CatboxClient.api_url} | Data: {data} | Headers: {headers}")
            print(f"_CatboxClient API URL: {_CatboxClient.api_url} | Data: | Headers: {headers}")
            
            async with self.__client.post(url=_CatboxClient.api_url, data=data) as res:
                res.raise_for_status()
                url = await res.text()
