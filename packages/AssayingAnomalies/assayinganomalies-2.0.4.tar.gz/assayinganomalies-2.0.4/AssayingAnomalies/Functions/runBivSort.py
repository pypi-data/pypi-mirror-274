...
def runBivSort(params, ret, ind, nptf1, nptf2, mcap, dates, **kwargs):
    ...
    if p['printResults'] != 0:
        ...
        grsres1 = f"{res['grsPval'][0]} {res['grsFstat'][0]} {round(res['grsDoF'][0][0])} {round(res['grsDoF'][0][1])}"
        grsres2 = f"{res['grsPval'][1]} {res['grsFstat'][1]} {round(res['grsDoF'][1][0])} {round(res['grsDoF'][1][1])}"
