...
def get_dict_data(config, body_key):
    """
    Retrieve and process the request body from the configuration.
    """
    if body_key in config:
        return replace_macros(config[body_key], MACROS)
    else:
...
def set_url(config, api_url):
    return {"api_url": api_url, "headers": None, "body": None}


@given(ParseUtils('domain {domain} path {path}'), target_fixture='request_config')
def set_env_domain_path(config, domain, path):
    Logger.info(config)
    domain_name = domain.format(**config)
    request_config = {"api_url": config[domain_name] + path, "headers": None, "body": None}
...
def define_method(request_config, api_method):
    ...
    request_config["response_body"] = request_any(api_method, request_config["api_url"],
                                                  headers=request_config["headers"], json=request_config["body"])
    return request_config


@then(ParseUtils('status {code:d}'))
def verify_endpoint_response_code(code, request_config):
    assert request_config["response_body"].status_code == code, (
        "Expected code: {}, actual code: {}".format(code, request_config["response_body"].status_code))


@then(ParseUtils('response key {fields}'))
def verify_endpoint_response_exist(fields, request_config):
...
def verify_endpoint_response_exist(substring, request_config):
