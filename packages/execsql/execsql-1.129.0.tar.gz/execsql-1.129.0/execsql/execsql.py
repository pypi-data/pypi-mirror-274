...
GUI_DISPLAY, GUI_CONNECT, GUI_CONSOLE, GUI_SELECTSUB, GUI_PAUSE, GUI_HALT, GUI_ENTRY, QUERY_CONSOLE, \
	...
	GUI_CREDENTIALS, GUI_MSG = range(17)
	...
	def monitor(var):
		...
		if gui_manager_stop_event.is_set():
			...
			if not gui_manager_queue.empty():
				...
				if msg.gui_type == GUI_MSG:
					ui = MsgUI(**msg.gui_args)
					ui.activate()
					msg.return_queue.put(None)
					ui = None
...
def add_hidecols_menu(parent, mnu, label, tv_widget):
	try:
	...
	def hidecols():
		allcols = tv_widget["columns"]
		dispcols = tv_widget["displaycolumns"]
		if len(dispcols)==1 and dispcols[0]=='#all':
			dispcols = allcols
		dlg = TableHideColsDialog(parent, allcols, dispcols)
		result = dlg.show()
		if result is not None:
			tv_widget["displaycolumns"] = result
	mnu.add_command(label=label, command=hidecols)
...
class MsgUI(object):
	def __init__(self, title, message):
		...
		self.msg_label = ttk.Label(msgframe, text=message, width=50, wraplength=400)
		...
		btnframe.grid(column=0, row=1, sticky=tk.EW)
		btnframe.columnconfigure(0, weight=1)
		close_btn = ttk.Button(btnframe, text="Close", underline=2, command=self.do_close)
		close_btn.grid(column=0, row=0, sticky=tk.E, padx=6)
		self.win.bind("<Return>", self.do_close)
		self.win.bind("<Alt-o>", self.do_close)
		# Other key bindings
		self.win.protocol("WM_DELETE_WINDOW", self.do_close)
		close_btn.focus()
...
class DisplayUI(object):
	...
	def __init__(self, title, message, button_list, selected_button=0, no_cancel=False, column_headers=None,
		...
		if rowset and column_headers:
			add_hidecols_menu(self.win, mnu, "Hide/show", self.tbl)
...
class CompareUI(object):
	def __init__(self, title, message, button_list, headers1, rows1,
		...
		orientation = tk.HORIZONTAL if sidebyside else tk.VERTICAL
		table_panes = ttk.PanedWindow(self.tablemaster, orient=orientation)
		self.tableframe1, self.tbl1 = treeview_table(table_panes, self.rows1, self.headers1, "browse")
		self.tableframe2, self.tbl2 = treeview_table(table_panes, self.rows2, self.headers2, "browse")
		...
		table_panes.grid(column=0, row=0, sticky=tk.NSEW)
		if sidebyside:
		...
		table_panes.add(self.tableframe1, weight=1)
		table_panes.add(self.tableframe2, weight=1)
		btnframe.grid(column=0, row=3, sticky=tk.EW)
		# Add 'save as' menu items
		mnu = tk.Menu(self.win)
		self.win.config(menu=mnu)
		add_saveas_menu(self.win, mnu, "Save 1 as...", rows1, headers1)
		add_hidecols_menu(self.win, mnu, "Hide/show 1", self.tbl1)
		add_saveas_menu(self.win, mnu, "Save 2 as...", rows2, headers2)
		add_hidecols_menu(self.win, mnu, "Hide/show 2", self.tbl2)
		# Create buttons.
		if self.help_url is not None:
			help_btn = ttk.Button(btnframe, text="Help", command=self.do_help)
			help_btn.grid(column=0, row=0, sticky=tk.W, padx=3)
		...
		for i, e in enumerate(self.entries):
			...
			if e.lookup_list:
				if e.entry_type == 'listbox':
					...
					e.entryctrl = tk.Listbox(itementryframe, listvariable=e.entryvar, height=e.height or 4, width=e.width or 20, selectmode=tk.MULTIPLE, exportselection=0)
					if e.required:
...
def x_msg(**kwargs):
	...
	enable_gui()
	return_queue = queue.Queue()
	gui_args = {"title": "Message",
				 "message": message
				 }
	gui_manager_queue.put(GuiSpec(GUI_MSG, gui_args, return_queue))
	user_response = return_queue.get(block=True)
	return None
...
metacommandlist.add(r'^\s*PROMPT(?:\s+MESSAGE)?\s+"(?P<message>(.|\n)*)"\s*$', x_msg)
...
See the documentation for more complete descriptions of the metacommands.
   ...
   PROMPT DISPLAY <table_or_view> MESSAGE "<text>"
