...
class BasePath:
    ...
    """Can be changed with :obj:`~BasePath.set_config`"""
    ...
    def set_config(cls, config: ConfigType):
        ...
        :obj:`~BasePath.client` to be recreated next time it is called.
    ...
    def _get_object(self, version_id: Union[str, None]) -> dict:
        raise NotImplementedError
    ...
    def _copy_object(
        self, destination: BasePath, version_id: Union[str, None] = None
    ) -> dict | ObjectWriteResult:
        raise NotImplementedError
    ...
    def _delete_one(self, version_id: Union[str, None]) -> dict | DeleteResult:
        raise NotImplementedError
    ...
    def _cast(self, iter_: Any):
        """Casts :inlinepython:`iter` into the appropriate type.
    ...
    def _get_version_from_revision(self, revision: int) -> str:
        """From a given revision of a given s3key, returns the corresponding
        version UUID.
        ...
        Returns:
            :
                The version UUID corresopnding to the specified revision.
    ...
    def _check_rm_inputs(
        ...
        if self.name == ".s3keep":
            message = (
                "Can not delete file .s3keep files: they serve to identify"
                " directories."
            )
            if self.verbose:
                logger.info(message)
    ...
    @typechecked
    def is_file(
        self, include_deleted: bool = False, version_id: Union[str, None] = None
    ) -> bool:
        raise NotImplementedError
    ...
    @typechecked
    def head(self, version_id: Union[str, None] = None) -> HeadResponse:
        raise NotImplementedError
    ...
    @getattr_safe_property
    def bucket(self) -> str:
        """The bucket name containing this S3Path, depeding on
        :obj:`~persistent`.
        """
        return (
    ...
    def handle_type(self) -> bool:
        """
        Is the path supposed to handle special types when reading and writing?
        """
    ...
    def handle_type(self, value: bool):
        if value and not self._CAN_HANDLE_TYPE:
            raise ModuleNotFoundError(handle_type_error)
    ...
    @path.setter
    @typechecked
    def path(self, path: Self | Path | Union[str, None]):
        if ".." in str(path):
    ...
    def object_type(self) -> str:
        ...
        .. code-block:: python
            ...
            path = S3Path("notebooks/myfolder/test.csv")  # path is None
            path.write(...)  # type is 'unknown'
    ...
    def metadata(self) -> dict:
        """Returns the metadata associated to the S3 object, if any"""
        return self.head().metadata

    @object_type.setter
    @typechecked
    def object_type(self, value: Union[str, None]):
    ...
    def __init__(
        ...
        """Prints informations to stdout when methods are called."""
        self.hidden: bool = self.name.startswith(".")
        ...
        self._object_type: Union[str, None] = None
        self.object_type: Union[str, None] = object_type
    ...
    def write(
        ...
        Args:
            ...
            read_kwargs: Not used if using MinIO, as it can natively append to
                existing S3 objects.
    ...
    @typechecked
    def upload(
        self, localpath: str | Path
    ) -> WriteResponse | list[WriteResponse]:
        """Upload a local file or directory to this location.

        If the current S3Path does not exist or is an existing file, and you try
        to upload a local file to it, the uploaded file name is the current
        S3Path. Instead, if the current S3Path is an existing directory, the
        uploaded file will be named after the local file and located inside the
        existing directory.

        You can also upload a directory. In that case, if the current S3Path
        does not exist, the content of the local directory will be uploaded in
        it. Instead, if the current S3Path is an existing directory, the local
        directory (and not just its content) is uploaded in it.

        Make sure you do not try to upload a local directory to an
        ...
        upload_to = self / localpath.name if self.is_dir() else copy(self)
        upload_to.verbose = False
    ...
    def rm(
        ...
        except self._S3Error as error:
            message = (
                f"Error while deleting {self.str_persistent}{self},"
                f" version={version_id}: {error}"
            )
            if self.verbose:
                logger.info(message)
            return DeleteResponse(
