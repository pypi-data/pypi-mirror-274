...
from etao_langchain.retrievers import EnsembleRetriever
from etao_langchain.vectorstores import ElasticKeywordsSearch, Milvus
from langchain.chains.question_answering import load_qa_chain
from etao_langchain.rag.init_retrievers import (
    BaselineVectorRetriever,
    KeywordRetriever,
    MixRetriever,
    SmallerChunksVectorRetriever,
)
from etao_langchain.rag.scoring.ragas_score import RagScore
from etao_langchain.rag.utils import import_by_type, import_class


class etaoRagPipeline:
    ...
    def retrieval_and_rerank(self, question, collection_name):
        ...
        if self.params['post_retrieval']['with_rank'] and len(docs):
            if not hasattr(self, 'ranker'):
                ...
                rerank_object = import_class(f'etao_langchain.rag.rerank.{rerank_type}')
    ...
    def question_answering(self):
        ...
        if 'prompt_type' in self.params['generate']:
            ...
            prompt = import_class(f'etao_langchain.rag.prompts.{prompt_type}')
        else:
            prompt = None
    ...
    def score(self):
        ...
        if metric_params['type'] == 'etao-ragas':
            score_params = {
...
if __name__ == '__main__':
    ...
    parser.add_argument('--params', type=str, default='config/test/baseline_s2b.yaml', help='etao rag params')
    # 解析参数
    args = parser.parse_args()

    rag = etaoRagPipeline(args.params)
