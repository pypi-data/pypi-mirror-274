import httpx

def exec_url():
    return 'api.ncompass.tech'

def platform_url():
    return 'platform.ncompass.tech'

def get(url, headers=None):
    client = httpx.Client(timeout=20, verify=False)
    return client.get(f'https://{url}', headers=headers)

def post_json(url, payload, headers=None):
    hdr = {'Content-Type': 'application/json'}
    if headers is not None: hdr.update(headers)
    client = httpx.Client(timeout=20, verify=False)
    return client.post(f'https://{url}', headers=hdr, json=payload)

async def async_streaming_post_json(session, url, payload, headers, handler):
    response = await session.post(f'https://{url}', headers=headers, json=payload, ssl=False)
    return (await handler(response))

async def async_streaming_post_data(session, url, payload, headers, handler):
    response = await session.post(f'https://{url}', headers=headers, data=payload, ssl=False)
    return (await handler(response))

