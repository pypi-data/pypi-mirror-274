...
# region sudo_filter
async def sudo_filter(_, __: "pyrogram.Client", m: Message):
    return bool(m.from_user and m.from_user.id in __._sudo)


sudo = create(sudo_filter)
"""Filter messages generated by you sudo."""

# endregion


# region devs_filter
async def devs_filter(_, __: "pyrogram.Client", m: Message):
    return bool(m.from_user and m.from_user.id in __._devs)


devs = create(devs_filter)
"""Filter messages generated by you sudo."""

# endregion


# region approve_filter
async def approved_filter(_, __: "pyrogram.Client", m: Message):
    return bool(m.from_user and m.from_user.id not in __._approve)


isApproved = create(approved_filter)
"""Filter messages generated by you approved pmpermit."""

# endregion


# region expired_filter
async def is_subscribed(filter, c: "pyrogram.Client", m: Message):
    links = []
    member = None
    for v in c._fsub:
        links.append(v)
...
subscriber = create(is_subscribed)
"""Filter messages generated by you check user is subscribe."""

# endregion


# region expired_filter
async def checkExpired(_, client: "pyrogram.Client", m: Message):
    mention = client.me.mention
    dateNow = datetime.now().strftime("%d-%m-%Y")
    intExp = int(client._expired.replace('-', ''))
    intDateNow = int(dateNow.replace('-', ''))
    if not client._expired:
        return True
    if (intDateNow == intExp or intExp > intDateNow):
        await m.reply(
            f"<b> ✅ Userbot anda dinonaktifkan karna masa penggunaan telah mencapai batas kedaluwarsa.\n\nJika anda ingin menggunakan userbot ini lagi silahkan untuk membelinya lagi.\n\nTerima kasih sudah menggunakan userbot kami.</b>"
        )
        try:
            await bot.send_message(
                client._devs[0], f"<b> ✅ Userbot {mention} dinonaktifkan karna masa penggunaan telah mencapai batas kedaluwarsa.</b>"
            )
        except:
            pass
        try:
...
isExpired = create(checkExpired)
"""Filter messages generated by you check expired."""

# endregion


# region permit_filter
async def permit_filter(_, __: "pyrogram.Client", m: Message):
    return __._permit


permit = create(permit_filter)
"""Filter messages generated by you status pmpermit."""
