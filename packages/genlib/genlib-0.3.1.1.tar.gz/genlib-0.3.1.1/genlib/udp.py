...
class UDP(socket.socket):
    ...
    def loopStart(self, trigger=True):
        ...
        self.__is_exit_recv_thread = False
    ...
    def loopStop(self, timeout=2):
        ...
        self.__is_exit_recv_thread = True
...
class UDPServer(UDP):    
    def __init__(self, iface=UDP.DEFAULT_INTERFACE, port=UDP.DEFAULT_PORT):
        ...
        self.local = (socket.gethostbyname(socket.gethostname()), port)
...
class UDPClient(UDP):    
    def __init__(self, ip=UDP.DEFAULT_SERVER_IP, port=UDP.DEFAULT_PORT):
        ...
        self.local = (socket.gethostbyname(socket.gethostname()), self.getsockname()[1]) #Proceed only after sending one packet
                
        self.appendRemote((ip, port))   


class MulticastReceiver(UDPServer):
    def __init__(self, iface=UDP.DEFAULT_INTERFACE, group=UDP.DEFAULT_GROUP, port=UDP.DEFAULT_PORT):        
