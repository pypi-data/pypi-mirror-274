...
def get_gcp_local_session_token() -> dict:
    auth_logger = None
    comp_logger = None
    auth_logger_level = 0
    comp_logger_level = 0
...
def get_long_lived_adc_to_forward():
    to_forward = {}

    # get *local* credentials
    adf = get_application_default_credentials()
    ...
    to_forward["files"] = {
        "~/.config/gcloud/application_default_credentials.json": adf_json,
        "~/.config/gcloud/configurations/config_default": default_config,
    }

    return to_forward


def send_application_default_credentials(cluster):
    to_forward = get_long_lived_adc_to_forward()
