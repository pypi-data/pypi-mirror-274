...
class BaseRouter(APIRouter, ABC):
    ...
    def __init__(self, extra_funcs: Union[Tuple[Tuple], List[Tuple]] = tuple(),
        ...
        actions = []
        already_written = [func.args[0] for name, func in methods]
        already_written_names = [name for name, func in methods]
        for name, func in extra_funcs:
            if func.args[0] not in already_written and name not in already_written_names:
                actions.append((name, func))
        actions = actions + list(methods)
