...
def create_jar_flag_list(context: CertoraContext, args: List[Attr.ContextAttribute]) -> List[str]:
    return_list = []
    for arg in args:
...
def get_typechecker_arguments(context: CertoraContext, typechecker_args: Optional[List[str]] = None) -> str:
    """
    Assembles a string of all the command-line options the typechecker cares about
    @param context: The context
    @param typechecker_args: A dict of additional options (as provided via "typechecker_args" in a conf file)
    @return: A string with all the options in the format the typechecker understands.
    """
    typecheck_cmd = ""
    if typechecker_args:
        typecheck_cmd = f"{typecheck_cmd} {' '.join(typechecker_args)}"

    relevant_args = filter(lambda arg: get_attr_value(context, arg),
                           [Attr.ContextAttribute.RULE, Attr.ContextAttribute.EXCLUDE_RULE])
    args_list = create_jar_flag_list(context, list(relevant_args))

    return f"{typecheck_cmd} {' '.join(args_list)}"
...
def get_local_run_cmd(context: CertoraContext) -> str:
    ...
    """
    run_args = []
    if context.is_tac or context.is_solana or context.is_wasm:
