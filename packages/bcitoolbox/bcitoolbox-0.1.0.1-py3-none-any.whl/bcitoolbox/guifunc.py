...
def import_csv(text_entry):
    ...
    try:
        ...
        for ndata in range (len(data_matrices)):
            ...
            if disorcon == True:
                [error,modelprop,dataprop,d] = simulateVV_GUI([0.5,1,1,3,3], 10000, data_matrices[ndata], 1, strategy = 'ave', fitType = 'mll', es_para = checkbox_vars, fixvalue = [0.5,0.4,0.8,4000,2,70000.5,70000.5])
...
def plot_func(ifsave):
    ...
    for ndata in range (len(data_matrices)):
        fig = plt.figure()
        if init_numt == True:
            [error,modelprop,dataprop,d] = simulateVV_GUI(parameters_list[ndata], 10000, data_matrices[ndata], 1, strategy = strategy_list[ndata], fitType = 'mll', es_para = checkbox_vars, fixvalue = [0.5,0.4,0.8,4000,2,0,0])

        else :
            [error,modelprop,dataprop,c] = simulateLCD_GUI(parameters_list[ndata], 10000, data_matrices[ndata], strategy = strategy_list[ndata], fittype = 'mll', es_para = checkbox_vars, fixvalue = [0.5,0.4,0.8,4000,2,0,0])
...
def plot_func_l(ifsave):
    ...
    for ndata in range (len(data_matrices)):
        ...
        for i in range(a[1]):
            ...
            if datamat[1, i] is not None and datamat[1, i][0] < 1000:
                D2 = datamat[1, i]
...
def run_code():
    ...
    global progressbar
    global start_time
    global parameters_list, error_list, strategy_list, vb_list
...
def open_welcome():
    ...
    version_label = tk.Label(window, text="Version 0.1.0.1", fg="gray")
