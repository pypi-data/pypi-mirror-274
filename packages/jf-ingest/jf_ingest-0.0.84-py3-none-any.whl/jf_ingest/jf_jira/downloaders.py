...
def download_projects_and_versions_and_components(
    ...
    # There appears to be itermitent 404s that get thrown for versions and components.
    # When we retry, they 404 goes away.
    http_codes_to_retry = []
    http_codes_to_retry.extend(DEFAULT_HTTP_CODES_TO_RETRY_ON)
    http_codes_to_retry.append(404)
    filters: list = (
    ...
    all_projects: list[Project] = retry_for_status(
        jira_connection.projects, statuses_to_retry=http_codes_to_retry
    )
        ...
        p.raw.update(
            {
                "components": [
                    ...
                    for c in retry_for_status(
                        jira_connection.project_components, p, statuses_to_retry=http_codes_to_retry
                    )
                ]
            }
        )
    ...
    for p in projects:
        versions = retry_for_status(
            jira_connection.project_versions, p, statuses_to_retry=http_codes_to_retry
        )
...
def download_boards_and_sprints(
    ...
    if download_sprints:
        retry_for_statuses = []
        retry_for_statuses.extend(DEFAULT_HTTP_CODES_TO_RETRY_ON)
...
def get_issue_list_to_download(
    ...
    * deletes since last pull:
    ...
    if len(project_to_issue_map) == 0 and len(jellyfish_issue_metadata) > 0:
        logger.warning("No valid projects found in local metadata, skipping issue download")
        return IssueListForDownload()
...
def download_worklogs(
    ...
    while True:
        try:
            ...
            logging_helper.send_to_agent_log_file(f'This error WILL NOT be raised')
            break
