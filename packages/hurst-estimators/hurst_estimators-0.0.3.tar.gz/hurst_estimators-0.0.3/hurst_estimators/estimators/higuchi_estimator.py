...
def higuchi_estimator(
    X: np.ndarray, min_window_size: int, max_window_size: int, *, num_windows: int = 20
    ...
    if N < max_window_size:
        raise ValueError(
            "The length of the time series must be greater than max_window_size."
        )
    if num_windows < 2:
    ...
    window_sizes = np.unique(
        np.geomspace(min_window_size, max_window_size, num=num_windows).astype(int)
    )
