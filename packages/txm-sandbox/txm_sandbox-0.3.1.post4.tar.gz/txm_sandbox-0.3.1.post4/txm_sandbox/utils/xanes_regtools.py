    ...
    def alignment_scheduler(
        ...
        if use_chnk:
            if ref_mode.upper() == "SINGLE":
                if (anchor_loc % chnk_sz) > 2:
                    left_num_chnk = anchor_loc // chnk_sz + 1
                else:
                    left_num_chnk = anchor_loc // chnk_sz

                for ii in range(left_num_chnk - 1):
                    algn_pair_lst.append(
                        [
                            anchor_loc - ii * chnk_sz,
                            max(anchor_loc - (ii + 1) * chnk_sz, 0),
                        ]
                    )
                algn_pair_lst.append([anchor_loc - (left_num_chnk - 1) * chnk_sz, 0])

                if ((n_pnts - anchor_loc) % chnk_sz) > 2:
                    right_num_chnk = (n_pnts - anchor_loc) // chnk_sz + 1
                else:
                    right_num_chnk = (n_pnts - anchor_loc) // chnk_sz

                for ii in range(right_num_chnk - 1):
                    algn_pair_lst.append(
                        [anchor_loc + ii * chnk_sz, anchor_loc + (ii + 1) * chnk_sz]
                    )
                algn_pair_lst.append(
                    [anchor_loc + (right_num_chnk - 1) * chnk_sz, n_pnts - 1]
                )

                algn_pair_lst = np.array(algn_pair_lst)
                algn_pair_lst = [
                    list(ii) for ii in algn_pair_lst[algn_pair_lst[:, 0].argsort()]
                ]
                tem = deepcopy(algn_pair_lst)

                for ii in range(left_num_chnk):
                    for jj in range(tem[ii][0] - 1, tem[ii][1], -1):
                        algn_pair_lst.append([tem[ii][0], jj])

                for ii in range(left_num_chnk, right_num_chnk + left_num_chnk):
                    for jj in range(tem[ii][1] - 1, tem[ii][0], -1):
                        algn_pair_lst.append([tem[ii][1], jj])
