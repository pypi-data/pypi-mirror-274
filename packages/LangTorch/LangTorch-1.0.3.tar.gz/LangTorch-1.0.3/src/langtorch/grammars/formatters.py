...
def f_concatenate_terminals(instance: 'Text') -> str:
    ...
    # Recursive cases
    if isinstance(instance, tuple) and len(instance) == 2 and isinstance(instance[0], str) and isinstance(instance[1], (tuple, list)):
        return ''.join((f_concatenate_terminals(child) for child in instance[1]) if isinstance(instance[1], list) else f_concatenate_terminals(instance[1]))
...
def wrap_line(line, max_width):
    if len(line) <= max_width:
        return [line]
    return [line[i:i + max_width - 1] + '↵' if i + max_width < len(line) else line[i:i + max_width - 1] for i in range(0, len(line), max_width - 1)]
