...
def compute_derivatives( data, width = 1 ):
    ...
    for i in range( len( data[2] ) ):
        ...
        derivative_data[2].append( gu.derivative( data[1], data[2][i], width ) )

    for i in range( len( data[9] ) ):

        derivative_data[9].append( gu.derivative( data[1], data[9][i], width ) )
...
def extract_rheology_features( output_directory, file_data, data, first_derivative_data, second_derivative_data ):
    ...
    for i in range( len( file_data ) ):
        ...
        feature_1.append( np.log10( data[2][i][0] ) )

    features = np.array( feature_1 )[:, np.newaxis]

    feature_names = ["Rhe_{:.2f}".format( data[1][0] )]
    ...
    for j in range( 1, len( data[1] ) ):
        ...
        feature_names.append( "Rhe_{:.2f}".format( data[1][j] ) )
    ...
    for j in range( len( data[3][1] ) ):
        ...
        feature_names.append( "Rhe_SM_{:.2f}".format( data[1][j] ) )
    ...
    for j in range( len( data[4][1] ) ):
        ...
        for i in range( len( file_data ) ):
            ...
            feature_1.append( np.log10( data[4][i][j] ) )

        features = np.hstack( (features, np.array( feature_1 )[:, np.newaxis]) )

        feature_names.append( "Rhe_LM_{:.2f}".format( data[1][j] ) )
    ...
    for j in range( len( data[5][1] ) ):
        ...
        for i in range( len( file_data ) ):
            ...
            feature_1.append( data[5][i][j] )

        features = np.hstack( (features, np.array( feature_1 )[:, np.newaxis]) )

        feature_names.append( "Rhe_Loss_{:.2f}".format( data[1][j] ) )
    ...
    for i in range( len( file_data ) ):
        ...
        log_change = (np.log10( 1 ) - np.log10( data[5][i][index_1] )) / (np.log10( data[5][i][index_2] ) - np.log10( data[5][i][index_1] ))

        crossover_point = 10 ** ((np.log10( data[1][index_2] ) - np.log10( data[1][index_1] )) * log_change + np.log10( data[1][index_1] ))
    ...
    for i in range( len( file_data ) ):
        ...
        m = (np.log( data[2][i][3] ) - np.log( data[2][i][1] )) / (np.log( data[1][3] ) - np.log( data[1][1] ))
        b = np.log( data[2][i][2] / data[1][2] ** m )

        extrapolated_value = data[1][len( data[1] ) - 1] ** m * np.exp( b )
        real_value = data[2][i][len( data[2][i] ) - 1]
...
def read_and_analyse_features( ip, file_data ):
    ...
    if not ip.feature_selection:
        ...
        ip.feature_selection = [0, 15, 30, 31, 46, 61, 62, 77, 92, 93, 108, 123, 124, 125]
        # ip.feature_selection = [0, 15, 30, 124, 125]
