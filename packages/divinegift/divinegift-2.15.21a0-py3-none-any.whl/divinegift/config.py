...
class Settings:
    ...
    def __init__(self, file_: Optional[str] = None, ck_fname: Optional[str] = 'key.ck', encoding: str = 'utf-8',
        ...
        self.possible_file_names = ['settings.yaml', 'config.yaml', 'configs.yaml', 'application.yaml']
    ...
    def find_config_file(self, file):
        ...
        raise ConfigNotFoundError(f'There is no config file {file} in directory {d} with allowed extensions: [{self.file_extensions_allowed}]')
    ...
    def parse_settings(self, file_: str = './settings.yaml', ck_fname: str = 'key.ck', encoding: str = 'utf-8',
        ...
        if not os.path.exists(file_):
            config_dir, config_file = os.path.split(file_)
            for possible_file in self.possible_file_names:
                try:
                    file_ = self.find_config_file(os.path.join(config_dir, possible_file))
